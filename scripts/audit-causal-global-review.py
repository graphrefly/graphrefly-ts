"""Replay the fixed cumulative b30ae176 -> 0c4cbdf6 coverage comparison; descriptive only."""
...
def audit(before,after):
 ...
 assert bf['profileIntervention']==af['profileIntervention']
 assert read(before/'dispatch.json')['nodeDigest']==read(after/'dispatch.json')['nodeDigest']
 assert set(bf['sources'])==set(af['sources'])
 changed=sorted(p for p in bf['sources'] if bf['sources'][p]!=af['sources'][p])
 assert bf['bundle']=='c64b4d1d0a85496b7bd49613f300ab76867db330aceb36c3614dfb459f915f44'
 assert af['bundle']=='13313f587f0ec59c762f4a301f09e8ca2f980afa609625b5e017354d09210176'
 assert changed==['examples/spending-alerts/causal-admission.ts','packages/ts/src/solutions/causal-occurrence/evidence.ts','packages/ts/src/solutions/causal-occurrence/identity.ts','packages/ts/src/solutions/causal-occurrence/transition.ts'],changed
