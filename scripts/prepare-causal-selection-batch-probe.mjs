/** Diagnostic-only selector wave grouping intervention. Never changes repository consumer code. */
...
const businessPath = "examples/spending-alerts/causal-business.ts";
const business = readFileSync(businessPath, "utf8");
const begin = business.indexOf("\tconst evaluationSelections = make<BusinessFrame<Evaluation>>(");
const end = business.indexOf("\tconst transaction = project(", begin);
assert.ok(begin >= 0 && end > begin);
let selection = business.slice(begin, end);
assert.equal(selection.split("(ctx) => {").length, 2);
selection = selection.replace("(ctx) => {", "(ctx) => {\n const diagnosticOutputs: any[] = [];");
assert.equal(selection.split("ctx.down(").length, 4);
selection = selection.replaceAll("ctx.down(", "diagnosticOutputs.push(...");
const tail = "\n\t\t},\n\t);";
assert.equal(selection.split(tail).length, 2);
selection = selection.replace(
	tail,
	"\n if(diagnosticOutputs.length) ctx.down(diagnosticOutputs);" + tail,
);
const grouped = business.slice(0, begin) + selection + business.slice(end);
const b = await build({
	stdin: {
		contents: entry,
	...
	plugins: [
		{
			name: "opt-in-profile",
			setup(api) {
				api.onLoad({ filter: /causal-business\.ts$/ }, () => ({
					contents: grouped,
					loader: "ts",
...
for (const p of [
	...
	"scripts/prepare-causal-selection-batch-probe.mjs",
	"pnpm-lock.yaml",
]) {
...
put("freeze.json", {
	...
	selectorIntervention: {
		original: sha(business),
		loaded: sha(grouped),
		boundary: "only selection output wave grouping; ordered DATA contents retained",
	},
	scope:
