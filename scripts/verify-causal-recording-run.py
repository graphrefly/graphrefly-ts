...
_spec = importlib.util.spec_from_file_location('child_verifier', Path(__file__).with_name('verify-causal-recording-child.py'))
...
def verify(root):
    ...
    require([j['round'] for j in jobs] == [i // 4 for i in range(16)], 'round blocks preserved')
    require(len(set(coordinates)) == 16 and set(coordinates) == expected, 'exact schedule coordinates')
    require(all(type(j['id']) is int for j in jobs) and [j['id'] for j in jobs] == list(range(16)), 'sequential job IDs')
    ...
    require(isinstance(result.get('children'), list) and len(result['children']) == 16, '16 result children')
    ...
    for job, summary in zip(jobs, result['children']):
        ...
        require(diagnostic['condition'] == 'CPU' and diagnostic['orientation'] == job['orientation'], 'diagnostic coordinates')
        recording = read(directory / 'recording.json')
        require(recording['condition']==job['condition'], 'recording coordinate')
        analysis = _child.verify(samples, diagnostic, recording)
        ...
        for orientation in ('U', 'V'):
            for batch in range(3):
                for arm in ('candidate', 'reference'):
                    for numerator, denominator in [('DEFERRED', 'EAGER')]:
    ...
    for orientation in ('U', 'V'):
        for batch in range(3):
            for arm in ('candidate', 'reference'):
                for contrast in ('DEFERRED/EAGER',):
    ...
    for r in range(4):
        for o in ('U','V'):
            a,b=([x['analysis']['whole'] for x in children if x['round']==r and x['analysis']['orientation']==o and x['analysis']['condition']==c][0] for c in ('DEFERRED','EAGER'))
            whole_pairs.append(dict(round=r,orientation=o,eager=b,deferred=a,ratios={k:a[k]/b[k] if b[k]>0 else None for k in b},zeroDenominators=[k for k in b if b[k]==0]))
    consistent=all(x['ratios'][k]<1 for x in ratios if x['batch']==0 for k in ('p95Ms','constructionSumMs'))
    return {'wholePairs':whole_pairs,'consistentFirstBatch':consistent,'schema': 'causal-recording-run-analysis-v1', 'children': children, 'ratios': ratios,
            'fourRoundRanges': ranges,
            'sampleCount': 38400, 'measuredBlockCount': 96,
