...
const [mode, dir, index] = process.argv.slice(2),
	root = resolve(dir);
...
if (mode === "prepare") {
	...
	for (const arm of ["before", "after"]) {
					...
					setup(api) {
						api.onLoad({ filter: /\.ts$/ }, (args) => {
							const p = relative(process.cwd(), args.path),
								bytes = readFileSync(resolve(sourceRoot, p));
							assert.equal(sha(bytes), frozen[p], p);
							return {
								contents: p.endsWith("spending-preset-performance-worker.ts")
									? adaptWorker(bytes.toString())
									: bytes.toString(),
								loader: "ts",
		...
		writeFileSync(resolve(root, arm + ".mjs"), b.outputFiles[0].contents);
		bundles[arm] = sha(b.outputFiles[0].contents);
	}
	const jobs = [];
	...
	writeFileSync(
		resolve(root, "P1-inputs.json"),
		readFileSync(resolve(root, "../before/P1-inputs.json")),
	);
	put("freeze.json", {
		bundles,
		sources,
		...
		warmup: 30,
		measured: 200,
		input: sha(readFileSync(resolve(root, "P1-inputs.json"))),
		scope: "Unprofiled cold construction; diagnostic only",
	});
} else {
	assert.equal(mode, "run");
	const f = read("freeze.json"),
		job = f.jobs[Number(index)];
	assert.equal(job.id, Number(index));
	...
	for (const arm of job.order) {
		...
		assert.equal(
			w.preflight(
				{ id: "cold-P1-" + job.mode, group: "cold", profile: "P1", mode: job.mode },
				read("P1-inputs.json"),
			).passed,
			true,
		);
		...
		for (let i = -f.warmup; i < f.measured; i++) {
			...
			try {
				...
				run = w.measurementArm("candidate", job.mode);
