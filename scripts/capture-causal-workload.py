...
supervisor = module('causal-workload-supervisor.py', 'supervisor')
verifier = module('verify-causal-workload-diagnostic.py', 'child_verifier')
...
def capture(prepared, scenario, approval, qualification, output, node):
    ...
    try:
        ...
        def run_one(cell):
            ...
            assert (diag['condition'],diag['orientation']) == (cell['condition'],cell['orientation'])
            verifier.verify(samples,diag)
            return result
        ...
        put(output/'failure.json',dict(error=f'{type(error).__name__}:{error}',
