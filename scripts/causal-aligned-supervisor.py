...
def schedule(rng):
    ...
    for round_id in range(2):
        cells = [(c, o) for c in ('CONTROL', 'GC', 'GC_CPU') for o in ('U', 'V')]
        rng.shuffle(cells)
        base = len(jobs)
...
def dispatch(jobs, run_one):
    ...
    if len(jobs) != 12 or [j['id'] for j in jobs] != list(range(12)):
        raise ValueError('12 frozen sequential coordinates required')
    if [j['round'] for j in jobs] != [i//6 for i in range(12)]:
    ...
    return dict(children=results, notRun=jobs[len(results):],
                completed=len(results)==12 and not results[-1]['result'].get('reason'))
