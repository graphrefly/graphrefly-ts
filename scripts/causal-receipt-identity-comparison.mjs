...
if (mode === "prepare") {
	...
	const before = execFileSync("git", ["show", `351a7ace:${target}`], { encoding: "utf8" });
	...
	for (let position = 0; position < job.order.length; position++) {
		...
		try {
			const trace = [];
			const stops = ["spending/evidence", "spending/causal/authority"].map((name) =>
			...
			if (priorTrace !== undefined)
				assert.deepEqual(trace, priorTrace, "exact evidence and authority DATA sequence");
			priorTrace = trace;
