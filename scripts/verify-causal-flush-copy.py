...
def verify(root):
 ...
 assert f['adapter']==sha(root/'worker-adapter.mjs') and f['transformedWorker']==sha(root/'transformed-worker.ts')
 assert f['tool']==sha(root/'tool.mjs') and f['beforeSource']==sha(root/'transition-before.ts')
 assert f['afterSource']==sha(root/'source/packages/ts/src/solutions/causal-occurrence/transition.ts')
 ...
 for p in ['P1','P3','P6']:expected.append(dict(id=len(expected),profile=p,count=2,repeat=0,control=True,order=['before','before']))
 ...
 for job,record in zip(expected,a['records']):
  ...
  x=read(root/f"job-{job['id']}.json");assert len(x['records'])==2;assert x['job']==job and x['preflights']==2 and x['stateComparison'] is True and x['formalQualification'] is False
