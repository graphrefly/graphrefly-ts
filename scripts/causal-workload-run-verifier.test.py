...
v = load('run_verifier', 'verify-causal-workload-run.py')
f = load('synthetic_child', 'causal-workload-verifier.test.py')
    ...
    for round_id in range(4):
        for condition in ('BASE', 'CPU', 'CPU_GC'):
            for orientation in ('U', 'V'):
                ...
                samples, diagnostic = f.fixture(condition, orientation)
                # Distinct construction durations yield non-1 instrumentation ratios.
                factor = {'BASE': 1, 'CPU': 2, 'CPU_GC': 3}[condition]
                for sample in samples:
                    sample['start'] /= 10
                    sample['ms'] = factor * .2
                    sample['end'] = sample['start'] + sample['ms']
                for cp in diagnostic['checkpoints']:
                    cp['wallBefore'] = cp['wallBefore'] / 10 + .2
                    cp['wallAfter'] = cp['wallAfter'] / 10 + .2
                if diagnostic['disconnectAt'] is not None:
                    diagnostic['disconnectAt'] /= 10
...
class RunVerifierTests(unittest.TestCase):
    def test_full_and_rejections(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(report['sampleCount'], 57600)
            self.assertEqual(len(report['ratios']), 96)
            self.assertEqual(report['ratios'][0]['ratios']['p95Ms'], 2)
            self.assertAlmostEqual(report['ratios'][1]['ratios']['constructionSumMs'], 1.5)
            ...
            for key, value in [('rssBytes', 256 * 1024 ** 2 + 1), ('gap', 1.1), ('childElapsed', 31), ('directoryBytes', 256 * 1024 ** 2 + 1)]:
