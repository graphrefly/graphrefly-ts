"""Independent partial coverage audit. Failed and not-run coordinates remain explicit."""
...
def verify(root):
 ...
 assert f['limits'] in [dict(processes=168,childSeconds=900,totalSeconds=7200,rssMiB=2048,retries=0),dict(processes=168,childSeconds=30,totalSeconds=1800,rssMiB=256,retries=0)]
 assert len(f['jobs'])==168
 assert a['complete'] is False and isinstance(a['failure'],str) and a['failure']
 assert a['attempted']==list(range(a['dispatched'])) and a['notRun']==list(range(a['dispatched'],168))
 ...
 for j,job in enumerate(f['jobs'][:a['dispatched']]):
  assert job['position']==j
  d=root/f'job-{j:03}'; cfg=read(d/'config.json');exit=read(d/'exit.json')
  row=job['row']; assert cfg['row']==row and cfg['identicalReference']==job['identicalReference']
  if j==a['dispatched']-1:
   assert exit['error'] is not None and not (d/'completion.json').exists(),'failed final process'
   continue
  meta=read(d/'worker.json')
 ...
 assert len(pids)==a['dispatched']-1 and samples_total==len(pids)*3600,'completed prefix'
 report=[]
 for item in rows.values():
  if 'main' not in item or 'control' not in item:
   item.update(incompletePair=True);report.append(item);continue
  ...
  report.append(item)
 return dict(kind='diagnostic-coverage-replay',complete=False,formalQualification=False,dispatched=a['dispatched'],completedJobs=len(pids),notRun=a['notRun'],failure=a['failure'],samplesInCompletedJobs=samples_total,maxObservedRSSMiB=maxrss,rows=report)
