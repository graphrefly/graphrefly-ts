...
spec = importlib.util.spec_from_file_location('verifier', Path(__file__).with_name('verify-causal-latency-baseline.py'))
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
source = Path(sys.argv[1])
cases = [
 ('control-uses-candidate', 'job-1.json', lambda x: x['records'][0].update(arm='candidate')),
 ('negative-time', 'job-0.json', lambda x: x['records'][0]['observations'][0].update(ms=-1)),
