...
old=cpu.old
SCHEDULE=['D-U','R-V','D-V','R-U','R-U','D-V','R-V','D-U']
SOURCES=['causal-release-diagnostic.py','derive-causal-release.mjs','causal-release-observer.mjs','verify-causal-release.py','causal-release.test.mjs','causal-release.test.py','causal-release-paths.test.mjs','causal-cleanup-observer.mjs','verify-causal-cleanup.py','causal-cpu-observer.mjs','verify-causal-cpu.py','verify-causal-position-pairs.py','verify-causal-block-history.py','verify-causal-control-crossover.py','causal-control-crossover.py']
DESIGN='11b753d7a9682d1ab9cd951c1c8e828ab663da6b5ac981354e6e51b31db9b069'
FLAGS=['--trace-gc','--trace-deopt','--log-deopt','--no-logfile-per-isolate','--logfile=v8.log']
FROZEN_D={'observed.mjs': 'a7b50438faa1b757612d408966c562675d69c9375ad70ead8c709471776d1cf1', 'derived-worker.mjs': '9e0ee5e980e38348b8e132c247ab25d20bfeeff0a873b29f21236e46b6e9650e', 'derived-worker-copy.mjs': '9e0ee5e980e38348b8e132c247ab25d20bfeeff0a873b29f21236e46b6e9650e', 'source/scripts/causal-cpu-observer.mjs': 'd95be8c25541cdc4df6c964a89823c802ae4b97992859a2dd81726167093f89b', 'source/scripts/causal-cleanup-observer.mjs': '025cbe33d4ffcf7975090c9d9842f04311c44246b82ec08b8af6fe3bb36099ec'}
...
def expected_entry(root,job,mode):
    ...
    for var,name in [('n','observed.mjs'),('m0',prefix+'worker.mjs'),('m1',prefix+'worker-copy.mjs')]:
    ...
    if mode=='R':s+='const {createReleaseGaps}=await import('+json.dumps((root/'source/scripts/causal-release-observer.mjs').as_uri())+');\n'
...
def base_verify_gaps(g,samples,e,mode):
...
def check_release_source(original,derived):
    assert hashlib.sha256(original.encode()).hexdigest()=='9e0ee5e980e38348b8e132c247ab25d20bfeeff0a873b29f21236e46b6e9650e'
    reverse=[('  release(opts = {}, diagnostic) {','  release(opts = {}) {'),('      reason: opts.reason ?? this.name\n    }, diagnostic);','      reason: opts.reason ?? this.name\n    });'),('  releaseNodes(nodes, opts, diagnostic) {\n    this.host._releaseNodes(nodes, opts, diagnostic);','  releaseNodes(nodes, opts) {\n    this.host._releaseNodes(nodes, opts);'),('  _releaseNodes(nodes, _opts = {}, diagnostic) {','  _releaseNodes(nodes, _opts = {}) {'),('group.release(void 0, diagnostic?.release);','group.release();')]
    clean=''.join(l for l in derived.splitlines(keepends=True) if not l.rstrip().endswith('// RELEASE'))
    for new,old in reverse:assert clean.count(new)==1;clean=clean.replace(new,old)
    assert clean==original,'unapproved release transform'
    marked=[l.strip() for l in derived.splitlines() if l.rstrip().endswith('// RELEASE')]
    assert marked==[f'if (diagnostic) diagnostic.r{i} = diagnostic.now(); // RELEASE' for i in range(7)]
    # Each new boundary is tied to an original successor/predecessor statement.
    links=[('_releaseNodes(nodes, _opts = {}, diagnostic) {',0,'const seen = /* @__PURE__ */ new Set();'),('const releaseIds = new Map(entries.map(({ node, entry }) => [node, entry.id]));',1,'for (const entry of this._entries.values()) {'),('}',2,'for (const { node, entry } of entries) {'),('}',3,'const releasedEvents = this._topologyObservers.size === 0 ? [] : entries.map(({ node, entry }) => ({'),('let releaseFailed = false;',4,'for (const { node } of entries) {'),('}',5,'if (!releaseFailed) {'),('if (releaseFailed) throw releaseError;',6,'}')]
    lines=[l.strip() for l in derived.splitlines()]
    for before,n,after in links:
        i=lines.index(marked[n]);assert lines[i-1]==before and lines[i+1]==after

def verify_gaps(g,samples,e,mode):
    assert mode in ('D','R')
    if mode=='D':
        assert 'releaseMode' not in g
        return base_verify_gaps(g,samples,e,'D')
    assert g.get('releaseMode')=='R'
    projected={**g,'samples':[{**s,'deep':{k:v for k,v in s['deep'].items() if k!='release'}} for s in g['samples']]}
    base=base_verify_gaps(projected,samples,e,'D');parts=[]
    labels=['entry-preparation','external-dependency-checks','quiescence-subscriber-checks','event-registry-removal','runtime-release','owner-event-finalization']
    for s in g['samples']:
        assert set(s['deep'])=={'d0','d1','d2','d3','d4','release'}
        r=s['deep']['release'];assert set(r)=={f'r{i}' for i in range(7)}
        values=[r[f'r{i}'] for i in range(7)];assert all(type(x) in (float,int) and math.isfinite(x) for x in values)
        assert values==sorted(values) and s['deep']['d3']<=values[0]<=values[-1]<=s['deep']['d4']
        p={name:values[i+1]-values[i] for i,name in enumerate(labels)}
        p['wrapper-remainder']=(values[0]-s['deep']['d3'])+(s['deep']['d4']-values[-1])
        assert math.isclose(sum(p.values()),s['deep']['d4']-s['deep']['d3'],rel_tol=1e-12,abs_tol=1e-9)
        parts.append(p)
    for i,row in enumerate(base):
        a=(i//2)*400+(100 if i%2 else 0);b=(i//2)*400+(400 if i%2 else 100)
        row['releaseSegments']={k:distribution([x[k] for x in parts[a:b]]) for k in parts[0]}
    return base
...
def admission(root):
    ...
    assert a['proposalCommit']=='720f9b18' and a['designDigest']=='sha256:'+DESIGN and sha(root/'approved-design.md')==DESIGN
    assert [a[k] for k in ('maxConsumerProcesses','maxFixtureProcesses','maxSamples','maxSecondsPerChild','maxSecondsTotal','maxOutputBytes','checkIntervalMs','retries')]==[8,0,19200,30,900,268435456,100,0]
    assert a['output']=='archive/evals/causal-release-boundaries-v1/run'
    ...
    check_release_source((root/'derived-worker.mjs').read_text(),(root/'release-worker.mjs').read_text())
    assert (root/'release-worker.mjs').read_bytes()==(root/'release-worker-copy.mjs').read_bytes()
    for name,digest in FROZEN_D.items():assert sha(root/name)==digest,'frozen D drift'
    assert read(root/'materials.json')=={n:sha(root/n) for n in ['worker.mjs','worker-copy.mjs','derived-worker.mjs','derived-worker-copy.mjs','position.mjs','observed.mjs','release-worker.mjs','release-worker-copy.mjs','P2-inputs.json','reference-qualification.json']}
    return r
...
def job_identity(root,position):
    ...
    c=dict(output=str(j),run=r['run']+f'-{position}',sourceDigest=sha(d/'entry.mjs'),mode='Q',gapMode='D',releaseMode=mode,profile='consumer',orientation=orientation,row=old.ROW,kind='control',control=True,scenarioPath=str(physical/'P2-inputs.json'))
...
def present_identities(root,position):
    r,d,c,ex=job_identity(root,position)
    for name in ('evidence.json','gaps.json'):
        if not (d/name).exists():continue
        value=read(d/name);expected=dict(run=c['run'],pid=ex['pid'],sourceDigest=c['sourceDigest'])
        if name=='evidence.json':expected.update(node=r['runtime']['node'],v8=r['runtime']['v8'])
        assert value['identity']==expected,'present partial evidence identity'
    if (d/'worker.json').exists():
        m=read(d/'worker.json');assert m['pid']==ex['pid'] and m['node']==r['runtime']['node'] and m['row']==old.ROW
    if (d/'entry-events.jsonl').exists():
        events=[json.loads(l) for l in (d/'entry-events.jsonl').read_text().splitlines()];assert events==[dict(module=n,pid=ex['pid']) for n in ('n','m0','m1')][:len(events)] and len(events)<=3

def verify_prefix(root,count):
    assert type(count) is int and 0<=count<=8
    r=admission(root);names={f'{p:02d}-{SCHEDULE[p]}' for p in range(count)}
    assert {p.name for p in root.iterdir() if p.is_dir() and p.name not in ('source','qualification')}==names,'prefix directories'
    runtime_files={'dispatch.json','exit.json','samples.jsonl','evidence.json','gaps.json','worker.json','entry-events.jsonl','completion.json'}
    for p in root.rglob('*'):
        assert not p.is_symlink()
        if p.is_file() and p.name in runtime_files:
            parts=p.relative_to(root).parts;assert len(parts)==2 and parts[0] in names,'prefix hidden execution'
    last=r['startMonotonic'];pids=set()
    for p in range(count):
        row=verify_job(root,p);d=root/f'{p:02d}-{SCHEDULE[p]}';ex=read(d/'exit.json')
        assert last<=ex['startMonotonic']<r['startMonotonic']+900 and ex['pid'] not in pids;pids.add(ex['pid']);last=ex['endMonotonic']
        command=read(d/'independent-command.json');assert command['exitCode']==0 and command['stderr']=='';cpu.same(json.loads(command['stdout']),dict(evidenceValid=True,**row))
    return dict(evidenceValid=True,prefixProcesses=count,samples=count*2400)
...
def verify(root):
    ...
    result=read(root/'result.json');attempted=result['attempted'];assert attempted==list(range(len(attempted))) and len(attempted)<=8 and result['notRun']==list(range(len(attempted),8))
    ...
    else:assert result['status']=='complete-diagnostic' and result['stopReason'] is None and len(attempted)==8
    ...
    if extra:
        assert failure and failure['stage']=='pre-dispatch' and len(attempted)<8
            ...
            if (d/'evidence.json').exists():
                try:
                    ...
                    if (d/'gaps.json').exists():
                        ...
                        failed['arithmeticSegments']=verify_gaps(g,raw,e,c['releaseMode'])
    ...
    assert result['capturedSamples']==sum(len(p.read_bytes().splitlines()) for p in root.glob('*/samples.jsonl'))<=19200
    if result['status']=='complete-diagnostic':assert len(verified)==8 and last-r['startMonotonic']<=result['elapsedSeconds']<=900
...
if __name__=='__main__':
    try:
        if len(sys.argv)==3 and sys.argv[1]=='--admission':admission(Path(sys.argv[2]));out=dict(evidenceValid=True,admission=True)
        elif len(sys.argv)==4 and sys.argv[1]=='--prefix':out=verify_prefix(Path(sys.argv[2]),int(sys.argv[3]))
        elif len(sys.argv)==4 and sys.argv[1]=='--job':out=dict(evidenceValid=True,**verify_job(Path(sys.argv[2]),int(sys.argv[3])))
