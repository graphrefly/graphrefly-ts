...
- `packages/cli`, `packages/mcp-server` — workspace consumers of `@graphrefly/graphrefly`. Their vitest configs alias `@graphrefly/graphrefly` directly to `packages/pure-ts/src/index.ts` (the shim is just re-exports; nothing new to test through it).
