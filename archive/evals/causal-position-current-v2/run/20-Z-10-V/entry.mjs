...
const config = {"row": {"id": "cold-P2-summary", "group": "cold", "profile": "P2", "mode": "summary"}, "output": "/Users/davidchenallio/src/graphrefly-ts/archive/evals/causal-position-current-v2/run/20-Z-10-V", "scenarioPath": "/Users/davidchenallio/src/graphrefly-ts/archive/evals/causal-position-current-v2/run/P2-inputs.json", "control": true, "kind": "control", "orientation": "V"};
assert.deepEqual(JSON.parse(readFileSync("/Users/davidchenallio/src/graphrefly-ts/archive/evals/causal-position-current-v2/run/20-Z-10-V/config.json","utf8")),config);
const record = module => appendFileSync("/Users/davidchenallio/src/graphrefly-ts/archive/evals/causal-position-current-v2/run/20-Z-10-V/entry-events.jsonl",JSON.stringify({module,pid:process.pid})+"\n");
...
await n.runRow("/Users/davidchenallio/src/graphrefly-ts/archive/evals/causal-position-current-v2/run/20-Z-10-V/config.json",[m0,m1]);
assert.deepEqual(m0.RECIPE,expected);assert.deepEqual(m1.RECIPE,expected);
writeFileSync("/Users/davidchenallio/src/graphrefly-ts/archive/evals/causal-position-current-v2/run/20-Z-10-V/recipe-after.json",JSON.stringify({pid:process.pid,recipe:expected}));
