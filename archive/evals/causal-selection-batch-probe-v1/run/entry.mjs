  ...
  const evaluationSelections = make(
    ...
    (ctx) => {
      ...
      const emitPending = () => {
        ...
        state.pending = [];
        if (rows.length || problems.length) diagnosticOutputs.push(...[["DATA", frame(rows, problems.splice(0))]]);
      };
      emitPending();
      for (const raw2 of depWaves(ctx, 1).flat()) {
        ...
        if (!result.valid) {
          problems.push(result.issue);
          state.pending = [];
          diagnosticOutputs.push(...[["DATA", frame([], problems.splice(0), false)]]);
          continue;
        }
      ...
      if (problems.length) diagnosticOutputs.push(...[["DATA", frame([], problems, false)]]);
      if (diagnosticOutputs.length) ctx.down(diagnosticOutputs);
    }
  );
...
function schedule(row, scenario2) {
  const before = [], action = [];
  const arrival = (refs2, count = 1) => ({
    lane: "arrivals",
    values: Array.from({ length: count }, () => ({ ...scenario2.arrivals, evaluationRefs: refs2 }))
  });
  if (row.group === "cold") return { before, action, preWaveNew: 0, frameItems: 0 };
  if (row.group === "recovery")
    return { before: scenario2.steps, action, preWaveNew: 0, frameItems: 0 };
  const refs = scenario2.arrivals.evaluationRefs;
  if (row.change === "duplicate") before.push(...scenario2.steps);
  else {
    before.push(...scenario2.setup);
    if (scenario2.id !== "P6")
      before.push({ lane: "verification", values: [scenario2.combined.verification] });
    if (row.change === "one-new") before.push(arrival(refs.slice(0, -1)));
  }
  action.push(arrival(refs, row.dataCount));
  if (scenario2.id === "P6" && row.change !== "duplicate")
    action.push(...scenario2.steps.filter((s) => s.lane === "verification"));
  return {
    before,
    action,
...
function materialHints(scenario2, plan) {
  const seen = /* @__PURE__ */ new Set();
  const flagged = new Set(
    scenario2.evaluations.filter((e) => oracleBusiness(e).flagged).map((e) => e.evaluationRef)
  );
...
function preflight(row, scenario2) {
  const a = graphArm("candidate", row.mode), b = graphArm("reference", row.mode), c = measurementArm("plain", row.mode);
  assert.ok("plain" in c);
  const plan = schedule(row, scenario2);
  const steps = row.group === "cold" ? scenario2.steps : [...plan.before, ...plan.action];
  const topology = {};
  let primary;
  try {
    ...
    assert.equal(assessments.length, scenario2.evaluations.length);
    for (const e of scenario2.evaluations) {
    ...
    return {
      passed: true,
      materialHints: materialHints(scenario2, plan),
      recoveryPorts,
      topology,
      beforeOccurrences,
      afterOccurrences,
...
// profile-entry.ts
var scenario = JSON.parse(readFileSync(new URL("./P1-inputs.json", import.meta.url), "utf8"));
var results = [];
for (const dataCount of [1, 2]) for (const arm of ["candidate", "reference"]) {
  ...
  try {
    ...
    const observations = [];
    for (let i = 0; i < 10; i++) {
      const before = run.graph.profile();
      const t = performance.now();
      for (const step of plan.action) run.send(step);
      const elapsed = performance.now() - t;
      const after = run.graph.profile();
      assert2.deepEqual(run.state(), baseline, "duplicate preserves authority");
