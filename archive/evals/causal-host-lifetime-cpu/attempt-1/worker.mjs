...
// spending-host-profile-worker.ts
...
__name(isInvalidErrorPayload, "isInvalidErrorPayload");
...
__name(messageTier, "messageTier");
function isDeferredTier(t) {
  return TIER[t] >= TIER_VALUE;
}
__name(isDeferredTier, "isDeferredTier");
function isValueTier(t) {
  return TIER[t] === TIER_VALUE;
}
__name(isValueTier, "isValueTier");
function isPauseBufferedTier(t) {
  const tier = TIER[t];
  return tier === TIER_VALUE || tier === TIER_SETTLE;
}
__name(isPauseBufferedTier, "isPauseBufferedTier");
...
__name(isUpAllowed, "isUpAllowed");
...
__name(isWaveActive, "isWaveActive");
function enterWave() {
  depth++;
}
__name(enterWave, "enterWave");
function exitWave() {
  depth--;
  if (depth === 0 && pendingHead < pendingCores.length) drain();
}
__name(exitWave, "exitWave");
...
__name(dropBoundaryTasksForBatch, "dropBoundaryTasksForBatch");
function drain() {
  let escaped = null;
...
__name(currentBatch, "currentBatch");
function currentBoundaryBatchToken() {
  return active ?? boundaryOwner ?? void 0;
}
__name(currentBoundaryBatchToken, "currentBoundaryBatchToken");
...
__name(deferToBatch, "deferToBatch");
...
__name(deferAfterBatchForTarget, "deferAfterBatchForTarget");
function commit(b) {
...
__name(nodeRuntimeHost, "nodeRuntimeHost");
...
__name(assertStrictJsonNumber, "assertStrictJsonNumber");
...
__name(sortedJsonValue, "sortedJsonValue");
function stableJsonString(value) {
  return JSON.stringify(sortedJsonValue(value));
}
__name(stableJsonString, "stableJsonString");
function strictStableJsonString(value) {
  return JSON.stringify(cloneStrictJsonValue(value));
}
__name(strictStableJsonString, "strictStableJsonString");
...
__name(jsonCodecFor, "jsonCodecFor");
...
__name(hasUnpairedSurrogate, "hasUnpairedSurrogate");
...
__name(assertNoUnpairedSurrogates, "assertNoUnpairedSurrogates");
...
__name(strictJsonDataErrorsInner, "strictJsonDataErrorsInner");
...
__name(cloneStrictJsonValue, "cloneStrictJsonValue");
...
__name(cloneStrictJsonObject, "cloneStrictJsonObject");
function assertNoDuplicateJsonObjectKeys(text2) {
  ...
  function readJsonString() {
    ...
    while (index < text2.length) {
      ...
      if (ch === '"') {
        index += 1;
        try {
          return JSON.parse(text2.slice(start2, index));
        } catch {
  ...
  __name(readJsonString, "readJsonString");
  ...
  __name(consumeLiteral, "consumeLiteral");
...
__name(assertNoDuplicateJsonObjectKeys, "assertNoDuplicateJsonObjectKeys");
...
__name(assertNodeVersionDataCompatible, "assertNodeVersionDataCompatible");
...
__name(snapshotNodeVersionData, "snapshotNodeVersionData");
...
__name(resolveNodeVersioningPolicy, "resolveNodeVersioningPolicy");
...
__name(createNodeVersion, "createNodeVersion");
...
__name(advanceNodeVersion, "advanceNodeVersion");
...
__name(cloneNodeVersion, "cloneNodeVersion");
...
__name(setRuntimeReleaseFailures, "setRuntimeReleaseFailures");
function runtimeReleaseFailuresOfNode(node) {
  const record = registrations.get(node);
  return record?.kind === "retired" ? record.failures : void 0;
}
__name(runtimeReleaseFailuresOfNode, "runtimeReleaseFailuresOfNode");
function nodeBackendContributor(node) {
  return liveRegistration(node)?.backendContributor;
}
__name(nodeBackendContributor, "nodeBackendContributor");
...
__name(withNodeCore, "withNodeCore");
function takeConstructingNodeCore() {
  const core = constructingCore;
  constructingCore = void 0;
  return core;
}
__name(takeConstructingNodeCore, "takeConstructingNodeCore");
...
__name(nodeOwnerForGraphUse, "nodeOwnerForGraphUse");
...
__name(setNodeOwner, "setNodeOwner");
...
__name(setNodeTopologyDepsChangedObserver, "setNodeTopologyDepsChangedObserver");
function notifyTopologyDepsChanged(node, prevDeps, deps) {
  liveRegistration(node)?.graphAttachment?.observer?.(node, prevDeps, deps);
}
__name(notifyTopologyDepsChanged, "notifyTopologyDepsChanged");
...
__name(cleanupNodeAcquisition, "cleanupNodeAcquisition");
...
__name(failNodeAcquisition, "failNodeAcquisition");
var ColdNodeAcquisitionError = class extends Error {
  static {
    __name(this, "ColdNodeAcquisitionError");
  }
  cleanupErrors;
...
var dispatcherHandleStatKey = /* @__PURE__ */ __name((h) => JSON.stringify([String(h.poolId), String(h.handleId)]), "dispatcherHandleStatKey");
...
var EnvironmentDrivers = class _EnvironmentDrivers {
  static {
    __name(this, "EnvironmentDrivers");
  }
  process;
  http;
  sse;
...
__name(normalizePullDemand, "normalizePullDemand");
...
__name(validateDownPayloads, "validateDownPayloads");
...
__name(nodeBuildCtx, "nodeBuildCtx");
function nodeMakeCtx(self, snapshot) {
  const ctx = {
    ...
    up: /* @__PURE__ */ __name((msgs, towardDep) => {
    ...
    onDeactivation: /* @__PURE__ */ __name((fn) => {
      if (self._released) return;
      self._hooks.onDeactivation.push(fn);
    }, "onDeactivation"),
    onInvalidate: /* @__PURE__ */ __name((fn) => {
      if (self._released) return;
      self._hooks.onInvalidate.push(fn);
    }, "onInvalidate"),
    environment: /* @__PURE__ */ __name(() => self._slot.environment, "environment"),
    // R-rewire-deferred (D47): defer a self-dep-set mutation to the committed boundary.
    rewireNext: {
      subscribeDep: /* @__PURE__ */ __name((dep, fn) => self._requestRewireNext({ kind: "add", dep, fn }), "subscribeDep"),
      unsubscribeDep: /* @__PURE__ */ __name((dep, fn) => self._requestRewireNext({ kind: "remove", dep, fn }), "unsubscribeDep"),
      replaceDeps: /* @__PURE__ */ __name((deps, fn) => self._requestRewireNext({ kind: "set", deps, fn }), "replaceDeps")
    },
    ...
    upNext: /* @__PURE__ */ __name((msgs, towardDep) => self._requestUpNext(msgs, towardDep), "upNext"),
    ...
    [CTX_NODE_BINDING]: {
      ...
      create: /* @__PURE__ */ __name((factory) => withEnvironmentDrivers(self._slot.environment, () => withNodeCore(self._core, factory)), "create")
    }
  };
...
__name(nodeMakeCtx, "nodeMakeCtx");
...
__name(nodeRefreshCtx, "nodeRefreshCtx");
function nodeMakeState(self) {
  return {
    get: /* @__PURE__ */ __name(() => self._privateState.value, "get"),
    set: /* @__PURE__ */ __name((v) => {
      self._privateState.value = v;
    }, "set"),
    persist: /* @__PURE__ */ __name((on = true) => {
      self._privateState.persist = on;
    }, "persist")
  };
}
__name(nodeMakeState, "nodeMakeState");
...
__name(nodeRecordDepProjection, "nodeRecordDepProjection");
...
__name(nodeDepProjectionHasData, "nodeDepProjectionHasData");
...
__name(nodeReceiveFromDep, "nodeReceiveFromDep");
...
__name(nodeReleaseDepDirty, "nodeReleaseDepDirty");
...
__name(nodeSettleAfterAbsorbedTerminal, "nodeSettleAfterAbsorbedTerminal");
...
__name(nodeMarkDirty, "nodeMarkDirty");
...
__name(nodeMaybeRun, "nodeMaybeRun");
...
__name(nodeSettleRewire, "nodeSettleRewire");
...
__name(nodeAllDepsSettled, "nodeAllDepsSettled");
...
__name(nodePassthroughEmit, "nodePassthroughEmit");
...
__name(nodeRunWave, "nodeRunWave");
...
__name(nodeActivate, "nodeActivate");
function nodeSubscribeDepAt(self, depNode, opts = {}) {
  ...
  const sink = /* @__PURE__ */ __name((msg, delivery) => {
  ...
  nodeRuntimeHost(depNode)._subscribeOwned(sink, {
    record: /* @__PURE__ */ __name((release) => {
      if (idx0 !== -1) {
...
__name(nodeSubscribeDepAt, "nodeSubscribeDepAt");
...
__name(nodeSeedRestoredDepAt, "nodeSeedRestoredDepAt");
...
__name(nodeDeactivate, "nodeDeactivate");
function nodeSubscriberCount(self) {
  return self._lifecycle.subscribers.size;
}
__name(nodeSubscriberCount, "nodeSubscriberCount");
...
__name(nodeIsRuntimeQuiescentForRelease, "nodeIsRuntimeQuiescentForRelease");
function nodeReleaseRuntime(self) {
  if (self._released) return;
  self._released = true;
  const node = self;
  const releaseErrors = [];
  const recordReleaseError = /* @__PURE__ */ __name((error, resource2, handle) => {
    releaseErrors.push({
      cause: error,
      resource: resource2,
...
__name(nodeReleaseRuntime, "nodeReleaseRuntime");
...
__name(nodeResetDepState, "nodeResetDepState");
...
function nodeDown(self, msgs) {
  ...
  const deliveryWave = {};
  const assertVersionDataCompatible = /* @__PURE__ */ __name((wave) => {
    for (const m of wave) {
      if (m[0] === "DATA") assertNodeVersionDataCompatible(self._version.policy, m[1]);
    }
  }, "assertVersionDataCompatible");
  const snapshotVersionData = /* @__PURE__ */ __name((wave) => wave.map(
...
__name(nodeMarkDemandRouted, "nodeMarkDemandRouted");
...
__name(nodePauseRelease, "nodePauseRelease");
...
__name(nodeDeliverPullDemand, "nodeDeliverPullDemand");
...
__name(nodeOnDemand, "nodeOnDemand");
...
__name(nodeFirePullDemand, "nodeFirePullDemand");
...
__name(nodeFireOwedDemandIfReady, "nodeFireOwedDemandIfReady");
...
__name(nodeShouldBufferOnPause, "nodeShouldBufferOnPause");
...
__name(nodeInvalidate, "nodeInvalidate");
...
__name(nodeAllDepsTerminal, "nodeAllDepsTerminal");
...
__name(nodeRollbackBatched, "nodeRollbackBatched");
function nodeDeferBoundary(self, fn, batchToken) {
  deferRewire(self._core, fn, {
    batchToken,
    isReady: /* @__PURE__ */ __name(() => !self._hasBoundaryPauseLock(), "isReady")
  });
}
__name(nodeDeferBoundary, "nodeDeferBoundary");
...
__name(nodeRequestRewireNext, "nodeRequestRewireNext");
...
__name(nodeRequestUpNext, "nodeRequestUpNext");
...
__name(nodeApplyRewireNext, "nodeApplyRewireNext");
...
__name(validateNodeRewire, "validateNodeRewire");
...
__name(toCheckpointJson, "toCheckpointJson");
...
__name(checkpointValue, "checkpointValue");
...
__name(checkpointTerminal, "checkpointTerminal");
...
__name(checkpointBackendStateOfNode, "checkpointBackendStateOfNode");
...
__name(initNodeWithCore, "initNodeWithCore");
...
__name(makeInitNode, "makeInitNode");
...
__name(operatorNodeFn, "operatorNodeFn");
function merge() {
  return {
    factory: "merge",
    opts: { partial: true },
    body: /* @__PURE__ */ __name((ctx) => {
...
var ColdConstructionError = class extends Error {
  ...
  cleanupErrors;
  static {
    __name(this, "ColdConstructionError");
  }
};
...
__name(stableBoundary, "stableBoundary");
function checkInputs(registrar, inputs2) {
  const seen = /* @__PURE__ */ new Set();
  const pending = [...inputs2];
  while (pending.length > 0) {
    const node = pending.pop();
    if (seen.has(node)) continue;
    seen.add(node);
    registrar.assertRegisteredNode(node, "construction input/dependency");
    const host2 = nodeRuntimeHost(node);
    if (host2._value.terminal !== void 0 && !host2._slot.resubscribable)
      throw new Error("construction input is terminal and non-resubscribable");
    pending.push(...node.deps);
  }
}
__name(checkInputs, "checkInputs");
function prepareConstruction(graph2, manifest) {
  stableBoundary();
  const registrar = graphRegistrations.get(graph2);
...
__name(prepareConstruction, "prepareConstruction");
var ConstructionScope = class {
  ...
  assertContext(graph2, startup, epoch3) {
    if (this.phase !== "cold" || graphRegistrations.get(graph2) !== this.registrar || startup !== this.startupNode || epoch3 !== this.manifest.epoch)
  ...
  abort(cause) {
    ...
    this.phase = "aborted";
    const errors = [];
    const attempt = /* @__PURE__ */ __name((resource2, cleanup, locator = {}) => {
      try {
        cleanup();
      } catch (error) {
        errors.push({ ...locator, resource: resource2, cause: error });
      }
    }, "attempt");
...
function startConstruction(graph2, owner) {
  stableBoundary();
  if (graphRegistrations.get(graph2)?.existingConstructions?.get(owner.instance) !== owner || owner.phase !== "owned")
...
__name(startConstruction, "startConstruction");
...
__name(causalBinding, "causalBinding");
function assertCausalCapabilities(graph2, full, binding2) {
  const root = issued.get(full);
  if (root === void 0 || root.graph !== graph2 || root.level !== "full" || root.binding.contract !== binding2.contract || root.binding.implementationRevision !== binding2.implementationRevision || root.binding.scope !== binding2.scope || root.binding.epoch !== binding2.epoch)
  ...
  for (const handle of [full.identity, full.execution, full.retained]) {
    ...
    if (record === void 0 || record.instance !== root.instance || record.graph !== graph2)
...
__name(assertCausalCapabilities, "assertCausalCapabilities");
function createCausalCapabilities(graph2, scope, name, ports, binding2) {
  ...
  for (const [level, handle] of [
    ...
    issued.set(handle, { graph: graph2, instance, binding: binding2, level });
  return full;
}
__name(createCausalCapabilities, "createCausalCapabilities");
...
__name(prepareCommittedEffectsView, "prepareCommittedEffectsView");

// packages/ts/src/identity.ts
function canonicalTupleKey(parts) {
  return JSON.stringify(parts);
}
__name(canonicalTupleKey, "canonicalTupleKey");
...
__name(causalOccurrenceDigest, "causalOccurrenceDigest");
function canonicalSnapshot(value) {
  const freeze2 = /* @__PURE__ */ __name((item) => {
...
__name(canonicalSnapshot, "canonicalSnapshot");
...
__name(canonicalEntry, "canonicalEntry");
...
__name(validSourceRef, "validSourceRef");
...
__name(validRef, "validRef");
function validAdmissionState(value) {
  return value === "admitted" || value === "rejected";
}
__name(validAdmissionState, "validAdmissionState");
...
__name(validOutcomeState, "validOutcomeState");
...
__name(validTerminal, "validTerminal");
function sameRef(left, right) {
  return refKey(left) === refKey(right);
}
__name(sameRef, "sameRef");
...
__name(validResult, "validResult");
function pushIssue(outputs, value) {
  outputs.push({ kind: "issue", value });
}
__name(pushIssue, "pushIssue");
var retainPending = /* @__PURE__ */ __name((opts, outputs, map2, key, value, label) => {
...
__name(exactOccurrence, "exactOccurrence");
...
__name(rejectDefinitiveMissingRef, "rejectDefinitiveMissingRef");
...
__name(maybeRelease, "maybeRelease");
...
__name(exactAdmission, "exactAdmission");
...
__name(currentnessMetadata, "currentnessMetadata");
...
__name(currentnessChanged, "currentnessChanged");
...
__name(recomputeCurrentness, "recomputeCurrentness");
...
__name(flushAdmissions, "flushAdmissions");
...
__name(receiveAdmissions, "receiveAdmissions");
...
__name(receiveWatermarks, "receiveWatermarks");
...
__name(emitCoverage, "emitCoverage");
...
__name(retainEvidence, "retainEvidence");
...
__name(flushEvidence, "flushEvidence");
...
__name(receiveEvidence, "receiveEvidence");
...
__name(isEvidenceTerminal, "isEvidenceTerminal");
...
__name(settledForEviction, "settledForEviction");
function emitConservation(context, occurrence3) {
  ...
  const count = /* @__PURE__ */ __name((value) => admitted.filter((record) => record.outcome?.state === value).length, "count");
  outputs.push({
    kind: "conservation",
    value: {
...
__name(emitConservation, "emitConservation");
...
__name(retainEffectProposal, "retainEffectProposal");
...
__name(flushEffectsAndTerminals, "flushEffectsAndTerminals");
...
__name(receiveTerminals, "receiveTerminals");
...
__name(receiveProposals, "receiveProposals");
...
__name(receiveEffectAdmissions, "receiveEffectAdmissions");
...
__name(receiveOutcomes, "receiveOutcomes");
...
__name(pendingObligations, "pendingObligations");
...
function transitionCausalAuthority(prior, arrivals2, opts) {
  ...
  const outputs = [];
  const emitIssue = /* @__PURE__ */ __name((value) => pushIssue(outputs, value), "emitIssue");
  const context = { state, opts, outputs, committedViewChanged: false };
  const acceptOccurrence = /* @__PURE__ */ __name((entry) => {
  ...
  const receiveOccurrences = /* @__PURE__ */ __name((arrival) => {
  ...
  const recomputeDomain = /* @__PURE__ */ __name((revisionDomain) => {
  ...
  const pendingWorkCount = /* @__PURE__ */ __name(() => state.admissions.size + state.pending.size + state.pendingTerminals.size + state.pendingEffectProposals.size + state.pendingEffectAdmissions.size + state.pendingEffectOutcomes.size + state.pendingEvidence.size, "pendingWorkCount");
  const refreshProjections = /* @__PURE__ */ __name(() => {
...
__name(transitionCausalAuthority, "transitionCausalAuthority");
...
__name(causalColdNodeNames, "causalColdNodeNames");
var PreparedCausalOptions = class {
  static {
    __name(this, "PreparedCausalOptions");
  }
  #options;
  constructor(options) {
...
__name(prepareCausalOptions, "prepareCausalOptions");
function lane(graph2, input, name, laneName) {
  return graph2.node(
    [input],
    (ctx) => {
...
__name(lane, "lane");
function projectFact(graph2, authority, name, kind, replayBuffer) {
  return graph2.node(
    [authority],
    (ctx) => {
...
__name(projectFact, "projectFact");
function buildCausalNodes(ownerGraph, graph2, startup, preparedOptions, binding2) {
  binding2 = causalBinding(binding2);
  graph2.assertContext(ownerGraph, startup, binding2.epoch);
  const opts = preparedOptions.options;
  const arrivals2 = graph2.initNode(
    merge(),
    [
      lane(graph2, opts.occurrences, `${opts.name}/input/occurrences`, "occurrences"),
      lane(graph2, opts.admissions, `${opts.name}/input/admissions`, "admissions"),
      lane(graph2, opts.branchTerminals, `${opts.name}/input/branch-terminals`, "branch-terminals"),
      lane(graph2, opts.effectProposals, `${opts.name}/input/effect-proposals`, "effect-proposals"),
      lane(
        graph2,
      ...
      lane(graph2, opts.effectOutcomes, `${opts.name}/input/effect-outcomes`, "effect-outcomes"),
      lane(graph2, opts.evidence, `${opts.name}/input/evidence`, "evidence"),
      lane(graph2, opts.watermarks, `${opts.name}/input/watermarks`, "watermarks")
    ],
    { name: `${opts.name}/arrivals` }
  );
  const authority = graph2.node(
    [arrivals2],
    (ctx) => {
  ...
  const releaseEvents = graph2.initNode(merge(), [released, rawRelease], {
    name: `${opts.name}/release-events`
  });
  const releaseController = graph2.node(
    [releaseEvents],
    (ctx) => {
  ...
  const result = {
    ...
    coverage: projectFact(graph2, authority, `${opts.name}/coverage`, "coverage", opts.maxEvidence),
    ...
    issues: projectFact(graph2, authority, `${opts.name}/issues`, "issue", opts.maxPending)
  };
  const committedEffects = graph2.node(
    [authority],
    (ctx) => {
  ...
  assertCausalOccurrenceTopology(graph2.readIncoming(), opts.name);
  const full = createCausalCapabilities(ownerGraph, graph2, opts.name, result, binding2);
...
__name(buildCausalNodes, "buildCausalNodes");
function causalOccurrenceRequiredEdges(name, description) {
  ...
  const sources2 = description === void 0 ? [] : inputLanes.flatMap((laneName) => {
    const target = `${name}/input/${laneName}`;
    return description.edges.filter((edge) => edge.to === target);
  });
  return Object.freeze([...sources2, ...internal]);
}
__name(causalOccurrenceRequiredEdges, "causalOccurrenceRequiredEdges");
function assertCausalOccurrenceTopology(description, name) {
  const incoming = /* @__PURE__ */ new Map();
  description.edges.forEach((edge) => {
    let sources2 = incoming.get(edge.to);
    if (sources2 === void 0) {
      sources2 = /* @__PURE__ */ new Set();
      incoming.set(edge.to, sources2);
    }
    sources2.add(edge.from);
  });
...
__name(assertCausalOccurrenceTopology, "assertCausalOccurrenceTopology");
...
function keys(value, expected2) {
  if (value === null || typeof value !== "object" || Array.isArray(value) || Object.keys(value).sort().join(",") !== expected2.split(",").sort().join(","))
...
__name(sameBinding, "sameBinding");
...
__name(proposalForMaterial, "proposalForMaterial");
...
__name(makeRequestMaterial, "makeRequestMaterial");
...
__name(makeMaterialSnapshot, "makeMaterialSnapshot");
...
__name(associationKey, "associationKey");
function validateMaterialSnapshot(raw, expected2) {
  ...
  try {
    ...
    if (!sameBinding(frame2.body, expected2) || frame2.body.packRef.kind !== expected2.packRef.kind || frame2.body.packRef.id !== expected2.packRef.id)
...
__name(validateMaterialSnapshot, "validateMaterialSnapshot");
...
__name(publicationFor, "publicationFor");
function buildSpendingPublication(ownerGraph, scope, startup, prepared, binding2, materialSource, expected2) {
  const asOf = freeze(JSON.parse(canonicalMaterial(expected2)));
...
__name(buildSpendingPublication, "buildSpendingPublication");
...
__name(inputDigest, "inputDigest");
...
__name(materialProfile, "materialProfile");
...
function checkInput(kind, raw, expected2) {
  try {
    ...
    if (kind === "arrivals") {
      ...
      if (!same(v.packRef, expected2.packRef)) throw new TypeError("pack-ref");
    } else {
      const names = {
      ...
      keys2(v, names[kind]);
      binding(v.binding);
      if (!same(v.binding, expected2)) throw new TypeError("binding");
      if (kind === "pack") {
...
__name(dyadic, "dyadic");
var width = /* @__PURE__ */ __name((x) => x.toString(2).length, "width");
function exponentOfRatio(a, b) {
  let e = width(a) - width(b);
  if (e >= 0 ? a < b << BigInt(e) : a << BigInt(-e) < b) e--;
  return e;
}
__name(exponentOfRatio, "exponentOfRatio");
...
__name(integerSqrt, "integerSqrt");
...
__name(exactVendorStats, "exactVendorStats");
...
__name(exactZScore, "exactZScore");
...
function nodeMaker(scope, prefix, edges) {
  return (name, deps, fn) => {
    const expected2 = Object.freeze([...deps]);
    const node = scope.node(
      expected2,
      (ctx) => {
        if (node.deps.length !== expected2.length || node.deps.some((d, i) => d !== expected2[i])) {
    ...
    edges.set(node, expected2);
    return node;
  };
}
__name(nodeMaker, "nodeMaker");
...
function buildBusiness(make, inputs2, binding2) {
  const evaluationSelections = make(
    "evaluationSelections",
    [inputs2.evaluations.pack, inputs2.evaluations.arrivals],
    (ctx) => {
      let state = ctx.state.get();
      if (!state) {
      ...
      const emitPending = /* @__PURE__ */ __name(() => {
...
__name(buildBusiness, "buildBusiness");
...
function buildAdmission(make, inputs2, binding2, business, materials) {
  const currentFacts = checked(
    make,
    "currentFacts",
    inputs2.evaluations.current,
    "current",
    binding2
  );
  ...
  const publicationPolicy = make(
    ...
    (ctx) => {
      ...
      let receiptConflict;
      let comparisonTexts;
      const comparisonKey = /* @__PURE__ */ __name((value) => {
      ...
      const sameInInvocation = /* @__PURE__ */ __name((left, right) => comparisonKey(left) === comparisonKey(right), "sameInInvocation");
  ...
  const occurrences = make(
    ...
    (ctx) => {
      const outputs = [];
      const emit = /* @__PURE__ */ __name((messages) => outputs.push(...messages), "emit");
...
__name(buildMaterials, "buildMaterials");
...
__name(spendingNodeNames, "spendingNodeNames");
function spendingInputNodes(inputs2) {
  if (!inputs2 || Object.keys(inputs2).sort().join(",") !== "evaluations,inbox,localAuthority,verification" || Object.keys(inputs2.evaluations).sort().join(",") !== "arrivals,current,pack" || Object.keys(inputs2.verification).join(",") !== "receipts" || Object.keys(inputs2.localAuthority).join(",") !== "facts" || Object.keys(inputs2.inbox).join(",") !== "facts")
    throw new TypeError("spending requires four exact input groups");
  const nodes = [
    inputs2.evaluations.pack,
    inputs2.evaluations.arrivals,
    inputs2.evaluations.current,
    inputs2.verification.receipts,
    inputs2.localAuthority.facts,
    inputs2.inbox.facts
...
__name(spendingInputNodes, "spendingInputNodes");
function buildSpendingPresetNodes(ownerGraph, scope, startup, inputs2, rawBinding, name, diagnostics = "off") {
  if (diagnostics !== "off" && diagnostics !== "summary")
    throw new TypeError("spending diagnostics");
  const binding2 = validateBinding(rawBinding);
  spendingInputNodes(inputs2);
  ...
  const business = buildBusiness(make, inputs2, binding2), materials = buildMaterials(make, business, binding2), admission = buildAdmission(make, inputs2, binding2, business, materials);
  ...
  for (const [node, expected2] of edges)
    if (node.deps.length !== expected2.length || node.deps.some((dep, i) => dep !== expected2[i]))
...
__name(buildSpendingPresetNodes, "buildSpendingPresetNodes");
...
var OfflineAlertResource = class {
  ...
  writeBytes;
  static {
    __name(this, "OfflineAlertResource");
  }
...
function composeOfflineSpending(graph2, inputs2, rawBinding, resource2, options) {
  const binding2 = validateBinding(rawBinding), name = options.name;
  if (!(resource2 instanceof OfflineAlertResource) || resource2.kind !== "offline-alert-resource" || !same(resource2.binding, binding2))
    throw new TypeError("offline resource binding");
  const scope = prepareConstruction(graph2, {
    name,
    epoch: binding2.compositionEpoch,
    names: [
  ...
  try {
    ...
    lease.transfer();
    startConstruction(graph2, owner);
...
__name(composeOfflineSpending, "composeOfflineSpending");
function buildOfflineSpendingHost(graph2, scope, startup, inputs2, binding2, lease, options) {
  scope.assertContext(graph2, startup, binding2.compositionEpoch);
  ...
  const frame2 = /* @__PURE__ */ __name(() => frozen({
    binding: binding2,
  ...
  const complete = /* @__PURE__ */ __name((r, state, code) => {
    if (r.outcome) return;
  ...
  const packFacts = scope.node(
    [inputs2.evaluations.pack],
    (ctx) => {
      if (packFacts.deps.length !== 1 || packFacts.deps[0] !== inputs2.evaluations.pack || depLatest(ctx, 0) === void 0) {
        ctx.down([
          [
            "DATA",
            {
  ...
  const guard = scope.node(
    ...
    (ctx) => {
      ...
      if (view.authorityId !== `${name}/causal/authority` || view.binding.epoch !== binding2.compositionEpoch || !same(material.body.packRef, binding2.packRef)) {
        fault = "authority-binding";
        return;
      }
      const checked2 = /* @__PURE__ */ __name((index) => {
      ...
      for (const effect of view.effects) {
        ...
        const request = material.body.materials.find(
          (m) => same(proposalForMaterial(m), effect.proposal)
        );
        if (!request || !same(
          {
        ...
        if (!same(
          ...
          material.body
        ) || body.hostEpoch !== binding2.hostEpoch || body.compositionEpoch !== binding2.compositionEpoch || body.sourceDigest !== binding2.sourceDigest || body.runtimeDigest !== binding2.runtimeDigest || !same(body.destinationRef, binding2.destinationRef))
          continue;
        ...
        const currents = current?.current.filter((c) => same(c.occurrence, effect.proposal.occurrence)) ?? [];
        const receipts = verification?.receipts.filter(
          (v) => same(v.occurrence, body.occurrence) && v.inputDigest === body.inputDigest && v.policyDigest === body.policyDigest && v.sourceDigest === body.sourceDigest && v.runtimeDigest === body.runtimeDigest && v.requestDigest === body.payloadDigest && v.verifierRevision === VERIFIER_REVISION && v.numericDomainRef === NUMERIC_DOMAIN
        ) ?? [];
        const grants = local?.grants.filter(
          (g) => same(g.occurrence, body.occurrence) && g.requestDigest === body.payloadDigest && same(g.destinationRef, body.destinationRef) && g.hostEpoch === body.hostEpoch && g.replayScope.compositionEpoch === body.compositionEpoch && g.replayScope.hostEpoch === body.hostEpoch
        ) ?? [];
        const evaluation2 = pack2?.valid ? pack2.value.evaluations.find((e) => same(e.occurrence, body.occurrence)) : void 0;
        const grant2 = grants[0], receipt2 = receipts[0], now = local?.tick;
        const denied = !local || local.stop || now === void 0 || currents.length !== 1 || currents[0].policyDigest !== body.policyDigest || currents[0].watermark < body.occurrence.revision || !evaluation2 || !same(currents[0].policyRef, evaluation2.policyRef) || receipts.length !== 1 || receipt2.verdict !== "pass" || grants.length !== 1 || grant2.revoked || admission.admissionRef.kind !== "spending-admission" || admission.admissionRef.id !== hash({
...
`;
  ...
  return Object.freeze({
    ...
    afterStart: /* @__PURE__ */ __name((owner) => {
    ...
    runEndReady,
    built,
    inspect: /* @__PURE__ */ __name(() => Object.freeze({
      fault,
...
__name(buildOfflineSpendingHost, "buildOfflineSpendingHost");
...
__name(normalizeTopologySnapshot, "normalizeTopologySnapshot");
...
__name(topologyString, "topologyString");
...
__name(graphBlueprintDiagnostics, "graphBlueprintDiagnostics");
...
__name(deriveEdges, "deriveEdges");
function labelForNode(id) {
  return id === "" ? "node" : `node '${id}'`;
}
__name(labelForNode, "labelForNode");
...
__name(topologyFromDescribe, "topologyFromDescribe");
function cloneTopologyMeta(meta) {
  return cloneTopologyValue(meta, /* @__PURE__ */ new WeakMap());
}
__name(cloneTopologyMeta, "cloneTopologyMeta");
...
__name(cloneTopologyValue, "cloneTopologyValue");
...
__name(isNonAuthoritativeCollectionHelperMeta, "isNonAuthoritativeCollectionHelperMeta");
...
__name(assertCheckpointQuiescentStatus, "assertCheckpointQuiescentStatus");
function topologyPathMatches(eventPath, path) {
  return eventPath === path || eventPath.startsWith(`${path}::`);
}
__name(topologyPathMatches, "topologyPathMatches");
function prefixTopologyPath(prefix, path) {
  return `${prefix}::${path}`;
}
__name(prefixTopologyPath, "prefixTopologyPath");
...
__name(cloneTopologyEvent, "cloneTopologyEvent");
...
function explainSubset(snap, chain) {
  ...
  const reach = /* @__PURE__ */ __name((start2, adj) => {
...
__name(assertGraphLocalNode, "assertGraphLocalNode");
...
var Graph = class {
  ...
  derived(deps, fn, opts = {}) {
    ...
    const ctxFn = /* @__PURE__ */ __name((ctx) => {
      try {
  ...
  _describe(_prefix, incoming) {
    ...
    const localId = /* @__PURE__ */ __name((n) => {
  ...
  observe(path) {
    ...
    return {
      subscribe: /* @__PURE__ */ __name((sink) => {
...
__name(emitRegisteredDepsChanged, "emitRegisteredDepsChanged");
...
__name(unitsFromBits, "unitsFromBits");
var units = /* @__PURE__ */ __name((x) => unitsFromBits(bits(x)), "units");
var unitDenominator = 1n << 1074n;
function referenceRound(numerator, denominator, squareRoot) {
  if (numerator === 0n) return 0;
  const compare = /* @__PURE__ */ __name((u, divisor = 1n) => squareRoot ? u * u * denominator - numerator * (unitDenominator * divisor) ** 2n : u * denominator - numerator * unitDenominator * divisor, "compare");
...
__name(referenceRound, "referenceRound");
...
__name(referenceNumbers, "referenceNumbers");
...
__name(referenceFixed, "referenceFixed");
...
__name(oracleEncoding, "oracleEncoding");
function oracleCanonical(input, maxBytes = 1048576) {
  return oracleEncoding(input, maxBytes).text;
}
__name(oracleCanonical, "oracleCanonical");
var oracleHash = /* @__PURE__ */ __name((s) => `sha256:${createHash2("sha256").update(s).digest("hex")}`, "oracleHash");
...
__name(oracleFreeze, "oracleFreeze");
...
__name(oracleMaterial, "oracleMaterial");
...
__name(oracleBusiness, "oracleBusiness");
...
__name(oracleRequest, "oracleRequest");
...
function evaluationFixture(index = 0, vendor = "coffee", prefixLength = 2, flagged = true) {
  ...
  const policy2 = { zThreshold: flagged ? 0.5 : 100, dailyRatioThreshold: 100 }, profile4 = { dailyAverage: 100, typicalCategories: ["coffee"] }, profileRef = { kind: "profile", id: "base" }, policyRef = { kind: "policy", id: flagged ? "alert" : "normal" };
  const value = {
    evaluationRef: id,
    subjectRef: id,
    inputDigest: oracleHash(oracleCanonical({ profileRef, profile: profile4, prefix })),
    profileRef,
    profile: profile4,
    policyRef,
...
__name(evaluationFixture, "evaluationFixture");
...
__name(evaluationPack, "evaluationPack");
...
__name(policyFacts, "policyFacts");
...
__name(drain2, "drain");
function inputsFor(graph2) {
  const sources2 = {
    pack: graph2.node([], null, { name: "pack" }),
    arrivals: graph2.node([], null, { name: "arrivals" }),
    current: graph2.node([], null, { name: "current" }),
    verification: graph2.node([], null, { name: "verification" }),
    local: graph2.node([], null, { name: "local" })
  };
  return {
    sources: sources2,
    inputs: {
      evaluations: { pack: sources2.pack, arrivals: sources2.arrivals, current: sources2.current },
      verification: { receipts: sources2.verification },
      localAuthority: { facts: sources2.local }
    }
  };
}
__name(inputsFor, "inputsFor");

// spending-host-profile-worker.ts
var mode = process.argv[2];
var window = process.argv[3];
var output = process.argv[4];
var start = window === "first8" ? 0 : 56;
var end = window === "first8" ? 7 : 63;
...
var expected = evaluations.map((e) => oracleRequest(e, presetBinding).body.payloadText + "\n");
var arrivals = evaluations.map((e) => ({ packRef: presetBinding.packRef, evaluationRefs: [e.evaluationRef] }));
var graph = new Graph({ name: "spending-lifetime-profile" });
var { sources, inputs } = inputsFor(graph);
var calls = [];
var resource = new OfflineAlertResource(presetBinding, async (payload) => {
  calls.push(payload);
  return { bytesWritten: Buffer.byteLength(payload) };
});
var host = composeOfflineSpending(graph, inputs, presetBinding, resource, { name: "spending", diagnostics: mode });
var stops = Object.values(host.consume.view).map((node) => node.subscribe(() => {
}));
var send = /* @__PURE__ */ __name((lane2, value) => sources[lane2].down([["DATA", value]]), "send");
var session = new Session();
session.connect();
var post = /* @__PURE__ */ __name((method, params = {}) => new Promise((yes, no) => session.post(method, params, (error, value) => error ? no(error) : yes(value))), "post");
var profile3;
try {
  ...
  for (let i = 0; i < 64; i++) {
    ...
    await drain2();
    if (i === end) {
      profile3 = (await post("Profiler.stop")).profile;
      writeFileSync(output, JSON.stringify(profile3));
    }
  }
  assert.deepEqual(calls, expected);
  const beforeStop = host.inspect();
  assert.equal(beforeStop.fault, void 0);
  assert.equal(beforeStop.records.length, 64);
  assert.ok(beforeStop.records.every((r) => r.outcome?.state === "succeeded"));
