...
def run(root):
 ...
 assert (root/'qualification.log').read_text().find('POSITION_LOADED_QUALIFICATION_PASS')>=0,'loaded qualification sentinel'
 ...
 try:
  for cell in cells:
   ...
   argv=[node,*c.FLAGS,str(job/'entry.mjs')];put(job/'dispatch.json',dict(**cell,argv=argv,startMonotonic=t,startWall=wall,wakeBefore=w))
   try:
    ...
    record=dict(**cell,pid=None if p is None else p.pid,exitCode=None if p is None else p.returncode,startMonotonic=t,endMonotonic=time.monotonic(),startWall=wall,endWall=time.time(),wakeBefore=w,wakeAfter=after,childError=failure,cleanupErrors=cleanup,wakeError=wake_error)
