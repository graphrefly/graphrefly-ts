...
def verify(root):
 ...
 assert f['limits']==dict(processes=168,childSeconds=30,totalSeconds=1800,rssMiB=256,retries=0)
 assert len(f['jobs'])==168
 assert a['complete'] is True and a['failure'] is None and a['dispatched']==168 and a['notRun']==[]
 assert 0<a['elapsed']<=1800 and a['formalQualification'] is False
 ...
 for j,job in enumerate(f['jobs']):
  ...
  assert exit['exitCode']==0 and exit['error'] is None and exit['cleanupErrors']==[] and exit['wakeError'] is None and 0<exit['end']-exit['start']<=30
  ...
  for x in exit['rss']:
   if x['code']==0 and x['stdout'].strip():
    pid,rss,state=x['stdout'].split();assert int(pid)==exit['pid'] and 0<=int(rss)<=262144
