...
def run(root):
 ...
 try:
  for job in f['jobs']:
   ...
   assert time.monotonic()-start<1800,'whole deadline'
   ...
   try:
    with (d/'stdout.log').open('xb') as out,(d/'stderr.log').open('xb') as err:
     ...
     while p.poll() is None:
      assert time.monotonic()-t<30 and time.monotonic()-start<1800,'deadline'
      ...
      if ps.returncode==0 and ps.stdout.strip():
       ...
       assert int(fields[1])<=256*1024,'RSS cap'
   ...
   assert end-t<=30 and end-start<=1800,'post-exit deadline'
