...
console.log(
	`Memory bench — graph size ${GRAPH_SIZE}, ${EMITS.toLocaleString()} emits`,
);
...
async function tsOnlyBench() {
	const { HandleRuntime } = await import(
		"/tmp/handle-core-compiled/bindings.js"
	);
	gcIfAvailable();
	...
	console.log(`  graph build:    heapUsed ${delta(before, afterBuild, "heapUsed")}, rss ${delta(before, afterBuild, "rss")}`);
	console.log(
		`  during emits:   heapUsed ${delta(afterBuild, peak, "heapUsed")} (peak), rss ${delta(afterBuild, peak, "rss")} (peak)`,
	);
	console.log(`  after gc:       heapUsed ${delta(afterBuild, afterGc, "heapUsed")} (retained), rss ${delta(afterBuild, afterGc, "rss")}`);
	console.log(`  elapsed:        ${elapsedMs.toFixed(1)}ms`);
	console.log(`  TOTAL rss growth: ${delta(before, afterGc, "rss")} (V8 heap dominates here — pure JS)`);
	return { elapsedMs, peakHeap: peak.heapUsed - afterBuild.heapUsed, peakRss: peak.rss - afterBuild.rss, totalRss: afterGc.rss - before.rss };
}
...
(async () => {
	...
	if (ts.totalRss > 0 && rust.totalRss > 0) {
		console.log(`    Ratio:        TS holds ${(ts.totalRss / rust.totalRss).toFixed(1)}× more total memory after GC`);
	}
	console.log("");
