 ...
 *
 * Local copy for use in resilience and IO primitives within the presentation layer.
 * Mirrors `@graphrefly/pure-ts` internal `core/_internal/ring-buffer.ts`.
 *
 * @internal
 */
...
export class RingBuffer<T> {
	...
	/** Append an item. If size equals capacity, drops the oldest entry. */
	push(item: T): void {
	...
	/** O(1) index lookup. Negative indices count from the tail. */
