...
describe("core operators (node sugar, D6/L1.5)", () => {
	it("map emits fn(value)", () => {
		const g = graph();
		const s = g.state(1);
		const m = g.map(s, (n) => n * 2);
		const msgs: Message[] = [];
	...
	it("filter emits only when pred holds", () => {
		const g = graph();
		const s = g.state(2);
		const evens = g.filter(s, (n) => n % 2 === 0);
		const msgs: Message[] = [];
	...
	it("scan accumulates with a seed", () => {
		const g = graph();
		const s = g.state(1);
		const sum = g.scan(s, (acc: number, v: number) => acc + v, 0);
		const msgs: Message[] = [];
	...
	it("distinctUntilChanged suppresses repeats", () => {
		const g = graph();
		const s = g.state(1);
		const d = g.distinctUntilChanged(s);
		const msgs: Message[] = [];
	...
	it("describe shows the real operator factory name (D6), not 'derived'", () => {
		...
		g.map(s, (n) => n + 1, { name: "inc" });
		g.filter(s, (n) => n > 0, { name: "pos" });
