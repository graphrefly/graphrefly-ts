...
import {
	persistD754LiveBundle,
	runD754InjectedNoNetworkQualification,
	validateD754LiveBundle,
} from "../../evals/empirical-memory-rerun-avoidance/d754-graph-native-live.js";
import {
	D754_IMPLEMENTATION_MANIFEST_DIGEST,
	type D754_PRIVATE_SOURCE_SHA256,
	type D754_TRACKED_SOURCE_SHA256,
	validateD754ImplementationBytes,
} from "../../evals/empirical-memory-rerun-avoidance/d754-implementation-manifest.js";
import {
	D754_TRANSPORT_EVIDENCE_SCHEMA,
	validateD754TransportDiagnosticGraphEvidence,
} from "../../evals/empirical-memory-rerun-avoidance/d754-transport-diagnostic-route-adapter.js";
...
function admission() {
	...
	return createD733GraphNativeRouteAdmission({
		...
		access: createD733RouteAccessProjection({
			profile,
			observationRevision: "d754.test.v1",
...
async function sourceBytes() {
	...
	const trackedNames: Readonly<Record<keyof typeof D754_TRACKED_SOURCE_SHA256, string>> = {
		...
		transportRouteAdapter: "d754-transport-diagnostic-route-adapter.ts",
		coordinates: "d754-coordinates.ts",
		claim: "d754-single-use-dispatch-claim.ts",
		live: "d754-graph-native-live.ts",
	};
	const privateNames: Readonly<Record<keyof typeof D754_PRIVATE_SOURCE_SHA256, string>> = {
		realRouteAdapter: "d754-private-real-route-adapter.ts",
		noNetworkRunner: "run-d754-no-network-pre-live.ts",
		liveRunner: "run-d754-live.ts",
	};
	return {
		...
		) as Record<keyof typeof D754_TRACKED_SOURCE_SHA256, Uint8Array>,
		...
		) as Record<keyof typeof D754_PRIVATE_SOURCE_SHA256, Uint8Array>,
	};
}

describe("D754 D752-qualified Graph-native live replacement", () => {
	it("runs and atomically persists the complete injected six-arm block", async () => {
		...
		const bundle = await runD754InjectedNoNetworkQualification({
			historicalBundleBytes: await readFile(join(root, "bundle.v1.json")),
			implementationManifestDigest: validateD754ImplementationBytes(await sourceBytes()),
			adapter: fixture.adapter,
			providerTransportCalls: fixture.providerCalls,
			signal: AbortSignal.timeout(30_000),
		});
		expect(validateD754LiveBundle(bundle)).toEqual(bundle);
		...
		const privateRoot = await mkdtemp(join(tmpdir(), "graphrefly-d754-persist-"));
		await chmod(privateRoot, 0o700);
		try {
			const receipt = await persistD754LiveBundle({ privateRoot, bundle });
	...
	it("rejects diagnostic evidence that is not derived from the canonical Graph facts", async () => {
		...
		const integration = await runD754InjectedNoNetworkQualification({
			historicalBundleBytes: await readFile(join(root, "bundle.v1.json")),
			implementationManifestDigest: D754_IMPLEMENTATION_MANIFEST_DIGEST,
			adapter: fixture.adapter,
			providerTransportCalls: fixture.providerCalls,
			signal: AbortSignal.timeout(30_000),
		});
		...
		const forgedMaterial = {
			schemaVersion: D754_TRANSPORT_EVIDENCE_SCHEMA,
			facts: [
				{
					schemaVersion: "graphrefly.b112.d754.graph-admitted-transport-diagnostic-fact.v1",
					runSequence: 0,
					effectSequence: 0,
