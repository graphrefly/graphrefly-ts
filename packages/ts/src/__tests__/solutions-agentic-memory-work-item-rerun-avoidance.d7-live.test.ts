...
} from "../../evals/empirical-memory-rerun-avoidance/d6-current-openrouter-adapter.js";
import {
	persistCurrentGraphLiveBundle,
	runCurrentGraphLiveMeasurement,
	validateCurrentGraphLiveBundle,
} from "../../evals/empirical-memory-rerun-avoidance/d7-current-live.js";
import {
	acquireCurrentGraphLiveDispatchClaimAtRootForTest,
	consumeCurrentGraphLiveDispatchClaim,
} from "../../evals/empirical-memory-rerun-avoidance/d7-current-live-claim.js";
import {
...
} from "../../evals/empirical-memory-rerun-avoidance/d7-current-live-coordinates.js";
import {
	admitCurrentGraphLiveZeroByok,
	composeCurrentGraphLivePreclaim,
	readCurrentGraphLiveOfficialPricing,
} from "../../evals/empirical-memory-rerun-avoidance/d7-current-live-preflight.js";
...
describe("graphrefly-ts:D7 Graph-native live composition", () => {
	it("preserves the same-process constructed bundle identity through live persistence wiring", async () => {
		const repositoryRoot = resolve(import.meta.dirname, "../../../..");
		const runnerSource = await readFile(
			join(repositoryRoot, "packages/ts/evals/empirical-memory-rerun-avoidance/run-d7-live.ts"),
			"utf8",
		);
	...
	it("runs the admitted six-arm path through the real adapter with injected no-network transport", async () => {
		const root = await mkdtemp(join(tmpdir(), "graphrefly-d7-"));
		roots.push(root);
		const repositoryRoot = resolve(import.meta.dirname, "../../../..");
		const credential = {
		...
		const zeroByokBytes = Buffer.from(
			JSON.stringify({
				schemaVersion: "graphrefly-ts.d7.current-graph-live-zero-byok-observation.v1",
				decisionRef: "graphrefly-ts:D7",
		...
		const implementationManifestDigest = empiricalStrictJsonDigest({ fixture: "d7-injected-v1" });
		...
		const claim = await acquireCurrentGraphLiveDispatchClaimAtRootForTest(
			await realpath(privateRoot),
			{
				...
				qualificationArtifactDigest: empiricalStrictJsonDigest({ injected: "d7" }),
				qualificationDigest: empiricalStrictJsonDigest({ injected: "d7-qualification" }),
			},
		);
	...
	it("admits executor failure as partial Graph evidence and never publishes success generation", async () => {
		const root = await mkdtemp(join(tmpdir(), "graphrefly-d7-failure-"));
		roots.push(root);
		const credential = {
		...
		const privateRoot = join(root, "private");
		await mkdir(privateRoot, { mode: 0o700 });
		const implementationManifestDigest = empiricalStrictJsonDigest({ fixture: "d7-failure" });
		const claim = await acquireCurrentGraphLiveDispatchClaimAtRootForTest(
			await realpath(privateRoot),
			{
				preclaim,
