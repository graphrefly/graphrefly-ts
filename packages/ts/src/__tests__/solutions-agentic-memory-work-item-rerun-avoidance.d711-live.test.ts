...
import { createD711CurrentKeyExecutionAdmission } from "../../evals/empirical-memory-rerun-avoidance/d711-current-key-execution-admission.js";
import {
	createD711PreflightCapability,
	D711_APPROVAL_REF,
	D711_APPROVAL_REVISION,
	D711_CLAIM_BOUNDARY,
	D711_PRICING_REVISION,
} from "../../evals/empirical-memory-rerun-avoidance/d711-fresh-pricing-live.js";
import {
	D711_LIVE_ATTEMPT_RECEIPT_FILE,
	persistD711LiveAttemptReceipt,
} from "../../evals/empirical-memory-rerun-avoidance/d711-live-attempt-receipt.js";
import { runD711OrderedLiveAdmissions } from "../../evals/empirical-memory-rerun-avoidance/d711-live-orchestration.js";
import {
	consumeD711OfficialPricingRead,
	readD711OfficialPricing,
	validateD711OfficialPricingRead,
} from "../../evals/empirical-memory-rerun-avoidance/d711-official-pricing-live.js";
import {
	assertD711QualifiedLiveImplementation,
	D711_QUALIFIED_NODE_VERSION,
} from "../../evals/empirical-memory-rerun-avoidance/d711-qualified-live-entrypoint.js";
import {
	acquireD711SingleUseDispatchClaimAtPrivateRoot,
	consumeD711SingleUseDispatchClaim,
	consumePersistedD711DispatchClaimForExecutionAtPrivateRoot,
	D711_LIVE_GENERATION_REF,
	D711_SINGLE_USE_DISPATCH_CLAIM_DIRECTORY,
} from "../../evals/empirical-memory-rerun-avoidance/d711-single-use-dispatch-claim.js";
import {
	consumeD711FreshZeroByokQualification,
	createD711ZeroByokChallenge,
	D711_ZERO_BYOK_ATTESTATION_FILE,
	D711_ZERO_BYOK_ATTESTATION_SCHEMA,
	D711_ZERO_BYOK_QUALIFICATION_REVISION,
	D711_ZERO_BYOK_WORKSPACE_REVISION,
	readD711FreshZeroByokQualification,
} from "../../evals/empirical-memory-rerun-avoidance/d711-zero-byok-qualification.js";
...
async function privateRoot() {
	const container = await mkdtemp(join(tmpdir(), "graphrefly-d711-claim-"));
...
async function currentKeyExecutionAdmission() {
	...
	return createD711CurrentKeyExecutionAdmission({ admission, credential: testCredential });
}

describe("D711 fresh-pricing-separated live authority", () => {
	it("freezes the numeric authority while retaining the immutable v3 route revision", () => {
		expect(D711_APPROVAL_REF).toBe("decision.D711");
		expect(D711_APPROVAL_REVISION).toBe("decision.D711.2026-08-10.v1");
		expect(D711_CLAIM_BOUNDARY).toContain("no-efficacy-claim");
		expect(D711_PRICING_REVISION).toBe(OPENROUTER_DEEPSEEK_V4_FLASH_PRICING_REVISION);
		expect(D711_LIVE_GENERATION_REF).toContain("d711");
	});

	it("keeps the D711 implementation gate immutable after the distinct D712 source delta", async () => {
		...
		await expect(
			assertD711QualifiedLiveImplementation({
				repositoryRoot,
				privateOperatorRoot,
				nodeVersion: D711_QUALIFIED_NODE_VERSION,
			}),
	...
	it("binds one exact official GET to the frozen schedule without exposing response bytes", async () => {
		...
		const read = await readD711OfficialPricing({
		...
		expect(validateD711OfficialPricingRead(structuredClone(read))).toEqual(read);
		expect(JSON.stringify(read)).not.toContain("provider_name");
		expect(JSON.stringify(read)).not.toContain("0.00000009");
		expect(consumeD711OfficialPricingRead(read).match.frozenScheduleRevision).toBe(
			OPENROUTER_DEEPSEEK_V4_FLASH_PRICING_REVISION,
		);
		expect(() => consumeD711OfficialPricingRead(read)).toThrow(/reused/);
	});
	...
	it("rejects accessor and caller-input substitution without invoking the pricing transport", async () => {
		...
		await expect(readD711OfficialPricing(accessor as never)).rejects.toThrow(/data property/);
		...
		const pending = readD711OfficialPricing(input);
	...
	it("atomically grants only one cross-process-shaped D711 claim", async () => {
		const { container, root } = await privateRoot();
		try {
			const contenders = await Promise.allSettled([
				acquireD711SingleUseDispatchClaimAtPrivateRoot(root),
				acquireD711SingleUseDispatchClaimAtPrivateRoot(root),
			]);
			...
			const acquired = contenders.find(
				(
					...
					Awaited<ReturnType<typeof acquireD711SingleUseDispatchClaimAtPrivateRoot>>
				> => result.status === "fulfilled",
			)?.value;
			expect(acquired).toBeDefined();
			expect(() => consumeD711SingleUseDispatchClaim(acquired)).toThrow(/single-use/);
			const persisted = await consumePersistedD711DispatchClaimForExecutionAtPrivateRoot(
				root,
				await currentKeyExecutionAdmission(),
			);
			expect(() => consumeD711SingleUseDispatchClaim(persisted)).not.toThrow();
			await expect(
				consumePersistedD711DispatchClaimForExecutionAtPrivateRoot(
					root,
					await currentKeyExecutionAdmission(),
			...
			const claimPath = join(root, D711_SINGLE_USE_DISPATCH_CLAIM_DIRECTORY);
			...
			await persistD711LiveAttemptReceipt({
				claim: acquired!,
			...
			expect((await stat(join(claimPath, D711_LIVE_ATTEMPT_RECEIPT_FILE))).mode & 0o777).toBe(
				0o600,
			);
		} finally {
	...
	it("consumes one fresh 0600 zero-BYOK observation bound to the credential workspace", async () => {
		const { container, root } = await privateRoot();
		const path = join(root, D711_ZERO_BYOK_ATTESTATION_FILE);
		try {
			const challenge = createD711ZeroByokChallenge({
				credentialBindingRef: "credential.ref",
				credentialBindingRevision: "credential.revision",
			...
			const material = {
				schemaVersion: D711_ZERO_BYOK_ATTESTATION_SCHEMA,
				...
				credentialBindingRevision: "credential.revision",
				workspaceRef: "workspace.ref",
				workspaceRevision: D711_ZERO_BYOK_WORKSPACE_REVISION,
				qualificationRevision: D711_ZERO_BYOK_QUALIFICATION_REVISION,
			};
			await writeFile(
				path,
			...
			const qualification = await readD711FreshZeroByokQualification({
			...
			expect(consumeD711FreshZeroByokQualification(qualification)).toBe(qualification);
			expect(() => consumeD711FreshZeroByokQualification(qualification)).toThrow(/fresh/);
	...
	it("locks pricing, credential, zero-BYOK, claim, current-key, then serial provider order", async () => {
		const events: string[] = [];
		const result = await runD711OrderedLiveAdmissions({
			readFreshPricing() {
				events.push("pricing");
				return "pricing";
			},
	...
	it("rejects a forged D711 preflight before any live block", () => {
		expect(() =>
			createD711PreflightCapability({
