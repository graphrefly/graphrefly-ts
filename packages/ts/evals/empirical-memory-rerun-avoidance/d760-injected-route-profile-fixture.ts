...
export interface D760InjectedRouteProfileFixtureV1 {
...
export function createD760InjectedRouteProfileFixture(inputValue: {
	...
	readonly forwardPhaseContinuation?: boolean;
	readonly retryForwardPhaseOnce?: boolean;
	readonly wrongForwardPhaseTool?: boolean;
	readonly hiddenVerifierFailOncePerRun?: boolean;
	readonly nonRetryableTransportFailureAtRunSequence?: number;
}): D760InjectedRouteProfileFixtureV1 {
	...
	const hiddenVerifierAttempts = new Map<number, number>();
	let providerCalls = 0;
	let forwardPhaseRetryInjected = false;
	let active = 0;
	let maxActive: 0 | 1 = 0;
	const enter = (): void => {
	...
	const adapter = createD734RouteBoundProviderAdapter({
				...
				return await invokeD734RouteBoundOpenRouterTurn({
					...
					transport: {
						async request(request) {
							...
							if (
								inputValue.retryForwardPhaseOnce === true &&
								!forwardPhaseRetryInjected &&
								executionInput.effectRequest.completionContext?.reason ===
									"objective-phase-advanced"
							) {
								forwardPhaseRetryInjected = true;
								return retryResponse(executionInput.effectRequest, {
		...
		async hiddenVerifier({ effectRequest }) {
			...
			try {
				...
				const attempt = (hiddenVerifierAttempts.get(effectRequest.runSequence) ?? 0) + 1;
				hiddenVerifierAttempts.set(effectRequest.runSequence, attempt);
				const status =
					inputValue.hiddenVerifierFailOncePerRun === true && attempt === 1
						? ("failed" as const)
						: ("passed" as const);
				return {
					actualCostMicrousd: 0,
					...
					result: {
						...
						status,
						workspaceStateDigest: workspace,
						evidenceDigest: digest({ effectRequest, attempt, status }),
					},
				};
			} finally {
				leave();
