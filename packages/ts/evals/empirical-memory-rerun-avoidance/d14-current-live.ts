...
import {
	consumeD14CurrentGraphLiveExecutionAuthority,
	type D14CurrentGraphLiveDispatchClaimV1,
	type D14CurrentGraphLiveExecutionAuthorityV1,
} from "./d14-current-live-claim.js";
import {
	D14_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST,
	D14_CURRENT_GRAPH_LIVE_DECISION_REF,
	D14_CURRENT_GRAPH_LIVE_GENERATION_REF,
	D14_CURRENT_GRAPH_LIVE_LIMITS,
	D14_CURRENT_GRAPH_LIVE_ROUTE,
	D14_CURRENT_GRAPH_LIVE_TASK,
	D14_D13_QUALIFICATION_ARTIFACT_DIGEST,
} from "./d14-current-live-coordinates.js";

export const D14_CURRENT_GRAPH_LIVE_BUNDLE_SCHEMA =
	"graphrefly-ts.d14.current-graph-live-bundle.v1" as const;
export const D14_CURRENT_GRAPH_LIVE_PARTIAL_GRAPH_SCHEMA =
	"graphrefly-ts.d14.current-graph-live-partial-graph.v1" as const;
export const D14_CURRENT_GRAPH_LIVE_GENERATION_SCHEMA =
	"graphrefly-ts.d14.current-graph-live-generation.v1" as const;
export const D14_CURRENT_GRAPH_LIVE_TERMINAL_RECEIPT_SCHEMA =
	"graphrefly-ts.d14.current-graph-live-terminal-receipt.v1" as const;
export const D14_CURRENT_GRAPH_LIVE_PERSISTENCE_SCHEMA =
	"graphrefly-ts.d14.current-graph-live-persistence.v1" as const;
export const D14_CURRENT_GRAPH_LIVE_PREEXECUTION_FAILURE_SCHEMA =
	"graphrefly-ts.d14.current-graph-live-preexecution-failure.v1" as const;
export const D14_CURRENT_GRAPH_LIVE_MAX_BUNDLE_BYTES = 4_194_304;

type D14CurrentGraphLiveProviderFactV1 = CurrentGraphProviderEvidenceV1["facts"][number];

export interface D14CurrentGraphLivePartialGraphV1 {
	readonly schemaVersion: typeof D14_CURRENT_GRAPH_LIVE_PARTIAL_GRAPH_SCHEMA;
	readonly decisionRef: typeof D14_CURRENT_GRAPH_LIVE_DECISION_REF;
	readonly routeDigest: string;
	readonly taskProfileDigest: string;
	readonly facts: readonly D14CurrentGraphLiveProviderFactV1[];
...
export interface D14CurrentGraphLiveBundleV1 {
	readonly schemaVersion: typeof D14_CURRENT_GRAPH_LIVE_BUNDLE_SCHEMA;
	readonly decisionRef: typeof D14_CURRENT_GRAPH_LIVE_DECISION_REF;
	...
	readonly d13QualificationArtifactDigest: string;
...
export async function persistD14CurrentGraphLivePreexecutionFailure(inputValue: {
	readonly privateRoot: string;
	readonly claim: D14CurrentGraphLiveDispatchClaimV1;
	readonly implementationManifestDigest: string;
	readonly pricingObservationDigest: string;
	readonly zeroByokObservationDigest: string;
	...
	if (
		claim.decisionRef !== D14_CURRENT_GRAPH_LIVE_DECISION_REF ||
		claim.generationRef !== D14_CURRENT_GRAPH_LIVE_GENERATION_REF ||
		claim.coordinatesDigest !== D14_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST ||
	...
	const material = strictSnapshot({
		schemaVersion: D14_CURRENT_GRAPH_LIVE_PREEXECUTION_FAILURE_SCHEMA,
		...
		coordinatesDigest: D14_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST,
	...
	const commitMaterial = strictSnapshot({
		schemaVersion: "graphrefly-ts.d14.current-graph-live-commit.v1",
		generationRef: D14_CURRENT_GRAPH_LIVE_GENERATION_REF,
		disposition: "partial-failure" as const,
		bundleDigest: null,
	...
	return Object.freeze({
		schemaVersion: D14_CURRENT_GRAPH_LIVE_PERSISTENCE_SCHEMA,
		generationRef: D14_CURRENT_GRAPH_LIVE_GENERATION_REF,
		disposition: "partial-failure" as const,
...
export function isD14CompleteSixArmMeasurementForTest(
	evidence: D11TransportFailureEvidenceV1,
): boolean {
	return completeSixArms(evidence);
}

export async function runD14CurrentGraphLiveMeasurement(inputValue: {
	readonly executionAuthority: D14CurrentGraphLiveExecutionAuthorityV1;
	readonly executionClass: "live-provider" | "injected-no-network";
	readonly executor: D14CurrentGraphLiveExecutorV1;
	readonly implementationManifestDigest: string;
	readonly d13QualificationArtifactDigest: string;
	readonly pricingObservationDigest: string;
	readonly zeroByokObservationDigest: string;
}): Promise<D14CurrentGraphLiveBundleV1> {
	...
	if (
		input.implementationManifestDigest !== D14_IMPLEMENTATION_MANIFEST_DIGEST ||
		input.d13QualificationArtifactDigest !== D14_D13_QUALIFICATION_ARTIFACT_DIGEST ||
	...
	const authority = consumeD14CurrentGraphLiveExecutionAuthority(input.executionAuthority);
	if (
	...
	const graphAuthority = createD11TransportFailureAuthority({
		...
		routeProfile: D14_CURRENT_GRAPH_LIVE_ROUTE,
		taskProfile: D14_CURRENT_GRAPH_LIVE_TASK,
	});
	const facts: D14CurrentGraphLiveProviderFactV1[] = [];
	...
	try {
		for (let guard = 0; guard < D14_CURRENT_GRAPH_LIVE_LIMITS.maxEffectFacts; guard += 1) {
	...
	const partialGraphEvidence = success
		...
		: (() => {
				const material = strictSnapshot({
					schemaVersion: D14_CURRENT_GRAPH_LIVE_PARTIAL_GRAPH_SCHEMA,
					decisionRef: D14_CURRENT_GRAPH_LIVE_DECISION_REF,
					routeDigest: D14_CURRENT_GRAPH_LIVE_ROUTE.routeDigest,
					taskProfileDigest: D14_CURRENT_GRAPH_LIVE_TASK.taskProfileDigest,
					facts,
					rejectionFacts,
					transportFacts,
				...
				}) as D14CurrentGraphLivePartialGraphV1;
			})();
	const generation = success
		? (() => {
				const material = strictSnapshot({
					schemaVersion: D14_CURRENT_GRAPH_LIVE_GENERATION_SCHEMA,
					generationRef: D14_CURRENT_GRAPH_LIVE_GENERATION_REF,
	...
	const terminalMaterial = strictSnapshot({
		schemaVersion: D14_CURRENT_GRAPH_LIVE_TERMINAL_RECEIPT_SCHEMA,
		decisionRef: D14_CURRENT_GRAPH_LIVE_DECISION_REF,
	...
	const bundleMaterial = strictSnapshot({
		schemaVersion: D14_CURRENT_GRAPH_LIVE_BUNDLE_SCHEMA,
		decisionRef: D14_CURRENT_GRAPH_LIVE_DECISION_REF,
		...
		qualificationArtifactDigest: authority.claim.qualificationArtifactDigest,
		qualificationDigest: authority.claim.qualificationDigest,
		d13QualificationArtifactDigest: digest(
			input.d13QualificationArtifactDigest,
			"current.live.d13QualificationArtifactDigest",
		),
		pricingObservationDigest: digest(
			input.pricingObservationDigest,
...
function validatePartial(value: unknown): D14CurrentGraphLivePartialGraphV1 {
	...
	if (
		candidate.schemaVersion !== D14_CURRENT_GRAPH_LIVE_PARTIAL_GRAPH_SCHEMA ||
		candidate.decisionRef !== D14_CURRENT_GRAPH_LIVE_DECISION_REF ||
		candidate.routeDigest !== D14_CURRENT_GRAPH_LIVE_ROUTE.routeDigest ||
		candidate.taskProfileDigest !== D14_CURRENT_GRAPH_LIVE_TASK.taskProfileDigest
	)
	...
	if (facts.length > D14_CURRENT_GRAPH_LIVE_LIMITS.maxEffectFacts)
	...
	return strictSnapshot(candidate) as unknown as D14CurrentGraphLivePartialGraphV1;
}

export function validateD14CurrentGraphLiveBundle(value: unknown): D14CurrentGraphLiveBundleV1 {
	...
	if (
		candidate.schemaVersion !== D14_CURRENT_GRAPH_LIVE_BUNDLE_SCHEMA ||
		candidate.decisionRef !== D14_CURRENT_GRAPH_LIVE_DECISION_REF ||
		candidate.coordinatesDigest !== D14_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST ||
	...
	if (
		candidate.implementationManifestDigest !== D14_IMPLEMENTATION_MANIFEST_DIGEST ||
		candidate.d13QualificationArtifactDigest !== D14_D13_QUALIFICATION_ARTIFACT_DIGEST
	)
	...
	if (
		terminal.schemaVersion !== D14_CURRENT_GRAPH_LIVE_TERMINAL_RECEIPT_SCHEMA ||
		terminal.decisionRef !== D14_CURRENT_GRAPH_LIVE_DECISION_REF ||
	...
	if (candidate.generation !== null) {
		...
		if (
			generation.schemaVersion !== D14_CURRENT_GRAPH_LIVE_GENERATION_SCHEMA ||
			generation.generationRef !== D14_CURRENT_GRAPH_LIVE_GENERATION_REF ||
			generation.graphEvidenceDigest !== graphEvidence?.evidenceDigest ||
			generation.coordinatesDigest !== D14_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST ||
	...
	return strictSnapshot(candidate) as unknown as D14CurrentGraphLiveBundleV1;
}

export async function persistD14CurrentGraphLiveBundle(inputValue: {
	readonly privateRoot: string;
	readonly bundle: D14CurrentGraphLiveBundleV1;
}) {
	...
	const bundle = validateD14CurrentGraphLiveBundle(input.bundle);
