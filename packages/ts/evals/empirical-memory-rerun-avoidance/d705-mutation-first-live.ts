...
import {
	type AcquiredD705SingleUseDispatchClaimV1,
...
export const D705_OBSERVATION_SCHEMA =
	"graphrefly.private-solution-eval.d705-mutation-first-live-observation.v1" as const;
export const D705_SCORECARD_SCHEMA =
	"graphrefly.private-solution-eval.d705-mutation-first-live-scorecard.v1" as const;
export const D705_GENERATION_SCHEMA =
	"graphrefly.private-solution-eval.d705-mutation-first-live-generation.v1" as const;
export const D705_CLAIM_BOUNDARY =
	"single-d704-exact-replacement-d702-mutation-first-live-no-efficacy-claim" as const;
export const D705_APPROVAL_REF = "decision.D705" as const;
export const D705_APPROVAL_REVISION = "decision.D705.2026-08-09.v1" as const;
export const D705_PRICING_REVISION =
	"openrouter-deepseek-v4-flash-0731-deepinfra-fp4-2026-08-09.v5" as const;
export const D705_BUDGET = Object.freeze({ ...D696_BUDGET });
export const D705_PRIVATE_PERSISTENCE_ROOT = D691_PRIVATE_PERSISTENCE_ROOT;
...
export interface D705MutationFirstObservationV1 {
	readonly schemaVersion: typeof D705_OBSERVATION_SCHEMA;
	readonly claimBoundary: typeof D705_CLAIM_BOUNDARY;
	readonly decisionRef: typeof D705_APPROVAL_REF;
	readonly decisionRevision: typeof D705_APPROVAL_REVISION;
...
export interface D705MutationFirstScorecardV1 {
	readonly schemaVersion: typeof D705_SCORECARD_SCHEMA;
	readonly claimBoundary: typeof D705_CLAIM_BOUNDARY;
	readonly causalAttribution: "undetermined";
	readonly efficacyClaim: "none";
...
export interface D705PreflightCapabilityV1 {
	readonly capabilityRef: "d705-exact-live-preflight";
	readonly capabilityRevision: typeof D705_APPROVAL_REVISION;
	readonly executionClass: "live-provider";
}

interface D705PreflightState {
...
const constructedPreflights = new WeakMap<object, D705PreflightState>();
const constructedObservations = new WeakSet<object>();
const constructedScorecards = new WeakSet<object>();

export function createD705PreflightCapability(value: unknown): D705PreflightCapabilityV1 {
	const input = record(value, "d705.preflight");
	exactKeys(
		input,
		[
			"d690OfflineEvidence",
	...
	literal(input.executionClass, "live-provider", "d705.preflight.executionClass");
	...
	literal(
		...
		D691_QUALIFIED_D690_OFFLINE_EVIDENCE_DIGEST,
		"d705.d690.evidenceDigest",
	);
	const capability = Object.freeze({
		capabilityRef: "d705-exact-live-preflight" as const,
		capabilityRevision: D705_APPROVAL_REVISION,
...
function captureBlock(value: unknown): OpenRouterMatchedTrialBlockInputV4 {
	const block = record(value, "d705.block");
	const host = record(block.host, "d705.block.host");
	for (const key of [
		...
		if (Object.hasOwn(host, key)) throw new TypeError(`D705 owns host.${key}`);
	}
	return Object.freeze({
		...block,
		routeQualification: strictSnapshot(record(block.routeQualification, "d705.block.route")),
		host: Object.freeze({
			...host,
			frozen: strictSnapshot(record(host.frozen, "d705.block.host.frozen")),
			qualificationReport: strictSnapshot(
				record(host.qualificationReport, "d705.block.host.qualificationReport"),
			),
			initialRequest: strictSnapshot(record(host.initialRequest, "d705.block.host.initialRequest")),
			taskProfile: strictSnapshot(record(host.taskProfile, "d705.block.host.taskProfile")),
		}),
...
function validateBlock(block: OpenRouterMatchedTrialBlockInputV4): void {
	...
	if (
		...
		block.routeQualification.budget.approvalRef !== D705_APPROVAL_REF ||
		block.routeQualification.budget.approvalRevision !== D705_APPROVAL_REVISION ||
		block.routeQualification.pricing.pricingRevision !== D705_PRICING_REVISION ||
		empiricalStrictJsonDigest(command) !== D694_FOCUSED_VALIDATION_COMMAND_DIGEST
	) {
		throw new TypeError("D705 block drifted from its exact live baseline");
	}
}
...
function noProgressObserver(
	...
	return Object.freeze({
		observerRef: "d705-no-progress-receipt-observer",
		observerRevision: D705_APPROVAL_REVISION,
		record(receipt: ClosedNoProgressReceiptV1) {
			if (receipts.length >= STAGES.length * 32) {
				throw new TypeError("D705 no-progress receipt bound exhausted");
			}
...
export async function runD705MutationFirstBlock(input: {
	readonly preflight: D705PreflightCapabilityV1;
	readonly dispatchClaim: AcquiredD705SingleUseDispatchClaimV1;
	readonly block: OpenRouterMatchedTrialBlockInputV4;
}): Promise<{
	readonly observation: D705MutationFirstObservationV1;
	readonly scorecard: D705MutationFirstScorecardV1;
	readonly protectionExecutor: EmpiricalExactPrivateNeedleProtectionExecutorV1;
}> {
	const outer = record(input, "d705.input");
	exactKeys(outer, ["block", "dispatchClaim", "preflight"], "d705.input");
	const state = constructedPreflights.get(outer.preflight as object);
	if (state === undefined) throw new TypeError("D705 requires its same-process preflight");
	constructedPreflights.delete(outer.preflight as object);
	const dispatchClaim = consumeD705SingleUseDispatchClaim(outer.dispatchClaim);
	...
	if (d703State.d690OfflineEvidence.evidenceDigest !== state.d690OfflineEvidence.evidenceDigest) {
		throw new TypeError("D705 D690 preflight evidence mismatch");
	}
	...
	if (
		...
		throw new TypeError("D705 block drifted from exact D699 historical coordinates");
	}
	...
	if (
		...
		throw new TypeError("D705 transfer memory drifted from D690 evidence");
	}
	...
	if (result.profile !== "smoke") throw new TypeError("D705 requires smoke evidence");
	...
	if (
		...
		throw new TypeError("D705 matched runner omitted treatment invocation facts");
	}
	...
	const material = strictSnapshot({
		schemaVersion: D705_OBSERVATION_SCHEMA,
		claimBoundary: D705_CLAIM_BOUNDARY,
	...
	validateD705Observation(observation);
	constructedObservations.add(observation);
	return Object.freeze({
		observation,
		scorecard: createD705Scorecard(observation),
		protectionExecutor: result.protectionExecutor,
	});
}

export function validateD705Observation(value: unknown): D705MutationFirstObservationV1 {
	const candidate = record(value, "d705.observation");
	const keys: readonly (keyof D705MutationFirstObservationV1)[] = [
		"causalAttribution",
		"claimBoundary",
	...
	exactKeys(candidate, keys, "d705.observation");
	literal(candidate.schemaVersion, D705_OBSERVATION_SCHEMA, "d705.schema");
	literal(candidate.claimBoundary, D705_CLAIM_BOUNDARY, "d705.claimBoundary");
	literal(candidate.decisionRef, D705_APPROVAL_REF, "d705.decisionRef");
	literal(candidate.decisionRevision, D705_APPROVAL_REVISION, "d705.decisionRevision");
	literal(candidate.causalAttribution, "undetermined", "d705.causalAttribution");
	literal(candidate.efficacyClaim, "none", "d705.efficacyClaim");
	literal(candidate.executionClass, "live-provider", "d705.executionClass");
	literal(candidate.d699ObservationDigest, D703_D699_OBSERVATION_DIGEST, "d705.d699");
	literal(candidate.d702QualificationDigest, D703_D702_QUALIFICATION_DIGEST, "d705.d702");
	literal(candidate.d703DryRunGenerationDigest, D703_DRY_RUN_GENERATION_DIGEST, "d705.d703");
	literal(
		candidate.d704DispatchClaimArtifactDigest,
		D704_CONSUMED_DISPATCH_CLAIM_ARTIFACT_DIGEST,
		"d705.d704ClaimArtifact",
	);
	literal(candidate.d704DispatchClaimDigest, D704_CONSUMED_DISPATCH_CLAIM_DIGEST, "d705.d704Claim");
	digest(candidate.dispatchClaimDigest, "d705.dispatchClaimDigest");
	literal(
	...
	literal(underlying.executionClass, "live-provider", "d705.underlying.executionClass");
	literal(underlying.route.budgetApprovalRef, D705_APPROVAL_REF, "d705.route.approvalRef");
	literal(
		underlying.route.budgetApprovalRevision,
		D705_APPROVAL_REVISION,
		"d705.route.approvalRevision",
	);
	literal(underlying.route.pricingRevision, D705_PRICING_REVISION, "d705.route.pricingRevision");
	literal(
		candidate.underlyingObservationDigest,
		empiricalStrictJsonDigest(underlying),
		"d705.underlyingDigest",
	);
	const focusedValidationReceipts = array(
		candidate.focusedValidationReceipts,
		"d705.focusedReceipts",
	...
	const mutationFirstInvocations = array(candidate.mutationFirstInvocations, "d705.mutations").map(
		validateD703MutationFirstInvocationFact,
	);
	const noProgressReceipts = array(candidate.noProgressReceipts, "d705.receipts").map(
		validateD703NoProgressReceipt,
	);
	if (
		...
		throw new TypeError("D705 mechanism evidence bound exceeded");
	}
	...
	literal(candidate.mutationFirstRecoveryObserved, recovery, "d705.recovery");
	literal(candidate.matchedPairEvaluable, matchedPairEvaluable(underlying), "d705.evaluable");
	literal(
		safeInteger(candidate.transportCalls, "d705.transportCalls", { min: 1, max: 576 }),
		underlying.result.requests,
		"d705.transportRequestBinding",
	);
	literal(candidate.maximumConcurrentTransportCalls, 1, "d705.maxConcurrency");
	const expectedRetryWaitCalls = STAGES.reduce(
		(total, stage) => total + (runForStage(underlying, stage)?.retryWaitTrace.length ?? 0),
		0,
	);
	literal(
		safeInteger(candidate.retryWaitCalls, "d705.retryWaitCalls", { min: 0, max: 12 }),
		expectedRetryWaitCalls,
		"d705.retryWaitBinding",
	);
	literal(candidate.fallbackUsed, false, "d705.fallbackUsed");
	literal(candidate.providerSwitchUsed, false, "d705.providerSwitchUsed");
	literal(candidate.workspaceResidueCount, 0, "d705.workspaceResidueCount");
	const material = strictSnapshot({
		...candidate,
		underlying,
		focusedValidationReceipts: derived.receipts,
		...
		matchedPairEvaluable: matchedPairEvaluable(underlying),
	}) as unknown as D705MutationFirstObservationV1;
	const { observationDigest, ...withoutDigest } = material;
	literal(observationDigest, empiricalStrictJsonDigest(withoutDigest), "d705.observationDigest");
	return strictSnapshot({ ...withoutDigest, observationDigest });
}

export function createD705Scorecard(
	value: D705MutationFirstObservationV1,
): D705MutationFirstScorecardV1 {
	const observation = validateD705Observation(value);
	const completed = observation.underlying.result.classification === "complete";
	const material = strictSnapshot({
		schemaVersion: D705_SCORECARD_SCHEMA,
		claimBoundary: D705_CLAIM_BOUNDARY,
		causalAttribution: "undetermined" as const,
		efficacyClaim: "none" as const,
...
export async function persistD705PrivateGeneration(input: {
	...
	const request = record(input, "d705.persistence");
	exactKeys(
		request,
	...
	if (!constructedObservations.has(request.observation as object)) {
		throw new TypeError("D705 persistence requires same-process observation");
	}
	const observation = validateD705Observation(request.observation);
	const scorecard = createD705Scorecard(observation);
	if (
		...
		throw new TypeError("D705 persistence requires same-process derived scorecard");
	}
	if (!isEmpiricalExactPrivateNeedleProtectionExecutor(request.protectionExecutor)) {
		throw new TypeError("D705 persistence requires constructed protection");
	}
	if (
		typeof request.generationRef !== "string" ||
		!/^[a-z0-9][a-z0-9._-]{0,127}$/.test(request.generationRef)
	) {
		throw new TypeError("D705 generation ref invalid");
	}
	const privateRoot = await assertSafePrivateRoot(D705_PRIVATE_PERSISTENCE_ROOT);
	if (request.privateRoot !== privateRoot) throw new TypeError("D705 persistence root drifted");
	const generationRef = request.generationRef;
	const generationMaterial = strictSnapshot({
		schemaVersion: D705_GENERATION_SCHEMA,
		generationRef,
		claimBoundary: D705_CLAIM_BOUNDARY,
		causalAttribution: "undetermined" as const,
		efficacyClaim: "none" as const,
	...
	for (const [label, subject] of [
		["D705 observation", observation],
		["D705 scorecard", scorecard],
		["D705 generation", generation],
	] as const) {
		assertPrivateArtifactProtection({
			label,
			subject,
	...
	try {
		...
		throw new TypeError("D705 generation already exists");
	} catch (error) {
		if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
	}
	const stagingPath = join(privateRoot, `.d705-staging-${randomUUID()}`);
	...
	try {
		...
		for (const file of files) {
			...
			if (!Buffer.from(persisted).equals(file.bytes)) {
				throw new TypeError(`D705 staging readback failed for ${file.file}`);
			}
		}
