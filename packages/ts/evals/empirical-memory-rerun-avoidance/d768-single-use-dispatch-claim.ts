...
import {
	...
	D768_DISPATCH_CLAIM_REF,
	D768_GENERATION_REF,
	D768_HISTORICAL_BUNDLE_DIGEST,
	D768_ROUTE_PROFILE_DIGEST,
...
export const D768_DISPATCH_CLAIM_SCHEMA =
	"graphrefly.b112.d768.single-use-dispatch-claim.v1" as const;
export const D768_PRIVATE_ROOT = resolve(
	import.meta.dirname,
...
export interface D768PersistedDispatchClaimV1 {
	readonly schemaVersion: typeof D768_DISPATCH_CLAIM_SCHEMA;
	readonly claimRef: typeof D768_DISPATCH_CLAIM_REF;
	readonly decisionRef: typeof D768_DECISION_REF;
	readonly decisionRevision: typeof D768_DECISION_REVISION;
	readonly generationRef: typeof D768_GENERATION_REF;
...
export interface D768ExecutionAuthorityV1 {
	readonly revision: "graphrefly.b112.d768.execution-authority.v1";
	readonly claim: D768PersistedDispatchClaimV1;
...
function claimMaterial(
	...
	const input = record(inputValue, "d768.claim.input");
	exactKeys(
		input,
	...
	literal(
		...
		"d768.claim.implementation",
	);
	return strictSnapshot({
		schemaVersion: D768_DISPATCH_CLAIM_SCHEMA,
		claimRef: D768_DISPATCH_CLAIM_REF,
		...
		pricingReadDigest: digest(input.pricingReadDigest, "d768.claim.pricing"),
		zeroByokObservationDigest: digest(input.zeroByokObservationDigest, "d768.claim.zeroByok"),
...
async function acquire(
...
): Promise<D768PersistedDispatchClaimV1> {
	const requestedRoot = resolve(privateRootValue);
	const privateRoot = await realpath(requestedRoot);
	if (privateRoot !== requestedRoot) throw new TypeError("D768 private root is not canonical");
	...
	if (
		...
		throw new TypeError("D768 private root ownership is invalid");
	const claimRoot = join(privateRoot, `.${D768_DISPATCH_CLAIM_REF}`);
	...
	}) as D768PersistedDispatchClaimV1;
	...
	try {
		...
		if (
			...
			throw new TypeError("D768 dispatch claim readback drifted");
	} finally {
		await reader.close();
	}
	claims.set(claim, {
...
export function acquireD768SingleUseDispatchClaim(input: Parameters<typeof claimMaterial>[1]) {
	return acquire(D768_PRIVATE_ROOT, "live-fixed-root", input);
}

export function acquireD768SingleUseDispatchClaimAtRootForTest(
	root: string,
	input: Parameters<typeof claimMaterial>[1],
) {
...
async function revalidate(claim: D768PersistedDispatchClaimV1, state: ClaimState): Promise<void> {
	...
	if (
		...
		throw new TypeError("D768 durable claim directory drifted");
	...
	try {
		...
		if (
			...
			throw new TypeError("D768 durable claim file drifted");
	} finally {
		await reader.close();
	}
	literal(
...
export async function consumeD768DispatchClaimForExecution(inputValue: {
	readonly claim: D768PersistedDispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
}): Promise<D768ExecutionAuthorityV1> {
	const input = record(inputValue, "d768.execution.input");
	exactKeys(input, ["claim", "currentKeyAdmission"], "d768.execution.input");
	const claim = input.claim as D768PersistedDispatchClaimV1;
	const state = typeof claim === "object" && claim !== null ? claims.get(claim) : undefined;
	if (state === undefined) throw new TypeError("D768 claim is not fresh and constructed");
	claims.delete(claim);
	...
	if (
		...
		admission.remainingMicrousd < D768_BUDGET_LIMITS.maxCostMicrousd ||
		admission.limitReset !== "none" ||
		admission.isManagementKey
	)
		throw new TypeError("D768 current-key admission is outside authority");
	...
	const authority = Object.freeze({
		revision: "graphrefly.b112.d768.execution-authority.v1" as const,
		claim,
...
export function consumeD768ExecutionAuthority(value: unknown): D768ExecutionAuthorityV1 {
	if (typeof value !== "object" || value === null || !authorities.delete(value))
		throw new TypeError("D768 live execution authority must be fixed-root and single-use");
	return value as D768ExecutionAuthorityV1;
}
