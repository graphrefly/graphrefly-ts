...
import type { CurrentGraphLivePreclaimV1 as D16CurrentGraphLivePreclaimV1 } from "./d8-current-live-preflight.js";
import { consumeCurrentGraphLivePreclaim as consumeD16CurrentGraphLivePreclaim } from "./d8-current-live-preflight.js";
import {
	D16_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST,
	D16_CURRENT_GRAPH_LIVE_DECISION_REF,
	D16_CURRENT_GRAPH_LIVE_DECISION_REVISION,
	D16_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF,
...
export const D16_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_SCHEMA =
	"graphrefly-ts.d16.current-graph-live-dispatch-claim.v1" as const;
export const D16_CURRENT_GRAPH_LIVE_EXECUTION_AUTHORITY_REVISION =
	"graphrefly-ts.d16.current-graph-live-execution-authority.v1" as const;
export const D16_CURRENT_GRAPH_LIVE_PRIVATE_ROOT = resolve(
	import.meta.dirname,
	"../.private/empirical-memory-rerun-avoidance/current-graph-native-d16",
);

type ClaimScope = "live-fixed-root" | "injected-test-root";

export interface D16CurrentGraphLiveDispatchClaimV1 {
	readonly schemaVersion: typeof D16_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_SCHEMA;
	readonly claimRef: typeof D16_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF;
	readonly decisionRef: typeof D16_CURRENT_GRAPH_LIVE_DECISION_REF;
	readonly decisionRevision: typeof D16_CURRENT_GRAPH_LIVE_DECISION_REVISION;
	readonly generationRef: typeof D16_CURRENT_GRAPH_LIVE_GENERATION_REF;
...
export interface D16CurrentGraphLiveExecutionAuthorityV1 {
	readonly revision: typeof D16_CURRENT_GRAPH_LIVE_EXECUTION_AUTHORITY_REVISION;
...
function claimMaterial(
	scope: ClaimScope,
	preclaim: D16CurrentGraphLivePreclaimV1,
	implementationManifestDigest: string,
	qualificationArtifactDigest: string,
	qualificationDigest: string,
) {
	return strictSnapshot({
		schemaVersion: D16_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_SCHEMA,
		claimRef: D16_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF,
...
async function acquire(
	...
	scope: ClaimScope,
	inputValue: {
		readonly preclaim: D16CurrentGraphLivePreclaimV1;
		readonly implementationManifestDigest: string;
		readonly qualificationArtifactDigest: string;
		readonly qualificationDigest: string;
	},
): Promise<D16CurrentGraphLiveDispatchClaimV1> {
	...
	const preclaim = consumeD16CurrentGraphLivePreclaim(input.preclaim);
	...
	const claimRoot = join(privateRoot, `.${D16_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF}`);
	...
	}) as D16CurrentGraphLiveDispatchClaimV1;
...
export function acquireD16CurrentGraphLiveDispatchClaim(input: {
	readonly preclaim: D16CurrentGraphLivePreclaimV1;
	readonly implementationManifestDigest: string;
	readonly qualificationArtifactDigest: string;
	readonly qualificationDigest: string;
}) {
	return acquire(D16_CURRENT_GRAPH_LIVE_PRIVATE_ROOT, "live-fixed-root", input);
}

export function acquireD16CurrentGraphLiveDispatchClaimAtRootForTest(
	root: string,
	input: {
		readonly preclaim: D16CurrentGraphLivePreclaimV1;
		readonly implementationManifestDigest: string;
...
async function revalidateClaim(
	claim: D16CurrentGraphLiveDispatchClaimV1,
	state: ClaimState,
): Promise<void> {
	...
	const material = claimMaterial(
		state.scope,
		{ preclaimDigest: claim.preclaimDigest } as D16CurrentGraphLivePreclaimV1,
...
export async function consumeD16CurrentGraphLiveDispatchClaim(inputValue: {
	readonly claim: D16CurrentGraphLiveDispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
	readonly allowInjectedTestScope?: boolean;
}): Promise<D16CurrentGraphLiveExecutionAuthorityV1> {
	...
	const claim = input.claim as D16CurrentGraphLiveDispatchClaimV1;
	...
	const material = strictSnapshot({
		revision: D16_CURRENT_GRAPH_LIVE_EXECUTION_AUTHORITY_REVISION,
		claim,
		currentKeyAdmission,
	});
	...
	}) as D16CurrentGraphLiveExecutionAuthorityV1;
	authorities.add(authority);
	return authority;
}

export function consumeD16CurrentGraphLiveExecutionAuthority(
	value: unknown,
): D16CurrentGraphLiveExecutionAuthorityV1 {
	...
	return value as D16CurrentGraphLiveExecutionAuthorityV1;
}
