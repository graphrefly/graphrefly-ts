...
export const D54_LIVE_CLAIM_SCHEMA = "graphrefly-ts.d54.dispatch-claim.v1" as const;
export const D54_LIVE_BUNDLE_SCHEMA = "graphrefly-ts.d54.live-bundle.v1" as const;
export const D54_LIVE_GENERATION_REF = "current-graph-native-task-outcome-live-d54-v1" as const;
export const D54_LIVE_CLAIM_REF = "current-graph-native-task-outcome-claim-d54-v1" as const;
export const D54_LIVE_PRIVATE_ROOT = resolve(
	import.meta.dirname,
	"../.private/empirical-memory-rerun-avoidance/current-graph-native-d54-live",
);

export interface D54PreclaimV1 {
...
export interface D54DispatchClaimV1 {
	readonly schemaVersion: typeof D54_LIVE_CLAIM_SCHEMA;
	readonly claimRef: typeof D54_LIVE_CLAIM_REF;
	readonly authorityRef: "graphrefly-ts:D54";
	readonly architectureRef: "graphrefly-ts:D52";
	readonly generationRef: typeof D54_LIVE_GENERATION_REF;
	readonly preclaimDigest: string;
...
export interface D54ExecutionAuthorityV1 {
	readonly claim: D54DispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
	readonly authorityDigest: string;
}

export type D54LiveBundleV1 = Readonly<{
	schemaVersion: typeof D54_LIVE_BUNDLE_SCHEMA;
	generationRef: typeof D54_LIVE_GENERATION_REF;
...
export function composeD54Preclaim(input: {
...
export async function acquireD54DispatchClaim(input: {
	readonly privateRoot: string;
	readonly preclaim: D54PreclaimV1;
	readonly implementationCommit: string;
	readonly implementationManifestDigest: string;
	readonly qualificationArtifactDigest: string;
	readonly qualificationDigest: string;
}): Promise<D54DispatchClaimV1> {
	if (!preclaims.delete(input.preclaim)) throw new TypeError("D54 preclaim is absent or consumed");
	const privateRoot = resolve(input.privateRoot);
	if ((await realpath(privateRoot)) !== privateRoot)
		throw new TypeError("D54 private root drifted");
	const claimRoot = join(privateRoot, `.${D54_LIVE_CLAIM_REF}`);
	await mkdir(claimRoot, { mode: 0o700 });
	await chmod(claimRoot, 0o700);
	const material = strictSnapshot({
		schemaVersion: D54_LIVE_CLAIM_SCHEMA,
		claimRef: D54_LIVE_CLAIM_REF,
		authorityRef: "graphrefly-ts:D54" as const,
		architectureRef: "graphrefly-ts:D52" as const,
		generationRef: D54_LIVE_GENERATION_REF,
...
export async function consumeD54DispatchClaim(input: {
	readonly claim: D54DispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
}): Promise<D54ExecutionAuthorityV1> {
	const state = claims.get(input.claim);
	if (state === undefined) throw new TypeError("D54 dispatch claim is absent or consumed");
	...
	try {
		if (!sameBytes(new Uint8Array(await reader.readFile()), state.bytes))
			throw new TypeError("D54 durable dispatch claim drifted");
	} finally {
		await reader.close();
	}
...
export function constructD54LiveBundle(input: {
	readonly authority: D54ExecutionAuthorityV1;
	readonly pricing: D44D45PricingObservationV1;
	readonly zeroByok: D44D45ZeroByokObservationV1;
	readonly implementationCommit: string;
	readonly implementationManifestDigest: string;
	...
	const partialGraphEvidence =
		input.measurement.disposition === "partial-failure"
			...
			: null;
	const material = strictSnapshot({
		schemaVersion: D54_LIVE_BUNDLE_SCHEMA,
		generationRef: D54_LIVE_GENERATION_REF,
...
export async function persistD54LiveBundle(input: {
	readonly privateRoot: string;
	readonly bundle: D54LiveBundleV1;
}): Promise<Readonly<{ artifactDigest: string; receiptDigest: string }>> {
	if (!isAbsolute(input.privateRoot)) throw new TypeError("D54 persistence root must be absolute");
	const generationRoot = join(input.privateRoot, D54_LIVE_GENERATION_REF);
...
export async function prepareD54PrivateRoot(path: string): Promise<void> {
	await mkdir(path, { recursive: true, mode: 0o700 });
	await chmod(path, 0o700);
	const stat = await lstat(path);
	if (!stat.isDirectory() || stat.isSymbolicLink() || (stat.mode & 0o777) !== 0o700)
		throw new TypeError("D54 private root identity failed");
}
