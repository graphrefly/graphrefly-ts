...
export const CURRENT_GRAPH_QUALIFICATION_SCHEMA =
	"graphrefly-ts.d1.current-graph-native-qualification.v1" as const;
export const CURRENT_GRAPH_GENERATION_SCHEMA =
	"graphrefly-ts.d1.current-graph-native-generation.v1" as const;
export const CURRENT_GRAPH_BUNDLE_SCHEMA =
	"graphrefly-ts.d1.current-graph-native-bundle.v1" as const;
export const CURRENT_GRAPH_GENERATION_REF =
	"current-graph-native-no-network-qualification-2026-08-14-v1" as const;
...
export async function runCurrentGraphNativeNoNetworkQualification(): Promise<CurrentGraphQualificationBundleV1> {
	if ((await measureCurrentGraphImplementation()) !== CURRENT_GRAPH_IMPLEMENTATION_MANIFEST_DIGEST)
		throw new TypeError("current Graph implementation manifest drifted before qualification");
	...
	if (
		...
		throw new TypeError("current Graph exact six-arm qualification lifecycle drifted");
	if (
		...
		throw new TypeError("current Graph finding provenance drifted");
	const qualificationMaterial = strictSnapshot({
		schemaVersion: CURRENT_GRAPH_QUALIFICATION_SCHEMA,
		decisionRef: "graphrefly-ts:D1" as const,
	...
	if (strictJsonCodec.encode(bundle).byteLength > CURRENT_GRAPH_MAX_BUNDLE_BYTES)
		throw new TypeError("current Graph qualification bundle exceeded its byte bound");
	constructedBundles.add(bundle);
	if ((await measureCurrentGraphImplementation()) !== CURRENT_GRAPH_IMPLEMENTATION_MANIFEST_DIGEST)
		throw new TypeError("current Graph implementation manifest drifted after qualification");
	return bundle;
}
...
export function validateCurrentGraphQualificationBundle(
	...
	if (
		...
		qualification.decisionRef !== "graphrefly-ts:D1" ||
		...
		throw new TypeError("current Graph qualification coordinates drifted");
	...
	if (
		...
		throw new TypeError("current Graph qualification cross-binding drifted");
	...
	if (empiricalStrictJsonDigest(qualificationMaterial) !== qualification.qualificationDigest)
		throw new TypeError("current Graph qualification digest drifted");
	...
	if (empiricalStrictJsonDigest(generationMaterial) !== generation.generationDigest)
		throw new TypeError("current Graph generation digest drifted");
	...
	if (empiricalStrictJsonDigest(bundleMaterial) !== candidate.bundleDigest)
		throw new TypeError("current Graph bundle digest drifted");
...
async function assertDirectoryIdentity(
	...
	if (
		...
		throw new TypeError("current Graph persistence directory identity drifted");
}

async function assertPrivateRoot(path: string): Promise<CurrentGraphFileIdentity> {
	if (!isAbsolute(path)) throw new TypeError("current Graph private root must be absolute");
	const stat = await lstat(path);
...
async function writePrivateFile(
	...
	try {
		...
		if (!stat.isFile() || (stat.mode & 0o777) !== 0o600 || stat.nlink !== 1)
			throw new TypeError("current Graph persistence artifact identity drifted");
...
async function assertPrivateFile(
	...
	try {
		...
		if (
			...
			throw new TypeError("current Graph persistence artifact readback drifted");
	} finally {
		await handle.close();
	}
}
...
async function removeOwnedGeneration(
	...
	await assertDirectoryIdentity(path, identity, 0o700);
	const tombstone = join(privateRoot, `.current-d1-tombstone-${randomUUID()}`);
	await rename(path, tombstone);
	const moved = await lstat(tombstone);
	if (moved.dev !== identity.dev || moved.ino !== identity.ino)
		throw new TypeError("current Graph cleanup tombstone ownership drifted");
...
export async function persistCurrentGraphQualificationBundle(input: {
	...
	if (typeof captured.bundle !== "object" || captured.bundle === null)
		throw new TypeError("current Graph persistence bundle is invalid");
	if (!constructedBundles.has(captured.bundle))
		throw new TypeError("current Graph persistence requires a same-process qualified bundle");
	...
	if (fault !== undefined && faultStage === undefined)
		throw new TypeError("current Graph persistence fault is forged or replayed");
	if (fault !== undefined) persistenceFaults.delete(fault as object);
	if (typeof captured.privateRoot !== "string")
		throw new TypeError("current Graph private root must be a string");
	...
	try {
		...
		if (parentStat.dev !== rootIdentity.dev || parentStat.ino !== rootIdentity.ino)
			throw new TypeError("current Graph private root handle drifted");
		...
		if (bytes.byteLength > CURRENT_GRAPH_MAX_BUNDLE_BYTES)
			throw new TypeError("current Graph persistence bundle exceeded its byte bound");
		...
		const markerMaterial = strictSnapshot({
			schemaVersion: "graphrefly-ts.d1.current-graph-native-commit.v1",
			generationRef: CURRENT_GRAPH_GENERATION_REF,
		...
		if (
			...
			throw new TypeError("current Graph stable persistence handle drifted");
		await assertDirectoryIdentity(artifactsRoot, artifactsIdentity, 0o700);
		await assertDirectoryIdentity(finalRoot, finalIdentity, 0o700);
	...
	if (closeErrors.length > 0)
		operationError = new AggregateError(
			...
			"current Graph persistence handle cleanup failed",
		);
	...
	if (operationError !== null) {
		...
		if (errors.length > 1)
			throw new AggregateError(errors, "current Graph persistence cleanup failed");
		throw operationError;
	}
	if (receipt === null) throw new TypeError("current Graph persistence did not linearize");
	return receipt;
}
