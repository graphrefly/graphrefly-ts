...
import {
	...
	createD12CurrentGraphOpenRouterExecutor as createD16CurrentGraphOpenRouterExecutor,
} from "./d12-current-openrouter-adapter.js";
import { validateD15QualificationBundle } from "./d15-current-pre-live-qualification.js";
import { D16_IMPLEMENTATION_MANIFEST_DIGEST } from "./d16-current-implementation-manifest.js";
import {
	type D16CurrentGraphLiveBundleV1,
	runD16CurrentGraphLiveMeasurement,
	validateD16CurrentGraphLiveBundle,
} from "./d16-current-live.js";
import {
	acquireD16CurrentGraphLiveDispatchClaimAtRootForTest,
	consumeD16CurrentGraphLiveDispatchClaim,
} from "./d16-current-live-claim.js";
import {
	D16_CURRENT_GRAPH_LIVE_DECISION_REF,
	D16_CURRENT_GRAPH_LIVE_PROVIDER_NAME,
	D16_CURRENT_GRAPH_LIVE_PROVIDER_TAG,
	D16_CURRENT_GRAPH_LIVE_QUALIFICATION_GENERATION_REF,
	D16_CURRENT_GRAPH_LIVE_QUANTIZATION,
	D16_CURRENT_GRAPH_LIVE_READABLE_FILES,
	D16_CURRENT_GRAPH_LIVE_REQUEST_MODEL,
	D16_CURRENT_GRAPH_LIVE_SELECTED_ENDPOINT_MODEL,
...
export const D16_QUALIFICATION_SCHEMA =
	"graphrefly-ts.d16.current-live-no-network-qualification.v1" as const;
export const D16_QUALIFICATION_BUNDLE_SCHEMA =
	"graphrefly-ts.d16.current-live-no-network-bundle.v1" as const;
export const D16_QUALIFICATION_GENERATION_SCHEMA =
	"graphrefly-ts.d16.current-live-no-network-generation.v1" as const;
export const D16_QUALIFICATION_PERSISTENCE_SCHEMA =
	"graphrefly-ts.d16.current-live-no-network-persistence.v1" as const;
export const D16_INJECTED_TEST_GENERATION_REF =
	"current-graph-native-live-no-network-injected-test-d16-v1" as const;
export const D16_MAX_QUALIFICATION_BYTES = 4_194_304;

export interface D16D15BaselineAdmissionV1 {
	readonly revision: "graphrefly-ts.d16.d15-baseline-admission.v1";
}

export interface D16QualificationBundleV1 {
	readonly schemaVersion: typeof D16_QUALIFICATION_BUNDLE_SCHEMA;
	readonly qualification: Readonly<{
		schemaVersion: typeof D16_QUALIFICATION_SCHEMA;
		decisionRef: typeof D16_CURRENT_GRAPH_LIVE_DECISION_REF;
		implementationManifestDigest: string;
		baselineBasis: "exact-d15-artifact" | "injected-test";
		d15QualificationArtifactDigest: typeof D16_D15_QUALIFICATION_ARTIFACT_DIGEST;
		d15QualificationBundleDigest: typeof D16_D15_QUALIFICATION_BUNDLE_DIGEST;
		d15QualificationDigest: typeof D16_D15_QUALIFICATION_DIGEST;
	...
	readonly graphBundle: D16CurrentGraphLiveBundleV1;
	readonly generation: Readonly<{
		schemaVersion: typeof D16_QUALIFICATION_GENERATION_SCHEMA;
		generationRef: typeof D16_CURRENT_GRAPH_LIVE_QUALIFICATION_GENERATION_REF;
		qualificationDigest: string;
		graphBundleDigest: string;
		implementationManifestDigest: string;
		causalAttribution: "undetermined";
...
const baselineStates = new WeakMap<object, "exact-d15-artifact" | "injected-test">();
const constructedBundles = new WeakSet<object>();

function baselineCapability(
	basis: "exact-d15-artifact" | "injected-test",
): D16D15BaselineAdmissionV1 {
	const capability = Object.freeze({
		revision: "graphrefly-ts.d16.d15-baseline-admission.v1" as const,
	});
	baselineStates.set(capability, basis);
	return capability;
}

export function admitD16D15QualificationBaseline(
	bytesValue: Uint8Array,
): D16D15BaselineAdmissionV1 {
	if (!(bytesValue instanceof Uint8Array))
		throw new TypeError("D16 D15 baseline bytes are invalid");
	const bytes = new Uint8Array(bytesValue);
	if (empiricalSha256(bytes) !== D16_D15_QUALIFICATION_ARTIFACT_DIGEST)
		throw new TypeError("D16 D15 qualification artifact drifted");
	const bundle = validateD15QualificationBundle(strictJsonCodec.decode(bytes));
	if (
		bundle.bundleDigest !== D16_D15_QUALIFICATION_BUNDLE_DIGEST ||
		bundle.qualification.qualificationDigest !== D16_D15_QUALIFICATION_DIGEST ||
		bundle.qualification.implementationManifestDigest !== D16_D15_IMPLEMENTATION_MANIFEST_DIGEST ||
		bundle.qualification.baselineBasis !== "exact-d6-artifact" ||
		bundle.qualification.fullSixArmIntegrationPassed !== true ||
		bundle.qualification.efficacyClaim !== "none" ||
		bundle.generation.generationDigest !== D16_D15_QUALIFICATION_GENERATION_DIGEST
	)
		throw new TypeError("D16 D15 qualification projection drifted");
	return baselineCapability("exact-d15-artifact");
}

export function createD16InjectedD15BaselineForTest(): D16D15BaselineAdmissionV1 {
...
function officialPricingResponse(): Response {
	const response = new Response(
		JSON.stringify({
			data: {
				id: D16_CURRENT_GRAPH_LIVE_REQUEST_MODEL,
				endpoints: [
					{
						provider_name: D16_CURRENT_GRAPH_LIVE_PROVIDER_NAME,
						tag: D16_CURRENT_GRAPH_LIVE_PROVIDER_TAG,
						quantization: D16_CURRENT_GRAPH_LIVE_QUANTIZATION,
						model: D16_CURRENT_GRAPH_LIVE_SELECTED_ENDPOINT_MODEL,
...
export async function runD16InjectedNoNetworkQualification(inputValue: {
	readonly repositoryRoot: string;
	readonly baseline: D16D15BaselineAdmissionV1;
	readonly implementationManifestDigest: string;
}): Promise<D16QualificationBundleV1> {
	const input = record(inputValue, "D16 qualification input");
	exactKeys(
		input,
		["baseline", "implementationManifestDigest", "repositoryRoot"],
		"D16 qualification input",
	);
	const baseline = input.baseline as D16D15BaselineAdmissionV1;
	const baselineBasis =
		typeof baseline === "object" && baseline !== null ? baselineStates.get(baseline) : undefined;
	if (baselineBasis === undefined) throw new TypeError("D16 D15 baseline is forged or replayed");
	baselineStates.delete(baseline);
	const implementationManifestDigest = digest(
		input.implementationManifestDigest,
		"D16 implementation manifest",
	);
	if (implementationManifestDigest !== D16_IMPLEMENTATION_MANIFEST_DIGEST)
		throw new TypeError("D16 qualification implementation manifest drifted");
	const repositoryRoot = await realpath(resolve(String(input.repositoryRoot)));
	const temporaryRoot = await mkdtemp(join(tmpdir(), "graphrefly-d16-no-network-"));
	await chmod(temporaryRoot, 0o700);
	try {
		const credential = Object.freeze({
			bearerToken: "sk-or-v1-test-current-graph-live-key-d16",
		...
		const zeroByokBytes = Buffer.from(
			JSON.stringify({
				...
				allowedModels: [D16_CURRENT_GRAPH_LIVE_REQUEST_MODEL],
				allowedProviders: [D16_CURRENT_GRAPH_LIVE_PROVIDER_NAME],
		...
		const claim = await acquireD16CurrentGraphLiveDispatchClaimAtRootForTest(
			await realpath(privateRoot),
			{
				preclaim,
				implementationManifestDigest,
				qualificationArtifactDigest: empiricalStrictJsonDigest({ injected: "d16" }),
				qualificationDigest: empiricalStrictJsonDigest({ injected: "d16-qualification" }),
			},
		);
			...
			try {
				...
				if (pendingRetryBodyDigest !== null) {
					if (pendingRetryBodyDigest !== currentBodyDigest)
						throw new TypeError("D16 retry request bytes drifted");
				...
				const calls = hasReadResult
					...
					: D16_CURRENT_GRAPH_LIVE_READABLE_FILES.map((path, index) => ({
		...
		const graphBundle = validateD16CurrentGraphLiveBundle(
			await runD16CurrentGraphLiveMeasurement({
				executionAuthority,
		...
		if (graphBundle.disposition !== "success" || graphBundle.graphEvidence === null)
			throw new TypeError("D16 no-network Graph integration did not complete");
		...
		if (
			...
			throw new TypeError("D16 no-network Graph transport projection drifted");
		await lstat(materializationRoot).then(
			() => {
				throw new TypeError("D16 no-network left workspace residue");
			},
			(error: unknown) => {
		...
		if (!source.includes(CURRENT_GRAPH_LIVE_FIXED_ADMISSION_BLOCK))
			throw new TypeError("D16 no-network mutated the source workspace");
		const qualificationMaterial = strictSnapshot({
			schemaVersion: D16_QUALIFICATION_SCHEMA,
		...
		const generationMaterial = strictSnapshot({
			schemaVersion: D16_QUALIFICATION_GENERATION_SCHEMA,
			generationRef: D16_CURRENT_GRAPH_LIVE_QUALIFICATION_GENERATION_REF,
		...
		const bundleMaterial = strictSnapshot({
			schemaVersion: D16_QUALIFICATION_BUNDLE_SCHEMA,
			qualification,
			graphBundle,
			generation,
		});
		...
		}) as D16QualificationBundleV1;
		if (
			strictJsonCodec.encode(bundle as unknown as StrictJsonValue).byteLength >
			D16_MAX_QUALIFICATION_BYTES
		)
			throw new TypeError("D16 qualification exceeded its byte bound");
...
export function validateD16QualificationBundle(value: unknown): D16QualificationBundleV1 {
	const candidate = record(value, "D16 qualification bundle");
	exactKeys(
		candidate,
		["bundleDigest", "generation", "graphBundle", "qualification", "schemaVersion"],
		"D16 qualification bundle",
	);
	if (candidate.schemaVersion !== D16_QUALIFICATION_BUNDLE_SCHEMA)
		throw new TypeError("D16 qualification bundle schema drifted");
	const graphBundle = validateD16CurrentGraphLiveBundle(candidate.graphBundle);
	if (graphBundle.executionClass !== "injected-no-network" || graphBundle.disposition !== "success")
		throw new TypeError("D16 qualification Graph bundle drifted");
	const graphEvidence = graphBundle.graphEvidence;
	if (graphEvidence === null) throw new TypeError("D16 qualification Graph evidence is missing");
	const graph = graphEvidence.d9Evidence.providerEvidence;
	const runs = graph.workflowEvidence.runs;
	const qualification = record(candidate.qualification, "D16 qualification");
	exactKeys(
		qualification,
		[
			"baselineBasis",
			...
			"d15QualificationArtifactDigest",
			"d15QualificationBundleDigest",
			"d15QualificationDigest",
			"decisionRef",
			"efficacyClaim",
	...
	if (
		qualification.schemaVersion !== D16_QUALIFICATION_SCHEMA ||
		qualification.decisionRef !== D16_CURRENT_GRAPH_LIVE_DECISION_REF ||
		qualification.implementationManifestDigest !== D16_IMPLEMENTATION_MANIFEST_DIGEST ||
		(qualification.baselineBasis !== "exact-d15-artifact" &&
			qualification.baselineBasis !== "injected-test") ||
		qualification.d15QualificationArtifactDigest !== D16_D15_QUALIFICATION_ARTIFACT_DIGEST ||
		qualification.d15QualificationBundleDigest !== D16_D15_QUALIFICATION_BUNDLE_DIGEST ||
		qualification.d15QualificationDigest !== D16_D15_QUALIFICATION_DIGEST ||
		...
		throw new TypeError("D16 qualification projection drifted");
	const generation = record(candidate.generation, "D16 qualification generation");
	exactKeys(
		generation,
		[
			"causalAttribution",
		...
		"D16 qualification generation",
	);
	const { generationDigest, ...generationMaterial } = generation;
	if (
		generation.schemaVersion !== D16_QUALIFICATION_GENERATION_SCHEMA ||
		generation.generationRef !== D16_CURRENT_GRAPH_LIVE_QUALIFICATION_GENERATION_REF ||
		...
		throw new TypeError("D16 qualification generation drifted");
	const { bundleDigest, ...bundleMaterial } = candidate;
	if (bundleDigest !== empiricalStrictJsonDigest(bundleMaterial))
		throw new TypeError("D16 qualification bundle digest drifted");
	return strictSnapshot(candidate) as unknown as D16QualificationBundleV1;
}

async function persistQualification(inputValue: {
	readonly privateRoot: string;
	readonly bundle: D16QualificationBundleV1;
	readonly expectedBasis: D16QualificationBundleV1["qualification"]["baselineBasis"];
	readonly generationRef: string;
}) {
	const input = record(inputValue, "D16 qualification persistence input");
	exactKeys(
		input,
		["bundle", "expectedBasis", "generationRef", "privateRoot"],
		"D16 qualification persistence input",
	);
	if (
		...
		throw new TypeError("D16 qualification persistence requires a fresh constructed bundle");
	const bundle = validateD16QualificationBundle(input.bundle);
	if (bundle.qualification.baselineBasis !== input.expectedBasis)
		throw new TypeError("D16 qualification persistence basis drifted");
	...
	const commitMaterial = strictSnapshot({
		schemaVersion: "graphrefly-ts.d16.current-live-no-network-commit.v1",
	...
	const receiptMaterial = strictSnapshot({
		schemaVersion: D16_QUALIFICATION_PERSISTENCE_SCHEMA,
		generationRef: String(input.generationRef),
		bundleArtifactDigest: empiricalSha256(bundleBytes),
		bundleDigest: bundle.bundleDigest,
		qualificationDigest: bundle.qualification.qualificationDigest,
...
export function persistD16QualificationBundle(input: {
	readonly privateRoot: string;
	readonly bundle: D16QualificationBundleV1;
}) {
	return persistQualification({
		...input,
		expectedBasis: "exact-d15-artifact",
		generationRef: D16_CURRENT_GRAPH_LIVE_QUALIFICATION_GENERATION_REF,
	});
}

export function persistD16InjectedQualificationForTest(input: {
	readonly privateRoot: string;
	readonly bundle: D16QualificationBundleV1;
}) {
	return persistQualification({
		...
		generationRef: D16_INJECTED_TEST_GENERATION_REF,
	});
}
