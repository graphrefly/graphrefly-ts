...
import type { CurrentGraphLivePreclaimV1 as D12CurrentGraphLivePreclaimV1 } from "./d8-current-live-preflight.js";
import { consumeCurrentGraphLivePreclaim as consumeD12CurrentGraphLivePreclaim } from "./d8-current-live-preflight.js";
import {
	D12_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST,
	D12_CURRENT_GRAPH_LIVE_DECISION_REF,
	D12_CURRENT_GRAPH_LIVE_DECISION_REVISION,
	D12_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF,
...
export const D12_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_SCHEMA =
	"graphrefly-ts.d12.current-graph-live-dispatch-claim.v1" as const;
export const D12_CURRENT_GRAPH_LIVE_EXECUTION_AUTHORITY_REVISION =
	"graphrefly-ts.d12.current-graph-live-execution-authority.v1" as const;
export const D12_CURRENT_GRAPH_LIVE_PRIVATE_ROOT = resolve(
	import.meta.dirname,
	"../.private/empirical-memory-rerun-avoidance/current-graph-native-d12",
);

type ClaimScope = "live-fixed-root" | "injected-test-root";

export interface D12CurrentGraphLiveDispatchClaimV1 {
	readonly schemaVersion: typeof D12_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_SCHEMA;
	readonly claimRef: typeof D12_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF;
	readonly decisionRef: typeof D12_CURRENT_GRAPH_LIVE_DECISION_REF;
	readonly decisionRevision: typeof D12_CURRENT_GRAPH_LIVE_DECISION_REVISION;
	readonly generationRef: typeof D12_CURRENT_GRAPH_LIVE_GENERATION_REF;
...
export interface D12CurrentGraphLiveExecutionAuthorityV1 {
	readonly revision: typeof D12_CURRENT_GRAPH_LIVE_EXECUTION_AUTHORITY_REVISION;
...
function claimMaterial(
	scope: ClaimScope,
	preclaim: D12CurrentGraphLivePreclaimV1,
	implementationManifestDigest: string,
	qualificationArtifactDigest: string,
	qualificationDigest: string,
) {
	return strictSnapshot({
		schemaVersion: D12_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_SCHEMA,
		claimRef: D12_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF,
...
async function acquire(
	...
	scope: ClaimScope,
	inputValue: {
		readonly preclaim: D12CurrentGraphLivePreclaimV1;
		readonly implementationManifestDigest: string;
		readonly qualificationArtifactDigest: string;
		readonly qualificationDigest: string;
	},
): Promise<D12CurrentGraphLiveDispatchClaimV1> {
	...
	const preclaim = consumeD12CurrentGraphLivePreclaim(input.preclaim);
	...
	const claimRoot = join(privateRoot, `.${D12_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF}`);
	...
	}) as D12CurrentGraphLiveDispatchClaimV1;
...
export function acquireD12CurrentGraphLiveDispatchClaim(input: {
	readonly preclaim: D12CurrentGraphLivePreclaimV1;
	readonly implementationManifestDigest: string;
	readonly qualificationArtifactDigest: string;
	readonly qualificationDigest: string;
}) {
	return acquire(D12_CURRENT_GRAPH_LIVE_PRIVATE_ROOT, "live-fixed-root", input);
}

export function acquireD12CurrentGraphLiveDispatchClaimAtRootForTest(
	root: string,
	input: {
		readonly preclaim: D12CurrentGraphLivePreclaimV1;
		readonly implementationManifestDigest: string;
...
async function revalidateClaim(
	claim: D12CurrentGraphLiveDispatchClaimV1,
	state: ClaimState,
): Promise<void> {
	...
	const material = claimMaterial(
		state.scope,
		{ preclaimDigest: claim.preclaimDigest } as D12CurrentGraphLivePreclaimV1,
...
export async function consumeD12CurrentGraphLiveDispatchClaim(inputValue: {
	readonly claim: D12CurrentGraphLiveDispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
	readonly allowInjectedTestScope?: boolean;
}): Promise<D12CurrentGraphLiveExecutionAuthorityV1> {
	...
	const claim = input.claim as D12CurrentGraphLiveDispatchClaimV1;
	...
	const material = strictSnapshot({
		revision: D12_CURRENT_GRAPH_LIVE_EXECUTION_AUTHORITY_REVISION,
		claim,
		currentKeyAdmission,
	});
	...
	}) as D12CurrentGraphLiveExecutionAuthorityV1;
	authorities.add(authority);
	return authority;
}

export function consumeD12CurrentGraphLiveExecutionAuthority(
	value: unknown,
): D12CurrentGraphLiveExecutionAuthorityV1 {
	...
	return value as D12CurrentGraphLiveExecutionAuthorityV1;
}
