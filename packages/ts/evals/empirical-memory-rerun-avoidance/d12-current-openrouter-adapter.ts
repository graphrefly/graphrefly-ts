...
import { executeD11TransportBoundary } from "./d11-current-transport-boundary.js";
import {
...
export interface D12CurrentGraphOpenRouterAdapterOptionsV1 {
...
async function parseBoundedJson(
	...
	if (signal.aborted) throw new DOMException("D12 provider body deadline", "AbortError");
	let abortListener: (() => void) | null = null;
	const abort = new Promise<never>((_resolve, reject) => {
		abortListener = () => {
			void response.body?.cancel().catch(() => undefined);
			reject(new DOMException("D12 provider body deadline", "AbortError"));
		};
		signal.addEventListener("abort", abortListener, { once: true });
	});
	try {
		const bytes = new Uint8Array(await Promise.race([response.arrayBuffer(), abort]));
...
export function createD12CurrentGraphOpenRouterExecutor(
	options: D12CurrentGraphOpenRouterAdapterOptionsV1,
): D12CurrentGraphLiveExecutorV1 {
	...
	const executeEffect = async (
		effect: CurrentGraphProviderAdmittedEffectV1,
	): Promise<CurrentGraphProviderEffectResultInputV1 | D11TransportResultEnvelopeV1> => {
		...
		try {
			...
			if (request.effectKind === "provider-request") {
				...
				const remainingElapsedMs = () =>
					Math.max(0, request.reservation.maxElapsedMs - Math.ceil(now() - started));
				const scheduleRemaining = (callback: () => void) => {
					const handle = setTimeout(callback, remainingElapsedMs());
					return () => clearTimeout(handle);
				};
				let response: Response | null = null;
				const requestEnvelope = await executeD11TransportBoundary({
					effect,
					phase: "request",
					scheduleTimeout: (callback) => scheduleRemaining(callback),
					invoke: async (signal) => {
						try {
				...
				if (requestEnvelope.transportProposal !== null) return requestEnvelope;
				if (response === null)
					return Object.freeze(requestEnvelope) as D11TransportResultEnvelopeV1;
				return await executeD11TransportBoundary({
					effect,
					phase: "response-body",
					scheduleTimeout: (callback) => scheduleRemaining(callback),
					invoke: async (signal) => {
						const parsed = await parseBoundedJson(response!, signal);
						const elapsed = boundedElapsed(started, now, request.reservation.maxElapsedMs);
						if (!response!.ok) {
	...
	return Object.freeze({
		async execute(effect: CurrentGraphProviderAdmittedEffectV1) {
			...
			if (
				...
				"schemaVersion" in outcome &&
				outcome.schemaVersion === D11_TRANSPORT_ENVELOPE_SCHEMA
			)
				return outcome as D11TransportResultEnvelopeV1;
			return Object.freeze({
