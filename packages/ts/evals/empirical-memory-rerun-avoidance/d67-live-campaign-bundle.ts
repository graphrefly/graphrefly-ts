...
import {
	type D67LiveCampaignEvidenceV1,
	type D67LivePartialCampaignEvidenceV1,
...
import {
	...
	type D67LiveDispatchClaimV1,
	type D67LivePreclaimV1,
...
export const D67_LIVE_BUNDLE_SCHEMA = "graphrefly-ts.d67.live-campaign-bundle.v1" as const;

export type D67LiveCampaignBundleV1 = Readonly<{
	schemaVersion: typeof D67_LIVE_BUNDLE_SCHEMA;
	generationRef: typeof D67_LIVE_GENERATION_REF;
	...
	graphEvidence: D67LiveCampaignEvidenceV1 | null;
	partialGraphEvidence: D67LivePartialCampaignEvidenceV1 | null;
...
function assertBundleCoordinates(input: {
	...
	if (
		...
		throw new TypeError("D67 live bundle lost its exact claim/preflight coordinates");
}

export function constructD67LiveCampaignBundle(input: {
	readonly claim: D67LiveDispatchClaimV1;
	readonly preclaim: D67LivePreclaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
	readonly pricing: D44D45PricingObservationV1;
	...
	readonly measurement:
		| Readonly<{ disposition: "success"; evidence: D67LiveCampaignEvidenceV1 }>
		| Readonly<{
				disposition: "partial-failure";
				partialEvidence: D67LivePartialCampaignEvidenceV1;
		  }>;
}): D67LiveCampaignBundleV1 {
	assertBundleCoordinates(input);
	if (!Number.isSafeInteger(input.providerCalls) || input.providerCalls < 0)
		throw new TypeError("D67 live provider-call reconciliation was invalid");
	const graphEvidence =
		input.measurement.disposition === "success"
			? validateD67LiveCampaignEvidence(input.measurement.evidence)
			: null;
	const partialGraphEvidence =
		input.measurement.disposition === "partial-failure"
			? validateD67LivePartialCampaignEvidence(input.measurement.partialEvidence)
			: null;
	...
	if (
		...
		throw new TypeError("D67 live Graph evidence lost its claim or provider reconciliation");
	const efficacyClaim = graphEvidence?.efficacyClaim ?? "none";
	const material = strictSnapshot({
		schemaVersion: D67_LIVE_BUNDLE_SCHEMA,
...
export function validateD67LiveCampaignBundle(value: unknown): D67LiveCampaignBundleV1 {
	const candidate = record(value, "D67 live campaign bundle");
	exactKeys(
		candidate,
		[
			"schemaVersion",
		...
		"D67 live campaign bundle",
	);
	const suppliedDigest = String(candidate.bundleDigest);
	const { bundleDigest: _bundleDigest, ...material } = candidate;
	if (
		candidate.schemaVersion !== D67_LIVE_BUNDLE_SCHEMA ||
		candidate.generationRef !== D67_LIVE_GENERATION_REF ||
		empiricalStrictJsonDigest(material) !== suppliedDigest
	)
		throw new TypeError("D67 live campaign bundle identity drifted");
	const graphEvidence =
		candidate.graphEvidence === null
			? null
			: validateD67LiveCampaignEvidence(candidate.graphEvidence);
	const partialGraphEvidence =
		candidate.partialGraphEvidence === null
			? null
			: validateD67LivePartialCampaignEvidence(candidate.partialGraphEvidence);
	if (
		...
		throw new TypeError("D67 live campaign bundle disposition or evidence drifted");
	return strictSnapshot(candidate) as unknown as D67LiveCampaignBundleV1;
}

export async function persistD67LiveCampaignBundle(input: {
	readonly privateRoot: string;
	readonly bundle: D67LiveCampaignBundleV1;
}): Promise<Readonly<{ artifactDigest: string; receiptDigest: string }>> {
	if (!isAbsolute(input.privateRoot)) throw new TypeError("D67 persistence root must be absolute");
	...
	const validated = validateD67LiveCampaignBundle(input.bundle);
	const generationRoot = join(privateRoot, D67_LIVE_GENERATION_REF);
	await mkdir(generationRoot, { mode: 0o700 });
	await chmod(generationRoot, 0o700);
	const target = join(generationRoot, "bundle.v1.json");
	const temp = join(privateRoot, `.d67-live-bundle-${process.pid}.tmp`);
	...
	try {
		...
		if (!sameBytes(persisted, bytes)) throw new TypeError("D67 persisted bundle bytes drifted");
		validateD67LiveCampaignBundle(strictJsonCodec.decode(persisted));
	} finally {
		await reader.close();
	}
