...
export const D51_LIVE_CLAIM_SCHEMA = "graphrefly-ts.d51.dispatch-claim.v1" as const;
export const D51_LIVE_BUNDLE_SCHEMA = "graphrefly-ts.d51.live-bundle.v1" as const;
export const D51_LIVE_GENERATION_REF = "current-graph-native-phase-composite-live-d51-v1" as const;
export const D51_LIVE_CLAIM_REF = "current-graph-native-phase-composite-claim-d51-v1" as const;
export const D51_LIVE_PRIVATE_ROOT = resolve(
	import.meta.dirname,
	"../.private/empirical-memory-rerun-avoidance/current-graph-native-d51-live",
);

export interface D51PreclaimV1 {
...
export interface D51DispatchClaimV1 {
	readonly schemaVersion: typeof D51_LIVE_CLAIM_SCHEMA;
	readonly claimRef: typeof D51_LIVE_CLAIM_REF;
	readonly authorityRef: "graphrefly-ts:D51";
	readonly architectureRef: "graphrefly-ts:D50";
	readonly generationRef: typeof D51_LIVE_GENERATION_REF;
	readonly preclaimDigest: string;
...
export interface D51ExecutionAuthorityV1 {
	readonly claim: D51DispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
	readonly authorityDigest: string;
}

export type D51LiveBundleV1 = Readonly<{
	schemaVersion: typeof D51_LIVE_BUNDLE_SCHEMA;
	generationRef: typeof D51_LIVE_GENERATION_REF;
...
export function composeD51Preclaim(input: {
...
export async function acquireD51DispatchClaim(input: {
	readonly privateRoot: string;
	readonly preclaim: D51PreclaimV1;
	readonly implementationCommit: string;
	readonly implementationManifestDigest: string;
	readonly qualificationArtifactDigest: string;
	readonly qualificationDigest: string;
}): Promise<D51DispatchClaimV1> {
	if (!preclaims.delete(input.preclaim)) throw new TypeError("D51 preclaim is absent or consumed");
	const privateRoot = resolve(input.privateRoot);
	if ((await realpath(privateRoot)) !== privateRoot)
		throw new TypeError("D51 private root drifted");
	const claimRoot = join(privateRoot, `.${D51_LIVE_CLAIM_REF}`);
	await mkdir(claimRoot, { mode: 0o700 });
	await chmod(claimRoot, 0o700);
	const material = strictSnapshot({
		schemaVersion: D51_LIVE_CLAIM_SCHEMA,
		claimRef: D51_LIVE_CLAIM_REF,
		authorityRef: "graphrefly-ts:D51" as const,
		architectureRef: "graphrefly-ts:D50" as const,
		generationRef: D51_LIVE_GENERATION_REF,
...
export async function consumeD51DispatchClaim(input: {
	readonly claim: D51DispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
}): Promise<D51ExecutionAuthorityV1> {
	const state = claims.get(input.claim);
	if (state === undefined) throw new TypeError("D51 dispatch claim is absent or consumed");
	...
	try {
		if (!sameBytes(new Uint8Array(await reader.readFile()), state.bytes))
			throw new TypeError("D51 durable dispatch claim drifted");
	} finally {
		await reader.close();
	}
...
export function constructD51LiveBundle(input: {
	readonly authority: D51ExecutionAuthorityV1;
	readonly pricing: D44D45PricingObservationV1;
	readonly zeroByok: D44D45ZeroByokObservationV1;
	readonly implementationCommit: string;
	readonly implementationManifestDigest: string;
	...
	const partialGraphEvidence =
		input.measurement.disposition === "partial-failure"
			...
			: null;
	const material = strictSnapshot({
		schemaVersion: D51_LIVE_BUNDLE_SCHEMA,
		generationRef: D51_LIVE_GENERATION_REF,
...
export async function persistD51LiveBundle(input: {
	readonly privateRoot: string;
	readonly bundle: D51LiveBundleV1;
}): Promise<Readonly<{ artifactDigest: string; receiptDigest: string }>> {
	if (!isAbsolute(input.privateRoot)) throw new TypeError("D51 persistence root must be absolute");
	const generationRoot = join(input.privateRoot, D51_LIVE_GENERATION_REF);
...
export async function prepareD51PrivateRoot(path: string): Promise<void> {
	await mkdir(path, { recursive: true, mode: 0o700 });
	await chmod(path, 0o700);
	const stat = await lstat(path);
	if (!stat.isDirectory() || stat.isSymbolicLink() || (stat.mode & 0o777) !== 0o700)
		throw new TypeError("D51 private root identity failed");
}
