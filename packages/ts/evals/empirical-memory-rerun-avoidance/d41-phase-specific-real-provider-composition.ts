...
import type {
	D41AdmittedEffectV1,
	D41InferenceWireReceiptV1,
} from "./d41-phase-specific-inference-authority.js";

export const D41_ADAPTER_REVISION =
	"graphrefly-ts.d41.phase-specific-real-provider-composition.v1" as const;
const D41_WIRE_RECEIPT_SCHEMA =
	"graphrefly-ts.d41.phase-specific-inference-wire-receipt.v1" as const;
const MAX_REQUEST_BYTES = 2 * 1_048_576;

export interface D41PhaseSpecificRealProviderOptionsV1
	extends Omit<D38PrematureFinalRealProviderOptionsV1, "authority"> {}

export interface D41PhaseSpecificRealProviderExecutorV1 {
	readonly execute: (admitted: D41AdmittedEffectV1) => Promise<
		Readonly<{
			admitted: D41AdmittedEffectV1;
			result: unknown;
			wireReceipt: D41InferenceWireReceiptV1 | null;
		}>
	>;
...
function bodyBytes(value: unknown): Uint8Array {
	...
	throw new TypeError("D41 provider request body is not bounded bytes");
}

function lowerBody(
	body: Uint8Array,
	admitted: D41AdmittedEffectV1,
): Readonly<{ bytes: Uint8Array; receipt: D41InferenceWireReceiptV1 }> {
	if (body.byteLength < 1 || body.byteLength > MAX_REQUEST_BYTES)
		throw new TypeError("D41 provider request body exceeded its bound");
	const directive = admitted.inferenceDirective;
	if (directive === null) throw new TypeError("D41 provider request has no inference directive");
	const decoded = new TextDecoder("utf-8", { fatal: true }).decode(body);
	const candidate = record(JSON.parse(decoded), "D41 provider request body");
	if (candidate.max_tokens !== 65_536)
		throw new TypeError("D41 provider request did not retain the frozen pre-lowering ceiling");
	const reasoning = record(candidate.reasoning, "D41 provider request reasoning");
	if (reasoning.effort !== "high")
		throw new TypeError("D41 provider request reasoning effort drifted");
	...
	if (lowered.byteLength < 1 || lowered.byteLength > MAX_REQUEST_BYTES)
		throw new TypeError("D41 lowered provider request exceeded its bound");
	const material = strictSnapshot({
		schemaVersion: D41_WIRE_RECEIPT_SCHEMA,
...
export function createD41PhaseSpecificRealProviderExecutor(
	options: D41PhaseSpecificRealProviderOptionsV1,
): D41PhaseSpecificRealProviderExecutorV1 {
	const { fetchImpl, ...baseOptions } = options;
	let active: D41AdmittedEffectV1 | null = null;
	let activeReceipt: D41InferenceWireReceiptV1 | null = null;
	let executing = false;
	...
	const base = createD38PrematureFinalRealProviderExecutor({
		...
		fetchImpl: async (url, init) => {
			...
			if (current === null || current.inferenceDirective === null)
				throw new TypeError("D41 transport lacks a Graph inference directive");
			...
			if (prior !== undefined && !Buffer.from(prior).equals(Buffer.from(lowered.bytes)))
				throw new TypeError("D41 same-logical-request retry wire bytes drifted");
	...
	return Object.freeze({
		async execute(admitted: D41AdmittedEffectV1) {
			if (executing) throw new TypeError("D41 observed parallel admitted effects");
			executing = true;
			...
			try {
				...
				if (admitted.inferenceDirective !== null && activeReceipt === null)
					throw new TypeError("D41 provider effect completed without a wire receipt");
				return Object.freeze({
					admitted,
		...
		async dispose() {
			if (executing) throw new TypeError("D41 cannot dispose an active effect");
			active = null;
			activeReceipt = null;
			retryBodies.clear();
			await base.dispose();
