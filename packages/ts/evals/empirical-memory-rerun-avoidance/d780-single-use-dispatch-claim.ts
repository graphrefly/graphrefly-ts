...
import {
	...
	D780_DISPATCH_CLAIM_REF,
	D780_GENERATION_REF,
	D780_HISTORICAL_BUNDLE_DIGEST,
	D780_ROUTE_PROFILE_DIGEST,
...
export const D780_DISPATCH_CLAIM_SCHEMA =
	"graphrefly.b112.d780.single-use-dispatch-claim.v1" as const;
export const D780_PRIVATE_ROOT = resolve(
	import.meta.dirname,
...
export interface D780PersistedDispatchClaimV1 {
	readonly schemaVersion: typeof D780_DISPATCH_CLAIM_SCHEMA;
	readonly claimRef: typeof D780_DISPATCH_CLAIM_REF;
	readonly decisionRef: typeof D780_DECISION_REF;
	readonly decisionRevision: typeof D780_DECISION_REVISION;
	readonly generationRef: typeof D780_GENERATION_REF;
...
export interface D780ExecutionAuthorityV1 {
	readonly revision: "graphrefly.b112.d780.execution-authority.v1";
	readonly claim: D780PersistedDispatchClaimV1;
...
function claimMaterial(
	...
	const input = record(inputValue, "d780.claim.input");
	exactKeys(
		input,
	...
	literal(
		...
		"d780.claim.implementation",
	);
	return strictSnapshot({
		schemaVersion: D780_DISPATCH_CLAIM_SCHEMA,
		claimRef: D780_DISPATCH_CLAIM_REF,
		...
		pricingReadDigest: digest(input.pricingReadDigest, "d780.claim.pricing"),
		zeroByokObservationDigest: digest(input.zeroByokObservationDigest, "d780.claim.zeroByok"),
...
async function acquire(
...
): Promise<D780PersistedDispatchClaimV1> {
	const requestedRoot = resolve(privateRootValue);
	const privateRoot = await realpath(requestedRoot);
	if (privateRoot !== requestedRoot) throw new TypeError("D780 private root is not canonical");
	...
	const claimRoot = join(privateRoot, `.${D780_DISPATCH_CLAIM_REF}`);
	...
	}) as D780PersistedDispatchClaimV1;
	...
	try {
		...
		if (
			...
			throw new TypeError("D780 dispatch claim readback drifted");
	} finally {
		await reader.close();
	}
	claims.set(claim, {
...
export function acquireD780SingleUseDispatchClaim(input: Parameters<typeof claimMaterial>[1]) {
	return acquire(D780_PRIVATE_ROOT, "live-fixed-root", input);
}

export function acquireD780SingleUseDispatchClaimAtRootForTest(
	root: string,
	input: Parameters<typeof claimMaterial>[1],
) {
...
async function revalidate(claim: D780PersistedDispatchClaimV1, state: ClaimState): Promise<void> {
	...
	if (
		...
		throw new TypeError("D780 durable claim directory drifted");
	...
	try {
		...
		if (
			...
			throw new TypeError("D780 durable claim file drifted");
	} finally {
		await reader.close();
	}
	literal(
...
export async function consumeD780DispatchClaimForExecution(inputValue: {
	readonly claim: D780PersistedDispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
}): Promise<D780ExecutionAuthorityV1> {
	const input = record(inputValue, "d780.execution.input");
	exactKeys(input, ["claim", "currentKeyAdmission"], "d780.execution.input");
	const claim = input.claim as D780PersistedDispatchClaimV1;
	const state = typeof claim === "object" && claim !== null ? claims.get(claim) : undefined;
	if (state === undefined) throw new TypeError("D780 claim is not fresh and constructed");
	claims.delete(claim);
	...
	if (
		...
		admission.remainingMicrousd < D780_BUDGET_LIMITS.maxCostMicrousd ||
		admission.limitReset !== "none" ||
		admission.isManagementKey
	)
		throw new TypeError("D780 current-key admission is outside authority");
	...
	const authority = Object.freeze({
		revision: "graphrefly.b112.d780.execution-authority.v1" as const,
		claim,
...
export function consumeD780ExecutionAuthority(value: unknown): D780ExecutionAuthorityV1 {
	if (typeof value !== "object" || value === null || !authorities.delete(value))
		throw new TypeError("D780 live execution authority must be fixed-root and single-use");
	return value as D780ExecutionAuthorityV1;
}
