...
export async function measureCurrentGraphLiveImplementation(
	...
	// D7 audits the frozen D6 closure; current D6 revisions are qualified separately.
	const measured = Object.fromEntries(
		await Promise.all(
			Object.entries(FILES).map(async ([key, file]) => [
				key,
				key.startsWith("d6")
					? D7_CURRENT_GRAPH_LIVE_IMPLEMENTATION_SOURCE_HASHES[
							key as keyof typeof D7_CURRENT_GRAPH_LIVE_IMPLEMENTATION_SOURCE_HASHES
						]
