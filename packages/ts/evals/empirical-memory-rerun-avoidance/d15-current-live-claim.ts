...
import type { CurrentGraphLivePreclaimV1 as D15CurrentGraphLivePreclaimV1 } from "./d8-current-live-preflight.js";
import { consumeCurrentGraphLivePreclaim as consumeD15CurrentGraphLivePreclaim } from "./d8-current-live-preflight.js";
import {
	D15_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST,
	D15_CURRENT_GRAPH_LIVE_DECISION_REF,
	D15_CURRENT_GRAPH_LIVE_DECISION_REVISION,
	D15_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF,
...
export const D15_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_SCHEMA =
	"graphrefly-ts.d15.current-graph-live-dispatch-claim.v1" as const;
export const D15_CURRENT_GRAPH_LIVE_EXECUTION_AUTHORITY_REVISION =
	"graphrefly-ts.d15.current-graph-live-execution-authority.v1" as const;
export const D15_CURRENT_GRAPH_LIVE_PRIVATE_ROOT = resolve(
	import.meta.dirname,
	"../.private/empirical-memory-rerun-avoidance/current-graph-native-d15",
);

type ClaimScope = "live-fixed-root" | "injected-test-root";

export interface D15CurrentGraphLiveDispatchClaimV1 {
	readonly schemaVersion: typeof D15_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_SCHEMA;
	readonly claimRef: typeof D15_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF;
	readonly decisionRef: typeof D15_CURRENT_GRAPH_LIVE_DECISION_REF;
	readonly decisionRevision: typeof D15_CURRENT_GRAPH_LIVE_DECISION_REVISION;
	readonly generationRef: typeof D15_CURRENT_GRAPH_LIVE_GENERATION_REF;
...
export interface D15CurrentGraphLiveExecutionAuthorityV1 {
	readonly revision: typeof D15_CURRENT_GRAPH_LIVE_EXECUTION_AUTHORITY_REVISION;
...
function claimMaterial(
	scope: ClaimScope,
	preclaim: D15CurrentGraphLivePreclaimV1,
	implementationManifestDigest: string,
	qualificationArtifactDigest: string,
	qualificationDigest: string,
) {
	return strictSnapshot({
		schemaVersion: D15_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_SCHEMA,
		claimRef: D15_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF,
...
async function acquire(
	...
	scope: ClaimScope,
	inputValue: {
		readonly preclaim: D15CurrentGraphLivePreclaimV1;
		readonly implementationManifestDigest: string;
		readonly qualificationArtifactDigest: string;
		readonly qualificationDigest: string;
	},
): Promise<D15CurrentGraphLiveDispatchClaimV1> {
	...
	const preclaim = consumeD15CurrentGraphLivePreclaim(input.preclaim);
	...
	const claimRoot = join(privateRoot, `.${D15_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF}`);
	...
	}) as D15CurrentGraphLiveDispatchClaimV1;
...
export function acquireD15CurrentGraphLiveDispatchClaim(input: {
	readonly preclaim: D15CurrentGraphLivePreclaimV1;
	readonly implementationManifestDigest: string;
	readonly qualificationArtifactDigest: string;
	readonly qualificationDigest: string;
}) {
	return acquire(D15_CURRENT_GRAPH_LIVE_PRIVATE_ROOT, "live-fixed-root", input);
}

export function acquireD15CurrentGraphLiveDispatchClaimAtRootForTest(
	root: string,
	input: {
		readonly preclaim: D15CurrentGraphLivePreclaimV1;
		readonly implementationManifestDigest: string;
...
async function revalidateClaim(
	claim: D15CurrentGraphLiveDispatchClaimV1,
	state: ClaimState,
): Promise<void> {
	...
	const material = claimMaterial(
		state.scope,
		{ preclaimDigest: claim.preclaimDigest } as D15CurrentGraphLivePreclaimV1,
...
export async function consumeD15CurrentGraphLiveDispatchClaim(inputValue: {
	readonly claim: D15CurrentGraphLiveDispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
	readonly allowInjectedTestScope?: boolean;
}): Promise<D15CurrentGraphLiveExecutionAuthorityV1> {
	...
	const claim = input.claim as D15CurrentGraphLiveDispatchClaimV1;
	...
	const material = strictSnapshot({
		revision: D15_CURRENT_GRAPH_LIVE_EXECUTION_AUTHORITY_REVISION,
		claim,
		currentKeyAdmission,
	});
	...
	}) as D15CurrentGraphLiveExecutionAuthorityV1;
	authorities.add(authority);
	return authority;
}

export function consumeD15CurrentGraphLiveExecutionAuthority(
	value: unknown,
): D15CurrentGraphLiveExecutionAuthorityV1 {
	...
	return value as D15CurrentGraphLiveExecutionAuthorityV1;
}
