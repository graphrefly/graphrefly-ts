...
import type { CurrentGraphLiveExecutorV1 } from "./d6-current-live.js";
import {
...
import type { CurrentGraphLiveCredentialV1 } from "./d6-current-live-preflight.js";
import type {
	...
	try {
		...
		const commit = await runProcess({
			...
			args: [
				...
				"--no-gpg-sign",
				"-m",
				"current Graph-native D6 task baseline",
			],
			cwd: root,
			timeoutMs: 30_000,
		});
