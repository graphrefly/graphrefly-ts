...
import {
	...
	D782_DISPATCH_CLAIM_REF,
	D782_GENERATION_REF,
	D782_HISTORICAL_BUNDLE_DIGEST,
	D782_ROUTE_PROFILE_DIGEST,
...
export const D782_DISPATCH_CLAIM_SCHEMA =
	"graphrefly.b112.d782.single-use-dispatch-claim.v1" as const;
export const D782_PRIVATE_ROOT = resolve(
	import.meta.dirname,
...
export interface D782PersistedDispatchClaimV1 {
	readonly schemaVersion: typeof D782_DISPATCH_CLAIM_SCHEMA;
	readonly claimRef: typeof D782_DISPATCH_CLAIM_REF;
	readonly decisionRef: typeof D782_DECISION_REF;
	readonly decisionRevision: typeof D782_DECISION_REVISION;
	readonly generationRef: typeof D782_GENERATION_REF;
...
export interface D782ExecutionAuthorityV1 {
	readonly revision: "graphrefly.b112.d782.execution-authority.v1";
	readonly claim: D782PersistedDispatchClaimV1;
...
function claimMaterial(
	...
	const input = record(inputValue, "d782.claim.input");
	exactKeys(
		input,
	...
	literal(
		...
		"d782.claim.implementation",
	);
	return strictSnapshot({
		schemaVersion: D782_DISPATCH_CLAIM_SCHEMA,
		claimRef: D782_DISPATCH_CLAIM_REF,
		...
		pricingReadDigest: digest(input.pricingReadDigest, "d782.claim.pricing"),
		zeroByokObservationDigest: digest(input.zeroByokObservationDigest, "d782.claim.zeroByok"),
...
async function acquire(
...
): Promise<D782PersistedDispatchClaimV1> {
	const requestedRoot = resolve(privateRootValue);
	const privateRoot = await realpath(requestedRoot);
	if (privateRoot !== requestedRoot) throw new TypeError("D782 private root is not canonical");
	...
	const claimRoot = join(privateRoot, `.${D782_DISPATCH_CLAIM_REF}`);
	...
	}) as D782PersistedDispatchClaimV1;
	...
	try {
		...
		if (
			...
			throw new TypeError("D782 dispatch claim readback drifted");
	} finally {
		await reader.close();
	}
	claims.set(claim, {
...
export function acquireD782SingleUseDispatchClaim(input: Parameters<typeof claimMaterial>[1]) {
	return acquire(D782_PRIVATE_ROOT, "live-fixed-root", input);
}

export function acquireD782SingleUseDispatchClaimAtRootForTest(
	root: string,
	input: Parameters<typeof claimMaterial>[1],
) {
...
async function revalidate(claim: D782PersistedDispatchClaimV1, state: ClaimState): Promise<void> {
	...
	if (
		...
		throw new TypeError("D782 durable claim directory drifted");
	...
	try {
		...
		if (
			...
			throw new TypeError("D782 durable claim file drifted");
	} finally {
		await reader.close();
	}
	literal(
...
export async function consumeD782DispatchClaimForExecution(inputValue: {
	readonly claim: D782PersistedDispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
}): Promise<D782ExecutionAuthorityV1> {
	const input = record(inputValue, "d782.execution.input");
	exactKeys(input, ["claim", "currentKeyAdmission"], "d782.execution.input");
	const claim = input.claim as D782PersistedDispatchClaimV1;
	const state = typeof claim === "object" && claim !== null ? claims.get(claim) : undefined;
	if (state === undefined) throw new TypeError("D782 claim is not fresh and constructed");
	claims.delete(claim);
	...
	if (
		...
		admission.remainingMicrousd < D782_BUDGET_LIMITS.maxCostMicrousd ||
		admission.limitReset !== "none" ||
		admission.isManagementKey
	)
		throw new TypeError("D782 current-key admission is outside authority");
	...
	const authority = Object.freeze({
		revision: "graphrefly.b112.d782.execution-authority.v1" as const,
		claim,
...
export function consumeD782ExecutionAuthority(value: unknown): D782ExecutionAuthorityV1 {
	if (typeof value !== "object" || value === null || !authorities.delete(value))
		throw new TypeError("D782 live execution authority must be fixed-root and single-use");
	return value as D782ExecutionAuthorityV1;
}
