...
import {
	...
	type D774ProviderResultEnvelopeV1,
	type D774RouteEvidenceV1,
	validateD774ProviderResultEnvelope,
} from "./d774-provider-result-route-authority.js";

export const D774_GRAPH_NATIVE_CORE_REVISION =
	"graphrefly.b112.d774.provider-envelope-graph-core.v1" as const;

export interface D774CallerExecutorV1 {
	readonly revision: "graphrefly.b112.d774.caller-executor.v1";
	readonly execute: (
		input: D720CallerEffectExecutionInputV2,
	) => Promise<D774ProviderResultEnvelopeV1>;
}

export interface D774GraphNativeCoreV1 {
...
function validateExecutor(value: unknown): D774CallerExecutorV1 {
	const candidate = record(value, "d774.executor");
	exactKeys(candidate, ["execute", "revision"], "d774.executor");
	if (candidate.revision !== "graphrefly.b112.d774.caller-executor.v1")
		throw new TypeError("D774 executor revision drifted");
	...
	if (
		...
		throw new TypeError("D774 executor must expose one own immutable execution function");
	return candidate as unknown as D774CallerExecutorV1;
}
...
function failedEffectResult(
	...
	if (request.effectKind === "tool-action") {
		if (request.toolIntent === null) throw new TypeError("D774 failed tool lacks Graph intent");
		const state =
			request.workspaceStateDigest ??
			empiricalStrictJsonDigest({ unavailable: request.requestDigest });
		return Object.freeze({
...
export async function runD774GraphNativeEvalCore(inputValue: {
...
}): Promise<D774GraphNativeCoreV1> {
	const input = record(inputValue, "d774.core");
	exactKeys(
		input,
		[
			"budgetLimits",
	...
	if (Object.hasOwn(input, "signal") && !(input.signal instanceof AbortSignal))
		throw new TypeError("D774 signal is invalid");
	...
	const routeAuthority = createD774RouteAuthority();
	...
	for (let runCount = 0; runCount < 12; runCount += 1) {
		...
		for (let effectCount = 0; effectCount < 512; effectCount += 1) {
			...
			if (effectRequest === null) throw new TypeError("D774 Graph effect request is missing");
			...
			let envelope: D774ProviderResultEnvelopeV1;
			try {
				...
				envelope = validateD774ProviderResultEnvelope(raw, effectRequest, admission);
			} catch {
			...
			if (effectRequest.effectKind === "provider-request") {
				...
				if (proposal === null) throw new TypeError("D774 provider result omitted route proposal");
	...
	if (takeNextD719CleanGraphRequest(ledger) !== null)
		throw new TypeError("D774 six-arm bound ended while Graph still exposed work");
	return Object.freeze({
...
export function createD774CallerExecutor(
	execute: (
		input: Readonly<{
...
): D774CallerExecutorV1 {
	if (typeof execute !== "function") throw new TypeError("D774 executor must be a function");
	return Object.freeze({
		revision: "graphrefly.b112.d774.caller-executor.v1" as const,
		execute,
	});
}
