...
import {
	createD40InjectedBaselineForTest,
	type D40D39BaselineAdmissionV1,
	type D40LiveBundleV1,
	persistD40LiveBundle,
	runD40InjectedMeasurementForTest,
	validateD40LiveBundle,
} from "./d40-phase-specific-inference-live.js";
import {
	acquireD40DispatchClaimAtRootForTest,
...
import { D40_IMPLEMENTATION_MANIFEST_DIGEST } from "./d40-phase-specific-inference-live-implementation-manifest.js";
import {
	admitD40ZeroByok,
	composeD40Preclaim,
...
import { createD40PhaseSpecificRealProviderExecutor } from "./d40-phase-specific-real-provider-composition.js";

export const D40_QUALIFICATION_SCHEMA =
	"graphrefly-ts.d40.phase-specific-inference-live-qualification.v1" as const;
export const D40_QUALIFICATION_BUNDLE_SCHEMA =
	"graphrefly-ts.d40.phase-specific-inference-live-qualification-bundle.v1" as const;
export const D40_QUALIFICATION_GENERATION_SCHEMA =
	"graphrefly-ts.d40.phase-specific-inference-live-qualification-generation.v1" as const;

export interface D40QualificationBundleV1 {
	readonly schemaVersion: typeof D40_QUALIFICATION_BUNDLE_SCHEMA;
	readonly baselineBasis: "consumed-d39-artifact" | "injected-test";
	readonly mainBundle: D40LiveBundleV1;
	readonly qualification: Readonly<{
		readonly schemaVersion: typeof D40_QUALIFICATION_SCHEMA;
		...
		readonly implementationManifestDigest: typeof D40_IMPLEMENTATION_MANIFEST_DIGEST;
	...
	readonly generation: Readonly<{
		readonly schemaVersion: typeof D40_QUALIFICATION_GENERATION_SCHEMA;
		readonly generationRef: typeof D40_QUALIFICATION_GENERATION_REF;
		readonly qualificationDigest: string;
		readonly mainBundleDigest: string;
		readonly implementationManifestDigest: typeof D40_IMPLEMENTATION_MANIFEST_DIGEST;
		readonly causalAttribution: "undetermined";
...
async function makeTestPreclaim(credential: D40CredentialV1, nowMs: number) {
...
async function issueTestAuthority(input: {
	...
	const claim = await acquireD40DispatchClaimAtRootForTest(input.privateRoot, {
	...
	try {
		await acquireD40DispatchClaimAtRootForTest(input.privateRoot, {
			preclaim: await makeTestPreclaim(input.credential, input.nowMs),
			nowMs: input.nowMs,
			implementationManifestDigest: D40_IMPLEMENTATION_MANIFEST_DIGEST,
			qualificationArtifactDigest: qualificationCoordinate,
	...
	if (!duplicateRejected) throw new TypeError("D40 duplicate test claim was admitted");
	...
	return consumeD40DispatchClaim({ claim, currentKeyAdmission, allowInjectedTestScope: true });
}
...
function deriveMain(bundleValue: unknown) {
	const bundle = validateD40LiveBundle(bundleValue);
	if (
		...
		throw new TypeError("D40 qualification main disposition drifted");
	...
	if (
		...
		throw new TypeError("D40 qualification six-arm lifecycle drifted");
	...
	if (
		...
		throw new TypeError("D40 qualification inference ceilings drifted");
	...
	if (providerFacts.length !== evidence.facts.length)
		throw new TypeError("D40 inference/provider fact reverse coverage drifted");
	for (const fact of evidence.facts) {
		...
		if (provider === undefined || provider.factDigest !== fact.providerFactDigest)
			throw new TypeError("D40 inference/provider fact binding drifted");
	}
	const waits = retained.phaseEvidence.workflowEvidence.providerEvidence.facts.filter(
		(fact) => fact.request.effectKind === "retry-wait",
	);
	if (waits.length !== 1) throw new TypeError("D40 retry wait count drifted");
	...
	if (
		...
		retried[0]![0]!.maxOutputTokens !== D40_MUTATION_MAX_OUTPUT_TOKENS
	)
		throw new TypeError("D40 retry wire identity drifted");
	...
	if (
		...
		throw new TypeError("D40 retry conservative accounting drifted");
...
async function qualifyPartialFailure(input: {
	...
	const partial = await runD40InjectedMeasurementForTest({
		executionAuthority,
		...
		executorFactory: () =>
			Object.freeze({
				async execute() {
					throw new TypeError("injected D40 executor failure");
				},
				async dispose() {},
			}),
	});
	const validated = validateD40LiveBundle(partial);
	if (
		...
		throw new TypeError("D40 partial failure evidence drifted");
	...
	await persistD40LiveBundle({ privateRoot: await realpath(persistenceRoot), bundle: partial });
	...
	if (!replayRejected) throw new TypeError("D40 partial persistence replay was admitted");
}

export async function runD40InjectedNoNetworkQualification(input: {
	readonly baseline: D40D39BaselineAdmissionV1;
	readonly baselineBasis: D40QualificationBundleV1["baselineBasis"];
	readonly repositoryRoot: string;
	readonly materializationRoot: string;
}): Promise<D40QualificationBundleV1> {
	const testRoot = await mkdtemp(join(tmpdir(), "graphrefly-d40-qualification-"));
	await chmod(testRoot, 0o700);
	const claimRoot = join(testRoot, "claim");
	await mkdir(claimRoot, { mode: 0o700 });
	await chmod(claimRoot, 0o700);
	const credential = Object.freeze({
		bearerToken: "d40-injected-no-network-credential",
	...
	try {
		...
		const main = await runD40InjectedMeasurementForTest({
			executionAuthority,
			baseline: input.baseline,
			implementationManifestDigest: D40_IMPLEMENTATION_MANIFEST_DIGEST,
			allowConsumedBaselineForQualification: input.baselineBasis === "consumed-d39-artifact",
			executorFactory: () =>
				createD40PhaseSpecificRealProviderExecutor({
					...
					fetchImpl: async (_url, init) => {
						...
						try {
							...
							const messages = array(body.messages, "D40 injected messages");
							...
							const toolName =
								choice === "required"
									? hasReadResult
										...
										: "read_file"
									: record(
											record(choice, "D40 injected tool choice").function,
											"D40 injected tool function",
										).name;
							if (toolName === "read_file") return batchReadResponse();
							if (toolName !== "replace_exact")
								throw new TypeError("D40 injected tool choice drifted");
							if (!retryInjected) {
								retryInjected = true;
								return new Response(
		...
		if (maxActiveTransport !== 1)
			throw new TypeError("D40 qualification observed non-serial transport");
		await qualifyPartialFailure({ testRoot, credential });
		const qualificationMaterial = strictSnapshot({
			schemaVersion: D40_QUALIFICATION_SCHEMA,
			decisionRef: D40_DECISION_REF,
		...
		const generationMaterial = strictSnapshot({
			schemaVersion: D40_QUALIFICATION_GENERATION_SCHEMA,
			generationRef: D40_QUALIFICATION_GENERATION_REF,
		...
		const material = strictSnapshot({
			schemaVersion: D40_QUALIFICATION_BUNDLE_SCHEMA,
...
function validateQualification(value: unknown, main: D40LiveBundleV1) {
	const candidate = record(value, "D40 qualification");
	exactKeys(
		candidate,
		[
			"causalAttribution",
	...
	if (
		...
		throw new TypeError("D40 qualification drifted");
	return Object.freeze({ ...material, qualificationDigest: candidate.qualificationDigest });
}

export function validateD40QualificationBundle(value: unknown): D40QualificationBundleV1 {
	const candidate = record(value, "D40 qualification bundle");
	exactKeys(
		candidate,
		["baselineBasis", "bundleDigest", "generation", "mainBundle", "qualification", "schemaVersion"],
		"D40 qualification bundle",
	);
	if (
		candidate.schemaVersion !== D40_QUALIFICATION_BUNDLE_SCHEMA ||
		(candidate.baselineBasis !== "consumed-d39-artifact" &&
			candidate.baselineBasis !== "injected-test")
	)
		throw new TypeError("D40 qualification bundle coordinates drifted");
	const mainBundle = validateD40LiveBundle(candidate.mainBundle);
	const qualification = validateQualification(candidate.qualification, mainBundle);
	const generation = record(candidate.generation, "D40 qualification generation");
	exactKeys(
		generation,
		[
			"causalAttribution",
	...
	if (
		...
		throw new TypeError("D40 qualification generation drifted");
	const material = strictSnapshot({
		schemaVersion: D40_QUALIFICATION_BUNDLE_SCHEMA,
	...
	if (candidate.bundleDigest !== empiricalStrictJsonDigest(material))
		throw new TypeError("D40 qualification bundle digest drifted");
	return Object.freeze({
		...material,
...
export async function persistD40Qualification(input: {
	readonly privateRoot: string;
	readonly bundle: D40QualificationBundleV1;
}) {
	if (!constructed.has(input.bundle as object))
		throw new TypeError("D40 qualification bundle is forged or replayed");
	constructed.delete(input.bundle as object);
	const bundle = validateD40QualificationBundle(input.bundle);
	if (bundle.baselineBasis !== "consumed-d39-artifact")
		throw new TypeError("D40 production qualification requires consumed D39 bytes");
	const commitMaterial = strictSnapshot({
		schemaVersion: "graphrefly-ts.d40.phase-specific-inference-live-qualification-commit.v1",
		generationRef: D40_QUALIFICATION_GENERATION_REF,
...
export function qualificationDigest(value: unknown): string {
	return digest(
		record(validateD40QualificationBundle(value).qualification, "D40 qualification")
			.qualificationDigest,
		"D40 qualification digest",
	);
}
...
export function qualificationProviderAttemptCount(value: unknown): number {
	return safeInteger(
		record(validateD40QualificationBundle(value).qualification, "D40 qualification")
			.providerAttemptCount,
		"D40 qualification provider attempt count",
	);
}
