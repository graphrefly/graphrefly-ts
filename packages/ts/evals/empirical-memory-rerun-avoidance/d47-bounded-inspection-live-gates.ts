...
export const D47_LIVE_CLAIM_SCHEMA = "graphrefly-ts.d47.dispatch-claim.v1" as const;
export const D47_LIVE_BUNDLE_SCHEMA = "graphrefly-ts.d47.live-bundle.v1" as const;
export const D47_LIVE_GENERATION_REF =
	"current-graph-native-bounded-inspection-live-d47-v1" as const;
export const D47_LIVE_CLAIM_REF = "current-graph-native-bounded-inspection-claim-d47-v1" as const;
export const D47_LIVE_PRIVATE_ROOT = resolve(
	import.meta.dirname,
	"../.private/empirical-memory-rerun-avoidance/current-graph-native-d47-live",
);

export interface D47PreclaimV1 {
...
export interface D47DispatchClaimV1 {
	readonly schemaVersion: typeof D47_LIVE_CLAIM_SCHEMA;
	readonly claimRef: typeof D47_LIVE_CLAIM_REF;
	readonly authorityRef: "graphrefly-ts:D47";
	readonly architectureRef: "graphrefly-ts:D46";
	readonly generationRef: typeof D47_LIVE_GENERATION_REF;
	readonly preclaimDigest: string;
...
export interface D47ExecutionAuthorityV1 {
	readonly claim: D47DispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
	readonly authorityDigest: string;
}

export type D47LiveBundleV1 = Readonly<{
	schemaVersion: typeof D47_LIVE_BUNDLE_SCHEMA;
	generationRef: typeof D47_LIVE_GENERATION_REF;
...
export function composeD47Preclaim(input: {
...
export async function acquireD47DispatchClaim(input: {
	readonly privateRoot: string;
	readonly preclaim: D47PreclaimV1;
	readonly implementationCommit: string;
	readonly implementationManifestDigest: string;
	readonly qualificationArtifactDigest: string;
	readonly qualificationDigest: string;
}): Promise<D47DispatchClaimV1> {
	if (!preclaims.delete(input.preclaim)) throw new TypeError("D47 preclaim is absent or consumed");
	const privateRoot = resolve(input.privateRoot);
	if ((await realpath(privateRoot)) !== privateRoot)
		throw new TypeError("D47 private root drifted");
	const claimRoot = join(privateRoot, `.${D47_LIVE_CLAIM_REF}`);
	await mkdir(claimRoot, { mode: 0o700 });
	await chmod(claimRoot, 0o700);
	const material = strictSnapshot({
		schemaVersion: D47_LIVE_CLAIM_SCHEMA,
		claimRef: D47_LIVE_CLAIM_REF,
		authorityRef: "graphrefly-ts:D47" as const,
		architectureRef: "graphrefly-ts:D46" as const,
		generationRef: D47_LIVE_GENERATION_REF,
...
export async function consumeD47DispatchClaim(input: {
	readonly claim: D47DispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
}): Promise<D47ExecutionAuthorityV1> {
	const state = claims.get(input.claim);
	if (state === undefined) throw new TypeError("D47 dispatch claim is absent or consumed");
	...
	try {
		if (!sameBytes(new Uint8Array(await reader.readFile()), state.bytes))
			throw new TypeError("D47 durable dispatch claim drifted");
	} finally {
		await reader.close();
	}
...
export function constructD47LiveBundle(input: {
	readonly authority: D47ExecutionAuthorityV1;
	readonly pricing: D44D45PricingObservationV1;
	readonly zeroByok: D44D45ZeroByokObservationV1;
	readonly implementationCommit: string;
	readonly implementationManifestDigest: string;
	...
	const partialGraphEvidence =
		input.measurement.disposition === "partial-failure"
			...
			: null;
	const material = strictSnapshot({
		schemaVersion: D47_LIVE_BUNDLE_SCHEMA,
		generationRef: D47_LIVE_GENERATION_REF,
...
export async function persistD47LiveBundle(input: {
	readonly privateRoot: string;
	readonly bundle: D47LiveBundleV1;
}): Promise<Readonly<{ artifactDigest: string; receiptDigest: string }>> {
	if (!isAbsolute(input.privateRoot)) throw new TypeError("D47 persistence root must be absolute");
	const generationRoot = join(input.privateRoot, D47_LIVE_GENERATION_REF);
...
export async function prepareD47PrivateRoot(path: string): Promise<void> {
	await mkdir(path, { recursive: true, mode: 0o700 });
	await chmod(path, 0o700);
	const stat = await lstat(path);
	if (!stat.isDirectory() || stat.isSymbolicLink() || (stat.mode & 0o777) !== 0o700)
		throw new TypeError("D47 private root identity failed");
}
