...
import { D12_IMPLEMENTATION_MANIFEST_DIGEST } from "./d12-current-implementation-manifest.js";
import {
	type D12CurrentGraphLiveBundleV1,
	runD12CurrentGraphLiveMeasurement,
	validateD12CurrentGraphLiveBundle,
} from "./d12-current-live.js";
import {
	acquireD12CurrentGraphLiveDispatchClaimAtRootForTest,
	consumeD12CurrentGraphLiveDispatchClaim,
} from "./d12-current-live-claim.js";
import {
	D12_CURRENT_GRAPH_LIVE_DECISION_REF,
...
export const D12_QUALIFICATION_SCHEMA =
	"graphrefly-ts.d12.current-live-no-network-qualification.v1" as const;
export const D12_QUALIFICATION_BUNDLE_SCHEMA =
	"graphrefly-ts.d12.current-live-no-network-bundle.v1" as const;
export const D12_QUALIFICATION_GENERATION_SCHEMA =
	"graphrefly-ts.d12.current-live-no-network-generation.v1" as const;
export const D12_QUALIFICATION_PERSISTENCE_SCHEMA =
	"graphrefly-ts.d12.current-live-no-network-persistence.v1" as const;
export const D12_INJECTED_TEST_GENERATION_REF =
	"current-graph-native-live-no-network-injected-test-d12-v1" as const;
export const D12_MAX_QUALIFICATION_BYTES = 4_194_304;

export interface D12D11BaselineAdmissionV1 {
	readonly revision: "graphrefly-ts.d12.d11-baseline-admission.v1";
}

export interface D12QualificationBundleV1 {
	readonly schemaVersion: typeof D12_QUALIFICATION_BUNDLE_SCHEMA;
	readonly qualification: Readonly<{
		schemaVersion: typeof D12_QUALIFICATION_SCHEMA;
		decisionRef: typeof D12_CURRENT_GRAPH_LIVE_DECISION_REF;
		implementationManifestDigest: string;
		baselineBasis: "exact-d11-artifact" | "injected-test";
		d11QualificationArtifactDigest: typeof D12_D11_QUALIFICATION_ARTIFACT_DIGEST;
		d11QualificationBundleDigest: typeof D12_D11_QUALIFICATION_BUNDLE_DIGEST;
		d11QualificationDigest: typeof D12_D11_QUALIFICATION_DIGEST;
	...
	readonly graphBundle: D12CurrentGraphLiveBundleV1;
	readonly generation: Readonly<{
		schemaVersion: typeof D12_QUALIFICATION_GENERATION_SCHEMA;
		generationRef: typeof D12_CURRENT_GRAPH_LIVE_QUALIFICATION_GENERATION_REF;
		qualificationDigest: string;
		graphBundleDigest: string;
		implementationManifestDigest: string;
		causalAttribution: "undetermined";
...
const baselineStates = new WeakMap<object, "exact-d11-artifact" | "injected-test">();
const constructedBundles = new WeakSet<object>();

function baselineCapability(
	basis: "exact-d11-artifact" | "injected-test",
): D12D11BaselineAdmissionV1 {
	const capability = Object.freeze({
		revision: "graphrefly-ts.d12.d11-baseline-admission.v1" as const,
	});
	baselineStates.set(capability, basis);
	return capability;
}

export function admitD12D11QualificationBaseline(
	bytesValue: Uint8Array,
): D12D11BaselineAdmissionV1 {
	if (!(bytesValue instanceof Uint8Array))
		throw new TypeError("D12 D11 baseline bytes are invalid");
	const bytes = new Uint8Array(bytesValue);
	if (empiricalSha256(bytes) !== D12_D11_QUALIFICATION_ARTIFACT_DIGEST)
		throw new TypeError("D12 D11 qualification artifact drifted");
	const bundle = validateD11QualificationBundle(strictJsonCodec.decode(bytes));
	if (
		bundle.bundleDigest !== D12_D11_QUALIFICATION_BUNDLE_DIGEST ||
		bundle.qualification.qualificationDigest !== D12_D11_QUALIFICATION_DIGEST ||
		bundle.qualification.implementationManifestDigest !== D12_D11_IMPLEMENTATION_MANIFEST_DIGEST
	)
		throw new TypeError("D12 D11 qualification projection drifted");
	return baselineCapability("exact-d11-artifact");
}

export function createD12InjectedD11BaselineForTest(): D12D11BaselineAdmissionV1 {
...
export async function runD12InjectedNoNetworkQualification(inputValue: {
	readonly repositoryRoot: string;
	readonly baseline: D12D11BaselineAdmissionV1;
	readonly implementationManifestDigest: string;
}): Promise<D12QualificationBundleV1> {
	const input = record(inputValue, "D12 qualification input");
	exactKeys(
		input,
		["baseline", "implementationManifestDigest", "repositoryRoot"],
		"D12 qualification input",
	);
	const baseline = input.baseline as D12D11BaselineAdmissionV1;
	const baselineBasis =
		typeof baseline === "object" && baseline !== null ? baselineStates.get(baseline) : undefined;
	if (baselineBasis === undefined) throw new TypeError("D12 D11 baseline is forged or replayed");
	baselineStates.delete(baseline);
	const implementationManifestDigest = digest(
		input.implementationManifestDigest,
		"D12 implementation manifest",
	);
	if (implementationManifestDigest !== D12_IMPLEMENTATION_MANIFEST_DIGEST)
		throw new TypeError("D12 qualification implementation manifest drifted");
	const repositoryRoot = await realpath(resolve(String(input.repositoryRoot)));
	const temporaryRoot = await mkdtemp(join(tmpdir(), "graphrefly-d12-no-network-"));
	await chmod(temporaryRoot, 0o700);
	try {
		const credential = Object.freeze({
			bearerToken: "sk-or-v1-test-current-graph-live-key-d12",
		...
		const claim = await acquireD12CurrentGraphLiveDispatchClaimAtRootForTest(
			await realpath(privateRoot),
			{
				preclaim,
				implementationManifestDigest,
				qualificationArtifactDigest: empiricalStrictJsonDigest({ injected: "d12" }),
				qualificationDigest: empiricalStrictJsonDigest({ injected: "d12-qualification" }),
			},
		);
			...
			try {
				...
				if (pendingRetryBodyDigest !== null) {
					if (pendingRetryBodyDigest !== currentBodyDigest)
						throw new TypeError("D12 retry request bytes drifted");
		...
		const graphBundle = validateD12CurrentGraphLiveBundle(
			await runD12CurrentGraphLiveMeasurement({
				executionAuthority,
		...
		if (graphBundle.disposition !== "success" || graphBundle.graphEvidence === null)
			throw new TypeError("D12 no-network Graph integration did not complete");
		...
		if (
			...
			throw new TypeError("D12 no-network Graph transport projection drifted");
		await lstat(materializationRoot).then(
			() => {
				throw new TypeError("D12 no-network left workspace residue");
			},
			(error: unknown) => {
		...
		if (!source.includes(CURRENT_GRAPH_LIVE_FIXED_ADMISSION_BLOCK))
			throw new TypeError("D12 no-network mutated the source workspace");
		const qualificationMaterial = strictSnapshot({
			schemaVersion: D12_QUALIFICATION_SCHEMA,
		...
		const generationMaterial = strictSnapshot({
			schemaVersion: D12_QUALIFICATION_GENERATION_SCHEMA,
			generationRef: D12_CURRENT_GRAPH_LIVE_QUALIFICATION_GENERATION_REF,
		...
		const bundleMaterial = strictSnapshot({
			schemaVersion: D12_QUALIFICATION_BUNDLE_SCHEMA,
			qualification,
			graphBundle,
			generation,
		});
		...
		}) as D12QualificationBundleV1;
		if (
			strictJsonCodec.encode(bundle as unknown as StrictJsonValue).byteLength >
			D12_MAX_QUALIFICATION_BYTES
		)
			throw new TypeError("D12 qualification exceeded its byte bound");
...
export function validateD12QualificationBundle(value: unknown): D12QualificationBundleV1 {
	const candidate = record(value, "D12 qualification bundle");
	exactKeys(
		candidate,
		["bundleDigest", "generation", "graphBundle", "qualification", "schemaVersion"],
		"D12 qualification bundle",
	);
	if (candidate.schemaVersion !== D12_QUALIFICATION_BUNDLE_SCHEMA)
		throw new TypeError("D12 qualification bundle schema drifted");
	const graphBundle = validateD12CurrentGraphLiveBundle(candidate.graphBundle);
	if (graphBundle.executionClass !== "injected-no-network" || graphBundle.disposition !== "success")
		throw new TypeError("D12 qualification Graph bundle drifted");
	const graphEvidence = graphBundle.graphEvidence;
	if (graphEvidence === null) throw new TypeError("D12 qualification Graph evidence is missing");
	const graph = graphEvidence.d9Evidence.providerEvidence;
	const runs = graph.workflowEvidence.runs;
	const qualification = record(candidate.qualification, "D12 qualification");
	exactKeys(
		qualification,
		[
			"baselineBasis",
			...
			"d11QualificationArtifactDigest",
			"d11QualificationBundleDigest",
			"d11QualificationDigest",
			"decisionRef",
			"efficacyClaim",
	...
	if (
		qualification.schemaVersion !== D12_QUALIFICATION_SCHEMA ||
		qualification.decisionRef !== D12_CURRENT_GRAPH_LIVE_DECISION_REF ||
		qualification.implementationManifestDigest !== D12_IMPLEMENTATION_MANIFEST_DIGEST ||
		(qualification.baselineBasis !== "exact-d11-artifact" &&
			qualification.baselineBasis !== "injected-test") ||
		qualification.d11QualificationArtifactDigest !== D12_D11_QUALIFICATION_ARTIFACT_DIGEST ||
		qualification.d11QualificationBundleDigest !== D12_D11_QUALIFICATION_BUNDLE_DIGEST ||
		qualification.d11QualificationDigest !== D12_D11_QUALIFICATION_DIGEST ||
		...
		throw new TypeError("D12 qualification projection drifted");
	const generation = record(candidate.generation, "D12 qualification generation");
	exactKeys(
		generation,
		[
			"causalAttribution",
		...
		"D12 qualification generation",
	);
	const { generationDigest, ...generationMaterial } = generation;
	if (
		generation.schemaVersion !== D12_QUALIFICATION_GENERATION_SCHEMA ||
		generation.generationRef !== D12_CURRENT_GRAPH_LIVE_QUALIFICATION_GENERATION_REF ||
		...
		throw new TypeError("D12 qualification generation drifted");
	const { bundleDigest, ...bundleMaterial } = candidate;
	if (bundleDigest !== empiricalStrictJsonDigest(bundleMaterial))
		throw new TypeError("D12 qualification bundle digest drifted");
	return strictSnapshot(candidate) as unknown as D12QualificationBundleV1;
}

async function persistQualification(inputValue: {
	readonly privateRoot: string;
	readonly bundle: D12QualificationBundleV1;
	readonly expectedBasis: D12QualificationBundleV1["qualification"]["baselineBasis"];
	readonly generationRef: string;
}) {
	const input = record(inputValue, "D12 qualification persistence input");
	exactKeys(
		input,
		["bundle", "expectedBasis", "generationRef", "privateRoot"],
		"D12 qualification persistence input",
	);
	if (
		...
		throw new TypeError("D12 qualification persistence requires a fresh constructed bundle");
	const bundle = validateD12QualificationBundle(input.bundle);
	if (bundle.qualification.baselineBasis !== input.expectedBasis)
		throw new TypeError("D12 qualification persistence basis drifted");
	...
	const commitMaterial = strictSnapshot({
		schemaVersion: "graphrefly-ts.d12.current-live-no-network-commit.v1",
	...
	const receiptMaterial = strictSnapshot({
		schemaVersion: D12_QUALIFICATION_PERSISTENCE_SCHEMA,
		generationRef: String(input.generationRef),
		bundleArtifactDigest: empiricalSha256(bundleBytes),
		bundleDigest: bundle.bundleDigest,
		qualificationDigest: bundle.qualification.qualificationDigest,
...
export function persistD12QualificationBundle(input: {
	readonly privateRoot: string;
	readonly bundle: D12QualificationBundleV1;
}) {
	return persistQualification({
		...input,
		expectedBasis: "exact-d11-artifact",
		generationRef: D12_CURRENT_GRAPH_LIVE_QUALIFICATION_GENERATION_REF,
	});
}

export function persistD12InjectedQualificationForTest(input: {
	readonly privateRoot: string;
	readonly bundle: D12QualificationBundleV1;
}) {
	return persistQualification({
		...
		generationRef: D12_INJECTED_TEST_GENERATION_REF,
	});
}
