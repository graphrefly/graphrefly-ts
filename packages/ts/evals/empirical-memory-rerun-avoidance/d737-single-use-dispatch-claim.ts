...
export const D737_DISPATCH_CLAIM_SCHEMA =
	"graphrefly.b112.d737.single-use-dispatch-claim.v1" as const;
export const D737_PRIVATE_ROOT = resolve(
	import.meta.dirname,
...
export interface D737PersistedDispatchClaimV1 {
	readonly schemaVersion: typeof D737_DISPATCH_CLAIM_SCHEMA;
	readonly claimRef: typeof D737_DISPATCH_CLAIM_REF;
	readonly decisionRef: typeof D737_DECISION_REF;
	readonly decisionRevision: typeof D737_DECISION_REVISION;
	readonly generationRef: typeof D737_GENERATION_REF;
...
export interface D737ExecutionAuthorityV1 {
	readonly revision: "graphrefly.b112.d737.execution-authority.v1";
	readonly claim: D737PersistedDispatchClaimV1;
...
function claimMaterial(
	...
	const input = record(inputValue, "d737.claim.input");
	exactKeys(
		input,
	...
	return strictSnapshot({
		schemaVersion: D737_DISPATCH_CLAIM_SCHEMA,
		claimRef: D737_DISPATCH_CLAIM_REF,
		...
		pricingReadDigest: digest(input.pricingReadDigest, "d737.claim.pricing"),
		zeroByokObservationDigest: digest(input.zeroByokObservationDigest, "d737.claim.zeroByok"),
		implementationManifestDigest: digest(
			input.implementationManifestDigest,
			"d737.claim.implementation",
		),
...
async function acquire(
...
): Promise<D737PersistedDispatchClaimV1> {
	const requestedRoot = resolve(privateRootValue);
	const privateRoot = await realpath(requestedRoot);
	if (privateRoot !== requestedRoot) throw new TypeError("D737 private root is not canonical");
	...
	if (
		...
		throw new TypeError("D737 private root ownership is invalid");
	const claimRoot = join(privateRoot, `.${D737_DISPATCH_CLAIM_REF}`);
	...
	}) as D737PersistedDispatchClaimV1;
	...
	try {
		...
		if (
			...
			throw new TypeError("D737 dispatch claim readback drifted");
	} finally {
		await reader.close();
	}
	claims.set(claim, {
...
export function acquireD737SingleUseDispatchClaim(input: Parameters<typeof claimMaterial>[1]) {
	return acquire(D737_PRIVATE_ROOT, "live-fixed-root", input);
}

export function acquireD737SingleUseDispatchClaimAtRootForTest(
	root: string,
	input: Parameters<typeof claimMaterial>[1],
) {
...
async function revalidate(claim: D737PersistedDispatchClaimV1, state: ClaimState): Promise<void> {
	...
	if (
		...
		throw new TypeError("D737 durable claim directory drifted");
	...
	try {
		...
		if (
			...
			throw new TypeError("D737 durable claim file drifted");
	} finally {
		await reader.close();
	}
	literal(
...
export async function consumeD737DispatchClaimForExecution(inputValue: {
	readonly claim: D737PersistedDispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
}): Promise<D737ExecutionAuthorityV1> {
	const input = record(inputValue, "d737.execution.input");
	exactKeys(input, ["claim", "currentKeyAdmission"], "d737.execution.input");
	const claim = input.claim as D737PersistedDispatchClaimV1;
	const state = typeof claim === "object" && claim !== null ? claims.get(claim) : undefined;
	if (state === undefined) throw new TypeError("D737 claim is not fresh and constructed");
	claims.delete(claim);
	...
	if (
		...
		admission.remainingMicrousd < D737_BUDGET_LIMITS.maxCostMicrousd ||
		admission.limitReset !== "none" ||
		admission.isManagementKey
	)
		throw new TypeError("D737 current-key admission is outside authority");
	...
	const authority = Object.freeze({
		revision: "graphrefly.b112.d737.execution-authority.v1" as const,
		claim,
...
export function consumeD737ExecutionAuthority(value: unknown): D737ExecutionAuthorityV1 {
	if (typeof value !== "object" || value === null || !authorities.delete(value))
		throw new TypeError("D737 live execution authority must be fixed-root and single-use");
	return value as D737ExecutionAuthorityV1;
}
