...
import {
	...
	D766_DISPATCH_CLAIM_REF,
	D766_GENERATION_REF,
	D766_HISTORICAL_BUNDLE_DIGEST,
	D766_ROUTE_PROFILE_DIGEST,
...
export const D766_DISPATCH_CLAIM_SCHEMA =
	"graphrefly.b112.d766.single-use-dispatch-claim.v1" as const;
export const D766_PRIVATE_ROOT = resolve(
	import.meta.dirname,
...
export interface D766PersistedDispatchClaimV1 {
	readonly schemaVersion: typeof D766_DISPATCH_CLAIM_SCHEMA;
	readonly claimRef: typeof D766_DISPATCH_CLAIM_REF;
	readonly decisionRef: typeof D766_DECISION_REF;
	readonly decisionRevision: typeof D766_DECISION_REVISION;
	readonly generationRef: typeof D766_GENERATION_REF;
...
export interface D766ExecutionAuthorityV1 {
	readonly revision: "graphrefly.b112.d766.execution-authority.v1";
	readonly claim: D766PersistedDispatchClaimV1;
...
function claimMaterial(
	...
	const input = record(inputValue, "d766.claim.input");
	exactKeys(
		input,
	...
	return strictSnapshot({
		schemaVersion: D766_DISPATCH_CLAIM_SCHEMA,
		claimRef: D766_DISPATCH_CLAIM_REF,
		...
		pricingReadDigest: digest(input.pricingReadDigest, "d766.claim.pricing"),
		zeroByokObservationDigest: digest(input.zeroByokObservationDigest, "d766.claim.zeroByok"),
		implementationManifestDigest: digest(
			input.implementationManifestDigest,
			"d766.claim.implementation",
		),
...
async function acquire(
...
): Promise<D766PersistedDispatchClaimV1> {
	const requestedRoot = resolve(privateRootValue);
	const privateRoot = await realpath(requestedRoot);
	if (privateRoot !== requestedRoot) throw new TypeError("D766 private root is not canonical");
	...
	if (
		...
		throw new TypeError("D766 private root ownership is invalid");
	const claimRoot = join(privateRoot, `.${D766_DISPATCH_CLAIM_REF}`);
	...
	}) as D766PersistedDispatchClaimV1;
	...
	try {
		...
		if (
			...
			throw new TypeError("D766 dispatch claim readback drifted");
	} finally {
		await reader.close();
	}
	claims.set(claim, {
...
export function acquireD766SingleUseDispatchClaim(input: Parameters<typeof claimMaterial>[1]) {
	return acquire(D766_PRIVATE_ROOT, "live-fixed-root", input);
}

export function acquireD766SingleUseDispatchClaimAtRootForTest(
	root: string,
	input: Parameters<typeof claimMaterial>[1],
) {
...
async function revalidate(claim: D766PersistedDispatchClaimV1, state: ClaimState): Promise<void> {
	...
	if (
		...
		throw new TypeError("D766 durable claim directory drifted");
	...
	try {
		...
		if (
			...
			throw new TypeError("D766 durable claim file drifted");
	} finally {
		await reader.close();
	}
	literal(
...
export async function consumeD766DispatchClaimForExecution(inputValue: {
	readonly claim: D766PersistedDispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
}): Promise<D766ExecutionAuthorityV1> {
	const input = record(inputValue, "d766.execution.input");
	exactKeys(input, ["claim", "currentKeyAdmission"], "d766.execution.input");
	const claim = input.claim as D766PersistedDispatchClaimV1;
	const state = typeof claim === "object" && claim !== null ? claims.get(claim) : undefined;
	if (state === undefined) throw new TypeError("D766 claim is not fresh and constructed");
	claims.delete(claim);
	...
	if (
		...
		admission.remainingMicrousd < D766_BUDGET_LIMITS.maxCostMicrousd ||
		admission.limitReset !== "none" ||
		admission.isManagementKey
	)
		throw new TypeError("D766 current-key admission is outside authority");
	...
	const authority = Object.freeze({
		revision: "graphrefly.b112.d766.execution-authority.v1" as const,
		claim,
...
export function consumeD766ExecutionAuthority(value: unknown): D766ExecutionAuthorityV1 {
	if (typeof value !== "object" || value === null || !authorities.delete(value))
		throw new TypeError("D766 live execution authority must be fixed-root and single-use");
	return value as D766ExecutionAuthorityV1;
}
