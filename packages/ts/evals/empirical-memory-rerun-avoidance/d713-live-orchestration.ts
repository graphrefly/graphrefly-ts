...
export interface D713OrderedLiveAdmissionSteps<
	Pricing,
	Credential,
	ZeroByok,
	Claim,
...
function ownFunction(value: Record<string, unknown>, key: string): (...args: never[]) => unknown {
	...
	if (
		...
		throw new TypeError(`D713 ordered live admission ${key} must be an own function`);
	}
	return descriptor.value as (...args: never[]) => unknown;
}

export async function runD713OrderedLiveAdmissions<
	Pricing,
	Credential,
	ZeroByok,
	Claim,
	...
	ProviderResult,
>(
	input: D713OrderedLiveAdmissionSteps<
		Pricing,
		Credential,
		ZeroByok,
		Claim,
	...
	const candidate = record(input, "d713.orderedLiveAdmissions");
	exactKeys(
		candidate,
		[
		...
		"d713.orderedLiveAdmissions",
	);
	const readFreshPricing = ownFunction(candidate, "readFreshPricing") as () => AwaitedStep<Pricing>;
	const loadCredential = ownFunction(candidate, "loadCredential") as D713OrderedLiveAdmissionSteps<
		Pricing,
		Credential,
		ZeroByok,
		Claim,
	...
	const readFreshZeroByok = ownFunction(
		candidate,
		"readFreshZeroByok",
	) as D713OrderedLiveAdmissionSteps<
		Pricing,
		Credential,
		ZeroByok,
		Claim,
	...
	const acquireDispatchClaim = ownFunction(
		...
		ProviderResult
	>["acquireDispatchClaim"];
	const readCurrentKey = ownFunction(candidate, "readCurrentKey") as D713OrderedLiveAdmissionSteps<
		Pricing,
		Credential,
		ZeroByok,
		Claim,
