...
import {
	D754_COORDINATES_DIGEST,
	D754_DECISION_REF,
	D754_DECISION_REVISION,
	D754_GENERATION_REF,
	D754_HISTORICAL_ARTIFACT_SHA256,
	D754_HISTORICAL_BUNDLE_DIGEST,
	D754_HISTORICAL_GENERATION_DIGEST,
	D754_HISTORICAL_QUALIFICATION_DIGEST,
	D754_ROUTE_PROFILE,
	D754_ROUTE_PROFILE_DIGEST,
} from "./d754-coordinates.js";
import {
	consumeD754ExecutionAuthority,
	type D754ExecutionAuthorityV1,
} from "./d754-single-use-dispatch-claim.js";
import {
	type D754TransportDiagnosticGraphEvidenceV1,
	type D754TransportDiagnosticRouteAdapterV1,
	finalizeD754TransportDiagnostics,
	validateD754TransportDiagnosticGraphEvidence,
} from "./d754-transport-diagnostic-route-adapter.js";

export const D754_QUALIFICATION_SCHEMA = "graphrefly.b112.d754.live-qualification.v1" as const;
export const D754_OBSERVATION_SCHEMA = "graphrefly.b112.d754.live-observation.v1" as const;
export const D754_GENERATION_SCHEMA = "graphrefly.b112.d754.success-generation.v1" as const;
export const D754_PARTIAL_SCHEMA = "graphrefly.b112.d754.partial-failure-generation.v1" as const;
export const D754_TERMINAL_SCHEMA = "graphrefly.b112.d754.terminal-receipt.v1" as const;
export const D754_BUNDLE_SCHEMA = "graphrefly.b112.d754.live-bundle.v1" as const;

export interface D754LiveBundleV1 {
	readonly schemaVersion: typeof D754_BUNDLE_SCHEMA;
	...
	readonly transportDiagnosticGraphEvidence: D754TransportDiagnosticGraphEvidenceV1;
...
function replayGraph(value: unknown): D722CanonicalGraphEvidenceV1 {
	const candidate = record(value, "d754.graphEvidence");
	const runs = array(candidate.effectRuns, "d754.graphEvidence.effectRuns");
	if (runs.length > 12) throw new TypeError("D754 Graph run bound exceeded");
	...
	return Object.freeze(
		graph.effectRuns.map((run) => {
			...
			if (facts.length !== 1 || facts[0]?.kind !== "graph-effect-result-admitted")
				throw new TypeError("D754 requires one Graph cleanup fact per arm");
			const fact = facts[0];
...
function validateHistoricalBundle(bytes: Uint8Array): void {
	if (!(bytes instanceof Uint8Array) || bytes.byteLength < 1 || bytes.byteLength > 16 * 1_048_576)
		throw new TypeError("D754 D752 qualification artifact bytes are invalid");
	literal(empiricalSha256(bytes), D754_HISTORICAL_ARTIFACT_SHA256, "d754.historical.artifact");
	const bundle = validateD752Bundle(strictJsonCodec.decode(new Uint8Array(bytes)));
	literal(bundle.bundleDigest, D754_HISTORICAL_BUNDLE_DIGEST, "d754.historical.bundle");
	literal(
		bundle.generation.generationDigest,
		D754_HISTORICAL_GENERATION_DIGEST,
		"d754.historical.generation",
	);
	literal(
		bundle.qualification.qualificationDigest,
		D754_HISTORICAL_QUALIFICATION_DIGEST,
		"d754.historical.qualification",
	);
}
...
function buildBundle(
	...
	const transportDiagnosticGraphEvidence = validateD754TransportDiagnosticGraphEvidence(
		input.transportDiagnosticGraphEvidence,
		graphEvidence,
	);
	for (const fact of routeEvidence.facts)
		literal(fact.routeProfileDigest, D754_ROUTE_PROFILE_DIGEST, "d754.route.profile");
	const failures = executorFailureFacts(graphEvidence);
	const cleanups = cleanupFacts(graphEvidence);
	const providerTransportCalls = safeInteger(input.providerTransportCalls, "d754.providerCalls", {
		max: 96,
	});
	...
	const qualificationMaterial = strictSnapshot({
		schemaVersion: D754_QUALIFICATION_SCHEMA,
		decisionRef: D754_DECISION_REF,
		decisionRevision: D754_DECISION_REVISION,
		coordinatesDigest: D754_COORDINATES_DIGEST,
		...
		implementationManifestDigest: digest(input.implementationManifestDigest, "d754.implementation"),
		...
		claimDigest: digest(input.claimDigest, "d754.claim"),
		currentKeyAdmissionDigest: digest(input.currentKeyAdmissionDigest, "d754.currentKey"),
		pricingReadDigest: digest(input.pricingReadDigest, "d754.pricingRead"),
		pricingObservationDigest: digest(input.pricingObservationDigest, "d754.pricingObservation"),
		zeroByokObservationDigest: digest(input.zeroByokObservationDigest, "d754.zeroByok"),
	...
	const observationMaterial = strictSnapshot({
		schemaVersion: D754_OBSERVATION_SCHEMA,
		disposition,
		model: D754_ROUTE_PROFILE.requestModel,
		selectedEndpointModel: D754_ROUTE_PROFILE.selectedEndpointModel,
		provider: D754_ROUTE_PROFILE.providerName,
		providerTag: D754_ROUTE_PROFILE.providerTag,
		quantization: D754_ROUTE_PROFILE.quantization,
		endpointProtocol: D754_ROUTE_PROFILE.endpointProtocol,
		reasoningEffort: D754_ROUTE_PROFILE.reasoningEffort,
	...
	const generationMaterial = strictSnapshot({
		schemaVersion: success ? D754_GENERATION_SCHEMA : D754_PARTIAL_SCHEMA,
	...
	const terminalMaterial = strictSnapshot({
		schemaVersion: D754_TERMINAL_SCHEMA,
		status: disposition,
		claimDigest: input.claimDigest,
	...
	const material = strictSnapshot({
		schemaVersion: D754_BUNDLE_SCHEMA,
		disposition,
		qualification,
		graphEvidence,
	...
	return bundle as unknown as D754LiveBundleV1;
}

export async function runD754LiveReplacement(inputValue: {
	readonly historicalBundleBytes: Uint8Array;
	readonly implementationManifestDigest: string;
	readonly diagnosticCapability: D754TransportDiagnosticRouteAdapterV1;
	readonly executionAuthority: D754ExecutionAuthorityV1;
	readonly pricingReadDigest: string;
	readonly pricingObservationDigest: string;
	readonly providerTransportCalls: () => number;
	readonly signal: AbortSignal;
}): Promise<D754LiveBundleV1> {
	const input = record(inputValue, "d754.run");
	exactKeys(
		input,
		[
	...
	const authority = consumeD754ExecutionAuthority(input.executionAuthority);
	const pricingReadDigest = digest(input.pricingReadDigest, "d754.run.pricingReadDigest");
	const implementationManifestDigest = digest(
		input.implementationManifestDigest,
		"d754.run.implementationManifestDigest",
	);
	literal(
		authority.claim.historicalBundleDigest,
		D754_HISTORICAL_BUNDLE_DIGEST,
		"d754.authority.historical",
	);
	literal(authority.claim.routeProfileDigest, D754_ROUTE_PROFILE_DIGEST, "d754.authority.route");
	literal(authority.claim.pricingReadDigest, pricingReadDigest, "d754.authority.pricing");
	literal(
		authority.claim.implementationManifestDigest,
		implementationManifestDigest,
		"d754.authority.implementation",
	);
	if (typeof input.providerTransportCalls !== "function")
		throw new TypeError("D754 provider call counter is invalid");
	const integration = await runD734RouteProfileSixArmLiveIntegration({
		...
		adapter: (input.diagnosticCapability as D754TransportDiagnosticRouteAdapterV1).adapter,
		objectivePhaseRecoveryPolicy: createD748GraphForwardPhaseContinuationPolicy(),
		signal: input.signal as AbortSignal,
	});
	const diagnosticFinalization = finalizeD754TransportDiagnostics(
		input.diagnosticCapability as D754TransportDiagnosticRouteAdapterV1,
...
export async function runD754InjectedNoNetworkQualification(inputValue: {
	readonly historicalBundleBytes: Uint8Array;
	readonly implementationManifestDigest: string;
	...
	const input = record(inputValue, "d754.injectedRun");
	exactKeys(
		input,
		[
			"adapter",
	...
	const implementationManifestDigest = digest(
		input.implementationManifestDigest,
		"d754.injectedRun.implementation",
	);
	if (typeof input.providerTransportCalls !== "function")
		throw new TypeError("D754 injected provider call counter is invalid");
	const integration = await runD734RouteProfileSixArmLiveIntegration({
		sourceDigest: empiricalStrictJsonDigest({
			decisionRef: D754_DECISION_REF,
	...
	const emptyDiagnosticMaterial = strictSnapshot({
		schemaVersion: "graphrefly.b112.d754.transport-diagnostic-graph-evidence.v1" as const,
		facts: Object.freeze([]),
	});
	const transportDiagnosticGraphEvidence = validateD754TransportDiagnosticGraphEvidence(
		strictSnapshot({
	...
	return buildBundle(integration, {
		claimDigest: empiricalStrictJsonDigest({ d754: "injected-claim" }),
		currentKeyAdmissionDigest: empiricalStrictJsonDigest({ d754: "injected-current-key" }),
		pricingReadDigest: empiricalStrictJsonDigest({ d754: "injected-pricing-read" }),
		pricingObservationDigest: empiricalStrictJsonDigest({ d754: "injected-pricing-observation" }),
		zeroByokObservationDigest: empiricalStrictJsonDigest({ d754: "injected-zero-byok" }),
		implementationManifestDigest,
		providerTransportCalls: (input.providerTransportCalls as () => number)(),
		transportDiagnosticGraphEvidence,
...
export function validateD754LiveBundle(value: unknown): D754LiveBundleV1 {
	const candidate = record(value, "d754.bundle");
	exactKeys(
		candidate,
		[
			"bundleDigest",
	...
	literal(candidate.schemaVersion, D754_BUNDLE_SCHEMA, "d754.bundle.schema");
	...
	const transportDiagnostic = validateD754TransportDiagnosticGraphEvidence(
	...
	const qualification = record(candidate.qualification, "d754.qualification");
	const observation = record(candidate.observation, "d754.observation");
	const generation = record(candidate.generation, "d754.generation");
	const terminalReceipt = record(candidate.terminalReceipt, "d754.terminalReceipt");
	...
	const providerTransportCalls = safeInteger(
		...
		"d754.qualification.providerTransportCalls",
		{ max: 96 },
	);
	const providerAttemptEvidenceDisposition =
		providerTransportCalls === graphProviderEffectCount
	...
	const terminalDiagnosticProposalCount = safeInteger(
		...
		"d754.qualification.terminalDiagnosticProposalCount",
		{ max: 24 },
	);
	const retryDiagnosticProposalCount = safeInteger(
		qualification.retryDiagnosticProposalCount,
		"d754.qualification.retryDiagnosticProposalCount",
		{ max: 24 },
	);
	literal(
	...
	for (const [object, path] of [
		[qualification, "d754.qualification"],
		[observation, "d754.observation"],
		[terminalReceipt, "d754.terminalReceipt"],
	] as const) {
		literal(
	...
	const expectedDisposition =
		...
		cleanups.every((fact) => fact.status === "succeeded")
			...
			: "partial-failure";
	literal(
		oneOf(candidate.disposition, ["success", "partial-failure"], "d754.disposition"),
		expectedDisposition,
		"d754.disposition",
	);
	for (const [object, key, path] of [
		[qualification, "qualificationDigest", "d754.qualification"],
		[observation, "observationDigest", "d754.observation"],
		[generation, "generationDigest", "d754.generation"],
		[terminalReceipt, "terminalReceiptDigest", "d754.terminalReceipt"],
	] as const) {
	...
	literal(
		...
		D754_COORDINATES_DIGEST,
		"d754.qualification.coordinates",
	);
	literal(
		qualification.historicalArtifactSha256,
		D754_HISTORICAL_ARTIFACT_SHA256,
		"d754.qualification.historicalArtifact",
	);
	literal(
		qualification.historicalBundleDigest,
		D754_HISTORICAL_BUNDLE_DIGEST,
		"d754.qualification.historicalBundle",
	);
	literal(
		qualification.historicalGenerationDigest,
		D754_HISTORICAL_GENERATION_DIGEST,
		"d754.qualification.historicalGeneration",
	);
	literal(
		qualification.historicalQualificationDigest,
		D754_HISTORICAL_QUALIFICATION_DIGEST,
		"d754.qualification.historicalQualification",
	);
	literal(qualification.routeProfileDigest, D754_ROUTE_PROFILE_DIGEST, "d754.qualification.route");
	literal(qualification.graphEvidenceDigest, graph.evidenceDigest, "d754.qualification.graph");
	literal(
		qualification.routeEvidenceDigest,
		route.evidenceDigest,
		"d754.qualification.routeEvidence",
	);
	literal(
		qualification.terminalHttpGraphEvidenceDigest,
		terminal.evidenceDigest,
		"d754.qualification.terminal",
	);
	literal(
		qualification.transportDiagnosticGraphEvidenceDigest,
		transportDiagnostic.evidenceDigest,
		"d754.qualification.transportDiagnostic",
	);
	literal(observation.graphEvidenceDigest, graph.evidenceDigest, "d754.observation.graph");
	literal(observation.routeEvidenceDigest, route.evidenceDigest, "d754.observation.route");
	literal(
		observation.transportDiagnosticGraphEvidenceDigest,
		transportDiagnostic.evidenceDigest,
		"d754.observation.transportDiagnostic",
	);
	literal(
		generation.transportDiagnosticGraphEvidenceDigest,
		transportDiagnostic.evidenceDigest,
		"d754.generation.transportDiagnostic",
	);
	literal(
		digest(generation.observationDigest, "d754.generation.observationDigest"),
		digest(observation.observationDigest, "d754.observation.observationDigest"),
		"d754.generation.observation",
	);
	literal(terminalReceipt.graphEvidenceDigest, graph.evidenceDigest, "d754.terminal.graph");
	literal(
		terminalReceipt.transportDiagnosticGraphEvidenceDigest,
		transportDiagnostic.evidenceDigest,
		"d754.terminal.transportDiagnostic",
	);
	for (const object of [qualification, observation, generation, terminalReceipt]) {
		literal(object.causalAttribution, "undetermined", "d754.attribution");
		literal(object.efficacyClaim, "none", "d754.efficacy");
	}
	const bundleDigest = digest(candidate.bundleDigest, "d754.bundle.digest");
	const { bundleDigest: _bundleDigest, ...material } = candidate;
	literal(bundleDigest, empiricalStrictJsonDigest(material), "d754.bundle.digest");
	return strictSnapshot(candidate) as unknown as D754LiveBundleV1;
}
...
async function assertDirectory(path: string, identity: Identity): Promise<void> {
	...
	if (
		...
		throw new TypeError("D754 persistence directory identity drifted");
}
...
async function writeArtifact(path: string, bytes: Uint8Array): Promise<Identity> {
	...
	try {
		...
		if (!stat.isFile() || (stat.mode & 0o777) !== 0o600 || stat.nlink !== 1)
			throw new TypeError("D754 persistence file identity drifted");
...
async function assertArtifact(path: string, identity: Identity, bytes: Uint8Array): Promise<void> {
	...
	try {
		...
		if (
			...
			throw new TypeError("D754 persistence file readback drifted");
	} finally {
		await handle.close();
	}
}

export async function persistD754LiveBundle(inputValue: {
	readonly privateRoot: string;
	readonly bundle: D754LiveBundleV1;
}) {
	const input = record(inputValue, "d754.persist");
	exactKeys(input, ["bundle", "privateRoot"], "d754.persist");
	if (
		typeof input.bundle !== "object" ||
		input.bundle === null ||
		!constructedBundles.delete(input.bundle)
	)
		throw new TypeError("D754 persistence requires the exact constructed bundle");
	const bundle = validateD754LiveBundle(input.bundle);
	if (typeof input.privateRoot !== "string" || resolve(input.privateRoot) !== input.privateRoot)
		throw new TypeError("D754 persistence root must be absolute");
	...
	if (!parentStat.isDirectory() || (parentStat.mode & 0o777) !== 0o700)
		throw new TypeError("D754 persistence root is invalid");
	const finalRoot = join(privateRoot, D754_GENERATION_REF);
	...
	try {
		...
		const staging = join(finalRoot, `.d754-staging-${randomUUID()}`);
		...
		if (
			...
			throw new TypeError("D754 persistence rename identity drifted");
		const commit = strictSnapshot({
			schemaVersion: "graphrefly.b112.d754.atomic-commit.v1",
			generationRef: D754_GENERATION_REF,
