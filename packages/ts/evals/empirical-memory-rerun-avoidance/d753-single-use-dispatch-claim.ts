...
export const D753_DISPATCH_CLAIM_SCHEMA =
	"graphrefly.b112.d753.single-use-dispatch-claim.v1" as const;
export const D753_PRIVATE_ROOT = resolve(
	import.meta.dirname,
...
export interface D753PersistedDispatchClaimV1 {
	readonly schemaVersion: typeof D753_DISPATCH_CLAIM_SCHEMA;
	readonly claimRef: typeof D753_DISPATCH_CLAIM_REF;
	readonly decisionRef: typeof D753_DECISION_REF;
	readonly decisionRevision: typeof D753_DECISION_REVISION;
	readonly generationRef: typeof D753_GENERATION_REF;
...
export interface D753ExecutionAuthorityV1 {
	readonly revision: "graphrefly.b112.d753.execution-authority.v1";
	readonly claim: D753PersistedDispatchClaimV1;
...
function claimMaterial(
	...
	const input = record(inputValue, "d753.claim.input");
	exactKeys(
		input,
	...
	return strictSnapshot({
		schemaVersion: D753_DISPATCH_CLAIM_SCHEMA,
		claimRef: D753_DISPATCH_CLAIM_REF,
		...
		pricingReadDigest: digest(input.pricingReadDigest, "d753.claim.pricing"),
		zeroByokObservationDigest: digest(input.zeroByokObservationDigest, "d753.claim.zeroByok"),
		implementationManifestDigest: digest(
			input.implementationManifestDigest,
			"d753.claim.implementation",
		),
...
async function acquire(
...
): Promise<D753PersistedDispatchClaimV1> {
	const requestedRoot = resolve(privateRootValue);
	const privateRoot = await realpath(requestedRoot);
	if (privateRoot !== requestedRoot) throw new TypeError("D753 private root is not canonical");
	...
	if (
		...
		throw new TypeError("D753 private root ownership is invalid");
	const claimRoot = join(privateRoot, `.${D753_DISPATCH_CLAIM_REF}`);
	...
	}) as D753PersistedDispatchClaimV1;
	...
	try {
		...
		if (
			...
			throw new TypeError("D753 dispatch claim readback drifted");
	} finally {
		await reader.close();
	}
	claims.set(claim, {
...
export function acquireD753SingleUseDispatchClaim(input: Parameters<typeof claimMaterial>[1]) {
	return acquire(D753_PRIVATE_ROOT, "live-fixed-root", input);
}

export function acquireD753SingleUseDispatchClaimAtRootForTest(
	root: string,
	input: Parameters<typeof claimMaterial>[1],
) {
...
async function revalidate(claim: D753PersistedDispatchClaimV1, state: ClaimState): Promise<void> {
	...
	if (
		...
		throw new TypeError("D753 durable claim directory drifted");
	...
	try {
		...
		if (
			...
			throw new TypeError("D753 durable claim file drifted");
	} finally {
		await reader.close();
	}
	literal(
...
export async function consumeD753DispatchClaimForExecution(inputValue: {
	readonly claim: D753PersistedDispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
}): Promise<D753ExecutionAuthorityV1> {
	const input = record(inputValue, "d753.execution.input");
	exactKeys(input, ["claim", "currentKeyAdmission"], "d753.execution.input");
	const claim = input.claim as D753PersistedDispatchClaimV1;
	const state = typeof claim === "object" && claim !== null ? claims.get(claim) : undefined;
	if (state === undefined) throw new TypeError("D753 claim is not fresh and constructed");
	claims.delete(claim);
	...
	if (
		...
		admission.remainingMicrousd < D753_BUDGET_LIMITS.maxCostMicrousd ||
		admission.limitReset !== "none" ||
		admission.isManagementKey
	)
		throw new TypeError("D753 current-key admission is outside authority");
	...
	const authority = Object.freeze({
		revision: "graphrefly.b112.d753.execution-authority.v1" as const,
		claim,
...
export function consumeD753ExecutionAuthority(value: unknown): D753ExecutionAuthorityV1 {
	if (typeof value !== "object" || value === null || !authorities.delete(value))
		throw new TypeError("D753 live execution authority must be fixed-root and single-use");
	return value as D753ExecutionAuthorityV1;
}
