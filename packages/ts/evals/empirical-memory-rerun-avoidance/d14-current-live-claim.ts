...
import type { CurrentGraphLivePreclaimV1 as D14CurrentGraphLivePreclaimV1 } from "./d8-current-live-preflight.js";
import { consumeCurrentGraphLivePreclaim as consumeD14CurrentGraphLivePreclaim } from "./d8-current-live-preflight.js";
import {
	D14_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST,
	D14_CURRENT_GRAPH_LIVE_DECISION_REF,
	D14_CURRENT_GRAPH_LIVE_DECISION_REVISION,
	D14_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF,
...
export const D14_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_SCHEMA =
	"graphrefly-ts.d14.current-graph-live-dispatch-claim.v1" as const;
export const D14_CURRENT_GRAPH_LIVE_EXECUTION_AUTHORITY_REVISION =
	"graphrefly-ts.d14.current-graph-live-execution-authority.v1" as const;
export const D14_CURRENT_GRAPH_LIVE_PRIVATE_ROOT = resolve(
	import.meta.dirname,
	"../.private/empirical-memory-rerun-avoidance/current-graph-native-d14",
);

type ClaimScope = "live-fixed-root" | "injected-test-root";

export interface D14CurrentGraphLiveDispatchClaimV1 {
	readonly schemaVersion: typeof D14_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_SCHEMA;
	readonly claimRef: typeof D14_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF;
	readonly decisionRef: typeof D14_CURRENT_GRAPH_LIVE_DECISION_REF;
	readonly decisionRevision: typeof D14_CURRENT_GRAPH_LIVE_DECISION_REVISION;
	readonly generationRef: typeof D14_CURRENT_GRAPH_LIVE_GENERATION_REF;
...
export interface D14CurrentGraphLiveExecutionAuthorityV1 {
	readonly revision: typeof D14_CURRENT_GRAPH_LIVE_EXECUTION_AUTHORITY_REVISION;
...
function claimMaterial(
	scope: ClaimScope,
	preclaim: D14CurrentGraphLivePreclaimV1,
	implementationManifestDigest: string,
	qualificationArtifactDigest: string,
	qualificationDigest: string,
) {
	return strictSnapshot({
		schemaVersion: D14_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_SCHEMA,
		claimRef: D14_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF,
...
async function acquire(
	...
	scope: ClaimScope,
	inputValue: {
		readonly preclaim: D14CurrentGraphLivePreclaimV1;
		readonly implementationManifestDigest: string;
		readonly qualificationArtifactDigest: string;
		readonly qualificationDigest: string;
	},
): Promise<D14CurrentGraphLiveDispatchClaimV1> {
	...
	const preclaim = consumeD14CurrentGraphLivePreclaim(input.preclaim);
	...
	const claimRoot = join(privateRoot, `.${D14_CURRENT_GRAPH_LIVE_DISPATCH_CLAIM_REF}`);
	...
	}) as D14CurrentGraphLiveDispatchClaimV1;
...
export function acquireD14CurrentGraphLiveDispatchClaim(input: {
	readonly preclaim: D14CurrentGraphLivePreclaimV1;
	readonly implementationManifestDigest: string;
	readonly qualificationArtifactDigest: string;
	readonly qualificationDigest: string;
}) {
	return acquire(D14_CURRENT_GRAPH_LIVE_PRIVATE_ROOT, "live-fixed-root", input);
}

export function acquireD14CurrentGraphLiveDispatchClaimAtRootForTest(
	root: string,
	input: {
		readonly preclaim: D14CurrentGraphLivePreclaimV1;
		readonly implementationManifestDigest: string;
...
async function revalidateClaim(
	claim: D14CurrentGraphLiveDispatchClaimV1,
	state: ClaimState,
): Promise<void> {
	...
	const material = claimMaterial(
		state.scope,
		{ preclaimDigest: claim.preclaimDigest } as D14CurrentGraphLivePreclaimV1,
...
export async function consumeD14CurrentGraphLiveDispatchClaim(inputValue: {
	readonly claim: D14CurrentGraphLiveDispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
	readonly allowInjectedTestScope?: boolean;
}): Promise<D14CurrentGraphLiveExecutionAuthorityV1> {
	...
	const claim = input.claim as D14CurrentGraphLiveDispatchClaimV1;
	...
	const material = strictSnapshot({
		revision: D14_CURRENT_GRAPH_LIVE_EXECUTION_AUTHORITY_REVISION,
		claim,
		currentKeyAdmission,
	});
	...
	}) as D14CurrentGraphLiveExecutionAuthorityV1;
	authorities.add(authority);
	return authority;
}

export function consumeD14CurrentGraphLiveExecutionAuthority(
	value: unknown,
): D14CurrentGraphLiveExecutionAuthorityV1 {
	...
	return value as D14CurrentGraphLiveExecutionAuthorityV1;
}
