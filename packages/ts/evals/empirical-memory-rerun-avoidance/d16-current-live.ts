...
import {
	consumeD16CurrentGraphLiveExecutionAuthority,
	type D16CurrentGraphLiveDispatchClaimV1,
	type D16CurrentGraphLiveExecutionAuthorityV1,
} from "./d16-current-live-claim.js";
import {
	D16_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST,
	D16_CURRENT_GRAPH_LIVE_DECISION_REF,
	D16_CURRENT_GRAPH_LIVE_GENERATION_REF,
	D16_CURRENT_GRAPH_LIVE_LIMITS,
	D16_CURRENT_GRAPH_LIVE_ROUTE,
	D16_CURRENT_GRAPH_LIVE_TASK,
	D16_D15_QUALIFICATION_ARTIFACT_DIGEST,
} from "./d16-current-live-coordinates.js";

export const D16_CURRENT_GRAPH_LIVE_BUNDLE_SCHEMA =
	"graphrefly-ts.d16.current-graph-live-bundle.v1" as const;
export const D16_CURRENT_GRAPH_LIVE_PARTIAL_GRAPH_SCHEMA =
	"graphrefly-ts.d16.current-graph-live-partial-graph.v1" as const;
export const D16_CURRENT_GRAPH_LIVE_GENERATION_SCHEMA =
	"graphrefly-ts.d16.current-graph-live-generation.v1" as const;
export const D16_CURRENT_GRAPH_LIVE_TERMINAL_RECEIPT_SCHEMA =
	"graphrefly-ts.d16.current-graph-live-terminal-receipt.v1" as const;
export const D16_CURRENT_GRAPH_LIVE_PERSISTENCE_SCHEMA =
	"graphrefly-ts.d16.current-graph-live-persistence.v1" as const;
export const D16_CURRENT_GRAPH_LIVE_PREEXECUTION_FAILURE_SCHEMA =
	"graphrefly-ts.d16.current-graph-live-preexecution-failure.v1" as const;
export const D16_CURRENT_GRAPH_LIVE_MAX_BUNDLE_BYTES = 4_194_304;

type D16CurrentGraphLiveProviderFactV1 = CurrentGraphProviderEvidenceV1["facts"][number];

export interface D16CurrentGraphLivePartialGraphV1 {
	readonly schemaVersion: typeof D16_CURRENT_GRAPH_LIVE_PARTIAL_GRAPH_SCHEMA;
	readonly decisionRef: typeof D16_CURRENT_GRAPH_LIVE_DECISION_REF;
	readonly routeDigest: string;
	readonly taskProfileDigest: string;
	readonly facts: readonly D16CurrentGraphLiveProviderFactV1[];
...
export interface D16CurrentGraphLiveBundleV1 {
	readonly schemaVersion: typeof D16_CURRENT_GRAPH_LIVE_BUNDLE_SCHEMA;
	readonly decisionRef: typeof D16_CURRENT_GRAPH_LIVE_DECISION_REF;
	...
	readonly d15QualificationArtifactDigest: string;
...
export async function persistD16CurrentGraphLivePreexecutionFailure(inputValue: {
	readonly privateRoot: string;
	readonly claim: D16CurrentGraphLiveDispatchClaimV1;
	readonly implementationManifestDigest: string;
	readonly pricingObservationDigest: string;
	readonly zeroByokObservationDigest: string;
	...
	if (
		claim.decisionRef !== D16_CURRENT_GRAPH_LIVE_DECISION_REF ||
		claim.generationRef !== D16_CURRENT_GRAPH_LIVE_GENERATION_REF ||
		claim.coordinatesDigest !== D16_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST ||
	...
	const material = strictSnapshot({
		schemaVersion: D16_CURRENT_GRAPH_LIVE_PREEXECUTION_FAILURE_SCHEMA,
		...
		coordinatesDigest: D16_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST,
	...
	const commitMaterial = strictSnapshot({
		schemaVersion: "graphrefly-ts.d16.current-graph-live-commit.v1",
		generationRef: D16_CURRENT_GRAPH_LIVE_GENERATION_REF,
		disposition: "partial-failure" as const,
		bundleDigest: null,
	...
	return Object.freeze({
		schemaVersion: D16_CURRENT_GRAPH_LIVE_PERSISTENCE_SCHEMA,
		generationRef: D16_CURRENT_GRAPH_LIVE_GENERATION_REF,
		disposition: "partial-failure" as const,
...
export function isD16CompleteSixArmMeasurementForTest(
	evidence: D11TransportFailureEvidenceV1,
): boolean {
	return completeSixArms(evidence);
}

export async function runD16CurrentGraphLiveMeasurement(inputValue: {
	readonly executionAuthority: D16CurrentGraphLiveExecutionAuthorityV1;
	readonly executionClass: "live-provider" | "injected-no-network";
	readonly executor: D16CurrentGraphLiveExecutorV1;
	readonly implementationManifestDigest: string;
	readonly d15QualificationArtifactDigest: string;
	readonly pricingObservationDigest: string;
	readonly zeroByokObservationDigest: string;
}): Promise<D16CurrentGraphLiveBundleV1> {
	...
	if (
		input.implementationManifestDigest !== D16_IMPLEMENTATION_MANIFEST_DIGEST ||
		input.d15QualificationArtifactDigest !== D16_D15_QUALIFICATION_ARTIFACT_DIGEST ||
	...
	const authority = consumeD16CurrentGraphLiveExecutionAuthority(input.executionAuthority);
	if (
	...
	const graphAuthority = createD11TransportFailureAuthority({
		...
		routeProfile: D16_CURRENT_GRAPH_LIVE_ROUTE,
		taskProfile: D16_CURRENT_GRAPH_LIVE_TASK,
	});
	const facts: D16CurrentGraphLiveProviderFactV1[] = [];
	...
	try {
		for (let guard = 0; guard < D16_CURRENT_GRAPH_LIVE_LIMITS.maxEffectFacts; guard += 1) {
	...
	const partialGraphEvidence = success
		...
		: (() => {
				const material = strictSnapshot({
					schemaVersion: D16_CURRENT_GRAPH_LIVE_PARTIAL_GRAPH_SCHEMA,
					decisionRef: D16_CURRENT_GRAPH_LIVE_DECISION_REF,
					routeDigest: D16_CURRENT_GRAPH_LIVE_ROUTE.routeDigest,
					taskProfileDigest: D16_CURRENT_GRAPH_LIVE_TASK.taskProfileDigest,
					facts,
					rejectionFacts,
					transportFacts,
				...
				}) as D16CurrentGraphLivePartialGraphV1;
			})();
	const generation = success
		? (() => {
				const material = strictSnapshot({
					schemaVersion: D16_CURRENT_GRAPH_LIVE_GENERATION_SCHEMA,
					generationRef: D16_CURRENT_GRAPH_LIVE_GENERATION_REF,
	...
	const terminalMaterial = strictSnapshot({
		schemaVersion: D16_CURRENT_GRAPH_LIVE_TERMINAL_RECEIPT_SCHEMA,
		decisionRef: D16_CURRENT_GRAPH_LIVE_DECISION_REF,
	...
	const bundleMaterial = strictSnapshot({
		schemaVersion: D16_CURRENT_GRAPH_LIVE_BUNDLE_SCHEMA,
		decisionRef: D16_CURRENT_GRAPH_LIVE_DECISION_REF,
		...
		qualificationArtifactDigest: authority.claim.qualificationArtifactDigest,
		qualificationDigest: authority.claim.qualificationDigest,
		d15QualificationArtifactDigest: digest(
			input.d15QualificationArtifactDigest,
			"current.live.d15QualificationArtifactDigest",
		),
		pricingObservationDigest: digest(
			input.pricingObservationDigest,
...
function validatePartial(value: unknown): D16CurrentGraphLivePartialGraphV1 {
	...
	if (
		candidate.schemaVersion !== D16_CURRENT_GRAPH_LIVE_PARTIAL_GRAPH_SCHEMA ||
		candidate.decisionRef !== D16_CURRENT_GRAPH_LIVE_DECISION_REF ||
		candidate.routeDigest !== D16_CURRENT_GRAPH_LIVE_ROUTE.routeDigest ||
		candidate.taskProfileDigest !== D16_CURRENT_GRAPH_LIVE_TASK.taskProfileDigest
	)
	...
	if (facts.length > D16_CURRENT_GRAPH_LIVE_LIMITS.maxEffectFacts)
	...
	return strictSnapshot(candidate) as unknown as D16CurrentGraphLivePartialGraphV1;
}

export function validateD16CurrentGraphLiveBundle(value: unknown): D16CurrentGraphLiveBundleV1 {
	...
	if (
		candidate.schemaVersion !== D16_CURRENT_GRAPH_LIVE_BUNDLE_SCHEMA ||
		candidate.decisionRef !== D16_CURRENT_GRAPH_LIVE_DECISION_REF ||
		candidate.coordinatesDigest !== D16_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST ||
	...
	if (
		candidate.implementationManifestDigest !== D16_IMPLEMENTATION_MANIFEST_DIGEST ||
		candidate.d15QualificationArtifactDigest !== D16_D15_QUALIFICATION_ARTIFACT_DIGEST
	)
	...
	if (
		terminal.schemaVersion !== D16_CURRENT_GRAPH_LIVE_TERMINAL_RECEIPT_SCHEMA ||
		terminal.decisionRef !== D16_CURRENT_GRAPH_LIVE_DECISION_REF ||
	...
	if (candidate.generation !== null) {
		...
		if (
			generation.schemaVersion !== D16_CURRENT_GRAPH_LIVE_GENERATION_SCHEMA ||
			generation.generationRef !== D16_CURRENT_GRAPH_LIVE_GENERATION_REF ||
			generation.graphEvidenceDigest !== graphEvidence?.evidenceDigest ||
			generation.coordinatesDigest !== D16_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST ||
	...
	return strictSnapshot(candidate) as unknown as D16CurrentGraphLiveBundleV1;
}

export async function persistD16CurrentGraphLiveBundle(inputValue: {
	readonly privateRoot: string;
	readonly bundle: D16CurrentGraphLiveBundleV1;
}) {
	...
	const bundle = validateD16CurrentGraphLiveBundle(input.bundle);
