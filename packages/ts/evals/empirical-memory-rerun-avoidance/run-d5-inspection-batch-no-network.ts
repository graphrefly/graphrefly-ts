...
} from "./d5-inspection-batch-qualification.js";
...
export async function runCurrentGraphNativePrivateQualification(privateRootValue: string) {
	...
	globals[transportKey] = async () => {
		networkCalls += 1;
		throw new TypeError("D5 Graph no-network sentinel");
	};
	try {
		...
		if (networkCalls !== 0) throw new TypeError("D5 Graph qualification attempted network");
...
if (entryPath === fileURLToPath(import.meta.url)) {
	...
	if (privateRoot === undefined) throw new TypeError("D5 Graph private root argument is required");
