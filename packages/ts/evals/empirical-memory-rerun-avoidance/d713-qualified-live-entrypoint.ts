...
export const D713_QUALIFIED_IMPLEMENTATION_ANCESTOR =
	"c62ad2ae4d7458052a757026001ef04bb50d1aa1" as const;
export const D713_QUALIFIED_NODE_VERSION = "v24.18.0" as const;
export const D713_QUALIFIED_EVAL_SOURCE_TREE_DIGEST =
	"sha256:2a5140162f7f5fbdde3621bc667462acb8b6eef0734dc7508f0f43144acacdec" as const;
export const D713_QUALIFIED_PRIVATE_SOURCE_TREE_DIGEST =
	"sha256:28988098947e1b64621b47376ea7c8dae678ab7f33fc0e6e2978af429adaf5f8" as const;
...
function normalizeEntrypointSource(bytes: Uint8Array): Uint8Array {
	...
	return new TextEncoder().encode(
		source.replace(
			/(D713_QUALIFIED_(?:EVAL|PRIVATE)_SOURCE_TREE_DIGEST\s*=\s*\n?\s*")[^"]+("\s+as const;)/g,
...
async function sourceTreeDigest(input: {
	...
	for (const name of names) {
		...
		entries.push([
			name,
			sha256(
				input.normalizeEntrypoint && name === "d713-qualified-live-entrypoint.ts"
...
export async function assertD713QualifiedLiveImplementation(input: {
	...
	if (evalSourceDigest !== D713_QUALIFIED_EVAL_SOURCE_TREE_DIGEST) {
		throw new TypeError("D713 package-private eval source tree drifted after qualification");
	}
	...
	if (privateSourceDigest !== D713_QUALIFIED_PRIVATE_SOURCE_TREE_DIGEST) {
		throw new TypeError("D713 private operator source tree drifted after qualification");
	}
	return join(input.privateOperatorRoot, "run-d713-live.ts");
}
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
	...
	const runnerPath = await assertD713QualifiedLiveImplementation({
		repositoryRoot,
		privateOperatorRoot,
