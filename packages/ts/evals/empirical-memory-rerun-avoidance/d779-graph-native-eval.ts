...
import {
	consumeD779ProviderResultEnvelope,
	type D779ConsumedProviderResultV1,
	type D779ProviderResultEnvelopeV1,
} from "./d779-provider-capable-composition.js";

export const D779_GRAPH_NATIVE_CORE_REVISION =
	"graphrefly.b112.d779.task-tool-provider-graph-core.v1" as const;

export interface D779CallerExecutorV1 {
	readonly revision: "graphrefly.b112.d779.caller-executor.v1";
	readonly execute: (
		input: D720CallerEffectExecutionInputV2,
	) => Promise<D779ProviderResultEnvelopeV1>;
}

export interface D779GraphNativeCoreV1 {
...
function validateExecutor(value: unknown): D779CallerExecutorV1 {
	const candidate = record(value, "d776.executor");
	exactKeys(candidate, ["execute", "revision"], "d776.executor");
	if (candidate.revision !== "graphrefly.b112.d779.caller-executor.v1")
	...
	return candidate as unknown as D779CallerExecutorV1;
}
...
export async function runD779GraphNativeEvalCore(inputValue: {
	...
	const taskExposureFacts: D778TaskExposureFactV1[] = [];
	const toolRejectionFacts: D778ToolRejectionFactV1[] = [];
	...
	for (let runCount = 0; runCount < 12; runCount += 1) {
		...
		for (let effectCount = 0; effectCount < 512; effectCount += 1) {
			...
			let consumed: D779ConsumedProviderResultV1;
			try {
				...
				consumed = consumeD779ProviderResultEnvelope(raw, effectRequest, admission);
			} catch {
			...
			const resultFactDigest = empiricalStrictJsonDigest(resultFactMaterial);
			...
			if (effectRequest.effectKind === "provider-request") {
				...
				if (envelope.taskEnvelope === null || consumed.taskWireReceipt === null)
					throw new TypeError("D779 provider result omitted task exposure capability");
				taskExposureFacts.push(
					admitD778TaskExposureProposal(
						createD778TaskExposureProposal({
							envelope: envelope.taskEnvelope,
							wireReceipt: consumed.taskWireReceipt,
							binding,
						}),
					),
				);
			...
			if (consumed.toolRejection !== null) {
				const diagnostic = consumed.toolRejection;
				toolRejectionFacts.push(
					admitD778ToolRejectionProposal(
						createD778ToolRejectionProposal({ ...diagnostic, binding }),
					),
				);
			}
...
export function createD779CallerExecutor(
	execute: (
		input: Readonly<{
