...
import { validateD38LiveBundle } from "./d38-premature-final-live.js";
import {
	admitD40EffectResult,
	createD40InferenceAuthority,
	type D40InferenceEvidenceV1,
...
import {
	consumeD40ExecutionAuthority,
	consumeD40PreexecutionFailureAuthority,
	type D40ExecutionAuthorityV1,
	type D40PreexecutionFailureAuthorityV1,
} from "./d40-phase-specific-inference-live-claim.js";
import {
	D40_COORDINATES_DIGEST,
	D40_D39_ARTIFACT_DIGEST,
	D40_D39_BUNDLE_DIGEST,
	D40_D39_IMPLEMENTATION_MANIFEST_DIGEST,
	D40_D39_PARTIAL_GRAPH_DIGEST,
	D40_D39_QUALIFICATION_ARTIFACT_DIGEST,
	D40_D39_QUALIFICATION_DIGEST,
	D40_DECISION_REF,
	D40_GENERATION_REF,
	D40_REPAIRED_LIVE_LIMITS,
...
import type { D40CredentialV1 } from "./d40-phase-specific-inference-live-preflight.js";
import {
	createD40PhaseSpecificRealProviderExecutor,
	type D40PhaseSpecificRealProviderExecutorV1,
} from "./d40-phase-specific-real-provider-composition.js";

export const D40_BASELINE_ADMISSION_REVISION =
	"graphrefly-ts.d40.d39-baseline-admission.v1" as const;
export const D40_BUNDLE_SCHEMA = "graphrefly-ts.d40.premature-final-live-bundle.v1" as const;
export const D40_GATE_SCHEMA = "graphrefly-ts.d40.positive-differential-gate.v1" as const;
export const D40_PARTIAL_SCHEMA = "graphrefly-ts.d40.partial-graph-evidence.v1" as const;
export const D40_GENERATION_SCHEMA = "graphrefly-ts.d40.live-generation.v1" as const;
export const D40_TERMINAL_SCHEMA = "graphrefly-ts.d40.live-terminal-receipt.v1" as const;
export const D40_MAX_BUNDLE_BYTES = 8_388_608;

export interface D40D39BaselineAdmissionV1 {
	readonly revision: typeof D40_BASELINE_ADMISSION_REVISION;
}

export interface D40PositiveDifferentialGateV1 {
	readonly schemaVersion: typeof D40_GATE_SCHEMA;
...
export interface D40PartialGraphEvidenceV1 {
	readonly schemaVersion: typeof D40_PARTIAL_SCHEMA;
	...
	readonly completedGraphEvidence: D40InferenceEvidenceV1 | null;
...
export interface D40LiveBundleV1 {
	readonly schemaVersion: typeof D40_BUNDLE_SCHEMA;
	readonly decisionRef: typeof D40_DECISION_REF;
	readonly executionClass: "live-provider" | "injected-no-network";
	readonly disposition: "success" | "partial-failure";
	readonly coordinatesDigest: string;
	readonly implementationManifestDigest: string;
	readonly d39ArtifactDigest: typeof D40_D39_ARTIFACT_DIGEST;
	readonly d39BundleDigest: typeof D40_D39_BUNDLE_DIGEST;
	readonly d39PartialGraphDigest: typeof D40_D39_PARTIAL_GRAPH_DIGEST;
	readonly d39QualificationArtifactDigest: typeof D40_D39_QUALIFICATION_ARTIFACT_DIGEST;
	readonly d39QualificationDigest: typeof D40_D39_QUALIFICATION_DIGEST;
	readonly d39ImplementationManifestDigest: typeof D40_D39_IMPLEMENTATION_MANIFEST_DIGEST;
	...
	readonly graphEvidence: D40InferenceEvidenceV1 | null;
	readonly partialGraphEvidence: D40PartialGraphEvidenceV1 | null;
	readonly gate: D40PositiveDifferentialGateV1;
...
const baselines = new WeakMap<object, "consumed-d39-artifact" | "injected-test">();
const constructed = new WeakSet<object>();

function makeBaseline(basis: "consumed-d39-artifact" | "injected-test") {
	const value = Object.freeze({ revision: D40_BASELINE_ADMISSION_REVISION });
	baselines.set(value, basis);
	return value;
}

export function admitD40D39Baseline(bytesValue: Uint8Array): D40D39BaselineAdmissionV1 {
	if (!(bytesValue instanceof Uint8Array))
		throw new TypeError("D40 D39 baseline bytes are invalid");
	const bytes = new Uint8Array(bytesValue);
	if (empiricalSha256(bytes) !== D40_D39_ARTIFACT_DIGEST)
		throw new TypeError("D40 D39 immutable artifact drifted");
	const bundle = validateD38LiveBundle(strictJsonCodec.decode(bytes));
	if (
		bundle.disposition !== "partial-failure" ||
		bundle.bundleDigest !== D40_D39_BUNDLE_DIGEST ||
		bundle.partialGraphEvidence?.partialGraphDigest !== D40_D39_PARTIAL_GRAPH_DIGEST ||
		bundle.qualificationArtifactDigest !== D40_D39_QUALIFICATION_ARTIFACT_DIGEST ||
		bundle.qualificationDigest !== D40_D39_QUALIFICATION_DIGEST ||
		bundle.implementationManifestDigest !== D40_D39_IMPLEMENTATION_MANIFEST_DIGEST ||
		bundle.efficacyClaim !== "none"
	)
		throw new TypeError("D40 D39 immutable coordinates drifted");
	return makeBaseline("consumed-d39-artifact");
}

export function createD40InjectedBaselineForTest(): D40D39BaselineAdmissionV1 {
	return makeBaseline("injected-test");
}

function consumeBaseline(
	value: unknown,
	expected: "consumed-d39-artifact" | "injected-test",
): void {
	if (value === null || typeof value !== "object") throw new TypeError("D40 baseline is invalid");
	const basis = baselines.get(value);
	baselines.delete(value);
	if (basis !== expected) throw new TypeError("D40 baseline is forged, replayed or drifted");
}

function credentialBinding(credential: D40CredentialV1): string {
...
function evaluateGate(
	evidence: D40InferenceEvidenceV1 | null,
	evaluated: boolean,
): D40PositiveDifferentialGateV1 {
...
function partialEvidence(input: {
	...
	readonly activeEffectKind: string | null;
	readonly completedGraphEvidence: D40InferenceEvidenceV1 | null;
	readonly failureCode: D40PartialGraphEvidenceV1["failureCode"];
}): D40PartialGraphEvidenceV1 {
	const material = strictSnapshot({
		schemaVersion: D40_PARTIAL_SCHEMA,
...
async function drive(input: {
	readonly executionAuthority: D40ExecutionAuthorityV1;
	readonly baseline: D40D39BaselineAdmissionV1;
	readonly executionClass: D40LiveBundleV1["executionClass"];
	readonly executorFactory: (
		authority: ReturnType<typeof createD40InferenceAuthority>,
...
}): Promise<D40LiveBundleV1> {
	consumeBaseline(
		input.baseline,
		input.executionClass === "live-provider" || input.allowConsumedBaselineForQualification === true
			? "consumed-d39-artifact"
			: "injected-test",
	);
	const executionAuthority = consumeD40ExecutionAuthority(input.executionAuthority);
	if (executionAuthority.claim.implementationManifestDigest !== input.implementationManifestDigest)
		throw new TypeError("D40 implementation authority drifted");
	const graphAuthority = createD40InferenceAuthority({
	...
	let graphEvidence: D40InferenceEvidenceV1 | null = null;
	let failureCode: D40PartialGraphEvidenceV1["failureCode"] | null = null;
	try {
		for (let guard = 0; guard < D40_REPAIRED_LIVE_LIMITS.maxEffectFacts; guard += 1) {
			const admitted = takeD40AdmittedEffect(graphAuthority);
			if (admitted === null) {
				try {
					graphEvidence = validateD40InferenceEvidence(
						snapshotD40InferenceEvidence(graphAuthority),
					);
				} catch {
			...
			let execution: Awaited<ReturnType<D40PhaseSpecificRealProviderExecutorV1["execute"]>>;
			try {
			...
			try {
				const outcome = admitD40EffectResult(
					graphAuthority,
					execution.admitted,
					execution.result,
	...
	const generation = success
		? (() => {
				const material = strictSnapshot({
					schemaVersion: D40_GENERATION_SCHEMA,
	...
	const terminalMaterial = strictSnapshot({
		schemaVersion: D40_TERMINAL_SCHEMA,
		decisionRef: D40_DECISION_REF,
	...
	const material = strictSnapshot({
		schemaVersion: D40_BUNDLE_SCHEMA,
		decisionRef: D40_DECISION_REF,
	...
	if (
		strictJsonCodec.encode(bundle as unknown as StrictJsonValue).byteLength > D40_MAX_BUNDLE_BYTES
	)
		throw new TypeError("D40 live bundle exceeded its byte bound");
	constructed.add(bundle);
	return bundle;
}

export async function runD40LiveMeasurement(input: {
	readonly executionAuthority: D40ExecutionAuthorityV1;
	readonly baseline: D40D39BaselineAdmissionV1;
	...
	if (
		...
		throw new TypeError("D40 live credential or claim scope drifted");
	return drive({
		...
		beforeProviderEffect: async () => {
			if (
				(await measureD40Implementation(input.repositoryRoot)) !==
				input.implementationManifestDigest
			)
				throw new TypeError("D40 implementation drifted before admitted provider effect");
		},
...
export async function runD40InjectedMeasurementForTest(input: {
	readonly executionAuthority: D40ExecutionAuthorityV1;
	readonly baseline: D40D39BaselineAdmissionV1;
	readonly implementationManifestDigest: string;
	readonly executorFactory: (
...
function validateD40PartialGraphEvidence(value: unknown): D40PartialGraphEvidenceV1 {
	const candidate = record(value, "D40 partial Graph evidence");
	exactKeys(
		candidate,
		[
			"activeAdmissionDigest",
		...
		"D40 partial Graph evidence",
	);
	if (
		candidate.schemaVersion !== D40_PARTIAL_SCHEMA ||
		candidate.decisionRef !== D40_DECISION_REF ||
		candidate.coordinatesDigest !== D40_COORDINATES_DIGEST ||
		!(
			[
				"executor-boundary-failed",
				"graph-admission-failed",
		...
		throw new TypeError("D40 partial Graph evidence coordinates drifted");
	for (const [key, value] of [
		["activeRequestDigest", candidate.activeRequestDigest],
		["activeAdmissionDigest", candidate.activeAdmissionDigest],
	] as const)
		if (value !== null) digest(value, `D40 partial ${key}`);
	if (
		...
		throw new TypeError("D40 partial active effect kind drifted");
	const providerFacts = array(candidate.providerFacts, "D40 partial provider facts");
	if (providerFacts.length > D40_REPAIRED_LIVE_LIMITS.maxEffectFacts)
		throw new TypeError("D40 partial provider fact bound exceeded");
	const completedGraphEvidence =
		candidate.completedGraphEvidence === null
			? null
			: validateD40InferenceEvidence(candidate.completedGraphEvidence);
	if (completedGraphEvidence !== null) {
		...
		for (const [index, factValue] of providerFacts.entries()) {
			const fact = record(factValue, `D40 partial provider facts[${index}]`);
			if (!completeDigests.has(digest(fact.factDigest, `D40 partial provider fact[${index}]`)))
				throw new TypeError("D40 partial provider facts are not bound to completed Graph evidence");
		}
	}
	const material = strictSnapshot({
		schemaVersion: D40_PARTIAL_SCHEMA,
	...
	if (candidate.partialGraphDigest !== empiricalStrictJsonDigest(material))
		throw new TypeError("D40 partial Graph evidence digest drifted");
	return Object.freeze({
		...material,
		partialGraphDigest: candidate.partialGraphDigest,
	}) as D40PartialGraphEvidenceV1;
}

function validateD40Generation(
	value: unknown,
	input: {
		...
		readonly qualificationArtifactDigest: string;
		readonly qualificationDigest: string;
		readonly efficacyClaim: D40LiveBundleV1["efficacyClaim"];
	},
): Readonly<Record<string, StrictJsonValue>> {
	const candidate = record(value, "D40 generation");
	exactKeys(
		candidate,
		[
			"causalAttribution",
	...
	const material = strictSnapshot({
		...
		implementationManifestDigest: D40_IMPLEMENTATION_MANIFEST_DIGEST,
	...
	if (
		...
		throw new TypeError("D40 generation drifted");
	return Object.freeze({ ...material, generationDigest: candidate.generationDigest });
}

export function validateD40LiveBundle(value: unknown): D40LiveBundleV1 {
	const candidate = record(value, "D40 live bundle");
	exactKeys(
		candidate,
		[
			"bundleDigest",
			...
			"d39ArtifactDigest",
			"d39BundleDigest",
			"d39ImplementationManifestDigest",
			"d39PartialGraphDigest",
			"d39QualificationArtifactDigest",
			"d39QualificationDigest",
			"decisionRef",
			"disposition",
			"efficacyClaim",
			"executionClass",
	...
	if (
		candidate.schemaVersion !== D40_BUNDLE_SCHEMA ||
		candidate.decisionRef !== D40_DECISION_REF ||
		...
		candidate.coordinatesDigest !== D40_COORDINATES_DIGEST ||
		candidate.implementationManifestDigest !== D40_IMPLEMENTATION_MANIFEST_DIGEST ||
		candidate.d39ArtifactDigest !== D40_D39_ARTIFACT_DIGEST ||
		candidate.d39BundleDigest !== D40_D39_BUNDLE_DIGEST ||
		candidate.d39PartialGraphDigest !== D40_D39_PARTIAL_GRAPH_DIGEST ||
		candidate.d39QualificationArtifactDigest !== D40_D39_QUALIFICATION_ARTIFACT_DIGEST ||
		candidate.d39QualificationDigest !== D40_D39_QUALIFICATION_DIGEST ||
		candidate.d39ImplementationManifestDigest !== D40_D39_IMPLEMENTATION_MANIFEST_DIGEST
	)
		throw new TypeError("D40 live coordinates drifted");
	for (const key of [
		"claimDigest",
		"currentKeyAdmissionDigest",
		"pricingObservationDigest",
		...
		digest(candidate[key], `D40 live ${key}`);
	const graphEvidence =
		candidate.graphEvidence === null ? null : validateD40InferenceEvidence(candidate.graphEvidence);
	const partialGraphEvidence =
		candidate.partialGraphEvidence === null
			? null
			: validateD40PartialGraphEvidence(candidate.partialGraphEvidence);
	if (
		...
		throw new TypeError("D40 disposition drifted");
	...
	if (
		...
		throw new TypeError("D40 gate drifted");
	...
	if (candidate.efficacyClaim !== expectedClaim || candidate.causalAttribution !== "undetermined")
		throw new TypeError("D40 efficacy claim drifted");
	const generation =
		candidate.generation === null
			? null
			: validateD40Generation(candidate.generation, {
					graphEvidenceDigest: graphEvidence!.evidenceDigest,
	...
	const terminal = record(candidate.terminalReceipt, "D40 terminal receipt");
	exactKeys(
		terminal,
		[
			"causalAttribution",
	...
	const providerAttempts = accountingFacts.filter((fact) => {
		const request = record(record(fact, "D40 accounting fact").request, "D40 accounting request");
	...
	const confirmedCostMicrousd = accountingFacts.reduce((sum, fact) => {
		const reconciliation = record(
			record(fact, "D40 accounting fact").reconciliation,
			"D40 accounting reconciliation",
		);
		return sum + safeInteger(reconciliation.actualCostMicrousd, "D40 actual cost");
	}, 0);
	const terminalMaterial = strictSnapshot({
		schemaVersion: D40_TERMINAL_SCHEMA,
		decisionRef: D40_DECISION_REF,
	...
	if (
		...
		throw new TypeError("D40 terminal receipt drifted");
	...
	if (candidate.bundleDigest !== empiricalStrictJsonDigest(material))
		throw new TypeError("D40 bundle digest drifted");
	return canonical as unknown as D40LiveBundleV1;
}

export async function persistD40LiveBundle(input: {
	readonly privateRoot: string;
	readonly bundle: D40LiveBundleV1;
}) {
	if (!constructed.has(input.bundle as object))
		throw new TypeError("D40 live bundle is forged or replayed");
	constructed.delete(input.bundle as object);
	const bundle = validateD40LiveBundle(input.bundle);
	const commitMaterial = strictSnapshot({
		schemaVersion: "graphrefly-ts.d40.live-commit.v1",
		generationRef: D40_GENERATION_REF,
...
export async function persistD40PreexecutionFailure(input: {
	readonly privateRoot: string;
	readonly failureAuthority: D40PreexecutionFailureAuthorityV1;
	readonly implementationManifestDigest: string;
}) {
	const authority = consumeD40PreexecutionFailureAuthority(input.failureAuthority);
	if (authority.claim.scope !== "live-fixed-root")
		throw new TypeError("D40 preexecution failure rejected a non-live claim");
	const material = strictSnapshot({
		schemaVersion: "graphrefly-ts.d40.live-preexecution-failure.v1",
		decisionRef: D40_DECISION_REF,
		generationRef: D40_GENERATION_REF,
		coordinatesDigest: D40_COORDINATES_DIGEST,
		...
		implementationManifestDigest: digest(
			input.implementationManifestDigest,
			"D40 preexecution implementation",
		),
	...
	const commitMaterial = strictSnapshot({
		schemaVersion: "graphrefly-ts.d40.live-preexecution-commit.v1",
		generationRef: D40_GENERATION_REF,
