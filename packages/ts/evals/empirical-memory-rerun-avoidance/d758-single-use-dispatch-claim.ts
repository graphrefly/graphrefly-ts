...
export const D758_DISPATCH_CLAIM_SCHEMA =
	"graphrefly.b112.d758.single-use-dispatch-claim.v1" as const;
export const D758_PRIVATE_ROOT = resolve(
	import.meta.dirname,
...
export interface D758PersistedDispatchClaimV1 {
	readonly schemaVersion: typeof D758_DISPATCH_CLAIM_SCHEMA;
	readonly claimRef: typeof D758_DISPATCH_CLAIM_REF;
	readonly decisionRef: typeof D758_DECISION_REF;
	readonly decisionRevision: typeof D758_DECISION_REVISION;
	readonly generationRef: typeof D758_GENERATION_REF;
...
export interface D758ExecutionAuthorityV1 {
	readonly revision: "graphrefly.b112.d758.execution-authority.v1";
	readonly claim: D758PersistedDispatchClaimV1;
...
function claimMaterial(
	...
	const input = record(inputValue, "d758.claim.input");
	exactKeys(
		input,
	...
	return strictSnapshot({
		schemaVersion: D758_DISPATCH_CLAIM_SCHEMA,
		claimRef: D758_DISPATCH_CLAIM_REF,
		...
		pricingReadDigest: digest(input.pricingReadDigest, "d758.claim.pricing"),
		zeroByokObservationDigest: digest(input.zeroByokObservationDigest, "d758.claim.zeroByok"),
		implementationManifestDigest: digest(
			input.implementationManifestDigest,
			"d758.claim.implementation",
		),
...
async function acquire(
...
): Promise<D758PersistedDispatchClaimV1> {
	const requestedRoot = resolve(privateRootValue);
	const privateRoot = await realpath(requestedRoot);
	if (privateRoot !== requestedRoot) throw new TypeError("D758 private root is not canonical");
	...
	if (
		...
		throw new TypeError("D758 private root ownership is invalid");
	const claimRoot = join(privateRoot, `.${D758_DISPATCH_CLAIM_REF}`);
	...
	}) as D758PersistedDispatchClaimV1;
	...
	try {
		...
		if (
			...
			throw new TypeError("D758 dispatch claim readback drifted");
	} finally {
		await reader.close();
	}
	claims.set(claim, {
...
export function acquireD758SingleUseDispatchClaim(input: Parameters<typeof claimMaterial>[1]) {
	return acquire(D758_PRIVATE_ROOT, "live-fixed-root", input);
}

export function acquireD758SingleUseDispatchClaimAtRootForTest(
	root: string,
	input: Parameters<typeof claimMaterial>[1],
) {
...
async function revalidate(claim: D758PersistedDispatchClaimV1, state: ClaimState): Promise<void> {
	...
	if (
		...
		throw new TypeError("D758 durable claim directory drifted");
	...
	try {
		...
		if (
			...
			throw new TypeError("D758 durable claim file drifted");
	} finally {
		await reader.close();
	}
	literal(
...
export async function consumeD758DispatchClaimForExecution(inputValue: {
	readonly claim: D758PersistedDispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
}): Promise<D758ExecutionAuthorityV1> {
	const input = record(inputValue, "d758.execution.input");
	exactKeys(input, ["claim", "currentKeyAdmission"], "d758.execution.input");
	const claim = input.claim as D758PersistedDispatchClaimV1;
	const state = typeof claim === "object" && claim !== null ? claims.get(claim) : undefined;
	if (state === undefined) throw new TypeError("D758 claim is not fresh and constructed");
	claims.delete(claim);
	...
	if (
		...
		admission.remainingMicrousd < D758_BUDGET_LIMITS.maxCostMicrousd ||
		admission.limitReset !== "none" ||
		admission.isManagementKey
	)
		throw new TypeError("D758 current-key admission is outside authority");
	...
	const authority = Object.freeze({
		revision: "graphrefly.b112.d758.execution-authority.v1" as const,
		claim,
...
export function consumeD758ExecutionAuthority(value: unknown): D758ExecutionAuthorityV1 {
	if (typeof value !== "object" || value === null || !authorities.delete(value))
		throw new TypeError("D758 live execution authority must be fixed-root and single-use");
	return value as D758ExecutionAuthorityV1;
}
