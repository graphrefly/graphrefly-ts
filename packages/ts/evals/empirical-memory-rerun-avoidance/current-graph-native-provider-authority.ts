...
export const CURRENT_GRAPH_PROVIDER_REVISION =
	"graphrefly-ts.d2.current-graph-native-provider-authority.v1" as const;
export const CURRENT_GRAPH_PROVIDER_EVIDENCE_SCHEMA =
	"graphrefly-ts.d2.current-graph-native-provider-evidence.v1" as const;
export const CURRENT_GRAPH_PROVIDER_ROUTE_SCHEMA =
	"graphrefly-ts.d2.current-graph-native-route-profile.v1" as const;
export const CURRENT_GRAPH_PROVIDER_TASK_SCHEMA =
	"graphrefly-ts.d2.current-graph-native-task-profile.v1" as const;
export const CURRENT_GRAPH_PROVIDER_ENVELOPE_SCHEMA =
	"graphrefly-ts.d2.current-graph-native-model-envelope.v1" as const;
...
interface CurrentGraphProviderRequestProjectionV1 {
	readonly schemaVersion: "graphrefly-ts.d2.current-graph-native-effect-request.v1";
...
export interface CurrentGraphProviderEffectAdmissionV1 {
	readonly schemaVersion: "graphrefly-ts.d2.current-graph-native-effect-admission.v1";
	readonly requestDigest: string;
...
export interface CurrentGraphProviderEvidenceV1 {
	...
	readonly decisionRef: "graphrefly-ts:D2";
	readonly d1Baseline: Readonly<{
		commit: "fa6a731695c63a9da6681e76c07bd1ed3182e7a2";
		bundleArtifactDigest: "sha256:1ef86a1aede3ba96614b0fe9d39ac3c14c630d37f27fd878982991afcf3937ae";
		implementationManifestDigest: "sha256:6a9151b68dcdcb983d818ec0af9eb3ec6819883ec78a883b012edeae6229bc6d";
	}>;
	readonly topology: Readonly<{
		runtimeFactNode: "current/provider/runtime-facts";
		canonicalProjectionNode: "current/provider/canonical-projection";
...
function createRuntimeFactNode(owner: ReturnType<typeof graph>) {
	return owner.node<RuntimeAdmittedFact>([], null, { name: "current/provider/runtime-facts" });
}
...
function schedule(
	...
	const projectionMaterial = strictSnapshot({
		schemaVersion: "graphrefly-ts.d2.current-graph-native-effect-request.v1" as const,
		sequence,
	...
	const admissionMaterial = strictSnapshot({
		schemaVersion: "graphrefly-ts.d2.current-graph-native-effect-admission.v1" as const,
...
export function createCurrentGraphProviderAuthority(inputValue: {
	...
	const owner = graph({ name: "current/provider/graph-native-eval" });
	...
	const projectionNode = owner.node<RuntimeAdmittedFact>(
		...
		{
			name: "current/provider/canonical-projection",
...
function topology() {
	const material = strictSnapshot({
		runtimeFactNode: "current/provider/runtime-facts" as const,
		canonicalProjectionNode: "current/provider/canonical-projection" as const,
	});
...
function d1Baseline() {
	return Object.freeze({
		commit: "fa6a731695c63a9da6681e76c07bd1ed3182e7a2" as const,
		bundleArtifactDigest:
			"sha256:1ef86a1aede3ba96614b0fe9d39ac3c14c630d37f27fd878982991afcf3937ae" as const,
		implementationManifestDigest:
			"sha256:6a9151b68dcdcb983d818ec0af9eb3ec6819883ec78a883b012edeae6229bc6d" as const,
	});
}
...
function validateFactShape(value: unknown, index: number): CurrentGraphProviderFactV1 {
	...
	if (
		request.schemaVersion !== "graphrefly-ts.d2.current-graph-native-effect-request.v1" ||
	...
	if (
		admission.schemaVersion !== "graphrefly-ts.d2.current-graph-native-effect-admission.v1" ||
...
export function validateCurrentGraphProviderEvidence(
	...
	if (
		...
		candidate.decisionRef !== "graphrefly-ts:D2" ||
	...
	const baseline = record(candidate.d1Baseline, "current.provider.evidence.d1Baseline");
	exactKeys(
		baseline,
		["bundleArtifactDigest", "commit", "implementationManifestDigest"],
		"current.provider.evidence.d1Baseline",
	);
	if (empiricalStrictJsonDigest(baseline) !== empiricalStrictJsonDigest(d1Baseline()))
		throw new TypeError("current provider D1 baseline drifted");
