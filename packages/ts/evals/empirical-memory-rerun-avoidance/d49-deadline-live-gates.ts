...
export const D49_LIVE_CLAIM_SCHEMA = "graphrefly-ts.d49.dispatch-claim.v1" as const;
export const D49_LIVE_BUNDLE_SCHEMA = "graphrefly-ts.d49.live-bundle.v1" as const;
export const D49_LIVE_GENERATION_REF = "current-graph-native-deadline-live-d49-v1" as const;
export const D49_LIVE_CLAIM_REF = "current-graph-native-deadline-claim-d49-v1" as const;
export const D49_LIVE_PRIVATE_ROOT = resolve(
	import.meta.dirname,
	"../.private/empirical-memory-rerun-avoidance/current-graph-native-d49-live",
);

export interface D49PreclaimV1 {
...
export interface D49DispatchClaimV1 {
	readonly schemaVersion: typeof D49_LIVE_CLAIM_SCHEMA;
	readonly claimRef: typeof D49_LIVE_CLAIM_REF;
	readonly authorityRef: "graphrefly-ts:D49";
	readonly architectureRef: "graphrefly-ts:D48";
	readonly generationRef: typeof D49_LIVE_GENERATION_REF;
	readonly preclaimDigest: string;
...
export interface D49ExecutionAuthorityV1 {
	readonly claim: D49DispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
	readonly authorityDigest: string;
}

export type D49LiveBundleV1 = Readonly<{
	schemaVersion: typeof D49_LIVE_BUNDLE_SCHEMA;
	generationRef: typeof D49_LIVE_GENERATION_REF;
...
export function composeD49Preclaim(input: {
...
export async function acquireD49DispatchClaim(input: {
	readonly privateRoot: string;
	readonly preclaim: D49PreclaimV1;
	readonly implementationCommit: string;
	readonly implementationManifestDigest: string;
	readonly qualificationArtifactDigest: string;
	readonly qualificationDigest: string;
}): Promise<D49DispatchClaimV1> {
	if (!preclaims.delete(input.preclaim)) throw new TypeError("D49 preclaim is absent or consumed");
	const privateRoot = resolve(input.privateRoot);
	if ((await realpath(privateRoot)) !== privateRoot)
		throw new TypeError("D49 private root drifted");
	const claimRoot = join(privateRoot, `.${D49_LIVE_CLAIM_REF}`);
	await mkdir(claimRoot, { mode: 0o700 });
	await chmod(claimRoot, 0o700);
	const material = strictSnapshot({
		schemaVersion: D49_LIVE_CLAIM_SCHEMA,
		claimRef: D49_LIVE_CLAIM_REF,
		authorityRef: "graphrefly-ts:D49" as const,
		architectureRef: "graphrefly-ts:D48" as const,
		generationRef: D49_LIVE_GENERATION_REF,
...
export async function consumeD49DispatchClaim(input: {
	readonly claim: D49DispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
}): Promise<D49ExecutionAuthorityV1> {
	const state = claims.get(input.claim);
	if (state === undefined) throw new TypeError("D49 dispatch claim is absent or consumed");
	...
	try {
		if (!sameBytes(new Uint8Array(await reader.readFile()), state.bytes))
			throw new TypeError("D49 durable dispatch claim drifted");
	} finally {
		await reader.close();
	}
...
export function constructD49LiveBundle(input: {
	readonly authority: D49ExecutionAuthorityV1;
	readonly pricing: D44D45PricingObservationV1;
	readonly zeroByok: D44D45ZeroByokObservationV1;
	readonly implementationCommit: string;
	readonly implementationManifestDigest: string;
	...
	const partialGraphEvidence =
		input.measurement.disposition === "partial-failure"
			...
			: null;
	const material = strictSnapshot({
		schemaVersion: D49_LIVE_BUNDLE_SCHEMA,
		generationRef: D49_LIVE_GENERATION_REF,
...
export async function persistD49LiveBundle(input: {
	readonly privateRoot: string;
	readonly bundle: D49LiveBundleV1;
}): Promise<Readonly<{ artifactDigest: string; receiptDigest: string }>> {
	if (!isAbsolute(input.privateRoot)) throw new TypeError("D49 persistence root must be absolute");
	const generationRoot = join(input.privateRoot, D49_LIVE_GENERATION_REF);
...
export async function prepareD49PrivateRoot(path: string): Promise<void> {
	await mkdir(path, { recursive: true, mode: 0o700 });
	await chmod(path, 0o700);
	const stat = await lstat(path);
	if (!stat.isDirectory() || stat.isSymbolicLink() || (stat.mode & 0o777) !== 0o700)
		throw new TypeError("D49 private root identity failed");
}
