...
import {
	...
	markPersistedD711DispatchClaimFailedAtPrivateRoot,
	type PersistedD711SingleUseDispatchClaimV1,
	validateD711ExecutionStartedMarker,
	validatePersistedD711DispatchClaimAtPrivateRoot,
...
export const D711_LIVE_ATTEMPT_RECEIPT_SCHEMA =
	"graphrefly.private-solution-eval.d711-live-attempt-receipt.v1" as const;
export const D711_LIVE_ATTEMPT_RECEIPT_FILE = "terminal-attempt.v1.json" as const;

export type D711LiveAttemptPhase =
	| "claim-acquired"
	| "current-key-admitted"
	| "provider-block"
	| "generation-persistence";

export interface D711LiveAttemptReceiptV1 {
	readonly schemaVersion: typeof D711_LIVE_ATTEMPT_RECEIPT_SCHEMA;
	...
	readonly terminalPhase: D711LiveAttemptPhase;
...
export async function persistD711LiveAttemptReceipt(input: {
	readonly claim: PersistedD711SingleUseDispatchClaimV1;
	readonly terminalStatus: "success" | "failed";
	readonly terminalPhase: D711LiveAttemptPhase;
	readonly currentKeyNetworkCalls: 0 | 1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1 | null;
	readonly providerTransportCalls: number;
}): Promise<D711LiveAttemptReceiptV1> {
	const candidate = record(input, "d711.attemptReceiptInput");
	exactKeys(
		candidate,
		[
			"claim",
		...
		"d711.attemptReceiptInput",
	);
	const claim = record(candidate.claim, "d711.attemptReceipt.claim");
	exactKeys(claim, ["claimDigest", "claimPath"], "d711.attemptReceipt.claim");
	if (
		typeof claim.claimPath !== "string" ||
		claim.claimPath.length === 0 ||
		claim.claimPath !== join(dirname(claim.claimPath), D711_SINGLE_USE_DISPATCH_CLAIM_DIRECTORY)
	) {
		throw new TypeError("D711 attempt receipt claim path is not exact");
	}
	if (typeof claim.claimDigest !== "string" || !/^sha256:[0-9a-f]{64}$/.test(claim.claimDigest)) {
		throw new TypeError("D711 attempt receipt claim digest is invalid");
	}
	...
	if (
		...
		throw new TypeError("D711 attempt receipt claim ownership is invalid");
	}
	const privateRoot = dirname(claim.claimPath);
	await validatePersistedD711DispatchClaimAtPrivateRoot(
		privateRoot,
		claim as unknown as PersistedD711SingleUseDispatchClaimV1,
	);
	const terminalStatus = oneOf(
		candidate.terminalStatus,
		["success", "failed"] as const,
		"d711.attemptReceipt.status",
	);
	...
	if (candidate.currentKeyAdmission !== null) {
		...
		exactKeys(
			...
			"d711.attemptReceipt.currentKey",
		);
		literal(
			current.schemaVersion,
			"graphrefly.private-solution-eval.openrouter-current-key-spend-admission.v1",
			"d711.attemptReceipt.currentKey.schema",
		);
		const currentMaterial = strictSnapshot({
			schemaVersion:
				"graphrefly.private-solution-eval.openrouter-current-key-spend-admission.v1" as const,
			limitMicrousd: safeInteger(current.limitMicrousd, "d711.attemptReceipt.currentKey.limit", {
				min: 32_000_000,
				max: 32_000_000,
			}),
			remainingMicrousd: safeInteger(
				current.remainingMicrousd,
				"d711.attemptReceipt.currentKey.remaining",
				{ min: 6_000_000, max: 32_000_000 },
			),
			usageMicrousd: safeInteger(current.usageMicrousd, "d711.attemptReceipt.currentKey.usage", {
				min: 0,
				max: 32_000_000,
			}),
			limitReset: literal(current.limitReset, "none", "d711.attemptReceipt.currentKey.reset"),
			isManagementKey: literal(
				current.isManagementKey,
				false,
				"d711.attemptReceipt.currentKey.management",
			),
		});
		literal(
			current.admissionDigest,
			empiricalStrictJsonDigest(currentMaterial),
			"d711.attemptReceipt.currentKey.digest",
		);
	...
	if (
		...
		throw new TypeError("D711 terminal receipt status/phase evidence is inconsistent");
	}
	if (terminalPhase === "claim-acquired") {
		await markPersistedD711DispatchClaimFailedAtPrivateRoot(
			privateRoot,
			claim as unknown as PersistedD711SingleUseDispatchClaimV1,
		);
	} else {
	...
	const material = strictSnapshot({
		schemaVersion: D711_LIVE_ATTEMPT_RECEIPT_SCHEMA,
	...
	const receiptPath = join(claim.claimPath, D711_LIVE_ATTEMPT_RECEIPT_FILE);
