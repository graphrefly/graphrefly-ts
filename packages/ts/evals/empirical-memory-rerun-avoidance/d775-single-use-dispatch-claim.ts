...
import {
	...
	D775_DISPATCH_CLAIM_REF,
	D775_GENERATION_REF,
	D775_HISTORICAL_BUNDLE_DIGEST,
	D775_ROUTE_PROFILE_DIGEST,
...
export const D775_DISPATCH_CLAIM_SCHEMA =
	"graphrefly.b112.d775.single-use-dispatch-claim.v1" as const;
export const D775_PRIVATE_ROOT = resolve(
	import.meta.dirname,
...
export interface D775PersistedDispatchClaimV1 {
	readonly schemaVersion: typeof D775_DISPATCH_CLAIM_SCHEMA;
	readonly claimRef: typeof D775_DISPATCH_CLAIM_REF;
	readonly decisionRef: typeof D775_DECISION_REF;
	readonly decisionRevision: typeof D775_DECISION_REVISION;
	readonly generationRef: typeof D775_GENERATION_REF;
...
export interface D775ExecutionAuthorityV1 {
	readonly revision: "graphrefly.b112.d775.execution-authority.v1";
	readonly claim: D775PersistedDispatchClaimV1;
...
function claimMaterial(
	...
	const input = record(inputValue, "d775.claim.input");
	exactKeys(
		input,
	...
	literal(
		...
		"d775.claim.implementation",
	);
	return strictSnapshot({
		schemaVersion: D775_DISPATCH_CLAIM_SCHEMA,
		claimRef: D775_DISPATCH_CLAIM_REF,
		...
		pricingReadDigest: digest(input.pricingReadDigest, "d775.claim.pricing"),
		zeroByokObservationDigest: digest(input.zeroByokObservationDigest, "d775.claim.zeroByok"),
...
async function acquire(
...
): Promise<D775PersistedDispatchClaimV1> {
	const requestedRoot = resolve(privateRootValue);
	const privateRoot = await realpath(requestedRoot);
	if (privateRoot !== requestedRoot) throw new TypeError("D775 private root is not canonical");
	...
	if (
		...
		throw new TypeError("D775 private root ownership is invalid");
	const claimRoot = join(privateRoot, `.${D775_DISPATCH_CLAIM_REF}`);
	...
	}) as D775PersistedDispatchClaimV1;
	...
	try {
		...
		if (
			...
			throw new TypeError("D775 dispatch claim readback drifted");
	} finally {
		await reader.close();
	}
	claims.set(claim, {
...
export function acquireD775SingleUseDispatchClaim(input: Parameters<typeof claimMaterial>[1]) {
	return acquire(D775_PRIVATE_ROOT, "live-fixed-root", input);
}

export function acquireD775SingleUseDispatchClaimAtRootForTest(
	root: string,
	input: Parameters<typeof claimMaterial>[1],
) {
...
async function revalidate(claim: D775PersistedDispatchClaimV1, state: ClaimState): Promise<void> {
	...
	if (
		...
		throw new TypeError("D775 durable claim directory drifted");
	...
	try {
		...
		if (
			...
			throw new TypeError("D775 durable claim file drifted");
	} finally {
		await reader.close();
	}
	literal(
...
export async function consumeD775DispatchClaimForExecution(inputValue: {
	readonly claim: D775PersistedDispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
}): Promise<D775ExecutionAuthorityV1> {
	const input = record(inputValue, "d775.execution.input");
	exactKeys(input, ["claim", "currentKeyAdmission"], "d775.execution.input");
	const claim = input.claim as D775PersistedDispatchClaimV1;
	const state = typeof claim === "object" && claim !== null ? claims.get(claim) : undefined;
	if (state === undefined) throw new TypeError("D775 claim is not fresh and constructed");
	claims.delete(claim);
	...
	if (
		...
		admission.remainingMicrousd < D775_BUDGET_LIMITS.maxCostMicrousd ||
		admission.limitReset !== "none" ||
		admission.isManagementKey
	)
		throw new TypeError("D775 current-key admission is outside authority");
	...
	const authority = Object.freeze({
		revision: "graphrefly.b112.d775.execution-authority.v1" as const,
		claim,
...
export function consumeD775ExecutionAuthority(value: unknown): D775ExecutionAuthorityV1 {
	if (typeof value !== "object" || value === null || !authorities.delete(value))
		throw new TypeError("D775 live execution authority must be fixed-root and single-use");
	return value as D775ExecutionAuthorityV1;
}
