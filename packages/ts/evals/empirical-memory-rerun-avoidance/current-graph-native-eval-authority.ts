...
export const CURRENT_GRAPH_NATIVE_EVAL_REVISION =
	"graphrefly-ts.d1.current-graph-native-eval-authority.v1" as const;
export const CURRENT_GRAPH_NATIVE_EVIDENCE_SCHEMA =
	"graphrefly-ts.d1.current-graph-native-evidence.v1" as const;
export const CURRENT_GRAPH_CORRECTION_SCHEMA =
	"graphrefly-ts.d1.graph-correction-directive.v1" as const;
...
export interface CurrentGraphEffectRequestV1 {
	readonly schemaVersion: "graphrefly-ts.d1.graph-effect-request.v1";
...
export interface CurrentGraphEffectAdmissionV1 {
	readonly schemaVersion: "graphrefly-ts.d1.graph-effect-admission.v1";
	readonly requestDigest: string;
...
export interface CurrentGraphNativeEvidenceV1 {
	...
	readonly decisionRef: "graphrefly-ts:D1";
	readonly topology: Readonly<{
		factNode: "current/facts";
		projectionNode: "current/projections";
...
function createCurrentFactNode(owner: ReturnType<typeof graph>) {
	return owner.node<CurrentGraphAdmittedFactV1>([], null, { name: "current/facts" });
}
...
function hasCorrectionHeadroom(state: AuthorityState, kind: "replacement" | "semantic"): boolean {
	...
	const localEffects = kind === "replacement" ? 7 : 6;
...
function schedule(
	...
	const base = strictSnapshot({
		schemaVersion: "graphrefly-ts.d1.graph-effect-request.v1" as const,
		sequence,
		arm: currentArm(state),
		runSequence: state.run.runSequence,
		effectKind,
	...
	const admissionMaterial = strictSnapshot({
		schemaVersion: "graphrefly-ts.d1.graph-effect-admission.v1" as const,
...
function exactToolOrder(phase: CurrentGraphPhase, tools: readonly CurrentGraphToolRef[]): boolean {
	if (phase === "none") return tools.length === 1 && tools[0] === "read-file";
	if (phase === "inspection")
		return (
			tools.length === 3 &&
...
function validateResult(
	...
	if (request.effectKind === "materialization") {
		...
		if (tools.length > 4) throw new TypeError("current provider tool intent bound exceeded");
...
export function createCurrentGraphNativeEvalAuthority(inputValue: {
	...
	const owner = graph({ name: "current/graph-native-eval" });
	...
	const projectionNode = owner.node<CurrentGraphAdmittedFactV1>(
		...
		{ name: "current/projections", factory: "currentGraphCanonicalProjection" },
	);
...
function evidenceMaterial(state: AuthorityState) {
	const topologyMaterial = strictSnapshot({
		factNode: "current/facts" as const,
		projectionNode: "current/projections" as const,
...
export function validateCurrentGraphNativeEvidence(value: unknown): CurrentGraphNativeEvidenceV1 {
	...
	if (
		...
		candidate.decisionRef !== "graphrefly-ts:D1" ||
