...
import {
	...
	createD12CurrentGraphOpenRouterExecutor as createD14CurrentGraphOpenRouterExecutor,
...
import { D14_IMPLEMENTATION_MANIFEST_DIGEST } from "./d14-current-implementation-manifest.js";
import {
	type D14CurrentGraphLiveBundleV1,
	runD14CurrentGraphLiveMeasurement,
	validateD14CurrentGraphLiveBundle,
} from "./d14-current-live.js";
import {
	acquireD14CurrentGraphLiveDispatchClaimAtRootForTest,
	consumeD14CurrentGraphLiveDispatchClaim,
} from "./d14-current-live-claim.js";
import {
	D14_CURRENT_GRAPH_LIVE_DECISION_REF,
	D14_CURRENT_GRAPH_LIVE_PROVIDER_NAME,
	D14_CURRENT_GRAPH_LIVE_PROVIDER_TAG,
	D14_CURRENT_GRAPH_LIVE_QUALIFICATION_GENERATION_REF,
	D14_CURRENT_GRAPH_LIVE_QUANTIZATION,
	D14_CURRENT_GRAPH_LIVE_READABLE_FILES,
	D14_CURRENT_GRAPH_LIVE_REQUEST_MODEL,
	D14_CURRENT_GRAPH_LIVE_SELECTED_ENDPOINT_MODEL,
...
export const D14_QUALIFICATION_SCHEMA =
	"graphrefly-ts.d14.current-live-no-network-qualification.v1" as const;
export const D14_QUALIFICATION_BUNDLE_SCHEMA =
	"graphrefly-ts.d14.current-live-no-network-bundle.v1" as const;
export const D14_QUALIFICATION_GENERATION_SCHEMA =
	"graphrefly-ts.d14.current-live-no-network-generation.v1" as const;
export const D14_QUALIFICATION_PERSISTENCE_SCHEMA =
	"graphrefly-ts.d14.current-live-no-network-persistence.v1" as const;
export const D14_INJECTED_TEST_GENERATION_REF =
	"current-graph-native-live-no-network-injected-test-d14-v1" as const;
export const D14_MAX_QUALIFICATION_BYTES = 4_194_304;

export interface D14D13BaselineAdmissionV1 {
	readonly revision: "graphrefly-ts.d14.d13-baseline-admission.v1";
}

export interface D14QualificationBundleV1 {
	readonly schemaVersion: typeof D14_QUALIFICATION_BUNDLE_SCHEMA;
	readonly qualification: Readonly<{
		schemaVersion: typeof D14_QUALIFICATION_SCHEMA;
		decisionRef: typeof D14_CURRENT_GRAPH_LIVE_DECISION_REF;
		implementationManifestDigest: string;
		baselineBasis: "exact-d13-artifact" | "injected-test";
		d13QualificationArtifactDigest: typeof D14_D13_QUALIFICATION_ARTIFACT_DIGEST;
		d13QualificationBundleDigest: typeof D14_D13_QUALIFICATION_BUNDLE_DIGEST;
		d13QualificationDigest: typeof D14_D13_QUALIFICATION_DIGEST;
	...
	readonly graphBundle: D14CurrentGraphLiveBundleV1;
	readonly generation: Readonly<{
		schemaVersion: typeof D14_QUALIFICATION_GENERATION_SCHEMA;
		generationRef: typeof D14_CURRENT_GRAPH_LIVE_QUALIFICATION_GENERATION_REF;
		qualificationDigest: string;
		graphBundleDigest: string;
		implementationManifestDigest: string;
		causalAttribution: "undetermined";
...
const baselineStates = new WeakMap<object, "exact-d13-artifact" | "injected-test">();
const constructedBundles = new WeakSet<object>();

function baselineCapability(
	basis: "exact-d13-artifact" | "injected-test",
): D14D13BaselineAdmissionV1 {
	const capability = Object.freeze({
		revision: "graphrefly-ts.d14.d13-baseline-admission.v1" as const,
	});
	baselineStates.set(capability, basis);
	return capability;
}

export function admitD14D13QualificationBaseline(
	bytesValue: Uint8Array,
): D14D13BaselineAdmissionV1 {
	if (!(bytesValue instanceof Uint8Array))
		throw new TypeError("D14 D13 baseline bytes are invalid");
	const bytes = new Uint8Array(bytesValue);
	if (empiricalSha256(bytes) !== D14_D13_QUALIFICATION_ARTIFACT_DIGEST)
		throw new TypeError("D14 D13 qualification artifact drifted");
	const bundle = validateD13QualificationBundle(strictJsonCodec.decode(bytes));
	if (
		bundle.bundleDigest !== D14_D13_QUALIFICATION_BUNDLE_DIGEST ||
		bundle.qualification.qualificationDigest !== D14_D13_QUALIFICATION_DIGEST ||
		bundle.qualification.implementationManifestDigest !== D14_D13_IMPLEMENTATION_MANIFEST_DIGEST ||
		bundle.qualification.baselineBasis !== "exact-d12-artifact" ||
		bundle.qualification.efficacyClaim !== "none" ||
		bundle.generation.generationDigest !== D14_D13_QUALIFICATION_GENERATION_DIGEST
	)
		throw new TypeError("D14 D13 qualification projection drifted");
	return baselineCapability("exact-d13-artifact");
}

export function createD14InjectedD13BaselineForTest(): D14D13BaselineAdmissionV1 {
...
function officialPricingResponse(): Response {
	const response = new Response(
		JSON.stringify({
			data: {
				id: D14_CURRENT_GRAPH_LIVE_REQUEST_MODEL,
				endpoints: [
					{
						provider_name: D14_CURRENT_GRAPH_LIVE_PROVIDER_NAME,
						tag: D14_CURRENT_GRAPH_LIVE_PROVIDER_TAG,
						quantization: D14_CURRENT_GRAPH_LIVE_QUANTIZATION,
						model: D14_CURRENT_GRAPH_LIVE_SELECTED_ENDPOINT_MODEL,
...
export async function runD14InjectedNoNetworkQualification(inputValue: {
	readonly repositoryRoot: string;
	readonly baseline: D14D13BaselineAdmissionV1;
	readonly implementationManifestDigest: string;
}): Promise<D14QualificationBundleV1> {
	const input = record(inputValue, "D14 qualification input");
	exactKeys(
		input,
		["baseline", "implementationManifestDigest", "repositoryRoot"],
		"D14 qualification input",
	);
	const baseline = input.baseline as D14D13BaselineAdmissionV1;
	const baselineBasis =
		typeof baseline === "object" && baseline !== null ? baselineStates.get(baseline) : undefined;
	if (baselineBasis === undefined) throw new TypeError("D14 D13 baseline is forged or replayed");
	baselineStates.delete(baseline);
	const implementationManifestDigest = digest(
		input.implementationManifestDigest,
		"D14 implementation manifest",
	);
	if (implementationManifestDigest !== D14_IMPLEMENTATION_MANIFEST_DIGEST)
		throw new TypeError("D14 qualification implementation manifest drifted");
	const repositoryRoot = await realpath(resolve(String(input.repositoryRoot)));
	const temporaryRoot = await mkdtemp(join(tmpdir(), "graphrefly-d14-no-network-"));
	await chmod(temporaryRoot, 0o700);
	try {
		const credential = Object.freeze({
			bearerToken: "sk-or-v1-test-current-graph-live-key-d14",
		...
		const zeroByokBytes = Buffer.from(
			JSON.stringify({
				...
				allowedModels: [D14_CURRENT_GRAPH_LIVE_REQUEST_MODEL],
				allowedProviders: [D14_CURRENT_GRAPH_LIVE_PROVIDER_NAME],
		...
		const claim = await acquireD14CurrentGraphLiveDispatchClaimAtRootForTest(
			await realpath(privateRoot),
			{
				preclaim,
				implementationManifestDigest,
				qualificationArtifactDigest: empiricalStrictJsonDigest({ injected: "d14" }),
				qualificationDigest: empiricalStrictJsonDigest({ injected: "d14-qualification" }),
			},
		);
			...
			try {
				...
				if (pendingRetryBodyDigest !== null) {
					if (pendingRetryBodyDigest !== currentBodyDigest)
						throw new TypeError("D14 retry request bytes drifted");
				...
				const calls = hasReadResult
					...
					: D14_CURRENT_GRAPH_LIVE_READABLE_FILES.map((path, index) => ({
		...
		const graphBundle = validateD14CurrentGraphLiveBundle(
			await runD14CurrentGraphLiveMeasurement({
				executionAuthority,
		...
		if (graphBundle.disposition !== "success" || graphBundle.graphEvidence === null)
			throw new TypeError("D14 no-network Graph integration did not complete");
		...
		if (
			...
			throw new TypeError("D14 no-network Graph transport projection drifted");
		await lstat(materializationRoot).then(
			() => {
				throw new TypeError("D14 no-network left workspace residue");
			},
			(error: unknown) => {
		...
		if (!source.includes(CURRENT_GRAPH_LIVE_FIXED_ADMISSION_BLOCK))
			throw new TypeError("D14 no-network mutated the source workspace");
		const qualificationMaterial = strictSnapshot({
			schemaVersion: D14_QUALIFICATION_SCHEMA,
		...
		const generationMaterial = strictSnapshot({
			schemaVersion: D14_QUALIFICATION_GENERATION_SCHEMA,
			generationRef: D14_CURRENT_GRAPH_LIVE_QUALIFICATION_GENERATION_REF,
		...
		const bundleMaterial = strictSnapshot({
			schemaVersion: D14_QUALIFICATION_BUNDLE_SCHEMA,
			qualification,
			graphBundle,
			generation,
		});
		...
		}) as D14QualificationBundleV1;
		if (
			strictJsonCodec.encode(bundle as unknown as StrictJsonValue).byteLength >
			D14_MAX_QUALIFICATION_BYTES
		)
			throw new TypeError("D14 qualification exceeded its byte bound");
...
export function validateD14QualificationBundle(value: unknown): D14QualificationBundleV1 {
	const candidate = record(value, "D14 qualification bundle");
	exactKeys(
		candidate,
		["bundleDigest", "generation", "graphBundle", "qualification", "schemaVersion"],
		"D14 qualification bundle",
	);
	if (candidate.schemaVersion !== D14_QUALIFICATION_BUNDLE_SCHEMA)
		throw new TypeError("D14 qualification bundle schema drifted");
	const graphBundle = validateD14CurrentGraphLiveBundle(candidate.graphBundle);
	if (graphBundle.executionClass !== "injected-no-network" || graphBundle.disposition !== "success")
		throw new TypeError("D14 qualification Graph bundle drifted");
	const graphEvidence = graphBundle.graphEvidence;
	if (graphEvidence === null) throw new TypeError("D14 qualification Graph evidence is missing");
	const graph = graphEvidence.d9Evidence.providerEvidence;
	const runs = graph.workflowEvidence.runs;
	const qualification = record(candidate.qualification, "D14 qualification");
	exactKeys(
		qualification,
		[
			"baselineBasis",
			...
			"d13QualificationArtifactDigest",
			"d13QualificationBundleDigest",
			"d13QualificationDigest",
			"decisionRef",
			"efficacyClaim",
	...
	if (
		qualification.schemaVersion !== D14_QUALIFICATION_SCHEMA ||
		qualification.decisionRef !== D14_CURRENT_GRAPH_LIVE_DECISION_REF ||
		qualification.implementationManifestDigest !== D14_IMPLEMENTATION_MANIFEST_DIGEST ||
		(qualification.baselineBasis !== "exact-d13-artifact" &&
			qualification.baselineBasis !== "injected-test") ||
		qualification.d13QualificationArtifactDigest !== D14_D13_QUALIFICATION_ARTIFACT_DIGEST ||
		qualification.d13QualificationBundleDigest !== D14_D13_QUALIFICATION_BUNDLE_DIGEST ||
		qualification.d13QualificationDigest !== D14_D13_QUALIFICATION_DIGEST ||
		...
		throw new TypeError("D14 qualification projection drifted");
	const generation = record(candidate.generation, "D14 qualification generation");
	exactKeys(
		generation,
		[
			"causalAttribution",
		...
		"D14 qualification generation",
	);
	const { generationDigest, ...generationMaterial } = generation;
	if (
		generation.schemaVersion !== D14_QUALIFICATION_GENERATION_SCHEMA ||
		generation.generationRef !== D14_CURRENT_GRAPH_LIVE_QUALIFICATION_GENERATION_REF ||
		...
		throw new TypeError("D14 qualification generation drifted");
	const { bundleDigest, ...bundleMaterial } = candidate;
	if (bundleDigest !== empiricalStrictJsonDigest(bundleMaterial))
		throw new TypeError("D14 qualification bundle digest drifted");
	return strictSnapshot(candidate) as unknown as D14QualificationBundleV1;
}

async function persistQualification(inputValue: {
	readonly privateRoot: string;
	readonly bundle: D14QualificationBundleV1;
	readonly expectedBasis: D14QualificationBundleV1["qualification"]["baselineBasis"];
	readonly generationRef: string;
}) {
	const input = record(inputValue, "D14 qualification persistence input");
	exactKeys(
		input,
		["bundle", "expectedBasis", "generationRef", "privateRoot"],
		"D14 qualification persistence input",
	);
	if (
		...
		throw new TypeError("D14 qualification persistence requires a fresh constructed bundle");
	const bundle = validateD14QualificationBundle(input.bundle);
	if (bundle.qualification.baselineBasis !== input.expectedBasis)
		throw new TypeError("D14 qualification persistence basis drifted");
	...
	const commitMaterial = strictSnapshot({
		schemaVersion: "graphrefly-ts.d14.current-live-no-network-commit.v1",
	...
	const receiptMaterial = strictSnapshot({
		schemaVersion: D14_QUALIFICATION_PERSISTENCE_SCHEMA,
		generationRef: String(input.generationRef),
		bundleArtifactDigest: empiricalSha256(bundleBytes),
		bundleDigest: bundle.bundleDigest,
		qualificationDigest: bundle.qualification.qualificationDigest,
...
export function persistD14QualificationBundle(input: {
	readonly privateRoot: string;
	readonly bundle: D14QualificationBundleV1;
}) {
	return persistQualification({
		...input,
		expectedBasis: "exact-d13-artifact",
		generationRef: D14_CURRENT_GRAPH_LIVE_QUALIFICATION_GENERATION_REF,
	});
}

export function persistD14InjectedQualificationForTest(input: {
	readonly privateRoot: string;
	readonly bundle: D14QualificationBundleV1;
}) {
	return persistQualification({
		...
		generationRef: D14_INJECTED_TEST_GENERATION_REF,
	});
}
