...
export interface D711OrderedLiveAdmissionSteps<
	Pricing,
	Credential,
	ZeroByok,
	Claim,
...
function ownFunction(value: Record<string, unknown>, key: string): (...args: never[]) => unknown {
	...
	if (
		...
		throw new TypeError(`D711 ordered live admission ${key} must be an own function`);
	}
	return descriptor.value as (...args: never[]) => unknown;
}

export async function runD711OrderedLiveAdmissions<
	Pricing,
	Credential,
	ZeroByok,
	Claim,
	...
	ProviderResult,
>(
	input: D711OrderedLiveAdmissionSteps<
		Pricing,
		Credential,
		ZeroByok,
		Claim,
	...
	const candidate = record(input, "d711.orderedLiveAdmissions");
	exactKeys(
		candidate,
		[
		...
		"d711.orderedLiveAdmissions",
	);
	const readFreshPricing = ownFunction(candidate, "readFreshPricing") as () => AwaitedStep<Pricing>;
	const loadCredential = ownFunction(candidate, "loadCredential") as D711OrderedLiveAdmissionSteps<
		Pricing,
		Credential,
		ZeroByok,
		Claim,
	...
	const readFreshZeroByok = ownFunction(
		candidate,
		"readFreshZeroByok",
	) as D711OrderedLiveAdmissionSteps<
		Pricing,
		Credential,
		ZeroByok,
		Claim,
	...
	const acquireDispatchClaim = ownFunction(
		...
		ProviderResult
	>["acquireDispatchClaim"];
	const readCurrentKey = ownFunction(candidate, "readCurrentKey") as D711OrderedLiveAdmissionSteps<
		Pricing,
		Credential,
		ZeroByok,
		Claim,
