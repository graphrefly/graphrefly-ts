...
export interface D719CallerArmResultV1 {
	...
	readonly execution: {
		...
		readonly armLocalStoppedReason?:
			| "arm-policy-violated"
			| "arm-provider-turn-bound-exhausted"
			| "provider-retry-exhausted";
		readonly retryExhaustionEvidenceDigest?: string;
...
function decisionFor(fact: D719AdmittedArmFactV1, limits: D719CleanBudgetLimitsV1) {
	...
	const armLocalStop =
		...
		stoppedReason === "provider-retry-exhausted";
...
function findingFor(decision: D719ArmDecisionProjectionV1): D719HarnessFindingV1 {
	...
	else if (decision.stoppedReason === "provider-retry-exhausted") code = "provider-retry-exhausted";
...
function validateCallerResult(value: unknown): D719CallerArmResultV1 {
	...
	exactKeys(
		...
		[
			...
			...(Object.hasOwn(execution, "armLocalStoppedReason")
				...
				: []),
			...(Object.hasOwn(execution, "retryExhaustionEvidenceDigest")
				? ["retryExhaustionEvidenceDigest" as const]
				: []),
		],
		"d719.clean.execution",
	);
	for (const key of Object.keys(execution).filter(
		(key) => key !== "armLocalStoppedReason" && key !== "retryExhaustionEvidenceDigest",
	)) {
	...
	if (Object.hasOwn(execution, "armLocalStoppedReason"))
		oneOf(
			...
			["arm-policy-violated", "arm-provider-turn-bound-exhausted", "provider-retry-exhausted"],
			"d719.clean.execution.armLocalStoppedReason",
		);
	if (Object.hasOwn(execution, "retryExhaustionEvidenceDigest"))
		digest(
			execution.retryExhaustionEvidenceDigest,
			"d719.clean.execution.retryExhaustionEvidenceDigest",
		);
	if (
		(execution.armLocalStoppedReason === "provider-retry-exhausted") !==
		Object.hasOwn(execution, "retryExhaustionEvidenceDigest")
	)
		throw new TypeError("D767 retry exhaustion reason and evidence must be exact-bound");
	if (
		Object.hasOwn(execution, "terminalProviderFailure") &&
...
export function snapshotD719CleanGraphEvidence(
	...
	const finalArmLocalStop =
		...
		last?.stoppedReason === "provider-retry-exhausted";
	const complete =
