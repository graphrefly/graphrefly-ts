...
import { createD774CallerExecutor, runD774GraphNativeEvalCore } from "./d774-graph-native-eval.js";
import {
...
import {
	...
	invokeD774AdmittedRouteTurn,
	validateD774RouteEvidence,
} from "./d774-provider-result-route-authority.js";

export const D774_QUALIFICATION_SCHEMA =
	"graphrefly.b112.d774.provider-envelope-phase-lowering-qualification.v2" as const;
export const D774_GENERATION_SCHEMA =
	"graphrefly.b112.d774.provider-envelope-phase-lowering-generation.v2" as const;
export const D774_BUNDLE_SCHEMA =
	"graphrefly.b112.d774.provider-envelope-phase-lowering-bundle.v2" as const;
export const D774_PERSISTENCE_SCHEMA =
	"graphrefly.b112.d774.provider-envelope-phase-lowering-persistence.v2" as const;
export const D774_GENERATION_REF = "d774-provider-envelope-phase-lowering-no-network-v2" as const;
...
const CEILINGS = Object.freeze({
	routeDigest: empiricalStrictJsonDigest({ d774: "injected-no-network-route" }),
...
const SOURCE_DIGEST = empiricalStrictJsonDigest({
	...
	baseline: "consumed-d773-partial-canonical-graph-evidence",
	executionClass: "simulated-contract",
});
const encoder = new TextEncoder();
const decoder = new TextDecoder("utf-8", { fatal: true });
const D773_BASELINE_ARTIFACT_SHA256 =
	"sha256:30cb7cf0e45eac32f10fb0dfad0f4e20589096d79b95d3c42549a730d67966ba" as const;
const D773_BASELINE_BUNDLE_DIGEST =
	"sha256:11f34343932a79df0d9b86d5f04418a8b09971036ac5e7b45b297d4ffef566a7" as const;
const D773_BASELINE_GRAPH_EVIDENCE_DIGEST =
	"sha256:5a62d13b0c6557b129886bb38ffe53ad10b3e24351bf2c1ed1b630436aaa5db4" as const;
const D773_BASELINE_ROUTE_EVIDENCE_DIGEST =
	"sha256:4471276fd5afb58a64002162911f5e5403049885e202244b1b039d2b1a7d5ac4" as const;

export interface D774QualificationBundleV1 {
	readonly schemaVersion: typeof D774_BUNDLE_SCHEMA;
	readonly executionClass: "simulated-contract";
	readonly graphEvidence: D771CanonicalGraphEvidenceV1;
	readonly routeEvidence: ReturnType<typeof validateD774RouteEvidence>;
	readonly wrongToolGraphEvidence: D771CanonicalGraphEvidenceV1;
	readonly wrongToolRouteEvidence: ReturnType<typeof validateD774RouteEvidence>;
	readonly omissionGraphEvidence: D771CanonicalGraphEvidenceV1;
	readonly omissionRouteEvidence: ReturnType<typeof validateD774RouteEvidence>;
	readonly terminalHttpGraphEvidence: D771CanonicalGraphEvidenceV1;
	readonly terminalHttpRouteEvidence: ReturnType<typeof validateD774RouteEvidence>;
	readonly hiddenFailureGraphEvidence: D771CanonicalGraphEvidenceV1;
	readonly hiddenFailureRouteEvidence: ReturnType<typeof validateD774RouteEvidence>;
...
export interface D774ConsumedD773BaselineV1 {
	readonly schemaVersion: "graphrefly.b112.d774.consumed-d773-baseline.v1";
	readonly basis: "real-artifact" | "injected-test";
	readonly artifactSha256: typeof D773_BASELINE_ARTIFACT_SHA256;
	readonly bundleDigest: typeof D773_BASELINE_BUNDLE_DIGEST;
	readonly graphEvidenceDigest: typeof D773_BASELINE_GRAPH_EVIDENCE_DIGEST;
	readonly routeEvidenceDigest: typeof D773_BASELINE_ROUTE_EVIDENCE_DIGEST;
	readonly disposition: "partial-failure";
	readonly receiptDigest: string;
}

export interface D774PersistenceReceiptV1 {
	readonly schemaVersion: typeof D774_PERSISTENCE_SCHEMA;
	readonly generationRef: typeof D774_GENERATION_REF;
...
function baselineMaterial(basis: "real-artifact" | "injected-test") {
	return strictSnapshot({
		schemaVersion: "graphrefly.b112.d774.consumed-d773-baseline.v1" as const,
		basis,
		artifactSha256: D773_BASELINE_ARTIFACT_SHA256,
		bundleDigest: D773_BASELINE_BUNDLE_DIGEST,
		graphEvidenceDigest: D773_BASELINE_GRAPH_EVIDENCE_DIGEST,
		routeEvidenceDigest: D773_BASELINE_ROUTE_EVIDENCE_DIGEST,
		disposition: "partial-failure" as const,
	});
}

function validateBaseline(value: unknown): D774ConsumedD773BaselineV1 {
	const candidate = record(value, "d774.baseline");
	exactKeys(
		candidate,
		[
			"artifactSha256",
	...
	if (candidate.basis !== "real-artifact" && candidate.basis !== "injected-test")
		throw new TypeError("D774 baseline basis drifted");
	...
	if (candidate.receiptDigest !== empiricalStrictJsonDigest(material))
		throw new TypeError("D774 baseline receipt digest drifted");
	return strictSnapshot(candidate) as unknown as D774ConsumedD773BaselineV1;
}

export function admitD774ConsumedD773Baseline(bytes: Uint8Array): D774ConsumedD773BaselineV1 {
	if (!(bytes instanceof Uint8Array) || bytes.byteLength < 1 || bytes.byteLength > 16 * 1_048_576)
		throw new TypeError("D774 D773 baseline bytes are outside the bound");
	literal(empiricalSha256(bytes), D773_BASELINE_ARTIFACT_SHA256, "d774.baseline.sha256");
	const bundle = record(strictJsonCodec.decode(bytes), "d774.baseline.bundle");
	literal(bundle.disposition, "partial-failure", "d774.baseline.bundle.disposition");
	literal(bundle.bundleDigest, D773_BASELINE_BUNDLE_DIGEST, "d774.baseline.bundle.digest");
	const graphEvidence = record(bundle.graphEvidence, "d774.baseline.bundle.graphEvidence");
	const routeEvidence = record(bundle.routeEvidence, "d774.baseline.bundle.routeEvidence");
	literal(
		graphEvidence.evidenceDigest,
		D773_BASELINE_GRAPH_EVIDENCE_DIGEST,
		"d774.baseline.graph.digest",
	);
	literal(
		routeEvidence.evidenceDigest,
		D773_BASELINE_ROUTE_EVIDENCE_DIGEST,
		"d774.baseline.route.digest",
	);
...
export function createD774InjectedBaselineForTest(): D774ConsumedD773BaselineV1 {
...
function routeAdmission() {
	...
	return createD733GraphNativeRouteAdmission({
		...
		access: createD733RouteAccessProjection({
			profile,
			observationRevision: "d774.injected-no-network.v1",
...
async function runInjectedSixArms(
	...
	const executor = createD774CallerExecutor(async ({ admission, effectRequest }) => {
		active += 1;
		maxActive = Math.max(maxActive, active);
		if (active !== 1) throw new TypeError("D774 observed parallel effects");
		try {
				...
				return createD774ProviderResultEnvelope({
					...
					execution: {
						...
						result: {
							...
							evidenceDigest: empiricalStrictJsonDigest({ d774: "materialized", effectRequest }),
						},
					},
				});
			}
			...
			if (effectRequest.effectKind === "provider-request") {
				if (workspace === undefined) throw new TypeError("D774 provider workspace is missing");
				...
				const envelope = await invokeD774AdmittedRouteTurn({
					effectRequest: effectRequest as never,
					admission,
					credential: {
						bearerToken: "not-a-live-d774-credential",
						credentialBindingRef: "d774.injected-no-network",
						credentialBindingRevision: "v1",
					},
					taskStatement: "D774 bounded injected repository repair",
					...
					transport: {
						async request(request) {
							...
							if (priorBody === undefined)
								...
								throw new TypeError("D774 same-request retry wire body drifted");
							if (injectRetry)
								return {
							...
							const body = record(JSON.parse(decoder.decode(request.body)), "d774.injected.body");
							const choice = body.tool_choice;
							let toolName: string | null = null;
							if (choice !== "none") {
								if (choice === "required" || choice === "auto") toolName = "read_file";
								else
									toolName = record(record(choice, "d774.choice").function, "d774.choice.fn")
										.name as string;
							}
							const injectWrongTool =
								mode === "wrong-correction-tool" &&
								effectRequest.runSequence === 0 &&
								toolName === "replace_exact" &&
								!negativeInjected;
							if (injectWrongTool) {
								negativeInjected = true;
								toolName = "read_file";
							...
							const calls =
								toolName === null
									? []
									: toolName === "read_file" && effectRequest.completionContext === undefined
				...
				return createD774ProviderResultEnvelope({
					...
					execution: {
						...
						result: {
							...
							status: "completed",
							evidenceDigest: empiricalStrictJsonDigest({ d774: "retry-wait", effectRequest }),
						},
					},
				});
			}
			if (effectRequest.effectKind === "tool-action") {
				if (workspace === undefined || effectRequest.toolIntent === null)
					throw new TypeError("D774 tool workspace is missing");
				const after =
				...
				return createD774ProviderResultEnvelope({
					...
					execution: {
						...
						result: {
							...
							evidenceDigest: empiricalStrictJsonDigest({ d774: "tool", effectRequest }),
						},
					},
				});
			}
			if (effectRequest.effectKind === "public-semantic-validation") {
				if (workspace === undefined) throw new TypeError("D774 semantic workspace is missing");
			...
			if (effectRequest.effectKind === "hidden-verifier") {
				if (workspace === undefined) throw new TypeError("D774 hidden workspace is missing");
				...
				return createD774ProviderResultEnvelope({
					...
					execution: {
						...
						result: {
							...
							workspaceStateDigest: workspace,
							evidenceDigest: empiricalStrictJsonDigest({ d774: "hidden", effectRequest }),
						},
					},
				});
			}
			...
			return createD774ProviderResultEnvelope({
				...
				execution: {
					...
					result: {
						effectKind: "cleanup",
						status: "succeeded",
						evidenceDigest: empiricalStrictJsonDigest({ d774: "cleanup", effectRequest }),
					},
				},
			});
		} finally {
	...
	if (
		workspaces.size !== 0 ||
		maxActive !== 1 ||
		(mode === "positive" && (retried.size !== 3 || retryWaits !== 3)) ||
		...
		throw new TypeError(
			`D774 injected six-arm operational qualification drifted: ${JSON.stringify({
...
function assertFailureProvenanceSeparated(
	...
	if (expected === "http-terminal") {
		...
		if (
			...
			throw new TypeError("D774 terminal HTTP provenance drifted");
		return;
	}
	if (
		!results.some(
		...
		throw new TypeError("D774 hidden-verifier failure provenance drifted");
}

function assertLifecycle(graph: D771CanonicalGraphEvidenceV1): void {
	if (graph.effectRuns.length !== 6) throw new TypeError("D774 qualification requires six runs");
	for (const [index, run] of graph.effectRuns.entries()) {
		...
		if (
			...
			throw new TypeError(`D774 run ${index} lifecycle drifted`);
	}
}
...
function assertRetryCoverage(graph: D771CanonicalGraphEvidenceV1): void {
	...
	if (JSON.stringify(deriveRetryCoverage(graph)) !== JSON.stringify(expected))
		throw new TypeError(
			`D774 retry policy coverage drifted: ${JSON.stringify(deriveRetryCoverage(graph))}`,
		);
}

function assertWrongToolRejected(graph: D771CanonicalGraphEvidenceV1): void {
	const run = graph.effectRuns[0];
	if (run === undefined) throw new TypeError("D774 wrong-tool run is missing");
	const facts = run.facts.flatMap((fact) =>
		fact.kind === "graph-effect-result-admitted" ? [fact] : [],
	);
	const semanticFailureIndex = facts.findIndex(
		(fact) =>
			fact.result.effectKind === "public-semantic-validation" && fact.result.status === "failed",
	);
	const cleanupIndex = facts.findIndex(
		(fact, index) => index > semanticFailureIndex && fact.result.effectKind === "cleanup",
	);
	const postFailureTools = facts
		.slice(semanticFailureIndex + 1, cleanupIndex)
		.filter((fact) => fact.result.effectKind === "tool-action");
	if (
		semanticFailureIndex < 0 ||
		cleanupIndex < 0 ||
		postFailureTools.length !== 0 ||
		!graph.ledger.findings.some(
			(finding) => finding.code === "arm-policy-violated" && finding.runSequence === 0,
		)
	)
		throw new TypeError("D774 wrong correction tool was not rejected before side effects");
}
...
function assertProposalOmissionSeparated(graph: D771CanonicalGraphEvidenceV1): void {
	...
	if (
		...
		throw new TypeError("D774 proposal omission did not remain executor-failure provenance");
}

export async function runD774InjectedNoNetworkQualification(
	baselineValue: D774ConsumedD773BaselineV1,
): Promise<D774QualificationBundleV1> {
	const baseline = validateBaseline(baselineValue);
	if ((await measureD774Implementation()) !== D774_IMPLEMENTATION_MANIFEST_DIGEST)
		throw new TypeError("D774 implementation manifest validation failed");
	const run = await runInjectedSixArms();
	const wrongTool = await runInjectedSixArms("wrong-correction-tool");
	...
	for (const phase of [
		...
		if (!routePhases.includes(phase)) throw new TypeError(`D774 phase ${phase} was not qualified`);
	const qualificationMaterial = strictSnapshot({
		schemaVersion: D774_QUALIFICATION_SCHEMA,
		decisionRef: "decision.D774.2026-08-13.v1",
		baselineDisposition: "consumed-d773-partial-canonical-graph-evidence",
		...
		implementationManifestDigest: D774_IMPLEMENTATION_MANIFEST_DIGEST,
	...
	const generationMaterial = strictSnapshot({
		schemaVersion: D774_GENERATION_SCHEMA,
		generationRef: D774_GENERATION_REF,
	...
	const material = strictSnapshot({
		schemaVersion: D774_BUNDLE_SCHEMA,
...
export function validateD774QualificationBundle(value: unknown): D774QualificationBundleV1 {
	const candidate = record(value, "d774.bundle");
	exactKeys(
		candidate,
		[
			"bundleDigest",
	...
	literal(candidate.schemaVersion, D774_BUNDLE_SCHEMA, "d774.bundle.schemaVersion");
	literal(candidate.executionClass, "simulated-contract", "d774.bundle.executionClass");
	literal(candidate.causalAttribution, "undetermined", "d774.bundle.causalAttribution");
	literal(candidate.efficacyClaim, "none", "d774.bundle.efficacyClaim");
	const graphCandidate = record(candidate.graphEvidence, "d774.bundle.graphEvidence");
	...
	if (graph.evidenceDigest !== graphCandidate.evidenceDigest)
		throw new TypeError("D774 canonical Graph replay drifted");
	...
	const wrongToolGraph = replayNegative(
		...
		"d774.bundle.wrongToolGraphEvidence",
	);
	...
	const terminalHttpGraph = replayNegative(
		...
		"d774.bundle.terminalHttpGraphEvidence",
	);
	...
	const hiddenFailureGraph = replayNegative(
		...
		"d774.bundle.hiddenFailureGraphEvidence",
	);
	...
	const omissionGraph = replayNegative(
		...
		"d774.bundle.omissionGraphEvidence",
	);
	...
	const qualification = record(candidate.qualification, "d774.bundle.qualification");
	exactKeys(
		qualification,
		[
	...
	if (
		...
		qualification.baselineArtifactSha256 !== D773_BASELINE_ARTIFACT_SHA256 ||
		qualification.baselineBundleDigest !== D773_BASELINE_BUNDLE_DIGEST ||
		qualification.baselineGraphEvidenceDigest !== D773_BASELINE_GRAPH_EVIDENCE_DIGEST ||
		qualification.baselineRouteEvidenceDigest !== D773_BASELINE_ROUTE_EVIDENCE_DIGEST ||
		qualification.graphEvidenceDigest !== graph.evidenceDigest ||
		qualification.implementationManifestDigest !== D774_IMPLEMENTATION_MANIFEST_DIGEST ||
		...
		throw new TypeError("D774 qualification projection drifted");
	...
	if (qualificationDigest !== empiricalStrictJsonDigest(qualificationMaterial))
		throw new TypeError("D774 qualification digest drifted");
	const generation = record(candidate.generation, "d774.bundle.generation");
	exactKeys(
		generation,
		[
			"causalAttribution",
		...
		"d774.bundle.generation",
	);
	if (
		generation.generationRef !== D774_GENERATION_REF ||
		...
		throw new TypeError("D774 generation binding drifted");
	const { generationDigest, ...generationMaterial } = generation;
	if (generationDigest !== empiricalStrictJsonDigest(generationMaterial))
		throw new TypeError("D774 generation digest drifted");
	const { bundleDigest, ...bundleMaterial } = candidate;
	if (bundleDigest !== empiricalStrictJsonDigest(bundleMaterial))
		throw new TypeError("D774 bundle digest drifted");
	return strictSnapshot(candidate) as unknown as D774QualificationBundleV1;
}
...
async function assertPrivateRoot(path: string) {
	...
	if (
		...
		throw new TypeError("D774 private root must be a canonical 0700 directory");
	return canonical;
}
...
async function writePrivateFile(path: string, bytes: Uint8Array): Promise<void> {
	...
	try {
		...
		if (!stat.isFile() || stat.nlink !== 1 || (stat.mode & 0o777) !== 0o600)
			throw new TypeError("D774 private artifact identity drifted");
	} finally {
		await handle.close();
	}
}
...
async function persistBundle(
	inputValue: {
		...
		readonly bundle: D774QualificationBundleV1;
	},
	options: {
		readonly allowInjected: boolean;
		readonly faultStage?: "after-claim" | "after-write" | "after-rename" | "after-marker";
	},
): Promise<D774PersistenceReceiptV1> {
	const input = record(inputValue, "d774.persist");
	exactKeys(input, ["bundle", "privateRoot"], "d774.persist");
	const bundle = validateD774QualificationBundle(input.bundle);
	const baselineBasis = record(bundle.qualification, "d774.persist.qualification").baselineBasis as
		| "real-artifact"
		| "injected-test";
	if (!options.allowInjected && baselineBasis !== "real-artifact")
		throw new TypeError("D774 production persistence rejects injected baseline evidence");
	const root = await assertPrivateRoot(input.privateRoot as string);
	const finalRoot = join(root, D774_GENERATION_REF);
	const stagingRoot = join(root, `.d774-${randomUUID()}.tmp`);
	...
	try {
		...
		if (options.faultStage === "after-claim")
			throw new TypeError("D774 injected after-claim fault");
		...
		if (options.faultStage === "after-write")
			throw new TypeError("D774 injected after-write fault");
		await rename(stagingRoot, join(finalRoot, "artifacts"));
		stagingIdentity = null;
		if (options.faultStage === "after-rename")
			throw new TypeError("D774 injected after-rename fault");
		...
		if (options.faultStage === "after-marker")
			throw new TypeError("D774 injected after-marker fault");
		...
		try {
			...
			if (!sameBytes(bytes, bundleBytes)) throw new TypeError("D774 final bundle readback drifted");
		} finally {
		...
		try {
			...
			if (!sameBytes(bytes, markerBytes))
				throw new TypeError("D774 commit marker readback drifted");
		} finally {
		...
		if (
			...
			throw new TypeError("D774 final generation identity drifted");
		const receiptMaterial = strictSnapshot({
			schemaVersion: D774_PERSISTENCE_SCHEMA,
			generationRef: D774_GENERATION_REF,
			baselineBasis,
			bundleDigest: bundle.bundleDigest,
		...
		if (finalIdentity !== null) {
			...
			if (
				...
				const tombstone = join(root, `.d774-cleanup-${randomUUID()}.tmp`);
				...
				if (moved !== null && moved.dev === finalIdentity.dev && moved.ino === finalIdentity.ino)
					...
					cleanupErrors.push(new TypeError("D774 cleanup tombstone identity drifted"));
			}
		}
		if (cleanupErrors.length > 0)
			throw new AggregateError([error, ...cleanupErrors], "D774 persistence and cleanup failed");
		throw error;
	}
}

export async function persistD774QualificationBundle(inputValue: {
	readonly privateRoot: string;
	readonly bundle: D774QualificationBundleV1;
}): Promise<D774PersistenceReceiptV1> {
...
export async function persistD774InjectedBundleForTest(
	inputValue: {
		readonly privateRoot: string;
		readonly bundle: D774QualificationBundleV1;
	},
