...
import {
	...
	D781_DISPATCH_CLAIM_REF,
	D781_GENERATION_REF,
	D781_HISTORICAL_BUNDLE_DIGEST,
	D781_ROUTE_PROFILE_DIGEST,
...
export const D781_DISPATCH_CLAIM_SCHEMA =
	"graphrefly.b112.d781.single-use-dispatch-claim.v1" as const;
export const D781_PRIVATE_ROOT = resolve(
	import.meta.dirname,
...
export interface D781PersistedDispatchClaimV1 {
	readonly schemaVersion: typeof D781_DISPATCH_CLAIM_SCHEMA;
	readonly claimRef: typeof D781_DISPATCH_CLAIM_REF;
	readonly decisionRef: typeof D781_DECISION_REF;
	readonly decisionRevision: typeof D781_DECISION_REVISION;
	readonly generationRef: typeof D781_GENERATION_REF;
...
export interface D781ExecutionAuthorityV1 {
	readonly revision: "graphrefly.b112.d781.execution-authority.v1";
	readonly claim: D781PersistedDispatchClaimV1;
...
function claimMaterial(
	...
	const input = record(inputValue, "d781.claim.input");
	exactKeys(
		input,
	...
	literal(
		...
		"d781.claim.implementation",
	);
	return strictSnapshot({
		schemaVersion: D781_DISPATCH_CLAIM_SCHEMA,
		claimRef: D781_DISPATCH_CLAIM_REF,
		...
		pricingReadDigest: digest(input.pricingReadDigest, "d781.claim.pricing"),
		zeroByokObservationDigest: digest(input.zeroByokObservationDigest, "d781.claim.zeroByok"),
...
async function acquire(
...
): Promise<D781PersistedDispatchClaimV1> {
	const requestedRoot = resolve(privateRootValue);
	const privateRoot = await realpath(requestedRoot);
	if (privateRoot !== requestedRoot) throw new TypeError("D781 private root is not canonical");
	...
	const claimRoot = join(privateRoot, `.${D781_DISPATCH_CLAIM_REF}`);
	...
	}) as D781PersistedDispatchClaimV1;
	...
	try {
		...
		if (
			...
			throw new TypeError("D781 dispatch claim readback drifted");
	} finally {
		await reader.close();
	}
	claims.set(claim, {
...
export function acquireD781SingleUseDispatchClaim(input: Parameters<typeof claimMaterial>[1]) {
	return acquire(D781_PRIVATE_ROOT, "live-fixed-root", input);
}

export function acquireD781SingleUseDispatchClaimAtRootForTest(
	root: string,
	input: Parameters<typeof claimMaterial>[1],
) {
...
async function revalidate(claim: D781PersistedDispatchClaimV1, state: ClaimState): Promise<void> {
	...
	if (
		...
		throw new TypeError("D781 durable claim directory drifted");
	...
	try {
		...
		if (
			...
			throw new TypeError("D781 durable claim file drifted");
	} finally {
		await reader.close();
	}
	literal(
...
export async function consumeD781DispatchClaimForExecution(inputValue: {
	readonly claim: D781PersistedDispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
}): Promise<D781ExecutionAuthorityV1> {
	const input = record(inputValue, "d781.execution.input");
	exactKeys(input, ["claim", "currentKeyAdmission"], "d781.execution.input");
	const claim = input.claim as D781PersistedDispatchClaimV1;
	const state = typeof claim === "object" && claim !== null ? claims.get(claim) : undefined;
	if (state === undefined) throw new TypeError("D781 claim is not fresh and constructed");
	claims.delete(claim);
	...
	if (
		...
		admission.remainingMicrousd < D781_BUDGET_LIMITS.maxCostMicrousd ||
		admission.limitReset !== "none" ||
		admission.isManagementKey
	)
		throw new TypeError("D781 current-key admission is outside authority");
	...
	const authority = Object.freeze({
		revision: "graphrefly.b112.d781.execution-authority.v1" as const,
		claim,
...
export function consumeD781ExecutionAuthority(value: unknown): D781ExecutionAuthorityV1 {
	if (typeof value !== "object" || value === null || !authorities.delete(value))
		throw new TypeError("D781 live execution authority must be fixed-root and single-use");
	return value as D781ExecutionAuthorityV1;
}
