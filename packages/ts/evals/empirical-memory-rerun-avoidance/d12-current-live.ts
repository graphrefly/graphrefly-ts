...
import {
	consumeD12CurrentGraphLiveExecutionAuthority,
	type D12CurrentGraphLiveDispatchClaimV1,
	type D12CurrentGraphLiveExecutionAuthorityV1,
} from "./d12-current-live-claim.js";
import {
	D12_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST,
...
export const D12_CURRENT_GRAPH_LIVE_BUNDLE_SCHEMA =
	"graphrefly-ts.d12.current-graph-live-bundle.v1" as const;
export const D12_CURRENT_GRAPH_LIVE_PARTIAL_GRAPH_SCHEMA =
	"graphrefly-ts.d12.current-graph-live-partial-graph.v1" as const;
export const D12_CURRENT_GRAPH_LIVE_GENERATION_SCHEMA =
	"graphrefly-ts.d12.current-graph-live-generation.v1" as const;
export const D12_CURRENT_GRAPH_LIVE_TERMINAL_RECEIPT_SCHEMA =
	"graphrefly-ts.d12.current-graph-live-terminal-receipt.v1" as const;
export const D12_CURRENT_GRAPH_LIVE_PERSISTENCE_SCHEMA =
	"graphrefly-ts.d12.current-graph-live-persistence.v1" as const;
export const D12_CURRENT_GRAPH_LIVE_PREEXECUTION_FAILURE_SCHEMA =
	"graphrefly-ts.d12.current-graph-live-preexecution-failure.v1" as const;
export const D12_CURRENT_GRAPH_LIVE_MAX_BUNDLE_BYTES = 4_194_304;

type D12CurrentGraphLiveProviderFactV1 = CurrentGraphProviderEvidenceV1["facts"][number];

export interface D12CurrentGraphLivePartialGraphV1 {
	readonly schemaVersion: typeof D12_CURRENT_GRAPH_LIVE_PARTIAL_GRAPH_SCHEMA;
	readonly decisionRef: typeof D12_CURRENT_GRAPH_LIVE_DECISION_REF;
	readonly routeDigest: string;
	readonly taskProfileDigest: string;
	readonly facts: readonly D12CurrentGraphLiveProviderFactV1[];
...
export interface D12CurrentGraphLiveBundleV1 {
	readonly schemaVersion: typeof D12_CURRENT_GRAPH_LIVE_BUNDLE_SCHEMA;
	readonly decisionRef: typeof D12_CURRENT_GRAPH_LIVE_DECISION_REF;
	...
	readonly d11QualificationArtifactDigest: string;
...
export async function persistD12CurrentGraphLivePreexecutionFailure(inputValue: {
	readonly privateRoot: string;
	readonly claim: D12CurrentGraphLiveDispatchClaimV1;
	readonly implementationManifestDigest: string;
	readonly pricingObservationDigest: string;
	readonly zeroByokObservationDigest: string;
	...
	if (
		claim.decisionRef !== D12_CURRENT_GRAPH_LIVE_DECISION_REF ||
		claim.generationRef !== D12_CURRENT_GRAPH_LIVE_GENERATION_REF ||
		claim.coordinatesDigest !== D12_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST ||
	...
	const material = strictSnapshot({
		schemaVersion: D12_CURRENT_GRAPH_LIVE_PREEXECUTION_FAILURE_SCHEMA,
		...
		coordinatesDigest: D12_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST,
	...
	const commitMaterial = strictSnapshot({
		schemaVersion: "graphrefly-ts.d12.current-graph-live-commit.v1",
		generationRef: D12_CURRENT_GRAPH_LIVE_GENERATION_REF,
		disposition: "partial-failure" as const,
		bundleDigest: null,
	...
	return Object.freeze({
		schemaVersion: D12_CURRENT_GRAPH_LIVE_PERSISTENCE_SCHEMA,
		generationRef: D12_CURRENT_GRAPH_LIVE_GENERATION_REF,
		disposition: "partial-failure" as const,
...
export function isD12CompleteSixArmMeasurementForTest(
	evidence: D11TransportFailureEvidenceV1,
): boolean {
	return completeSixArms(evidence);
}

export async function runD12CurrentGraphLiveMeasurement(inputValue: {
	readonly executionAuthority: D12CurrentGraphLiveExecutionAuthorityV1;
	readonly executionClass: "live-provider" | "injected-no-network";
	readonly executor: D12CurrentGraphLiveExecutorV1;
	readonly implementationManifestDigest: string;
	readonly d11QualificationArtifactDigest: string;
	readonly pricingObservationDigest: string;
	readonly zeroByokObservationDigest: string;
}): Promise<D12CurrentGraphLiveBundleV1> {
	...
	if (
		input.implementationManifestDigest !== D12_IMPLEMENTATION_MANIFEST_DIGEST ||
		input.d11QualificationArtifactDigest !== D12_D11_QUALIFICATION_ARTIFACT_DIGEST ||
	...
	const authority = consumeD12CurrentGraphLiveExecutionAuthority(input.executionAuthority);
	if (
	...
	const graphAuthority = createD11TransportFailureAuthority({
		limits: D12_CURRENT_GRAPH_LIVE_LIMITS,
		routeProfile: D12_CURRENT_GRAPH_LIVE_ROUTE,
		taskProfile: D12_CURRENT_GRAPH_LIVE_TASK,
	});
	const facts: D12CurrentGraphLiveProviderFactV1[] = [];
	...
	try {
		for (let guard = 0; guard < D12_CURRENT_GRAPH_LIVE_LIMITS.maxEffectFacts; guard += 1) {
	...
	const partialGraphEvidence = success
		...
		: (() => {
				const material = strictSnapshot({
					schemaVersion: D12_CURRENT_GRAPH_LIVE_PARTIAL_GRAPH_SCHEMA,
					decisionRef: D12_CURRENT_GRAPH_LIVE_DECISION_REF,
				...
				}) as D12CurrentGraphLivePartialGraphV1;
			})();
	const generation = success
		? (() => {
				const material = strictSnapshot({
					schemaVersion: D12_CURRENT_GRAPH_LIVE_GENERATION_SCHEMA,
					generationRef: D12_CURRENT_GRAPH_LIVE_GENERATION_REF,
	...
	const terminalMaterial = strictSnapshot({
		schemaVersion: D12_CURRENT_GRAPH_LIVE_TERMINAL_RECEIPT_SCHEMA,
		decisionRef: D12_CURRENT_GRAPH_LIVE_DECISION_REF,
	...
	const bundleMaterial = strictSnapshot({
		schemaVersion: D12_CURRENT_GRAPH_LIVE_BUNDLE_SCHEMA,
		decisionRef: D12_CURRENT_GRAPH_LIVE_DECISION_REF,
		...
		qualificationArtifactDigest: authority.claim.qualificationArtifactDigest,
		qualificationDigest: authority.claim.qualificationDigest,
		d11QualificationArtifactDigest: digest(
			input.d11QualificationArtifactDigest,
			"current.live.d11QualificationArtifactDigest",
		),
		pricingObservationDigest: digest(
			input.pricingObservationDigest,
...
function validatePartial(value: unknown): D12CurrentGraphLivePartialGraphV1 {
	...
	if (
		candidate.schemaVersion !== D12_CURRENT_GRAPH_LIVE_PARTIAL_GRAPH_SCHEMA ||
		candidate.decisionRef !== D12_CURRENT_GRAPH_LIVE_DECISION_REF ||
		candidate.routeDigest !== D12_CURRENT_GRAPH_LIVE_ROUTE.routeDigest ||
		candidate.taskProfileDigest !== D12_CURRENT_GRAPH_LIVE_TASK.taskProfileDigest
	)
	...
	if (facts.length > D12_CURRENT_GRAPH_LIVE_LIMITS.maxEffectFacts)
	...
	return strictSnapshot(candidate) as unknown as D12CurrentGraphLivePartialGraphV1;
}

export function validateD12CurrentGraphLiveBundle(value: unknown): D12CurrentGraphLiveBundleV1 {
	...
	if (
		candidate.schemaVersion !== D12_CURRENT_GRAPH_LIVE_BUNDLE_SCHEMA ||
		candidate.decisionRef !== D12_CURRENT_GRAPH_LIVE_DECISION_REF ||
		candidate.coordinatesDigest !== D12_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST ||
	...
	if (
		candidate.implementationManifestDigest !== D12_IMPLEMENTATION_MANIFEST_DIGEST ||
		candidate.d11QualificationArtifactDigest !== D12_D11_QUALIFICATION_ARTIFACT_DIGEST
	)
	...
	if (
		terminal.schemaVersion !== D12_CURRENT_GRAPH_LIVE_TERMINAL_RECEIPT_SCHEMA ||
		terminal.decisionRef !== D12_CURRENT_GRAPH_LIVE_DECISION_REF ||
	...
	if (candidate.generation !== null) {
		...
		if (
			generation.schemaVersion !== D12_CURRENT_GRAPH_LIVE_GENERATION_SCHEMA ||
			generation.generationRef !== D12_CURRENT_GRAPH_LIVE_GENERATION_REF ||
			generation.graphEvidenceDigest !== graphEvidence?.evidenceDigest ||
			generation.coordinatesDigest !== D12_CURRENT_GRAPH_LIVE_COORDINATES_DIGEST ||
	...
	return strictSnapshot(candidate) as unknown as D12CurrentGraphLiveBundleV1;
}

export async function persistD12CurrentGraphLiveBundle(inputValue: {
	readonly privateRoot: string;
	readonly bundle: D12CurrentGraphLiveBundleV1;
}) {
	...
	const bundle = validateD12CurrentGraphLiveBundle(input.bundle);
