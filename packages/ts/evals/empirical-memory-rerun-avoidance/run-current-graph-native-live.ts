	...
	return await new Promise((resolvePromise, reject) => {
		...
		child.once("close", (code) => {
			if (code !== 0)
				reject(
					new TypeError(
						`current live gate failed (${command} ${args.join(" ")}): ${Buffer.concat(stderr).toString("utf8").slice(0, 4_096)}`,
					),
				);
...
async function boundedPrivateFile(path: string, maxBytes: number): Promise<Uint8Array> {
	...
	try {
		...
		if (
			...
			throw new TypeError(`current live private artifact identity is invalid: ${path}`);
		...
		try {
			...
			if (secondStat.dev !== stat.dev || secondStat.ino !== stat.ino || !sameBytes(first, second))
				throw new TypeError(`current live private artifact changed while read: ${path}`);
		} finally {
...
async function assertAbsent(path: string, label: string): Promise<void> {
	await lstat(path).then(
		() => {
			throw new TypeError(`current live ${label} already exists`);
		},
		(error: unknown) => {
...
async function loadCredential() {
	...
	for (const line of text.split(/\r?\n/u)) {
		...
		if (token !== null) throw new TypeError("current live credential file contains duplicates");
	...
	if (token === null || token.length < 16 || token.length > 4_096)
		throw new TypeError("current live credential is unavailable");
...
async function offlinePreflight() {
	if (process.version !== "v24.18.0") throw new TypeError("current live Node toolchain drifted");
	...
	if ((await realpath(CURRENT_GRAPH_LIVE_PRIVATE_ROOT)) !== CURRENT_GRAPH_LIVE_PRIVATE_ROOT)
		throw new TypeError("current live private root is not canonical");
	...
	if (status.trim() !== "") throw new TypeError("current live tracked worktree is dirty");
	...
	if (qualification.qualification.implementationManifestDigest !== implementationManifestDigest)
	...
	await runCommand("pnpm", [
		...
		"vitest",
		"run",
		"packages/ts/src/__tests__/solutions-agentic-memory-work-item-rerun-avoidance.current-graph-live.test.ts",
