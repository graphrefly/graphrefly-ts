...
export const D754_DISPATCH_CLAIM_SCHEMA =
	"graphrefly.b112.d754.single-use-dispatch-claim.v1" as const;
export const D754_PRIVATE_ROOT = resolve(
	import.meta.dirname,
...
export interface D754PersistedDispatchClaimV1 {
	readonly schemaVersion: typeof D754_DISPATCH_CLAIM_SCHEMA;
	readonly claimRef: typeof D754_DISPATCH_CLAIM_REF;
	readonly decisionRef: typeof D754_DECISION_REF;
	readonly decisionRevision: typeof D754_DECISION_REVISION;
	readonly generationRef: typeof D754_GENERATION_REF;
...
export interface D754ExecutionAuthorityV1 {
	readonly revision: "graphrefly.b112.d754.execution-authority.v1";
	readonly claim: D754PersistedDispatchClaimV1;
...
function claimMaterial(
	...
	const input = record(inputValue, "d754.claim.input");
	exactKeys(
		input,
	...
	return strictSnapshot({
		schemaVersion: D754_DISPATCH_CLAIM_SCHEMA,
		claimRef: D754_DISPATCH_CLAIM_REF,
		...
		pricingReadDigest: digest(input.pricingReadDigest, "d754.claim.pricing"),
		zeroByokObservationDigest: digest(input.zeroByokObservationDigest, "d754.claim.zeroByok"),
		implementationManifestDigest: digest(
			input.implementationManifestDigest,
			"d754.claim.implementation",
		),
...
async function acquire(
...
): Promise<D754PersistedDispatchClaimV1> {
	const requestedRoot = resolve(privateRootValue);
	const privateRoot = await realpath(requestedRoot);
	if (privateRoot !== requestedRoot) throw new TypeError("D754 private root is not canonical");
	...
	if (
		...
		throw new TypeError("D754 private root ownership is invalid");
	const claimRoot = join(privateRoot, `.${D754_DISPATCH_CLAIM_REF}`);
	...
	}) as D754PersistedDispatchClaimV1;
	...
	try {
		...
		if (
			...
			throw new TypeError("D754 dispatch claim readback drifted");
	} finally {
		await reader.close();
	}
	claims.set(claim, {
...
export function acquireD754SingleUseDispatchClaim(input: Parameters<typeof claimMaterial>[1]) {
	return acquire(D754_PRIVATE_ROOT, "live-fixed-root", input);
}

export function acquireD754SingleUseDispatchClaimAtRootForTest(
	root: string,
	input: Parameters<typeof claimMaterial>[1],
) {
...
async function revalidate(claim: D754PersistedDispatchClaimV1, state: ClaimState): Promise<void> {
	...
	if (
		...
		throw new TypeError("D754 durable claim directory drifted");
	...
	try {
		...
		if (
			...
			throw new TypeError("D754 durable claim file drifted");
	} finally {
		await reader.close();
	}
	literal(
...
export async function consumeD754DispatchClaimForExecution(inputValue: {
	readonly claim: D754PersistedDispatchClaimV1;
	readonly currentKeyAdmission: OpenRouterCurrentKeySpendAdmissionV1;
}): Promise<D754ExecutionAuthorityV1> {
	const input = record(inputValue, "d754.execution.input");
	exactKeys(input, ["claim", "currentKeyAdmission"], "d754.execution.input");
	const claim = input.claim as D754PersistedDispatchClaimV1;
	const state = typeof claim === "object" && claim !== null ? claims.get(claim) : undefined;
	if (state === undefined) throw new TypeError("D754 claim is not fresh and constructed");
	claims.delete(claim);
	...
	if (
		...
		admission.remainingMicrousd < D754_BUDGET_LIMITS.maxCostMicrousd ||
		admission.limitReset !== "none" ||
		admission.isManagementKey
	)
		throw new TypeError("D754 current-key admission is outside authority");
	...
	const authority = Object.freeze({
		revision: "graphrefly.b112.d754.execution-authority.v1" as const,
		claim,
...
export function consumeD754ExecutionAuthority(value: unknown): D754ExecutionAuthorityV1 {
	if (typeof value !== "object" || value === null || !authorities.delete(value))
		throw new TypeError("D754 live execution authority must be fixed-root and single-use");
	return value as D754ExecutionAuthorityV1;
}
