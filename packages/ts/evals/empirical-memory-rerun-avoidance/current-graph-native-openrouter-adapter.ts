...
import type { CurrentGraphLiveExecutorV1 } from "./current-graph-native-live.js";
import {
...
import type { CurrentGraphLiveCredentialV1 } from "./current-graph-native-live-preflight.js";
import type {
	...
	try {
		...
		const commit = await runProcess({
			...
			args: [
				...
				"--no-gpg-sign",
				"-m",
				"current Graph-native D3 task baseline",
			],
			cwd: root,
			timeoutMs: 30_000,
		});
				...
				if (request.effectKind === "tool-action") {
					...
					if (args.toolRef === "read-file") {
						...
						const validation = await runProcess({
							command: "pnpm",
							args: ["exec", "biome", "check", CURRENT_GRAPH_LIVE_WRITABLE_FILE],
							cwd: state.root,
							timeoutMs: 120_000,
						});
				...
				if (request.effectKind === "public-semantic-validation") {
					const validation = await runProcess({
						...
						args: [
							...
							"-t",
							"admits only a fresh D419 managed remote run",
						],
						cwd: state.root,
						timeoutMs: 120_000,
					});
