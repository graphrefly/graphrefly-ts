 ...
 * The native arm's `Graph` Proxy traps `tagFactory` to throw loudly per
 * `wrapNativeGraph` in `impls/rust.ts`; every test here is
 * `test.runIf(impl.name === "pure-ts")`-gated until `@graphrefly/native`
 * ships a `tag_factory` napi binding (lifts D004's R3.1.2 deferral in
 * `graphrefly-rs/docs/porting-deferred.md:1676`).
 *
...
describe.each(impls)("R3.1.2 tagFactory parity — $name", (impl) => {
	...
	test.runIf(runIfPureTs)(
		"tagFactory(factory, args) surfaces factory + factoryArgs at the top of describe()",
		async () => {
	...
	test.runIf(runIfPureTs)(
		"second tagFactory call WITHOUT args clears stale args (QA F8 invariant)",
		async () => {
			const g = new impl.Graph("root");
			await g.state<number>("a", 1);
	...
	test.runIf(runIfPureTs)(
		"tagFactory(factory) without factoryArgs does not populate the factoryArgs key",
		async () => {
			const g = new impl.Graph("root");
			await g.state<number>("a", 1);
	...
	test.runIf(runIfPureTs)(
		"factory tag persists across describe({reactive:true}) snapshots triggered by other mutations",
		async () => {
