...
- Before implementing a new public API in `src/patterns/` or `src/extra/`
...
1. **`--diff` (or no args)** — review the new public symbols introduced in the current uncommitted diff. Run `git diff --name-only HEAD` and `git status --short` to enumerate. Filter to files in `src/patterns/`, `src/extra/`, `src/core/`, `src/graph/`, `src/compat/`, or `src/integrations/` that introduce new exports.
...
If the target imports anything from `src/patterns/`, `~/src/graphrefly/COMPOSITION-GUIDE.md` is **mandatory** reading — composition primitives have non-obvious load-bearing patterns documented there.
