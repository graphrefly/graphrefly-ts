...
- **B2 — `mapFromSnapshot` defensive helper deleted in Tier 10.1.** Live emit path is always a `Map`, but `JsonGraphCodec` round-trips `Map` as plain `{}` on snapshot-restore. Without the helper, downstream `.entries()` / `.size` accesses silently fail. **Corrected** via D2: helper restored at `extra/composite.ts` and parallel `instanceof Map` check added at `ai/adapters/core/capabilities.ts`. Cleanup-tier safety checklist added to [docs/docs.jsonl](docs/docs.jsonl) so future Tier 10.x cleanups verify both live-emit AND snapshot-restore paths.
- **B3 — `process/start` initial γ-7-B used `freeze: false`.** Copied from memory-primitive precedent (where 768-dim vector freeze is a real tax). Process state objects are tiny — `freeze: false` opened a post-record state-mutation hazard. **Corrected** via D4: `freeze: true`. Migration shape-preservation rule added to docs.jsonl.
...
- ✅ **B2** — cleanup hook field rename (Lock 4.A: `beforeRun → onRerun`, `deactivate → onDeactivation`, `invalidate → onInvalidate`; Lock 4.A′ all firing sites updated). Landed 2026-05-03. **Carry RESOLVED 2026-05-16 (`/dev-dispatch`):** Lock 4.A drop-`() => void` shorthand sweep landed — type leg + 4 runtime firing sites removed from `core/node.ts`, `legacyCleanup` faithful-shim helper added, ~115 NodeFn cleanup returns across 53 source files migrated, 2 obsolete shim tests retired; archived `resolved-decisions.jsonl` id `lock-4a-cleanup-shorthand-sweep-landed-2026-05-16`. Two cosmetic/test-hygiene follow-ups tracked in `docs/optimizations.md`.
...
4. **Parity test layer** at `packages/parity-tests/` — vitest runner. **Current (2026-05-25):** `describe.each(impls)` runs `[pureTsImpl, rustImpl]` when `@graphrefly/native` is built or installed; see `packages/parity-tests/README.md`. Phase 13.9.B target shape (deferred): contract-trace replay — record trace from one impl, assert byte-equal sequence against the other (per rigor-infra Project 2). Divergences fail loud when the rust arm is active.
...
- **✅ LANDED (v1, pre-2026-05-18):** `refineExecutor(config)` — adapter that plugs `refineLoop` into the EXECUTE slot of `harnessLoop`. Composition E from §9.0. Shipped at [src/presets/harness/refine-executor.ts:123](../src/presets/harness/refine-executor.ts:123) with per-item `switchMap` lifecycle (Option A); regression test + `refine-executor.test.ts` landed alongside v1. The earlier "Land when a real consumer surfaces" gate was stale — corrected 2026-05-18 (consumption-demand constraint relaxed). Follow-ups (Option B persistent re-seed, `dispose` hardening) remain in `docs/optimizations.md`.
...
- **✅ LANDED 2026-05-18 (`/dev-dispatch`, consumption-demand constraint relaxed):** `toolInterceptor(opts?)` at [src/utils/ai/agents/tool-interceptor.ts](../src/utils/ai/agents/tool-interceptor.ts), exported via `@graphrefly/graphrefly/utils/ai`. Returns a `(calls) => Node<readonly ToolCall[]>` transform shaped to slot directly into `agentLoop.interceptToolCalls` (the 2026-04-22 splice). Per the approved design leans: modelled as **reactive deny-predicates** (NOT a buffering `valve` — a security kill-switch must drop, never buffer-and-replay) — `enabled?: NodeInput<boolean>` kill-switch + `allow?: NodeInput<(call) => boolean>[]` per-call predicates (every-must-pass; `null` predicate = pass-through, mirroring `toolSelector`); full-deny → `[RESOLVED]` (mirrors `toolCallsRaw`'s empty-batch gate). 5 tests in `ai.test.ts`. **Caveat:** full-deny *inside agentLoop* strands the loop in `"acting"` — a pre-existing agent-loop edge (effResults early-returns on empty), logged in `docs/optimizations.md`; the primitive's node contract is correct and the agentLoop integration test covers the supported partial-deny path.

#### 14.5.3 `mockLLM` promotion to public testing export
*Source: roadmap.md Phase 7.6 "Mock LLM fixture system"*
- **✅ LANDED 2026-05-18 (`/dev-dispatch`, consumption-demand constraint relaxed):** scenario-scripted `mockLLM` promoted from the private `src/__tests__/helpers/mock-llm.ts` to the public **`@graphrefly/graphrefly/testing`** subpath (`src/testing/mock-llm.ts` + `src/testing/index.ts` barrel; universal tier — `testing/` sits outside the 4-layer model, like `compat/`, and is unranked by the layer-boundary checker). Wired into `tsup.config.ts` ENTRY_POINTS + root `package.json` `exports["./testing"]`. Old helper moved to `TRASH/` (no-backward-compat); 4 in-repo importers retargeted. 5 public-surface tests in `src/__tests__/testing/mock-llm.test.ts` (stage detection, per-stage cycling, fallback, reset, stream deltas).
...
|----|-------|-------|-------|
| **DS-2.7.A** ✅ LOCKED 2026-05-19 / TS impl ✅ LANDED + /qa-PASSED 2026-05-20 | §2.7 first-run-gate cross-port convergence | Q1=Rust-canonical (`R2.7.0`–`R2.7.3`: RESOLVED doesn't settle; terminal doesn't by default — Reduce-class opts in via `NodeOptions.terminalAsRealInput`; `partial:true` truly disables gate, fn must guard SENTINEL). **TS impl slice** (`/dev-dispatch` + `/qa`, 2026-05-20): new `NodeOptions.terminalAsRealInput?: boolean` + gate-scan predicate rewritten in `packages/pure-ts/src/core/node.ts`; opted in: `reduce`/`scan`/`last`/`take`/`takeWhile` (Q5 audit expanded scope from `reduce`/`scan`/`last`-only); audit-discovered consumer opt-ins in `restoreEffect`/`approvalGate.output`/`catch`/`toArray` (the audit's 5th proposed site — `error-watcher` — was reverted in /qa as having no observable benefit); `takeWhile` fn-body fix (F1) added an explicit terminal branch alongside the opt-in. **20 tests landed**: 8 TS unit tests; 5 parity scenarios (structural; `Impl` NOT widened — Rust slice still pending); 4 consumer-opt-in regression tests; 3 operator-on-empty regression tests. Closes `optimizations.md` ② (premise unreproducible; real defect was 3-port divergence + stale TS spec text vs Phase 10.5 impl truth). Spec `GRAPHREFLY-SPEC.md` §2.7 + §2.5 amended + cross-track-ledger §2 row. **One follow-on slice remaining**: thin Rust slice — predicate already conformant; only needs to surface the Reduce-class terminal-aware behavior as a user-facing `NodeOptions.terminalAsRealInput` flag matching `R2.7.1`. PY carved out (Q4) → `[py-parity-am]` umbrella. See [archive/docs/SESSION-DS-2.7.A-first-run-gate.md](../archive/docs/SESSION-DS-2.7.A-first-run-gate.md). | Core (spec-level); TS slice ✅ ; Rust slice = small surface-only |
...
| **DS-14** ✅ LOCKED 2026-05-05 (main) + 2026-05-08 (storage WAL, landed Phase 14.6) + 2026-05-15 (residuals R1/R2 locked + landed + QA-passed; R3 = doc-supersede only) | Unified changesets/diff design | All 5 threads locked: op-log changesets / worker-bridge wire B (wire shape `{t:"c",lifecycle,path,change}` pre-locked — implementation a separate session, backpressure threshold deferred to real-consumer signal) / `lens.flow` delta (R3 decline-with-rationale) / `reactiveLog.scan` / `restoreSnapshot mode:"diff"` + WAL replay. Residuals R1 (TTL/LRU prune-record drain — `MapBackend.drainPruneRecords` in [reactive-map.ts:260,486,667](../packages/pure-ts/src/extra/data-structures/reactive-map.ts)) + R2 (pubsub `mutationLog` opt in [pubsub.ts:113,152,188](../src/base/composition/pubsub.ts)) ✅ landed + QA-passed 2026-05-15 (pure-ts 1172/1172, pubsub 21/21, biome-clean); R3 = decline-with-rationale, doc supersede only. See [SESSION-DS-14-changesets-design.md](../archive/docs/SESSION-DS-14-changesets-design.md), [SESSION-DS-14-storage-wal-replay.md](../archive/docs/SESSION-DS-14-storage-wal-replay.md), [SESSION-DS-14-changeset-residuals.md](../archive/docs/SESSION-DS-14-changeset-residuals.md). ⚠ D282 native-batch resolution PARTIAL 2026-05-25 via D288 (Path D locked) + D289 (native binding) + D290 (paired TS slice — `Impl.batch(fn: (ctx: BatchCtx) => void)` widening + scenario rebase) + D291 Item 1 (Case 5 cross-arm). **Case 15a stays `runIf(pure-ts)` pending D292** — cross-arm verification surfaced that `BenchBatchContext::commit` is sync napi → libuv blocks → TSFN-backed sinks deadlock on `BatchGuard::Drop`'s success path. D292 (queued in `docs/optimizations.md` "🔴 DESIGN SESSION FIRST") converts commit/rollback to async napi (mirrors D267). Tracked in `~/src/graphrefly-rs/docs/porting-deferred.md` § "D291 → D292 carry"; cross-track-ledger §1 D282 row stays open until D292 lands. | Phase 14; design ✅ done; impl ⚠ Case 15a pending D292 |
...
| **DS-14.7** ✅ IMPLEMENTED 2026-05-15 (locked 2026-05-13) | Reactive Fact Store / Live Knowledge Graph pattern | Static-topology memory substrate (~12 fixed operator nodes + columnar `state<FactStore>`) satisfying MEME L2/L3 (cascade + obsolescence) + Hassabis filter/consolidate/continual-learning frame. 9Q walk complete; Q9-open items 1–9 all resolved. Locks: `cascadeMaxIterations=8`, `shardBy` hash-mod 4 default, MemoryFragment shape (incl. `embedding?` / `parent_fragment_id?` / `provenance?`), synchronous `dependentsIndex` updates, `(fragment, storeReadHandle) => number` scoring, dedicated `consolidated` topic with default wire-back, structured `MemoryQuery` surface, bi-temporal as pattern-only (envelope deferred to v2), `simpleFactStore()` wrapper deferred to v1.1. Spec + DS-14 envelope: zero shape change. **v1 landed** as `reactiveFactStore<T>()` in `src/utils/memory/fact-store.ts` (re-exported via the `utils/memory` barrel); 20 tests in `src/__tests__/utils/memory/fact-store.test.ts` (topology / MEME L1·L2·L3 / cascade fixpoint+overflow / sharding / 4 extension faces / audit replay / describe·explain cycle visibility / SENTINEL guards). Deferred: recipe library (`recipes/*`), `simpleFactStore()` (v1.1), MEME 100-episode parity harness (Phase 15 / DS-15), PY parity (under `[py-parity-am]`). See [archive/docs/SESSION-DS-14.7-reactive-fact-store.md](../archive/docs/SESSION-DS-14.7-reactive-fact-store.md). | Phase 14.5; substantial |
| **DS-15** | Eval program shape | Two-tier design + catalog automation + scorecard | Phase 15; substantial. **Now consumes DS-14.7** — MEME parity (target ≥40% L2 vs 3% baseline, no Opus-judge-at-read tax) is a prime scorecard candidate. |
