...
|----|-------|-------|-------|
| **DS-2.7.A** ✅ LOCKED 2026-05-19 | §2.7 first-run-gate cross-port convergence | Q1=Rust-canonical (`R2.7.0`–`R2.7.3`: RESOLVED doesn't settle; terminal doesn't by default — Reduce-class opts in via `NodeOptions.terminalAsRealInput`; `partial:true` truly disables gate, fn must guard SENTINEL). Closes `optimizations.md` ② (premise unreproducible — see probes; real defect was 3-port divergence + stale TS spec text vs Phase 10.5 impl truth). Spec `GRAPHREFLY-SPEC.md` §2.7 + §2.5 amended + cross-track-ledger §2 row. **Two follow-on slices** (next sessions): TS slice (gate-predicate update + `terminalAsRealInput` flag + opt `reduce`/`scan`/`last` into it + new parity scenario) AND a thin Rust slice (predicate already conformant — only needs to surface the Reduce-class terminal-aware behavior as a user-facing `NodeOptions.terminalAsRealInput` flag matching `R2.7.1`). PY carved out (Q4) → `[py-parity-am]` umbrella. See [archive/docs/SESSION-DS-2.7.A-first-run-gate.md](../archive/docs/SESSION-DS-2.7.A-first-run-gate.md). | Core (spec-level); TS slice = small; Rust slice = small surface-only |
