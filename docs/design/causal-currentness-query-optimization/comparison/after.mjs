...
function recomputeCurrentness(context, revisionDomain) {
  ...
  const evaluated = occurrences.map((occurrence4) => ({
    occurrence: occurrence4,
    admission: exactAdmission(context, occurrence4)
  }));
  ...
  ) && evaluated.filter(({ occurrence: occurrence4 }) => occurrence4.revision > floor).every(({ admission }) => admission !== void 0);
  const latestAdmitted = /* @__PURE__ */ new Map();
  for (const { occurrence: occurrence4, admission } of evaluated) {
  ...
  for (const records of [state.evidence, state.coverageGaps]) {
    for (const entry of records.values()) {
      if (entry.occurrence.revisionDomain !== revisionDomain || !terminalCoverage.has(entry.coverage))
        continue;
      const key2 = refKey(entry.occurrence);
      let kinds = covered.get(key2);
      if (kinds === void 0) {
        kinds = /* @__PURE__ */ new Set();
        covered.set(key2, kinds);
      }
      kinds.add(entry.evidenceKind);
    }
  }
  return occurrences.every((occurrence4) => {
    const kinds = covered.get(refKey(occurrence4));
...
function materialDigest(text3) {
  return `sha256:${createHash2("sha256").update(text3, "utf8").digest("hex")}`;
}
function freeze(v) {
  if (v !== null && typeof v === "object") {
...
function number2(v, max, integral = false, min = 0) {
  if (typeof v !== "number" || !Number.isFinite(v) || v < min || v > max || integral && !Number.isSafeInteger(v))
    throw new TypeError("number");
}
function tick(v) {
  number2(v, Number.MAX_SAFE_INTEGER, true);
}
function epoch2(v) {
  number2(v, Number.MAX_SAFE_INTEGER, true, 1);
}
function boolean(v) {
...
function policy(v) {
  ...
  number2(v.zThreshold, 1e6);
  number2(v.dailyRatioThreshold, 1e6);
}
  ...
  try {
    ...
    if (kind === "arrivals") {
      ...
      if (kind === "pack") {
        ...
        number2(v.readiness.availableSlots, 1, true);
        array(v.outcomes);
...
async function runRow(configPath) {
...
`);
  const twin = config.control ? await import(config.copyModule) : void 0;
  const makeArm = config.control ? (arm, mode) => arm === "candidate" ? twin.measurementArm("reference", mode) : measurementArm("reference", mode) : measurementArm;
  const checked3 = preflight(row, scenario);
  if (twin) assert.deepEqual(twin.preflight(row, scenario), checked3, "identical-copy semantic preflight");
