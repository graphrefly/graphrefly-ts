...
for (const [name, hash] of Object.entries(binding.files))
	assert.equal(digest(readFileSync(path.join(root, name))), hash);
...
try {
	for (const name of ["B.mjs", "C.mjs", "B-copy.mjs"]) {
		...
		const micro = [];
		for (const row of ["inactive-60", "active-diamond-5", "inactive-2", "inactive-1"])
			micro.push({ row, ...sample(mod, row, scenario, undefined, () => 0) });
