...
with tempfile.TemporaryDirectory() as temp:
 ...
 put(root/'frozen.json',{'assets':assets});prepared=sha((root/'frozen.json').read_bytes());assets['frozen.json']=prepared
 r={'preparedDigest':prepared,'assets':assets,'bits':bits,'schedule':schedule,'runtime':runtime,'environment':{'PATH':'/test','LANG':'C','TZ':'UTC'},'wake':'test','originalRoot':str(root),'budget':{'children':48,'samples':6720,'warmup':20,'measured':50,'rssBytes':268435456,'childSeconds':30,'seconds':900,'retries':0},'continuity':{'clockSkewSeconds':.5,'childObservationGapSeconds':1}}
 put(root/'reservation.json',r);put(root/'continuity.json',[{'wall':1000,'mono':10,'phase':'start'}]);assert inspect(root,0)['passed']
 for j in schedule:
 ...
 points=[{'wall':1000,'mono':10,'phase':'start'}]
 for j in range(48):
  for delta in [0,.3]:points.append({'wall':1000+j+.1+delta,'mono':10+j+.1+delta,'phase':'child:'+str(j)})
 points.append({'wall':1049,'mono':59,'phase':'finish'});put(root/'continuity.json',points)
 ...
 mutation('host-clock-shift','continuity.json',lambda x:x[10].update(wall=x[10]['wall']+5))
 mutation('host-reversed','continuity.json',lambda x:x[10].update(mono=0))
 mutation('host-finish','continuity.json',lambda x:x.pop())
 mutation('host-gap','continuity.json',lambda x:x[2].update(wall=x[2]['wall']+2,mono=x[2]['mono']+2))
