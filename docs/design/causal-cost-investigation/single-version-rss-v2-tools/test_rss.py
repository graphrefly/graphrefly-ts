...
for case in ['positive','terminal-zero','terminal-empty','live-zero','zero-with-running-state','zombie-unreaped','wrong-pid','negative','malformed','ps-failure','over-limit','exiting-settled','exiting-timeout','exiting-error']:
 with tempfile.TemporaryDirectory() as tmp:
  ...
  class Fake(Process):
   ...
   def wait(self,**k):
    if case in ['zombie-unreaped','exiting-timeout'] and k.get('timeout')==.5:raise TimeoutError('settlement timeout')
    if case=='exiting-error' and k.get('timeout')==.5:self.returncode=1;return 1
    return super().wait(**k)
  class Runtime(Subprocess):
   ...
   def run(self,*a,**k):
    ...
    if n==2:
     ...
     elif case in ['exiting-settled','exiting-timeout','exiting-error']:out=str(proc.pid)+' ?Es 0'
  ...
  ok=collect.capture(root);assert ok==(case in ['positive','terminal-zero','terminal-empty','exiting-settled']),case
  v=inspect(root)
  if ok:
   assert v['completed']==32 and v['samples']==3840
   assert v['maxRssBytes']==1048576 and v['maxObservedRssGapSeconds']>0
   if case=='exiting-settled':
    p=root/'jobs/00/exit.json';raw=p.read_bytes()
    for key,val in [('code',1),('end',99),('error','failure')]:
     z=json.loads(raw);z['rssObservations'][-1]['settlement'][key]=val;put(p,z)
     try:inspect(root);raise AssertionError('bad settlement '+key)
