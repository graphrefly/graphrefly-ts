...
def inspect(root,complete=None):
 ...
 for j in schedule[:complete]:
  ...
  # Independent reconstruction from retained ps process-state evidence.
  derived=[];terminalSeen=False;previousEnd=0
  require(bool(out.get('rssObservations')),'missing raw RSS observations')
  for obs in out['rssObservations']:
   require(not terminalSeen,'observations after terminal')
   require(set(obs)=={'start','end','pollBefore','pollAfter','returncode','stdout','stderr','settlement'},'RSS observation shape')
   require(all(type(obs[k]) in (int,float) and math.isfinite(obs[k]) for k in ['start','end']) and previousEnd<=obs['start']<=obs['end']<=out['seconds'],'RSS observation times')
   previousEnd=obs['end']
   require(all(obs[k] is None or type(obs[k]) is int for k in ['pollBefore','pollAfter']),'RSS poll identity')
   require(obs['pollBefore'] is None or obs['pollAfter']==obs['pollBefore'],'RSS exit reversal')
   require(all(obs[k] is None or obs[k]==out['code'] for k in ['pollBefore','pollAfter']),'RSS final exit mismatch')
   fields=obs['stdout'].split();size=None;state=None
   if obs['returncode']==0 and len(fields)==3 and fields[0]==str(ident['pid']):
    state=fields[1]
    try:size=int(fields[2])*1024
    except ValueError:pass
   effectiveExit=obs['pollAfter'];settle=obs['settlement']
   if settle is not None:
    require(size==0 and state is not None and (state.startswith('Z') or 'E' in state) and obs['pollAfter'] is None,'RSS settle eligibility')
    require(set(settle)=={'start','end','code','error'} and all(type(settle[k]) in (int,float) and math.isfinite(settle[k]) for k in ['start','end']),'RSS settle shape')
    require(obs['end']<=settle['start']<=settle['end']<=out['seconds'] and settle['end']-settle['start']<=.55,'RSS settle interval')
    require(settle['error'] is None and type(settle['code']) is int and settle['code']==out['code'],'RSS settle exit')
    effectiveExit=settle['code'];previousEnd=settle['end']
   if size is not None and size>0:derived.append([obs['end'],size])
   else:
    terminalSeen=effectiveExit==out['code'] and effectiveExit is not None and ((size==0 and state is not None and (state.startswith('Z') or 'E' in state)) or (obs['returncode']==1 and not obs['stdout'].strip() and not obs['stderr'].strip()))
    require(terminalSeen,'unproven terminal RSS observation')
   if effectiveExit is not None:terminalSeen=True
  require(out['rss']==derived,'RSS raw/projection mismatch')
