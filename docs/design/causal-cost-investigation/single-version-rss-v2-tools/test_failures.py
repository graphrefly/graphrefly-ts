...
for case in ['initial-probe','spawn','interrupt','rss','observation-gap','cleanup-race','cleanup-error','wake-after','asset-drift','cleanup-wake-error','wake-before-spawn','node-identity','slot-drift','source-after-check']:
 with tempfile.TemporaryDirectory() as temp:
  ...
  def wake():
   wakes[0]+=1
   if case=='source-after-check' and wakes[0]==2:
    for arm in ['B','C']:(root/(arm+'.mjs')).write_bytes(b'changed')
   if case=='cleanup-wake-error' and wakes[0]>=4:raise RuntimeError('synthetic wake failure')
   return 'different' if (case=='wake-after' and wakes[0]>=4) or (case=='wake-before-spawn' and wakes[0]>=3) else 'same'
  collect.wake=wake
  class Fake(Process):
   def __init__(self,*a,**k):
    ...
    if case=='slot-drift':(root/'run-slot/bundle.mjs').write_bytes(b'changed')
   def poll(self):
  ...
  class Runtime(Subprocess):
   ...
   def check_output(self,*a,**k):
    ...
    return json.dumps({**json.loads(metadata),'execPath':'/wrong/node'}).encode() if case=='node-identity' else metadata
   def run(self,*a,**k):
    if case=='cleanup-race':raise RuntimeError('synthetic ps')
    return types.SimpleNamespace(returncode=0,stdout=a[0][-1]+' R '+('262145' if case in ['rss','cleanup-error','cleanup-wake-error'] else '1024'),stderr='' )
  ...
  if case in ['wake-before-spawn','source-after-check']:assert not active
  if case=='asset-drift':assert v['assetDrift']==['B.mjs'] and not list((root/'jobs').iterdir())
  if case not in ['initial-probe','asset-drift','node-identity']:
   ...
   if case=='rss':
    ...
    samples.write_text(json.dumps({'block':0,'position':0,'index':0,'slot':0,'phase':'warmup','clocks':[2,1]})+'\n')
    assert inspect(root)['failedJob']['invalidSuffix'] is not None
