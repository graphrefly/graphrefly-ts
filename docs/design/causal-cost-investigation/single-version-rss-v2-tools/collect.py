...
def capture(root,prepared_digest=None):
 ...
 try:
  for j in schedule:
   ...
   dispatched+=1;child=None;err=None;rss=[];observations=[];begin=time.monotonic();wallBegin=time.time();before=None
   try:
    slot.stage(j['arm'],d)
    before=wake()
    ...
    with (d/'stdout.log').open('xb') as out,(d/'stderr.log').open('xb') as stderr:
     ...
     while child.poll() is None:
      ...
      pollBefore=child.poll();obsStart=time.monotonic()-begin
      q=subprocess.run(['/bin/ps','-o','pid=,stat=,rss=','-p',str(child.pid)],capture_output=True,text=True,timeout=2)
      pollAfter=child.poll();obsEnd=time.monotonic()-begin
      obs={'start':obsStart,'end':obsEnd,'pollBefore':pollBefore,'pollAfter':pollAfter,'returncode':q.returncode,'stdout':q.stdout,'stderr':q.stderr,'settlement':None}
      observations.append(obs)
      fields=q.stdout.split();value=None;state=None
      if q.returncode==0 and len(fields)==3 and fields[0]==str(child.pid):
       state=fields[1]
       try:value=int(fields[2])*1024
       except ValueError:pass
      if value==0 and state is not None and (state.startswith('Z') or 'E' in state) and pollAfter is None:
       settle={'start':time.monotonic()-begin,'end':None,'code':None,'error':None};obs['settlement']=settle
       try:pollAfter=child.wait(timeout=.5);settle['code']=pollAfter
       except (Exception,KeyboardInterrupt) as e:settle['error']=reason(e)
       finally:settle['end']=time.monotonic()-begin
      if value is not None and value>0:
       rss.append([obsEnd,value])
       if value>268435456:raise ValueError('RSS guard')
      elif pollAfter is not None and ((value==0 and state is not None and (state.startswith('Z') or 'E' in state)) or (q.returncode==1 and not q.stdout.strip() and not q.stderr.strip())):
       break
      else:raise ValueError('invalid RSS observation')
      time.sleep(.1)
     clock.record('child:'+str(j['id']))
    ...
    if child is not None:
     try:
      if child.poll() is None:
       ...
       except OSError as e:err=(err+'; ' if err else '')+'signal error: '+reason(e)
    ...
    put(d/'exit.json',{'pid':child.pid if child else None,'code':child.returncode if child else None,'seconds':max(time.monotonic()-begin,time.time()-wallBegin),'failure':err,'rss':rss,'rssObservations':observations,'wakeBefore':before,'wakeAfter':after})
