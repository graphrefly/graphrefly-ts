...
for (const name of ["same-object-duplicate", "equal-clone-duplicate", "local-stop-change"]) {
	...
	for (const [label, mod] of [
		...
		function capture(phase) {
			if (mod.__take) phases.push({ phase, stats: mod.__take() });
		}
		try {
			run = mod.candidate();
			capture("construct");
			...
			const before = mod.graphSnapshot(run);
			...
			if (name === "local-stop-change") {
				action = [structuredClone(scenario.steps.find((s) => s.lane === "local"))];
				action[0].values[0].stop = true;
			}
			for (const step of action) run.send(step);
			capture("action");
			const after = mod.graphSnapshot(run);
			...
			if (name !== "local-stop-change") assert.deepEqual(after, expected);
				...
				assert.equal(after.effects[0].outcome, null);
			}
			run.cleanup();
			cleaned = true;
			...
			results.push({ name, label, phases, after });
		} finally {
...
writeFileSync(
	path.join(out, "counts.json"),
	JSON.stringify(
		{
			passed: true,
			totalInstances: 12,
			candidateLifecycles: 8,
