...
199文件失败归档回放一致。用户授权修复直到完整证据后，RSS观测/退出边界已修复。
[完整单版本证据](single-version-rss-v2-capture/README.md)：32/32进程、3840样本、47.16秒，
RSS峰值156.41MiB；572文件回放一致。此前29/32部分尝试独立保留。
构造p50/p95八组均下降但控制未过，动作总量p50混合；本方法结束，不追加采样，收益未确认。
