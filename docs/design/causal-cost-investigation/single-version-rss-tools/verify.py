...
def inspect(root,complete=None):
 ...
 for j in schedule[:complete]:
  ...
  for obs in out['rssObservations']:
   ...
   require(set(obs)=={'start','end','pollBefore','pollAfter','returncode','stdout','stderr'},'RSS observation shape')
   ...
   if size is not None and size>0:derived.append([obs['end'],size])
    ...
    terminalSeen=obs['pollAfter']==out['code'] and obs['pollAfter'] is not None and ((size==0 and state is not None and state.startswith('Z')) or (obs['returncode']==1 and not obs['stdout'].strip() and not obs['stderr'].strip()))
    require(terminalSeen,'unproven terminal RSS observation')
   if obs['pollAfter'] is not None:terminalSeen=True
