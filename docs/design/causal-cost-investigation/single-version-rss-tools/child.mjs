...
import { coordinates, sample } from "./driver.mjs";
...
try {
	...
	assert.deepEqual(entry.modules, ["run-slot/bundle.mjs"]);
	const modules = [];
	...
	assert.equal(entry.row, "P2-lifecycle");
	...
	assert.equal(digest(readFileSync(path.join(root, "run-slot/bundle.mjs"))), entry.moduleDigests["run-slot/bundle.mjs"]);
