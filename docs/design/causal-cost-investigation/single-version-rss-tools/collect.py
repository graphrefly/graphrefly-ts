   ...
   try:
    ...
    with (d/'stdout.log').open('xb') as out,(d/'stderr.log').open('xb') as stderr:
     ...
     while child.poll() is None:
      ...
      obs={'start':obsStart,'end':obsEnd,'pollBefore':pollBefore,'pollAfter':pollAfter,'returncode':q.returncode,'stdout':q.stdout,'stderr':q.stderr}
      ...
      elif pollAfter is not None and ((value==0 and state is not None and state.startswith('Z')) or (q.returncode==1 and not q.stdout.strip() and not q.stderr.strip())):
       break
