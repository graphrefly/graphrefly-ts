...
for (const [role, entry] of Object.entries(entries)) {
	...
	for (const file of inputs) {
		...
		const frozen = execFileSync("git", ["show", `${revision}:${file}`], {
			cwd: root,
			maxBuffer: 16000000,
		});
		...
		files[file] = {
			sha256: hash(current),
			gitBlob: execFileSync("git", ["rev-parse", `${revision}:${file}`], {
				cwd: root,
				encoding: "utf8",
			}).trim(),
		};
	...
	closures[role] = {
		entry,
		inputs,
		externalImports: result.metafile.outputs[Object.keys(result.metafile.outputs)[0]].imports,
		bundleSha256: hash(result.outputFiles[0].contents),
	};
}
const c = new Set(closures.candidateBuilder.inputs),
	r = new Set(closures.referenceBuilder.inputs);
	...
	JSON.stringify(
		{
			...
			limitation:
				"Module import closures, not executed-path or per-node cost attribution; harness includes both branches and oracle. Type-only imports omitted by bundling.",
			entries: closures,
			sharedBuilderInputs: shared,
			candidateOnly: [...c].filter((x) => !r.has(x)).sort(),
			referenceOnly: [...r].filter((x) => !c.has(x)).sort(),
