...
const commits = {
	B: "60190fcc2f660df1164a0c351ff2083925b71baf",
	C: "f7ea7a5281550060620c2e671d12d06885a07fd2",
};
...
assert.deepEqual(different, ["packages/ts/src/solutions/causal-occurrence/identity.ts"]);
...
const start = after.indexOf("// This call site receives a canonicalSnapshot-owned occurrence");
const end = after.indexOf("/** D160: fixed identity transition helper", start);
assert.ok(start >= 0 && end > start);
after = after.slice(0, start) + after.slice(end);
assert.equal(after.split("currentnessChanged(prior, value, occurrence)").length, 2);
after = after.replace(
	"currentnessChanged(prior, value, occurrence)",
	"dataKey(prior) !== dataKey(value)",
);
assert.equal(before, after, "only approved comparator differs");
