...
- **~~`equals` must never see undefined/None (all phases, noted 2026-04-05, resolved 2026-04-05):~~** `_emitAutoValue` / `_emit_auto_value` now skips `equals()` when `_hasEmittedData` / `_has_emitted_data` is `false`. Previously, custom `equals` functions received `undefined`/`None` as the first argument on the first computation, causing crashes for Map/Array/object comparisons. Spec §2.5 updated with explicit contract. Both TS and PY fixed. Resolution now uses the `_hasEmittedData` flag instead of the `_cached !== undefined` guard. Test `test_from_any_none_scalar` updated (PY) — first DATA(None) is now correctly treated as changed per spec §2.5.
- **~~`_cached !== undefined` sentinel is ambiguous with `[[DATA, undefined]]` and INVALIDATE (noted 2026-04-05, resolved 2026-04-05):~~** Added `_hasEmittedData: boolean` flag (initially `false`, set `true` on first `_emitAutoValue`, reset on INVALIDATE/`resetOnTeardown`). Both TS and PY. Disambiguates "never emitted" from "emitted undefined" and "reset via INVALIDATE". The counter-argument about exotic paths was valid but INVALIDATE reset had accidental semantics.
...
- **`toJSON()` → `toObject()` + `toJSONString()` / `to_json()` → `to_dict()` + `to_json_string()` (Phase 1.4, noted 2026-04-05):** Current API: TS `toJSON()` returns a plain object (ECMAScript `JSON.stringify` hook); PY `to_json()` returns a JSON string. **Decision:** rename to `toObject()` (TS) / `to_dict()` (PY) for the structured return, and add `toJSONString()` (TS, already exists) / `to_json_string()` (PY, new) for the deterministic string. TS `toJSON()` hook stays as an alias of `toObject()` for `JSON.stringify(graph)` compatibility. PY `to_json()` becomes an alias of `to_json_string()` for backward compat during transition. Pre-1.0 — deprecation aliases are optional.
- **Initial value, cached state, and equals interaction (all phases, noted 2026-04-05):** The `initial` option, `_cached` field, `_hasEmittedData` flag, INVALIDATE reset, `resetOnTeardown`, and `equals` function form a tightly coupled state machine with several edge cases: (1) `initial` sets `_cached` but `_hasEmittedData` stays `false` — first `_emitAutoValue` skips `equals` even if value matches `initial`; is this correct? (2) After INVALIDATE, `_cached = undefined`/`None` and `_hasEmittedData = false` — next recompute always emits DATA even if value is the same as pre-invalidation; arguably correct (cache was cleared) but means INVALIDATE → same-value-recompute → DATA instead of RESOLVED. (3) `actions.emit(undefined)` sets `_cached = undefined` — subsequent emissions with `_hasEmittedData = true` call `equals(undefined, newValue)` which is fine for default `Object.is`/`is` but custom `equals` might not expect `undefined`. (4) Could a `_SENTINEL` symbol/object replace both `_hasEmittedData` and the `initial` default, unifying the state machine? (5) **Resubscribable nodes:** after terminal (COMPLETE/ERROR), `_hasEmittedData` stays `true`. On resubscribe, if the producer's first emission matches `_cached`, the node emits RESOLVED — new subscriber never sees DATA. Should `_hasEmittedData` reset alongside `_terminal = false` in the resubscribe path? (6) **Reconnect (disconnect/reconnect cycle):** when all sinks unsubscribe then a new sink arrives, `_hasEmittedData` is still `true`. If recomputed value matches `_cached`, node emits RESOLVED. Arguably correct — the cache is valid and `get()` returns it — but new subscriber's first signal is RESOLVED without prior DATA from its perspective. Deferred: discuss systematically in next chat — may eliminate `_hasEmittedData` entirely.
