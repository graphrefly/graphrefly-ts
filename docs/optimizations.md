...
- **~~Sink adapter silent error swallowing (Phase 5.2b–5.2d, noted 2026-04-04, resolved 2026-04-04):~~** Resolved with option (c): all sinks now return a unified `SinkHandle` type with `{ dispose(), errors: Node<SinkTransportError | null>, flush?() }`. `BufferedSinkHandle` extends `SinkHandle`. A shared `createSinkErrorHandler` / `_create_sink_error_handler` helper routes errors to both the user's `onTransportError` callback (if provided) and a reactive `errors` state node. Both TS and PY sinks return `SinkHandle` (per-record) or `BufferedSinkHandle` (buffered). `dispose()` sends `TEARDOWN` to the errors node on both sides. Pre-1.0, no backward compat concern.
...
- **~~Synchronous SQLite blocking in `toSqlite` / `to_sqlite` sink (Phase 5.2b, noted 2026-04-04, resolved 2026-04-04):~~** Both TS and PY now accept `batchInsert` / `batch_insert` (default `false`) and `maxBatchSize` / `max_batch_size` (default `1000`). DATA values are buffered and flushed inside a `BEGIN`/`COMMIT` transaction on terminal messages (`messageTier >= 3`), at `maxBatchSize` threshold, or on `dispose()`. On insert error, the first error triggers `break` + `ROLLBACK`. BEGIN failure preserves pending data for retry via manual `flush()`. Both return `BufferedSinkHandle` when batch mode is enabled.
- **CheckpointAdapter refactored to key-value (Phase 3.1c, noted 2026-04-05, resolved 2026-04-05):** `CheckpointAdapter` changed from blob-based (`save(data)` / `load()`) to key-value (`save(key, data)` / `load(key)` / `clear(key)`). Unifies checkpoint persistence and cache storage under one interface. Both TS and PY updated. `saveGraphCheckpoint` / `restoreGraphCheckpoint` pass `graph.name` as key. `autoCheckpoint` / `auto_checkpoint` passes `self.name` as key. `FileCheckpointAdapter` now takes a directory (one file per key, sanitized filenames). `DictCheckpointAdapter` no longer takes an internal key in constructor. `SqliteCheckpointAdapter` no longer takes a fixed key — caller provides it. Pre-1.0, all downstream consumers updated, no legacy shims.
- **`tieredStorage` uses `CheckpointAdapter[]` directly (Phase 3.1c, noted 2026-04-05, resolved 2026-04-05):** With key-value `CheckpointAdapter`, `tieredStorage` / `tiered_storage` wraps adapters as `CacheTier`s naturally. No separate `CacheTier` interface needed for end users — `CheckpointAdapter` serves both checkpoint and cache use cases.
- **Demo shell external-graph reactivity (Phase 7.2, noted 2026-04-05, resolved 2026-04-05):** The demo shell graph visualizes a separate demo `Graph` instance. Resolved with `state("demo/graph-ref")` + `state("demo/graph-tick")` pattern: framework bindings subscribe to the demo graph's `observe()` and bump `demo/graph-tick` on each event, which triggers `derived("graph/mermaid")` / `derived("graph/describe")` / `derived("inspect/*")` re-derivation. Alternative considered: `producer` node that internally subscribes to the demo graph — rejected because the producer has no reactive way to learn which graph to observe (no deps), and coupling two Graph lifecycles inside a producer complicates teardown. The framework binding is the natural bridge. TS-only for now (demo shell); PY parity not needed.
- **~~`equals` must never see undefined/None (all phases, noted 2026-04-05, resolved 2026-04-05):~~** Superseded by `NO_VALUE` sentinel below. Original fix used `_hasEmittedData` flag; the sentinel approach replaced both.
- **~~`_cached !== undefined` sentinel is ambiguous with `[[DATA, undefined]]` and INVALIDATE (noted 2026-04-05, resolved 2026-04-05):~~** Superseded by `NO_VALUE` sentinel below.
- **~~Derived node error observability gap (all phases, noted 2026-04-05, resolved 2026-04-05):~~** When a derived node's `fn` or `equals` throws, the node emits `[[ERROR, err]]` downstream and status becomes `"errored"`. Resolution: (a) spec §2.2 updated to emphasize `status` as source of truth and `describe()` as the primary diagnostic; (b) `test-guidance.md` updated with debugging section; (c) both `fn` and `equals` errors are now wrapped with `Node "${name}": fn threw` / `Node "${name}": equals threw` using `Error` with `{ cause }`. Both TS and PY.

- **Dual-bookkeeping of derived deps + connect edges (all phases, noted 2026-04-05, resolved 2026-04-05):** `Graph.add()` now auto-registers edges from the node's constructor deps. When a node is `add()`-ed, the graph inspects `_deps` and registers edges for any dep already registered in the same graph. Eliminates the dual-bookkeeping bug surface. `connect()` remains available for explicit wiring. Pre-1.0, no backward compat.
- **ResettableTimer primitive (§5.10, noted 2026-04-05, resolved 2026-04-05):** Created `ResettableTimer` class in `core/timer.ts` (TS) and `core/timer.py` (PY). Refactored `retry`, `rateLimiter`/`rate_limiter`, and `timeout` to use it. Eliminates the inline `timer`/`timerGen`/`cancelTimer` boilerplate. Exported from `core/`.
- **Sequential batch delivery strategy (noted 2026-04-05, resolved 2026-04-05):** TS `emitWithBatch` now accepts `options?: { strategy?: "partition" | "sequential" }`. Sequential mode walks messages one-at-a-time, deferring phase-2 during batching — matching PY's existing `strategy="sequential"` in `emit_with_batch`.
- **DATA requires a payload (spec §1.2, noted 2026-04-05, resolved 2026-04-05):** Spec updated: `[DATA, value]` MUST include the second element. `undefined`/`None`/`null` are valid values. A bare `[DATA]` (missing payload) is a protocol violation — implementations SHOULD reject or ignore it.
- **Sink dispose() error routing (Phase 5.2-5.3, noted 2026-04-05, resolved 2026-04-05):** PY sinks now route suppressed `dispose()` errors to the `errors` companion node before suppressing, instead of silently swallowing. Observable via `observe()`.
- **`from_django_orm` / `from_tortoise` shared iteration (Phase 5.2, noted 2026-04-05, resolved 2026-04-05):** Extracted `_from_sync_rows` helper in PY. Both entry points kept with distinct validation and docstrings.
- **`from_csv` / `from_ndjson` thread cleanup (Phase 5.2c, noted 2026-04-05, resolved 2026-04-05):** PY now calls `t.join(timeout=1)` after setting the stop flag. Best-effort; daemon thread ensures no process-exit blocking.
- **~~`toJSON()` → `toObject()` + `toJSONString()` / `to_json()` → `to_dict()` + `to_json_string()` (Phase 1.4, noted 2026-04-05, resolved 2026-04-05):~~** TS: `toJSON()` renamed to `toObject()`; `toJSON()` kept as ECMAScript `JSON.stringify` hook (delegates to `toObject()`). PY: `to_json()` renamed to `to_json_string()`; `to_dict()` added as alias of `snapshot()`; `to_json` kept as class-level alias of `to_json_string`. Spec §3.8 updated.
- **~~Initial value, cached state, and equals interaction (all phases, noted 2026-04-05, resolved 2026-04-05):~~** Resolved with `NO_VALUE` sentinel (TS: `Symbol.for("graphrefly/NO_VALUE")`, PY: `_SENTINEL = object()`). Replaces the `_hasEmittedData` boolean flag entirely. One field (`_cached`) instead of two — impossible to desync. Key semantics: (1) When `initial` option is present (even as `undefined`/`None`), `_cached = initial` — `equals` IS called on first emission, so `initial: 5` + compute returns 5 → RESOLVED. (2) When `initial` is absent, `_cached = NO_VALUE` — first emission always DATA. (3) INVALIDATE / `resetOnTeardown` set `_cached = NO_VALUE` — next emission always DATA. (4) Resubscribable: terminal reset now also sets `_cached = NO_VALUE` — new subscriber always gets DATA (fixes edge case 5). (5) Reconnect (disconnect/reconnect): cache retained → same-value emits RESOLVED — correct, cache is valid, `get()` returns it. (6) `get()` returns `undefined`/`None` when `_cached === NO_VALUE`. Spec §2.5 updated with new `initial` semantics and `equals` contract.
- **Auto-edge registration is local-only (Phase 1.1, noted 2026-04-05 — design note, not an open decision):** `Graph.add()` auto-registers edges for deps within the same `Graph` instance only. Cross-subgraph deps (node depends on a node in a mounted subgraph or vice versa) still require explicit `connect()`. This is consistent with the spec: cross-subgraph edges are an explicit wiring concern, and `describe()` / diagram export already require them to be registered.
- **Core `bridge()` helper (Phase 8.2, noted 2026-04-06, QA 2026-04-06):** Both TS and PY export `bridge(from, to, opts?)` / `bridge(from_node, to_node, *, name, down)` from `core/bridge`. Creates a graph-visible effect node that intercepts messages from `from` via `onMessage` and forwards to `to.down()`. Default forwards: **all standard types** (DATA, DIRTY, RESOLVED, COMPLETE, ERROR, TEARDOWN, PAUSE, RESUME, INVALIDATE). Configurable via `down` option (allowlist). Terminal types (COMPLETE/ERROR) always cause bridge self-termination regardless of `down` filter — this prevents immortal bridges when terminals are excluded. `funnel()` explicitly excludes TEARDOWN from its inter-stage bridges (stage lifecycle is managed by the parent graph). `DEFAULT_DOWN` exported for callers that need to customize. Both repos: 8 bridge tests each (TS: `bridge.test.ts`, PY: `test_bridge.py`).
- **GraphSpec cross-language parity (Phase 8.3, noted 2026-04-06):** Both TS and PY implement `compileSpec`/`compile_spec`, `decompileGraph`/`decompile_graph`, `llmCompose`/`llm_compose`, `llmRefine`/`llm_refine`, `specDiff`/`spec_diff`, and `validateSpec`/`validate_spec` in `patterns/graphspec`. Key alignment: (1) `GraphSpec` schema is identical JSON shape — `nodes`, `templates`, `feedback` top-level keys. TS uses TypeScript types; PY uses TypedDict. (2) `compileSpec` resolves nodes in dependency order (state/producer first, then derived/effect/operator). Catalog is passed explicitly (`GraphSpecCatalog`) — no global registry. (3) Template instantiation creates mounted subgraphs via `graph.mount()`. `$param` bindings resolve to top-level nodes. (4) Feedback edges wire via §8.1 `feedback()` primitive. (5) `decompileGraph` uses `describe(detail="standard")` / `describe(format="spec")`, skips `__meta__` and `__feedback_*` internal nodes. Template detection via structural fingerprinting of mounted subgraphs (groups identical structures). (6) `specDiff` is pure JSON comparison — template-aware, feedback-aware. (7) LLM APIs (`llmCompose`/`llmRefine`) share identical system prompt and validation pipeline. TS uses `resolveNodeInput` helper; PY uses direct `_resolve_llm_response`. Both repos: 35 tests each.
- **~~Feedback bare DATA to reentry/counter — deferred to 8.2 (Phase 8.1, noted 2026-04-06, resolved 2026-04-06):~~** Resolved in 8.2. `feedback()` now uses a graph-registered effect node (`__feedback_effect_<condition>`) that intercepts condition messages via `onMessage` and forwards to reentry/counter. The effect participates in two-phase push naturally. Old subscribe-based bridge removed. Both TS and PY.
- **`llmCompose`/`llmRefine` async (TS) vs sync (PY) — intentional divergence (Phase 8.3, noted 2026-04-06):** TS: `async function` returning `Promise<GraphSpec>`. PY: synchronous, adapter must return `LLMResponse` directly. PY design invariant: no `async def` / `Awaitable` in public APIs. TS spec §5.10 allows `await` at system boundaries (LLM adapter is external I/O, not reactive scheduling). Both are correct for their language idiom. Adapter contracts differ: PY adapters must be sync; TS adapters return Promise-compatible values.
- **Domain templates cross-language parity (Phase 8.2, noted 2026-04-06):** Both TS and PY implement `observabilityGraph`/`observability_graph`, `issueTrackerGraph`/`issue_tracker_graph`, `contentModerationGraph`/`content_moderation_graph`, `dataQualityGraph`/`data_quality_graph` in `patterns/domain-templates` / `patterns/domain_templates`. Key alignment: (1) All four use source injection (option B) — user passes a source node, template wires the topology. (2) Same well-known node names across both: `"source"`, `"correlate"`, `"slo_value"`, `"slo_verified"`, `"alerts"`, `"output"`, `"extract"`, `"verify"`, `"known_patterns"`, `"regression"`, `"priority"`, `"classify"`, `"review_queue"`, `"policy"`, `"validate"`, `"anomaly"`, `"baseline"`, `"drift"`, `"remediate"`. (3) Null guard on derived fns: TS returns `undefined` (no auto-emit), PY returns `None` — both skip computation when source hasn't emitted real data. (4) PY `derived` fn signature is `(vals, actions)` vs TS `(vals) => ...` — PY lambdas take `_a` second arg. (5) Options: TS uses interface types, PY uses `@dataclass`. (6) Meta keys: `domain_template: true`, `template_type: "<name>"`. Both repos: TS 24 tests, PY 21 tests.
...
- **~~`contentModerationGraph` review accumulator read-modify-write (Phase 8.2, noted 2026-04-06 QA, resolved 2026-04-06):~~** Replaced `state<ModerationResult[]>` + read-modify-write accumulator with `reactiveLog<ModerationResult>` (Phase 3.2). O(1) `append()` replaces O(n) copy-on-write. Optional `maxQueueSize` / `max_queue_size` option bounds queue growth. Feedback condition reads from `ReactiveLogSnapshot` entries. `dataQualityGraph` baseline updater retains `batch()` wrapping (single value, not append-only). Both TS and PY.
- **Feedback effect perpetually dirty status (Phase 8.1/8.2, noted 2026-04-06 QA — documented, accepted):** The `__feedback_effect_<condition>` node forwards DIRTY via default dispatch (`return false`) but consumes DATA (`return true`). The effect stays in "dirty" status between emissions. This is **by design** — the status is technically correct (the node IS dirty: it received DIRTY and never settled). Effects don't need to settle, and the effect's fn is `undefined` (no-op). `describe()` may show "dirty" for the feedback effect between cycles. Not a bug. These nodes now carry `meta._internal: true` (see bridge/feedback meta tag below) and can be filtered from `describe()` output when undesired.
- **Reduction primitives cross-language parity (Phase 8.1, noted 2026-04-06, QA 2026-04-06, updated 8.2 2026-04-06):** Both TS and PY implement `stratify`, `funnel`, `feedback`, `budgetGate`/`budget_gate`, and `scorer` in `patterns/reduction`. All five follow the orchestration factory pattern (`_base_meta`, `_register_step`). Key alignment: (1) `stratify` buffers DIRTY until DATA arrives — on classifier miss, emits `[DIRTY, RESOLVED]` to preserve spec §1.3.1 (both). (2) `funnel` now uses `bridge()` from `core/bridge` for inter-stage wiring — graph-visible effect nodes (`__bridge_<from>→<stage>_input`) replace the old subscribe-based bridges. Visible in `describe()`, participates in two-phase push. (3) `feedback` counter node is source of truth (resettable via `graph.set()`); feedback wiring now uses a graph-registered effect node (`__feedback_effect_<condition>`) instead of raw subscribe. Counter name is `__feedback_<condition>` to support multiple loops per graph. (4) `budgetGate`/`budget_gate` force-flushes all buffered items on terminal regardless of budget; sends RESUME before terminal if paused; forwards constraint ERROR downstream, silences constraint COMPLETE, forwards unknown constraint types via default. (5) `scorer` coerces `undefined`/`None` to 0 before multiplication (no NaN/TypeError divergence). TS `ScoredItem` is a plain object; PY `ScoredItem` is a class with `__slots__` and `__eq__`. Meta keys: `reduction: true`, `reduction_type: "<name>"`. Both repos: 22 tests each.

- **`stratify` two-dep gating (Phase 8.1/9.0, noted roadmap, resolved 2026-04-06, QA 2026-04-06):** `stratify` `onMessage` previously intercepted only source messages (dep 0) and let rules messages (dep 1) fall through to default bitmask handling. When both source and rules updated in the same `batch()`, source DATA was classified against stale rules (rules DATA hadn't drained yet), and a spurious second emission occurred from default settlement of dep 1. Fix: `onMessage` now intercepts **all** messages from both deps, implementing manual two-dep gating (same diamond-resolution pattern as `budgetGate`). Classification deferred until all dirty deps settle. Rules-only changes produce no downstream emission ("future items only"). QA hardening: (1) PAUSE/RESUME/INVALIDATE from rules dep swallowed (internal impl detail, not leaked downstream). (2) `classify` exceptions caught and treated as non-match (branch not killed). (3) `completeWhenDepsComplete: false` explicit (branch lifecycle driven by source terminal, not auto-complete from both deps). Both TS and PY.

- **Infrastructure node `_internal` meta tag (Phase 8.2, noted 2026-04-06, resolved 2026-04-06):** `bridge()` and `feedback()` effect nodes now carry `meta: { _internal: true }`. `decompileGraph` / `decompile_graph` checks `meta._internal` first (robust, convention-independent), with legacy name-prefix fallback (`__bridge_`, `__feedback_effect_`). Both TS and PY.
- **Cleanup wrapper `{ cleanup, value? }` (all phases, noted 2026-04-06, resolved 2026-04-06):** Node fn can now return `{ cleanup: () => void, value?: T }` to explicitly separate cleanup from data. When returned: `cleanup` is registered; if `value` key is present, it's emitted as data. Plain function returns remain cleanup for backward compat. Exported as `CleanupResult<T>` (TS) / documented dict pattern (PY). Resolves the documented limitation where returning a callable as data was silently consumed as cleanup.
- **PY `SqliteCheckpointAdapter` thread safety (Phase 3.1, noted 2026-04-05, resolved 2026-04-06):** `check_same_thread=False` + `threading.Lock` wrapping all `execute` / `commit` calls. Safe for `auto_checkpoint` debounce timer threads calling `save()` while main thread calls `load()`. PY only (TS is single-threaded).
- **PY `budget_gate` O(n) `buffer.pop(0)` (Phase 8.1, noted 2026-04-06 QA, resolved 2026-04-06):** Replaced `list` with `collections.deque`; `pop(0)` → `popleft()`. PY only.
- **PY `stratify` `pending_dirty` not reset on TEARDOWN (Phase 8.1, noted 2026-04-06 QA, resolved 2026-04-06):** Added `MessageType.TEARDOWN` to the terminal handler in `on_message` that clears `pending_dirty`. PY only.
- **PY drain cycle detection (cross-language, noted 2026-04-06, already present):** PY already has `_MAX_DRAIN_ITERATIONS = 1000` in `core/protocol.py`. Summary table updated.

- **`gate` rename → `valve` + new human-approval `gate` (Phase 9.0, noted 2026-04-06):** The boolean-control `gate` operator (extra/operators, patterns/orchestration) renamed to `valve` across both TS and PY. A new `gate` primitive (queue-based human-in-the-loop approval with `pending`, `count`, `isOpen` reactive surfaces and `approve`/`reject`/`modify`/`open`/`close` controls) added to `patterns/orchestration` in both repos. Key decisions: (1) `modify(fn, n?)` signature is `(value, index, pending) → value` (Array.map-style), not the simpler callbag-recharge `(value) → value`. (2) Each approved/modified value emits as a separate `[[DATA, v]]` (not batched) — each approval is a separate reactive cycle, consistent with callbag-recharge's "Tier 2: cycle boundary" annotation. (3) Internal state nodes (`pending`, `isOpen`/`is_open`, `count`) registered via `graph.mount()` subgraph in both TS and PY (`${name}_state::pending`, etc.). Aligned 2026-04-07.
- **`promptNode` / `prompt_node` factory (Phase 9.0, noted 2026-04-06, parity aligned 2026-04-07):** Universal LLM transform wrapping prompt template + adapter into a reactive derived node. Both TS and PY now use `switchMap` / `switch_map` internally — new dep values cancel stale in-flight LLM calls. Both create an intermediate `messages` derived node from deps, then `switchMap`/`switch_map` maps each message list to an LLM invocation. Supports `format: "json"` (auto-parse with markdown fence stripping in both ports), `retries`, `cache` (Map-keyed by serialized `[role, content]` tuples). Composes with existing `LLMAdapter` interface. TS returns `Node<T | null>`; PY returns `Node[Any]`. Both pass `systemPrompt`/`system_prompt` in adapter invoke options. Both have defensive response content extraction (handles `LLMResponse`, plain string, and fallback `String()` / `str()`).
- **Parity fixes applied (Phase 9.0, 2026-04-07):** (1) TS `promptNode` now passes `systemPrompt` in adapter invoke opts (was missing). (2) TS `promptNode` now strips markdown fences before `JSON.parse` for `format: "json"` (aligned to PY `_extract_content`). (3) TS `promptNode` now uses defensive content extraction matching PY's multi-path approach. (4) TS cache key serialization aligned to PY: `JSON.stringify(msgs.map(m => [m.role, m.content]))` (explicit field extraction). (5) PY `valve` `describe_kind` changed from `"valve"` to `"operator"` (spec Appendix B compliance). (6) PY `gate` error messages now include method name (aligned to TS). (7) PY `gate` `_sync_reactive` split into `_sync_pending` / `_sync_open` to avoid unnecessary reactive cycles (aligned to TS pattern). (8) PY `GateController` type annotations fixed (`approve`/`reject` now `Callable[..., None]` reflecting optional `count` param).

- **Whole-repo `emit` → `down` audit + `up` / backpressure / `messageTier` sweep (all phases, noted 2026-04-07):** Systematic audit across both TS and PY:
  1. **Rename `emit`-named methods, variables, and keys to `down`** everywhere the operation only applies to the downstream direction. The `emit` naming creates a "fire and forget" mental model that obscures the bidirectional protocol and causes AI agents (and humans) to ignore `up()`. Exceptions: names that genuinely apply to both directions (e.g. `emitWithBatch` which orchestrates both phases) may keep their name if renaming is misleading.
  ...
  3. **Use `messageTier()` / `message_tier()` for all tier-based gating** — no hardcoded type checks (`=== COMPLETE || === ERROR`). Replace with `messageTier(t) >= 3` for terminal checks, `>= 2` for phase-2 gating, etc.
  Pre-1.0, no backward compat concern on any rename.

## Cross-language parity fixes (2026-04-05)

- **~~PY error wrapping missing original message (resolved 2026-04-05):~~** PY `RuntimeError(f'Node "{name}": fn threw')` omitted the original error message. TS included it: `fn threw: ${errMsg}`. Fixed all 3 PY sites (`fn threw`, `equals threw`, `on_message threw`) to include `{err}` in the f-string. TS `onMessage` error was also not wrapped (raw error forwarded) — now wraps as `Node "${name}": onMessage threw: ${errMsg}` with `{ cause }`.
- **~~PY `Graph.add()` missing reverse edge scan (resolved 2026-04-05):~~** PY only did forward auto-edge registration (new node's deps → graph). TS did both forward + reverse (existing nodes whose deps include the new node). Fixed: PY now does the reverse scan, matching TS.
...
- **Storage & sink adapter pattern parity (Phase 5.2d, 2026-04-04):** All 5.2d sinks follow the same pattern in both TS and PY: duck-typed client protocols, `onMessage` intercepting `DATA`, `SinkTransportError` for serialize/send failures. All sinks return a unified `SinkHandle` with `dispose()` + `errors: Node<SinkTransportError | null>`. `dispose()` sends `TEARDOWN` to the errors node. Buffered sinks (`toClickHouse`/`to_clickhouse`, `toS3`/`to_s3`, `toFile`/`to_file`, `toCSV`/`to_csv`) return `BufferedSinkHandle` adding `flush()`. TS `SinkHandle` has optional `flush?: ...`; PY uses separate `BufferedSinkHandle` dataclass (intentional — more Pythonic). Checkpoint adapters (`checkpointToS3`/`checkpoint_to_s3`, `checkpointToRedis`/`checkpoint_to_redis`) wire `graph.autoCheckpoint()`/`graph.auto_checkpoint()`. TS uses `setTimeout` for flush timers; PY uses `threading.Timer`. PY `to_postgres` calls `client.execute(sql, params)` (psycopg2/3 style); TS calls `client.query(sql, params)` (pg style). PY `to_s3` uses `json.dumps` which includes spaces after separators; TS `JSON.stringify` does not — NDJSON output is semantically equivalent but not byte-identical across languages.
...
| Batch emit API | `emit_with_batch` (+ `dispatch_messages` alias); optional `subgraph_lock` for node emissions; `strategy="sequential"` | `emitWithBatch`; `options.strategy: "partition" \| "sequential"` |
...
| `_hasEmittedData` / `_has_emitted_data` | `_has_emitted_data: bool` on `NodeImpl` | `_hasEmittedData: boolean` on `NodeImpl` |
| `ResettableTimer` | `core/timer.py` `ResettableTimer` class; used by `retry`, `rate_limiter`, `timeout` | `core/timer.ts` `ResettableTimer` class; used by `retry`, `rateLimiter`, `timeout` |
| Auto-edge registration in `Graph.add()` | `Graph.add()` inspects `_deps`, auto-registers edges | `Graph.add()` inspects `_deps`, auto-registers edges |
| Sequential batch strategy | `emit_with_batch(..., strategy="sequential")` (existing) | `emitWithBatch(..., { strategy: "sequential" })` (new, aligned with PY) |
...
1. **~~`_is_cleanup_fn` / `isCleanupFn` treats any callable return as cleanup (resolved 2026-03-31, superseded 2026-04-06).~~** Now resolved: `{ cleanup: fn, value?: T }` wrapper added (see `CleanupResult` type). Plain function returns remain cleanup for backward compat; `{ cleanup }` is the explicit path when a callable needs to be emitted as data (return the callable directly, use the wrapper only for cleanup registration).
...
- **~~`sample` + `undefined` as `T` (Tier 2, resolved 2026-04-07):~~** Fixed. `sample` now tracks source DATA values via a local `NO_VALUE` sentinel instead of reading `source.get()`. `undefined` as `T` correctly emits DATA. Python had no ambiguity.
...
| Item | Resolution |
|------|------------|
| **~~keepalive subscription cleanup on destroy (resolved 2026-04-07 — already implemented)~~** | `ChatStreamGraph`, `ToolRegistryGraph`, and `AgentMemoryGraph` all have `destroy()` methods that unsubscribe keepalive sinks. `systemPromptBuilder` returns a `SystemPromptHandle` with `dispose()`. No leak. |
...
| Item | Status | Notes |
|------|--------|-------|
| **~~`sample` + `undefined` as `T`~~** | Resolved (2026-04-07) | Fixed: `sample` now tracks source DATA via local `NO_VALUE` sentinel instead of `source.get()`. `undefined` as `T` correctly emits DATA. |
