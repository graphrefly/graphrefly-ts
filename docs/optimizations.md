...
- **`contentModerationGraph` review accumulator read-modify-write (Phase 8.2, noted 2026-04-06 QA):** The `__review_accumulator` effect uses `reviewQueue.get()` + spread + `reviewQueue.down()` to append items. TS is single-threaded and each DATA settles the effect before the next arrives, so no concurrent race. **PY parity note:** PY's GIL protects against true races but the pattern is fragile under async event loops — consider using a reactive scan/reduce instead if PY ever supports async node fns.
- **Feedback effect perpetually dirty status (Phase 8.1/8.2, noted 2026-04-06 QA):** The `__feedback_effect_<condition>` node forwards DIRTY via default dispatch (`return false`) but consumes DATA (`return true`). The effect stays in "dirty" status between emissions. This is cosmetic — effects don't need to settle, and the effect's fn is `undefined` (no-op). `describe()` may show "dirty" for the feedback effect between cycles. Not a bug; acceptable for internal infrastructure nodes.
...
| Drain cycle detection | Should align: `MAX_DRAIN_ITERATIONS = 1000` cap (same as TS) | `MAX_DRAIN_ITERATIONS = 1000` cap |
