...
WebSocket sockets, ack callbacks, and microservice contexts stay host-private. Graph-visible DATA carries only the selected payload envelope and reply correlation uses `requestId` plus `bindingId`. Adapter diagnostics are host-side snapshots by default; graph-visible diagnostics require the explicit diagnostics boundary and contain only sanitized data fields.
